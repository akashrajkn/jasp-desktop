...
test_that(
    "Main table results match",
    {
      ...
      results <- JASPTools::run("TTestBayesianPairedSamples", "debug.csv",
                                options, view = FALSE, quiet = TRUE)
      table <- results[["results"]][["ttest"]][["data"]]
      expect_equal_tables(table, list("contNormal", "-", "contGamma",
                                      293915424756.037, 1.32616895200622e-19))
    })
...
test_that("Prior posterior plot matches",
          {
            ...
            expect_equal_plots(
                testPlot, "prior-posterior", dir = "TTestBayesianPairedSamples")
          })
...
test_that("BF robustness check plot matches",
          {
            ...
            results <- JASPTools::run("TTestBayesianPairedSamples", "debug.csv",
                                      options, view = FALSE, quiet = TRUE)
            testPlot <- results[["state"]][["figures"]][[1]]
            expect_equal_plots(testPlot, "robustness-check",
                               dir = "TTestBayesianPairedSamples")
          })
...
test_that("Descriptives plot matches",
          {
            ...
            testPlot <- results[["state"]][["figures"]][[1]]
            expect_equal_plots(
                testPlot, "descriptives", dir = "TTestBayesianPairedSamples")
          })
    ...
    "Descriptives table matches",
    {
      ...
      expect_equal_tables(
          table, list("contNormal", 100, -0.18874858754, 1.05841360919316,
                      0.105841360919316, -0.398760810055083, 0.0212636349750834,
                      "contGamma", 100, 2.03296079621, 1.53241112621044,
                      0.153241112621044, 1.72889718286736, 2.33702440955264))
    })
