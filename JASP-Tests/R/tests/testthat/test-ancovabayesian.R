...
test_that(
    "Main table results match",
    {
      ...
      refTables <- list(
          nullModelTop = list(
              "Null model (incl. facFive)", 0.25, -0.557159597272472,
              0.0609616076207224, 0, "", "facGender", 0.25, -0.293047614582535,
              0.493235485474366, 0.264111982689937, 29.6656710072444,
              "contGamma", 0.25, -1.12934210184216, -0.61871766039617,
              -0.572182504569689, 15.885810482332, "facGender + contGamma",
              0.25, -0.856200286718923, -0.313955333846312, -0.299040689446451,
              9.83211717425022),
          bestModelTop = list(
              "facGender", 0.25, -0.267241266752112, 0.547550995086289, 0, "",
              "Null model (incl. facFive)", 0.25, -0.581045856142161,
              0.0282510445044043, -0.313804589390049, 10.5414217188718,
              "facGender + contGamma", 0.25, -0.904975901109745,
              -0.370131448423163, -0.637734634357633, 14.932174588461,
              "contGamma", 0.25, -1.13850449013975, -0.628606544880642,
      ...
      for (order in c("nullModelTop", "bestModelTop")) {
        ...
        results <- JASPTools::run(
            "AncovaBayesian", "debug.csv", options, view = FALSE, quiet = TRUE)
...
test_that(
    "Effects table results match",
    {
      ...
      refTables <- list(
          allModels = list(
              "contGamma", 0.6, 0.226438035053417, 0.195147508378382,
              "contBinom", 0.6, 0.244646736629283, 0.215922578651091,
              "contGamma<unicode><unicode><unicode>contBinom", 0.2,
              0.0356936478531283, 0.148059370442442),
          matchedModels = list(
              "contGamma", 0.4, 0.193766080181001, 0.251463756140853,
              "contBinom", 0.4, 0.211903947712595, 0.281631799589681,
              "contGamma<unicode><unicode><unicode>contBinom", 0.2,
              0.0356812041725066, 0.784742840790345))
