...
test_that(
    "Main table results match",
    {
      ...
      refTables <- list(
          nullModelTop = list(
              "Null model (incl. facExperim)", 0.2, -0.594672751877366,
              0.134816578924395, 0, "", "facGender", 0.2, -0.28722095052267,
              0.630131621377202, 0.307451801354696, 13.7355911669398, "facFive",
              0.2, -1.24424524718345, -0.61670420719205, -0.649572495306089,
              12.4382643482459, "facGender + facFive", 0.2, -0.997414136054835,
              -0.349308383227457, -0.40274138417747, 9.31843477413068,
              "facGender + facFive + facGender<unicode><unicode><unicode>facFive",
              0.2, -1.14281197270266, -0.508311164396966, -0.548139220825293,
              27.3699066120848),
          bestModelTop = list(
              "facGender", 0.2, -0.224979999753508, 0.77036505277832, 0, "",
              "Null model (incl. facExperim)", 0.2, -0.63697968879273,
              0.0789763886292554, -0.411999689039223, 20.3818205689136,
              "facGender + facFive", 0.2, -1.06635989051332, -0.42532677067774,
              -0.84137989075981, 21.4710504386678, "facFive", 0.2,
              -1.33455292282619, -0.711911214299153, -1.10957292307269,
              21.0528996183343,
              "facGender + facFive + facGender<unicode><unicode><unicode>facFive",
      ...
      for (order in c("nullModelTop", "bestModelTop")) {
        ...
        results <- JASPTools::run(
            "AnovaBayesian", "debug.csv", options, view = FALSE, quiet = TRUE)
        table <- results[["results"]][["model comparison"]][["data"]]
        expect_equal_tables(
            table, refTables[[order]], label = paste("Table with order", order))
      }
    })

test_that(
    "Effects table results match",
    {
      set.seed(0)
      options <- initOpts()
      options$dependent <- "contNormal"
      ...
      refTables <- list(
          allModels = list(
              "facFive", 0.6, 0.130329318915725, 0.0999070274533764,
              "contBinom", 0.6, 0.215198578129195, 0.182805120830189,
              "facFive<unicode><unicode><unicode>contBinom", 0.2,
              0.00408798728805838, 0.0164190701020926),
          matchedModels = list(
              "facFive", 0.4, 0.125726268310478, 0.144492921488286, "contBinom",
              0.4, 0.210639434646256, 0.26825960145967,
              "facFive<unicode><unicode><unicode>contBinom", 0.2,
              0.0041531263651184, 0.182973144016417))
      ...
      for (effectsType in c("allModels", "matchedModels")) {
        ...
        expect_equal_tables(
            table, refTables[[effectsType]],
            label = paste("Table with effects type", effectsType))
      }
    })

test_that(
    "Post-hoc Comparisons table results match",
    {
      options <- JASPTools::analysisOptions("AnovaBayesian")
      options$dependent <- "contNormal"
...
test_that(
    "Analysis handles errors",
    {
      ...
      results <- JASPTools::run(
          "AnovaBayesian", "debug.csv", options, view = FALSE, quiet = TRUE)
      expect_identical(
          results[["results"]][["model comparison"]][["error"]][["errorType"]],
          "badData", label = "Inf check")
      ...
      expect_identical(
          results[["results"]][["model comparison"]][["error"]][["errorType"]],
          "badData", label = "1-level factor check")
      ...
      expect_identical(
          results[["results"]][["model comparison"]][["error"]][["errorType"]],
          "badData", label = "All nuisance check")
      ...
      expect_identical(
          results[["results"]][["model comparison"]][["error"]][["errorType"]],
          "badData", label = "Too few obs check")
    })
