...
test_that(
    "Main table results match",
    {
      ...
      results <- JASPTools::run("CorrelationBayesianPairs", "debug.csv",
                                options, view = FALSE, quiet = TRUE)
      table <- results[["results"]][["correlation"]][["data"]]
      expect_equal_tables(
          table, list("contcor1", "-", "contcor2", 0.657010063712354,
                      64323420095.6619, 0.754530668995209, 0.526024457777752))
    })
...
test_that("Scatterplot matches",
          {
            ...
            expect_equal_plots(
                testPlot, "scatterplot", dir = "CorrelationBayesianPairs")
          })
...
test_that("Prior posterior plot matches",
          {
            ...
            testPlot <- results[["state"]][["figures"]][[1]]
            expect_equal_plots(
                testPlot, "prior-posterior", dir = "CorrelationBayesianPairs")
          })
...
test_that("BF robustness check plot matches",
          {
            ...
            testPlot <- results[["state"]][["figures"]][[1]]
            expect_equal_plots(
                testPlot, "robustness-check", dir = "CorrelationBayesianPairs")
          })
