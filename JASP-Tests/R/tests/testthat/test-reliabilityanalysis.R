...
test_that(
    "Main table results match",
    {
      ...
      results <- JASPTools::run("ReliabilityAnalysis", "debug.csv", options,
      ...
      expect_equal_tables(
          table, list("scale", -0.757822989578577, -0.0677657928415725,
                      ...
                      -0.02217061461, 0.144515070286093, -1.45211881901153,
                      -0.235388804018903))
    })

test_that(
    "Item Statistics table matches",
    {
      options <- JASPTools::analysisOptions("ReliabilityAnalysis")
      options$variables <- c("contcor1", "contcor2", "contNormal")
      ...
      table <- results[["results"]][[
                   "reliabilityItemsObj"]][["reliabilityItems"]][["data"]]
      expect_equal_tables(
          table, list("contcor1", 0.0618194975467092, 0.0319398198963565,
                      0.061902485553013, 0.560156128034403, 0.05254867287,
                      1.01183864387684, "contcor2", 0.277152727398941,
                      0.161031927910319, 0.27739448681683, 0.442807451055322,
                      0.06968807084, 1.0041493380131, "contNormal",
                      0.79299280264282, 0.657010063712354, 0.793006727117146,
                      0.106272823965938, -0.18874858754, 1.05841360919316))
    })
