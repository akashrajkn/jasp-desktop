...
test_that(
    "Main table results match",
    {
      ...
      results <- JASPTools::run("ContingencyTablesBayesian", "debug.csv",
...
test_that(
    "Bayesian Contingency Tables Tests table results match",
    {
      options <- JASPTools::analysisOptions("ContingencyTablesBayesian")
      options$rows <- "facExperim"
      options$columns <- "contBinom"
      ...
      samplingModels <- c(
          "poisson", "jointMultinomial", "independentMultinomialRowsFixed",
          "independentMultinomialColumnsFixed", "hypergeometric")
      refTables <- list(
          poisson = list(
              "BF<unicode><unicode> Poisson", "N", 100, 0.523118843924781),
          jointMultinomial = list("BF<unicode><unicode> joint multinomial", "N",
                                  100, 0.440084106793853),
          independentMultinomialRowsFixed = list(
              "BF<unicode><unicode> independent multinomial", "N", 100,
              0.35545254779504),
          independentMultinomialColumnsFixed = list(
              "BF<unicode><unicode> independent multinomial", "N", 100,
              0.364069579256729),
          hypergeometric = list("BF<unicode><unicode> hypergeometric", "N", 100,
                                0.269648117146104))
      ...
      for (samplingModel in samplingModels) {
        ...
        expect_equal_tables(table, refTables[[samplingModel]],
                            label = paste("Sampling model", samplingModel))
      }
    })
...
test_that("Log Odds Ratio table results match",
          {
            ...
            expect_equal_tables(
                table, list("Odds ratio", -0.325226942981456,
                            -0.981898524010587, 0.337174584207703, "TRUE"))
          })
