...
test_that("Main table results match",
          {
            ...
            results <- JASPTools::run("TTestBayesianOneSample", "debug.csv",
                                      options, view = FALSE, quiet = TRUE)
            table <- results[["results"]][["ttest"]][["data"]]
            expect_equal_tables(table, list("contNormal", 0.508160332089536,
                                            2.80907441042415e-05))
          })
...
test_that("Prior posterior plot matches",
          {
            ...
            expect_equal_plots(
                testPlot, "prior-posterior", dir = "TTestBayesianOneSample")
          })
...
test_that("BF robustness check plot matches",
          {
            ...
            testPlot <- results[["state"]][["figures"]][[1]]
            expect_equal_plots(
                testPlot, "robustness-check", dir = "TTestBayesianOneSample")
          })
...
test_that("Sequential analysis plot matches",
          {
            ...
            testPlot <- results[["state"]][["figures"]][[1]]
            expect_equal_plots(
                testPlot, "sequential-analysis", dir = "TTestBayesianOneSample")
          })
...
test_that("Descriptives plot matches",
          {
            ...
            testPlot <- results[["state"]][["figures"]][[1]]
            expect_equal_plots(
                testPlot, "descriptives", dir = "TTestBayesianOneSample")
          })
...
test_that(
    "Descriptives table matches",
    {
      ...
      results <- JASPTools::run("TTestBayesianOneSample", "debug.csv", options,
                                view = FALSE, quiet = TRUE)
      table <- results[["results"]][["descriptives"]][["descriptivesTable"]][[
          "data"]]
      expect_equal_tables(table, list("contNormal", 100, -0.18874858754,
                                      1.05841360919316, 0.105841360919316,
