...
test_that("Main table results match",
          {
            ...
            results <- JASPTools::run("BinomialTestBayesian", "debug.csv",
                                      options, view = FALSE, quiet = TRUE)
            table <- results[["results"]][["binomial"]][["data"]]
            expect_equal_tables(
                table, list("contBinom", 0, 58, 100, 0.58, 4.32337507642424e-15,
                            "TRUE", "contBinom", 1, 42, 100, 0.42,
                            3.43240614623212e-05, "FALSE"))
          })
...
test_that("Prior posterior plots match",
          {
            ...
            expect_equal_plots(
                testPlot, "prior-posterior-1", dir = "BinomialTestBayesian")

            testPlot <- results[["state"]][["figures"]][[2]]
            expect_equal_plots(
                testPlot, "prior-posterior-2", dir = "BinomialTestBayesian")
          })
...
test_that("Sequential analysis plots match",
          {
            ...
            testPlot <- results[["state"]][["figures"]][[1]]
            expect_equal_plots(
                testPlot, "sequential-analysis-1", dir = "BinomialTestBayesian")

            testPlot <- results[["state"]][["figures"]][[2]]
            expect_equal_plots(
                testPlot, "sequential-analysis-2", dir = "BinomialTestBayesian")
          })
