...
.generateErrorMessage <- function(
    type, opening = FALSE, concatenate = NULL, grouping = NULL, ...) {
  ...
  if (grepl(' ', type, fixed = TRUE) == TRUE) {
    ...
    if (is.null(message)) {
      stop(
          'Could not find error message for "', type,
          '" (if you were trying to pass on a message, note that it must be a complete sentence)')
    }
  }
  ...
  toBeReplaced <- regmatches(message, gregexpr("(?<=\\{{)\\S*?(?=\\}})",
                                               message, perl = TRUE))[[1]]
  if (base::identical(toBeReplaced, character(0)) == FALSE) {  # Were there any
                                                               # {{string}}'s?

    if (all(toBeReplaced %in% names(args)) == FALSE) {  # Were all replacements
                                                        # provided in the
                                                        # arguments?
      missingReplacements <- toBeReplaced[!toBeReplaced %in% names(args)]
      stop('Missing required replacement(s): "',
           paste(missingReplacements, collapse = ','), '"')
    }

    for (i in 1:length(toBeReplaced)) {
      value <- args[[toBeReplaced[i]]]
      if (length(value) > 1) {  # Some arguments may have multiple values, e.g.
                                # amount = c('< 3', '> 5000').
      ...
      message <- gsub(
          paste0('{{', toBeReplaced[i], '}}'), value, message, fixed = TRUE)
    }
  ...
  for (i in 1:length(replaceInMessage)) {
    if (grepl(names(replaceInMessage)[i], message)) {
      message <- gsub(
          names(replaceInMessage)[i], replaceInMessage[[i]], message)
    }
  }
  ...
  if (is.character(concatenate) && length(concatenate) == 1) {
    endOfString <- substr(
        concatenate, nchar(concatenate) - 4, nchar(concatenate))
    if (endOfString == '</ul>') {
...
.hasErrors <- function(dataset, perform, type = NULL, custom = NULL,
                       message = 'default', exitAnalysisIfErrors = FALSE, ...) {
  ...
  if (!isTRUE(nrow(dataset) > 0) ||
      perform != 'run' || (length(type) == 0 && length(custom) == 0)) {
    return(FALSE)
  }
  ...
  checks <- list()
  checks[[
      'infinity']] <- list(callback = .checkInfinity, addGroupingMsg = FALSE)
  checks[['factorLevels']] <- list(callback = .checkFactorLevels)
  checks[['variance']] <- list(callback = .checkVariance, addGroupingMsg = TRUE)
  checks[['observations']] <- list(
      callback = .checkObservations, addGroupingMsg = TRUE)
  checks[['varCovMatrix']] <- list(
      callback = .checkVarCovMatrix, addGroupingMsg = FALSE)
  checks[['limits']] <- list(callback = .checkLimits, addGroupingMsg = FALSE)
  checks[['varCovData']] <- list(
      callback = .checkVarCovData, addGroupingMsg = TRUE)
  ...
  if (length(custom) > 0) {
    if (is.function(custom)) {
      checks[['_custom']] <- list(
          callback = custom, isCustom = TRUE, hasNamespace = FALSE)
      type <- c(type, '_custom')
      ...
      for (i in 1:length(custom)) {
        if (is.function(custom[[i]])) {
          checks[[names(custom)[i]]] <- list(
              callback = custom[[i]], isCustom = TRUE, hasNamespace = namespace)
  ...
  for (i in 1:length(type)) {
    ...
    isCustom <-
        !is.null(check[['isCustom']])  # Is it an analysis-specific check?
    hasNamespace <-
        !isCustom || check[['hasNamespace']] == TRUE  # Is it a named check?
    ...
    if (hasNamespace) {
      ...
      if (length(funcArgs) > 0) {
        ...
        names(funcArgs)[names(funcArgs) != 'dataset'] <- paste0(
            type[[i]], '.', names(funcArgs)[names(funcArgs) != 'dataset'])
        ...
        if (paste0(type[[i]], '.target') %in% names(funcArgs) &&
            !paste0(type[[i]], '.target') %in% names(args)) {
        ...
        if (length(reqArgs) > 0 &&
            all(names(reqArgs) %in% names(args)) == FALSE) {
          missingArgs <- reqArgs[!names(reqArgs) %in% names(args)]
          stop('Missing required argument(s): "',
               paste(names(missingArgs), collapse = ','), '"')
        }

        if (length(optArgs) > 0 &&
            all(names(optArgs) %in% names(args)) == FALSE) {
    ...
    if (hasNamespace && length(funcArgs) > 0) {
      ...
      names(callingArgs) <- gsub(
          paste0(type[[i]], '.'), '', names(callingArgs), fixed = TRUE)
    ...
    if ((!isCustom && checkResult[['error']] != TRUE) ||
        (isCustom && (!is.character(checkResult) || checkResult == ''))) {
      next
    }
    ...
    if (!(message == 'short' && !is.null(errors[['message']]))) {
      ...
      grouping <- NULL
      if (!is.null(check[['addGroupingMsg']]) &&
          check[['addGroupingMsg']] == TRUE &&
      ...
      errors[['message']] <- base::do.call(
          .generateErrorMessage,
          c(list(type = msgType, opening = opening,
                 concatenate = errors[['message']], variables = varsToAdd,
                 grouping = grouping), args))
    }

    if (!hasNamespace) {
      next  # We won't add info of the error to the list if we don't have a
            # name.
    }
...
.checkInfinity <- function(
    dataset, target, grouping = NULL, groupingLevel = NULL) {
  ...
  for (v in target) {
    ...
    } else {  # Makes no sense to check all subgroups for infinity rather than
              # the entire variable at once.
...
.checkVariance <- function(
    dataset, target, equalTo = 0, grouping = NULL, groupingLevel = NULL) {
  ...
  for (v in target) {
    ...
    if (length(grouping) > 0) {
      variance <- .applyOnGroups(
          getVariance, dataset, v, grouping, groupingLevel)
    } else {
...
.checkObservations <- function(
    dataset, target, amount, grouping = NULL, groupingLevel = NULL) {
  ...
  for (v in target) {
    ...
    if (length(grouping) > 0) {
      obs <- .applyOnGroups(
          getObservations, dataset, v, grouping, groupingLevel)
    } else {
    ...
    for (checkAmount in amount) {
      ...
      if (any(sapply(expr, function(x) eval(parse(text = x))))) {  # See if any
                                                                   # of the
                                                                   # expressions
                                                                   # is true.
        result$error <- TRUE
...
.checkVarCovMatrix <- function(
    dataset, nrow = TRUE, symm = TRUE, posdef = TRUE, ...) {
...
.checkVarCovData <- function(
    dataset, target, corFun = NULL, grouping = NULL, corArgs = NULL, ...) {
  ...
  if (is.null(corFun)) corFun <- stats::cor
