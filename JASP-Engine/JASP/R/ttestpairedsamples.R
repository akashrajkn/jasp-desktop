...
TTestPairedSamples <- function(
  ...
  if (options$descriptivesPlots) {
    ...
    plotTitle <- ifelse(
        length(options$pairs) > 1, "Descriptives Plots", "Descriptives Plot")
    descriptivesPlots <- .pairedSamplesTTestDescriptivesPlot(
        dataset, options, perform)
...
.ttestPairedSamples <- function(dataset, options, perform) {
  ...
  if (sum(allTests) == 2) {
    fields <- append(
        fields, list(list(name = "test", type = "string", title = "Test")), 3)
  }
  ...
  if (options$hypothesis == "groupOneGreater") {
    direction <- "greater"
    message <-
        "All tests, hypothesis is measurement one greater than measurement two."
    .addFootnote(footnotes, symbol = "<em>Note.</em>", text = message)

  } else if (options$hypothesis == "groupTwoGreater") {
    direction <- "less"
    message <-
        "All tests, hypothesis is measurement one less than measurement two."
    .addFootnote(footnotes, symbol = "<em>Note.</em>", text = message)
  } else {
    direction <- "two.sided"
  }
        ...
        if (p1 != "" && p2 != "") {
          ...
          if (is.null(errorMessage)) {
                ...
                expr = {
                  ...
                  if (test == 2) {
                    ...
                    if (direction == "two.sided") {
                      confIntEffSize <- sort(
                          c(tanh(zmbiss + qnorm((1 - percentConfidenceEffSize) /
                                                2) * mrSE),
                            tanh(zmbiss + qnorm((1 + percentConfidenceEffSize) /
                                                2) * mrSE)))
                    } else if (direction == "less") {
                      confIntEffSize <- sort(
                          c(-Inf, tanh(zmbiss +
                                       qnorm(percentConfidenceEffSize) * mrSE)))
                    } else if (direction == "greater") {
                      confIntEffSize <- sort(
                          c(tanh(zmbiss +
                                 qnorm((1 - percentConfidenceEffSize)) * mrSE),
                            Inf))
                    }
                    ## else run a simple paired t-test
                  } else {
                    res <- stats::t.test(c1, c2, paired = TRUE,
                                         conf.level = percentConfidenceMeanDiff,
                                         alternative = direction)
                    df <- ifelse(
                        is.null(res$parameter), "", as.numeric(res$parameter))
                    d <- .clean(mean(c1 - c2) / sd(c1 - c2))
                    t <- as.numeric(res$statistic)
                    confIntEffSize <- c(0, 0)
                    if (wantsConfidenceEffSize) {

                      alphaLevels <- sort(c((1 - percentConfidenceEffSize),
                                            percentConfidenceEffSize))
                      ...
                      confIntEffSize <- sort(c(ncp1 / sqrt(n), ncp2 / sqrt(n)))[
                          order(c(1 - percentConfidenceEffSize,
                                  percentConfidenceEffSize))]
            ...
            if (class(result) != "try-error") {
              ...
              df <- ifelse(
                  is.null(result$parameter), "", as.numeric(result$parameter))
              sed <- ifelse(is.null(result$parameter), "", sed)

              # add things to the intermediate results object
              row <- list(df = df, p = p, md = m, d = d,
                          lowerCIlocationParameter = ciLow,
                          upperCIlocationParameter = ciUp,
                          lowerCIeffectSize = ciLowEffSize,
                          upperCIeffectSize = ciUpEffSize, sed = sed,
          ...
          if (!is.null(errorMessage)) {
            ...
            row <- list(df = "", p = "", md = "", d = "", lowerCI = "",
                        upperCI = "", sed = "", .footnotes = row.footnotes)
  ...
  for (pair in pairs) {
    ...
    if (perform == "run" && length(options$pairs) > 0 && p1 != p2) {
      ...
      errors <- .hasErrors(
          ...
          type = c('observations', 'variance', 'infinity'),
          all.target = c(p1, p2), observations.amount = c('< 3', '> 5000'))
      ...
      if (!error) {
        ...
        result <- list(
            v1 = p1, sep = "-", v2 = p2, W = W, p = p, .isNewGroup = newGroup)
      } else {
        newGroup <- length(normalityTests.results) == 0
        result <- list(v1 = p1, sep = "-", v2 = p2, W = "NaN", p = "NaN",
      ...
      newGroup <- length(normalityTests.results) == 0
      result <- list(
          v1 = p1, sep = "-", v2 = p2, W = ".", p = ".", .isNewGroup = newGroup)
    }
  ...
  for (i in .indices(options$pairs)) {
    ...
    if (perform == "run" && pair[[1]] != "" && pair[[2]] != "") {
      ...
      if (!identical(errors, FALSE)) {
        ...
        ####
        data <- data.frame(
            id = rep(1:length(c1), 2), dependent = c(c1, c2),
            groupingVariable = c(
                rep(paste("1.", pair[[1]], sep = ""), length(c1)),
                rep(paste("2.", pair[[2]], sep = ""), length(c2))))

        summaryStat <- .summarySEwithin(
            data, measurevar = "dependent", withinvars = "groupingVariable",
            idvar = "id",
