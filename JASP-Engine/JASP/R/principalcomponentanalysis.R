...
PrincipalComponentAnalysis <- function(
  ...
  return(mainFunctionPCAEFA(
             type = "pca", dataset = NULL, options, perform = "run",
             callback = function(...) list(status = "ok"), state = NULL, ...))
}

mainFunctionPCAEFA <- function(
    type, dataset = NULL, options, perform = "run",
    callback = function(...) list(status = "ok"), state = NULL, ...) {
  ...
  ## initialize result ## ----
  results[["title"]] = ifelse(type == "pca", "Principal Component Analysis",
  ...
  stateKey <- list(
      analysisResults = c(
          'rotationMethod', 'orthogonalSelector', 'obliqueSelector',
          'variables', 'factorMethod', 'eigenValuesBox', 'numberOfFactors'),
      pathDiagram = c('rotationMethod', 'orthogonalSelector', 'obliqueSelector',
                      'variables', 'factorMethod', 'eigenValuesBox',
                      'numberOfFactors', 'highlightSlider', 'highlightText',
                      'plotWidthPathDiagram', 'plotHeightPathDiagram'),
      screePlot = c(
          'rotationMethod', 'orthogonalSelector', 'obliqueSelector',
          'variables', 'factorMethod', 'eigenValuesBox', 'numberOfFactors',
          'plotWidthScreePlot', 'plotHeightScreePlot'))
  ...
  if (is.null(state[["analysisResults"]])) {
    ...
    if (perform == "run" && !is.null(dataset) && nrow(dataset) > 1) {
      customChecksPCAEFA <- list(
          function() {
            if (length(options$variables) > 0 &&
              ...
              return(paste0("Too many factors requested (",
                            options$numberOfFactors,
                            ") for the amount of included variables"))
            }
          }, function() {
               if (nrow(dataset) < 3) {
                 return(paste0("Not enough valid cases (", nrow(dataset),
                ...
                function() {
                  ...
                  for (i in 1:nrow(dataset)) {
                    if (sd(dataset[i, ], na.rm = TRUE) == 0)
                      varianceZero <- varianceZero + 1
                  }
      ...
      error <- .hasErrors(
          dataset = dataset, perform = perform,
          type = c("infinity", "variance"), custom = customChecksPCAEFA,
  ...
  results[[
      "goodnessOfFit"]] <- .goodnessOfFit(analysisResults, options, perform)
  # Create factor correlation table:
  results[["factorLoadings"]] <- .getLoadings(
      analysisResults, dataset, options, perform, "pca")

  if (options$incl_correlations) {
    results[["factorCorrelations"]] <- .getFactorCorrelations(
        analysisResults, options, perform, "pca")
  }
  ...
  if (isTRUE(options$incl_pathDiagram)) {
    p <- try(silent = FALSE,
             expr = {
               .pathDiagramPCAEFA(analysisResults, options, perform, type,
                                  oldPlot = state[["pathDiagram"]])
             })
  ...
  #save state
  state <- list(options = options, analysisResults = analysisResults,
...
.estimateNFactorPCAEFA <- function(dataset, options, perform, type) {
  ...
  if (perform == "run" &&
      !is.null(dataset) && nrow(dataset) > 1 && nVariable > 0) {
...
.getLoadings <- function(analysisResults, dataset, options, perform, type) {
  ...
  if (is.null(analysisResults) || isTryError(analysisResults)) {
    ...
    colnames(
        loadingsMatrix) <- c(paste(colName, seq_len(nFactor)), "Uniqueness")
    ...
    # Add uniqueness:
    loadingsMatrix <- cbind(
        loadingsMatrix, Uniqueness = unname(analysisResults$uniquenesses))
  }

  # Add columns:
  Loadings[["schema"]][[
      "fields"]][[1]] <- list(name = "VAR", title = "", type = "string")

  for (j in seq_len(ncol(loadingsMatrix))) {
    Loadings[["schema"]][["fields"]][[j + 1]] <- list(
        name = colnames(loadingsMatrix)[j], title = colnames(loadingsMatrix)[j],
  ...
  for (i in seq_len(nrow(loadingsMatrix))) {
    ...
    for (j in seq_along(dat)) {
      if (is.na(dat[j])) {
        ...
        Loadings[["data"]][[i]][[
            j + 1]] <- unname(ifelse(abs(dat[j]) < options$highlightText &
                                     j != length(dat), ".", dat[j]))
      }
    }
    names(Loadings[["data"]][[i]]) <- sapply(
        Loadings[["schema"]][["fields"]], "[[", 'name')
  }
  return(Loadings)
}
...
# Path diagram:
.pathDiagramPCAEFA <- function(
    analysisResults, options, perform, type, oldPlot = NULL) {
  ...
  pathDiagram$custom <- list(
      width = "plotWidthPathDiagram", height = "plotHeightPathDiagram")
  ...
  if (is.null(analysisResults) || isTryError(analysisResults)) {
    ...
    # Factor loadings
    if (type == "pca") {
      E_loadings <- data.frame(from = rep(labels, times = nFactor),
                               to = rep(factors, each = nIndicator),
                               weight = c(LY), stringsAsFactors = FALSE)
    } else {
      E_loadings <- data.frame(from = rep(factors, each = nIndicator),
                               to = rep(labels, times = nFactor),
                               weight = c(LY), stringsAsFactors = FALSE)
    }
    ...
    # Factor correlations:
    E_cor <- data.frame(from = c(factors[col(PS)]), to = c(factors[row(PS)]),
                        weight = c(PS), stringsAsFactors = FALSE)
    ...
    L <- cbind(
        c(rep(-1, nFactor), rep(1, nIndicator)), c(sq(nFactor), sq(nIndicator)))
    ...
    # Scale to plot width:
    Scale <- sqrt(pathDiagram$width ^ 2 + pathDiagram$height ^ 2) /
        sqrt(480 ^ 2 + 300 ^ 2)
    dist <- 1 / Scale * dist
    ...
    ECP[nrow(E_loadings) + nrow(E_resid) + seq_len(nrow(E_cor)), 1:2] <- 1.5 *
        pi
    if (type == "pca") {
    ...
    # bidirectional:
    bidir <- c(
        rep(FALSE, nrow(E_loadings) + nrow(E_resid)), rep(TRUE, nrow(E_cor)))
    ...
    .plotFunc <- function() {
      qgraph::qgraph(
          E, layout = L, directed = TRUE, bidirectional = bidir,
          residuals = TRUE, residScale = 10, labels = c(factors, labels),
          curve = curve, curveScale = FALSE, edgeConnectPoints = ECP,
          loopRotation = loopRotation, shape = shape, vsize = size1,
          vsize2 = size2, label.scale.equal = label.scale.equal, residScale = 2,
          mar = c(5, 10, 5, 12), normalize = FALSE, label.fill.vertical = 0.75,
          cut = options$highlightText, bg = "transparent")
    }

    content <- .writeImage(
        width = pathDiagram$width, height = pathDiagram$height,
...
.getFactorCorrelations <- function(analysisResults, options, perform, type) {
  ...
  FactorCorrelations[["title"]] <- ifelse(
      type == "pca", "Component Correlations", "Factor Correlations")

  # Extract loadings
  if (is.null(analysisResults) || isTryError(analysisResults)) {  # || perform
                                                                  # == "init"
    if (is.null(options$numberOfFactors)) {
      nFact <- 0
    } else {
  ...
  xName =
      ifelse(type == "pca", ifelse(Rotation == "none", "PC", "RC"), "Factor")

  if (ncol(corMatrix) > 0) {
    colnames(corMatrix) <-
        rownames(corMatrix) <- paste(xName, seq_len(ncol(corMatrix)))
  }


  # Add columns:
  FactorCorrelations[["schema"]] <- list(fields = list())
  FactorCorrelations[["schema"]][[
      "fields"]][[1]] <- list(name = "VAR", title = "", type = "string")

  for (j in seq_len(ncol(corMatrix))) {
    FactorCorrelations[["schema"]][["fields"]][[j + 1]] <- list(
        name = colnames(corMatrix)[j], title = colnames(corMatrix)[j],
        type = "number", format = "dp:3")
  }
  ...
  for (i in seq_len(nrow(corMatrix))) {
    ...
    names(
        FactorCorrelations[["data"]][[
            i]]) <- sapply(FactorCorrelations[["schema"]][["fields"]], "[[",
                           'name')[seq_along(FactorCorrelations[["data"]][[i]])]
  }
...
.goodnessOfFit <- function(analysisResults, options, perform) {
  ...
  # Create the columns:
  goodnessOfFit[["schema"]] <- list(
      fields = list(list(name = "model", title = "", type = "string"),
                    list(name = "chisq", title = "Value", type = "number",
                         format = "dp:3"),
  ...
  # Extract loadings:
  if (is.null(analysisResults) || isTryError(analysisResults)) {
    Fits <- list(CHI = ".", PVAL = ".", DF = ".")

  } else {
    analysisResults <- analysisResults$Results

    Fits <- list(CHI = .DotIfNULLPCAEFA(analysisResults$STATISTIC),
  ...
  goodnessOfFit[["data"]] <- list(
      list(model = "Model", chisq = ifelse(Fits$DF > 0, Fits$CHI, "."),
...
.fitMeasures <- function(analysisResults, options, perform) {
  ...
  # Extract loadings:
  if (is.null(analysisResults) || isTryError(analysisResults)) {
    Fits <- list(CHI = ".", PVAL = ".", DF = ".", RMSEA = ".", RMSEAlower = ".",
                 RMSEAupper = ".", TLI = ".", RMS = ".", CRMS = ".", BIC = ".")
  ...
  FitMeasures[["data"]] <- list(
      list(model = "Model", RMSEA = Fits$RMSEA,
           RMSEAci = paste(Fits$RMSEAlower, "-", Fits$RMSEAupper),
           TLI = Fits$TLI, BIC = Fits$BIC))
...
.screePlot <- function(dataset, options, perform, oldPlot = NULL, plotType) {
  ...
  if (!is.null(oldPlot) &&
      !identical(oldPlot[["data"]], "") && !is.null(oldPlot[["data"]])) {
    return(oldPlot)
  }
  ...
  screePlot$custom <- list(
      width = "plotWidthScreePlot", height = "plotHeightScreePlot")
  ...
  if (!is.null(dataset) && nrow(dataset) > 1 && length(options$variables) > 1) {
    ...
    # Eigenvalues:
    EV <- data.frame(id = seq_len(ncol(dataset)), ev = ev_ev, type = "Data")

    # Parallel analysis:
    PA <- data.frame(id = seq_len(ncol(dataset)), ev = pa_ev,
                     type = "Simulated (95th quantile)")

    combined <- rbind(EV, PA)

    p <- ggplot2::ggplot(
             combined, ggplot2::aes_string(
                           x = "id", y = "ev", lty = "type", pch = "type")) +
        ggplot2::geom_point(na.rm = TRUE, size = 3) +
        ggplot2::xlab("") + ggplot2::ylab("Eigenvalue") +
        ggplot2::xlab(xName) + ggplot2::geom_line(na.rm = TRUE) +
        ...
        ggplot2::theme(
            ...
            legend.justification = c(1, 1),  # legend.key =
    ...
    content <- .writeImage(
        width = options$plotWidthScreePlot,
        height = options$plotHeightScreePlot, plot = p, obj = TRUE)
