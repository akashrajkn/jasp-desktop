...
NetworkAnalysis <- function(
  ...
  if (is.null(dataset)) {
    ...
    if (perform == "run") {
      ...
      dataset <- .readDataSetToEnd(
          columns.as.numeric = variables, columns.as.factor = varsAsFactor,
          exclude.na.listwise = NULL)
      newOrder <- match(.unv(colnames(dataset)), variables, nomatch = 0L)
      variables <- variables[newOrder]

      if (options[["colorNodesBy"]] != "") {  # load data from variable that
                                              # indicates groups
        options[[
            "colorNodesByData"]] <- .readDataSetToEnd(
                                        columns = options[["colorNodesBy"]],
                                        exclude.na.listwise = options[[
                                            "colorNodesBy"]])[[1]]
        colorNodes <- .networkAnalysisSanitizeColorNodesByData(
            variables, options)
        options[["colorNodesByData"]] <- colorNodes[["newData"]]
        options[["colorNodesByDataMessage"]] <- colorNodes[["message"]]

      }
      if (options[["mgmVariableType"]] != "") {  # load data from variable that
                                                 # indicates variable type
        options[["mgmVariableTypeData"]] <- .readDataSetToEnd(
                                                columns = options[[
                                                    "mgmVariableType"]],
                                                exclude.na.listwise = options[[
                                                    "mgmVariableType"]])[[1]]
        ...
        # some robustness
        if (length(options[["mgmVariableTypeData"]]) <
            length(variables)) {  # too short
          options[["mgmVariableTypeDataOkay"]] <- -1
        } else if (length(options[["mgmVariableTypeData"]]) >
                   length(variables)) {  # too long
          options[["mgmVariableTypeDataOkay"]] <- 1
        }
        options[["mgmVariableTypeData"]] <- options[["mgmVariableTypeData"]][
            seq_along(variables)]
        options[[
            "mgmVariableTypeData"]][is.na(options[["mgmVariableTypeData"]])] <-
            "g"  # set missing to gaussian (in case of too few types supplied)

      }
      if (options[["layoutX"]] != "" && options[["layoutY"]] != "") {

        options[[
            "layoutXData"]] <- .readDataSetToEnd(columns = options[["layoutX"]],
                                                 exclude.na.listwise = options[[
                                                     "layoutX"]])[[1]]
        options[[
            "layoutYData"]] <- .readDataSetToEnd(columns = options[["layoutY"]],
                                                 exclude.na.listwise = options[[
                                                     "layoutY"]])[[1]]
      ...
      if (anyNA(c(options[["layoutXData"]], options[["layoutYData"]])) ||
        ...
        options[["layoutInvalid"]] <-
            TRUE  # TODO: footnote in table if this one is true
    ...
    dataset <- .vdf(dataset, columns.as.numeric = variables,
                    columns.as.factor = varsAsFactor)
  ...
  defArgs <- c(# data
               ...
               "estimator", # arguments for the estimator
                            "correlationMethod", "tuningParameter", "criterion",
               "isingEstimator", "nFolds", "split", "rule", "sampleSize",
               "thresholdBox", "thresholdString", "thresholdValue",
               # general arguments
  ...
  stateKey <- list(
      ...
      network = defArgs, # depends also on bootstrap options
                         bootstrap = bootstrapArgs,
      # depends also on normalization of centrality measures
      centrality = c(
          defArgs, "normalizeCentrality", "maxEdgeStrength", "minEdgeStrength"),
      # depends also on plotting arguments
      networkPLT = c(
          defArgs, # depends also on plotting arguments
                   "plotWidthNetwork", "plotHeightNetwork", "layout",
          "edgeColors", "repulsion", "edgeSize", "nodeSize", "colorNodesBy",
          "maxEdgeStrength", "minEdgeStrength", "cut", "showDetails",
          "nodeColors", "showLegend", "legendNumber", "showMgmVariableType",
          "showVariableNames", "graphSize", "scaleLabels", "labelSize",
          "abbreviateLabels", "abbreviateNoChars", "keepLayoutTheSame",
          "layoutX", "layoutY"),
      centralityPLT = c(
          defArgs, "plotWidthCentrality", "plotHeightCentrality",
          "normalizeCentrality", "abbreviateLabels", "abbreviateNoChars"),
      layout = c(defArgs, "layout", "repulsion", "layoutX", "layoutY"),
      bootstrapEdgePLT = bootstrapArgs, bootstrapCentPLT = bootstrapArgs)

  if (options[["keepLayoutTheSame"]])  # ensure layout remains unchanged unless
                                       # the data changes
  ...
  if (perform == "run" && options[["estimator"]] == "mgm" &&
                          !is.null(options[["mgmVariableTypeData"]])) {

    options[["mgmVariableTypeData"]] <- .networkAnalysisSanitizeMgmVariableType(
        variables = variables, options = options)

    results[["generalTB"]] <- .networkAnalysisGeneralTable(
        NULL, dataset, options, perform = "init")  # any errors will appear top
                                                   # of this table
    results[["mgmTB"]] <- .networkAnalysisMgmVariableInfoTable(
        network, variables, options, perform)
  ...
  if (length(variables) > 2) {
    ...
    if (is.null(state[["network"]])) {
      ...
      if (options[["groupingVariable"]] != "") {
        ...
        .hasErrors(dataset = dataset[.v(groupingVariable)], perform = perform,
                   ...
                   factorLevels.amount = "< 2", observations.amount = "< 10",
      ...
      if (options[["estimator"]] == "mgm" &&
          "c" %in% options[["mgmVariableTypeData"]]) {
      ...
      # check for errors
      fun <- cor
      if (options[["correlationMethod"]] == "cov") fun <- cov
      .hasErrors(
          dataset = dataset, perform = perform, type = checks,
          variance.target = variables,  # otherwise the grouping variable always
                                        # has variance == 0
          variance.grouping = groupingVariable,
          factorLevels.target = categoricalVars,
          factorLevels.amount = "> 10",  # probably a misspecification of
                                         # mgmVariableType when this happens.
          factorLevels.grouping = groupingVariable,
          observations.amount = " < 3",
          observations.grouping = groupingVariable,
          varCovData.grouping = groupingVariable, varCovData.corFun = fun,
          varCovData.corArgs = list(use = "pairwise"),
          exitAnalysisIfErrors = TRUE)
    }

    network <- .networkAnalysisRun(
        dataset = dataset, options = options, variables = variables,
        perform = perform, oldNetwork = state)
    ...
    if (options[["bootstrapOnOff"]]) {
      ...
      network <- .networkAnalysisBootstrap(
          network, options, variables, perform, oldNetwork = state,
          results = results, callback = callback)
  ...
  keep <- NULL
  results[["generalTB"]] <- .networkAnalysisGeneralTable(
      network, dataset, options, perform)

  if (!is.null(network) && network[["status"]] == "error") {
    results[["generalTB"]][["error"]] <- list(
        errorMessage = "A proper network could not be estimated. To receive assistance with this problem, please report the message above at: https://jasp-stats.org/bug-reports",
        errorType = "badData")
  } else {
    ...
    if (options[["bootstrapOnOff"]]) {
      results[["bootstrapTB"]] <- .networkAnalysisBootstrapTable(
          network, dataset, options, perform)

      if (options[["StatisticsEdges"]]) {
        results[["bootstrapEdgePLT"]] <- .networkAnalysisBootstrapPlot(
            network, options, perform, statistic = "edge",
            oldPlot = state[["bootstrapEdgePLT"]])
        allBootstrapPlots <- results[["bootstrapEdgePLT"]][["collection"]]
        for (bt in allBootstrapPlots) keep <- c(keep, bt[["data"]])
      }
      if (options[["StatisticsCentrality"]]) {
        results[["bootstrapCentPLT"]] <- .networkAnalysisBootstrapPlot(
            network, options, perform,
            statistic = c("strength", "betweenness", "closeness"),
            oldPlot = state[["bootstrapCentPLT"]])
    ...
    if (options[["tableWeightsMatrix"]]) {
      results[["weightmatrixTB"]] <- .networkAnalysisWeightMatrixTable(
          network, options, variables, perform)
    }
    if (options[["tableLayout"]]) {
      results[["layoutTB"]] <- .networkAnalysisLayoutTable(
          network, options, variables, perform)
    }
    if (options[["tableCentrality"]]) {
      results[["centralityTB"]] <- .networkAnalysisCentralityTable(
          network, options, perform)
    }
    if (options[["plotNetwork"]]) {
      results[["networkPLT"]] <- .networkAnalysisNetworkPlot(
          network, options, perform, oldPlot = state[["networkPLT"]])
      allNetworkPlots <- results[["networkPLT"]][["collection"]]
      for (nw in allNetworkPlots) keep <- c(keep, nw[["data"]])
    }
    if (options[["plotCentrality"]]) {
      results[["centralityPLT"]] <- .networkAnalysisCentralityPlot(
          network, options, perform, oldPlot = state[["centralityPLT"]])
  ...
  if (perform == "init") {
    ...
    # Save State
    state <- list(
        options = options, network = network[["network"]],
        bootstrap = network[["bootstrap"]],
        centrality = network[["centrality"]], layout = network[["layout"]],
...
.networkAnalysisRun <- function(
    dataset, options, variables, perform, oldNetwork = NULL) {
  ...
  networkList <- list(
      ...
      centrality = oldNetwork[["centrality"]], layout = oldNetwork[["layout"]])
  ...
  if (is.null(networkList[["network"]])) {  # estimate network
    ...
    options[["isingEstimator"]] <- switch(
        options[["isingEstimator"]], "pseudoLikelihood" = "pl",
        "univariateRegressions" = "uni", "bivariateRegressions" = "bi",
        "logLinear" = "ll")
    ...
    if (options[["estimator"]] == "mgm") {
      ...
      if (is.null(options[["mgmVariableTypeData"]])) {
        ...
        type[invalidType] <-
            "g"  # set missing to gaussian. TODO: add to table message.

        if (any(invalidType))
          networkList[["message"]] <- c(
              networkList[["message"]],
              sprintf(
                  "The variable types supplied in %s contain missing values or values that do not start with 'g', 'c', or 'p'. These have been reset to gaussian ('g'). They were indices %s.",
                  options[["mgmVariableType"]],
                  paste(which(invalidType), sep = ", ")))

        # find out the levels of each categorical variable
        for (i in which(type == "c"))
          level[i] <- max(
              1, nlevels(dataset[[1]][[i]]), length(unique(dataset[[1]][[i]])))
    ...
    .dots <- list(
        ...
        method = options[["isingEstimator"]], rule = options[["rule"]],
        nFolds = options[["nFolds"]], weighted = options[["weightedNetwork"]],
        signed = options[["signedNetwork"]], split = options[["split"]],
        criterion = options[["criterion"]],
        sampleSize = options[["sampleSize"]], type = type, lev = level,
        threshold = threshold)
    ...
    funArgs <- formals(
        utils::getFromNamespace(
            paste0("bootnet_", options[["estimator"]]), ns = "bootnet"))
    ...
    if (options[["estimator"]] == "huge") {
      ...
      if (.dots[["criterion"]] == "cv") .dots[["criterion"]] == "ebic"

    } else if (options[["estimator"]] == "mgm") {

      .dots[["criterion"]] <- toupper(.dots[["criterion"]])  # this function
                                                             # wants capitalized
                                                             # arguments
      ...
      if (is.na(.dots[["nFolds"]]) || .dots[["nFolds"]] < 2)  # estimator
                                                              # crashes
                                                              # otherwise
        .dots[["nFolds"]] <- 2
    ...
    for (nw in seq_along(dataset)) {
      ...
      msg <- capture.output(network <- bootnet::estimateNetwork(
                                data = data, default = options[["estimator"]],
                                .dots = .dots), type = "message")
      t1 <- Sys.time()

      dev.off()  # close the fake png
      file.remove(tempFileName)  # remove the fake png file

      timing[nw] <- difftime(t1, t0, units = "secs")  # ensure times are always
                                                      # seconds
  ...
  if (is.null(networkList[["centrality"]])) {  # calculate centrality measures
    ...
    for (nw in seq_along(dataset)) {
      ...
      cent <- qgraph::centrality(
          network[["graph"]], weighted = options[["weightedNetwork"]],
          signed = options[["signedNetwork"]], all.shortest.paths = FALSE)
      ...
      TBcent <- as.data.frame(
          cent[c("Betweenness", "Closeness", "InDegree", "OutDegree")])

      # adapted from qgraph::centrality_auto
      wmat <- qgraph::getWmat(network$graph)
      if (options[["weightedNetwork"]]) wmat <- sign(wmat)

      if (options[["signedNetwork"]]) wmat <- abs(wmat)

      directedGraph <- ifelse(
          base::isSymmetric.matrix(object = wmat, tol = 1e-12), FALSE, TRUE)
      weightedGraph <- ifelse(
          all(qgraph::mat2vec(wmat) %in% c(0, 1)), FALSE, TRUE)
      ...
      if (options[["normalizeCentrality"]] == "normalized") {  # normalize only
                                                               # if variances
                                                               # are nonzero
        ...
        for (i in 1:ncol(TBcent)) {  # code modified from
      ...
      nc <- ncol(TBcent)
      TBcent <- TBcent[c(nc, 1:(nc - 1))]  # put columns in intended order (of
                                           # schema).
      cent <- TBcent
  ...
  if (is.null(networkList[["layout"]])) {
    ...
    if (options[["layout"]] != "data" || options[["layoutInvalid"]]) {
      ...
      if (options[["layout"]] == "data") options[["layout"]] <- "circle"

      # this calls qgraph::qgraph so we need the png trick again
      tempFileName <- .networkAnalysisGetTempFileName()
      grDevices::png(filename = tempFileName)

      layout <- qgraph::averageLayout(
          networkList[["network"]], layout = options[["layout"]],
...
.networkAnalysisBootstrap <- function(network, options, variables, perform,
                                      oldNetwork = NULL, results, callback) {
  ...
  if (perform == "run" && is.null(network[["bootstrap"]])) {
    ...
    callbackBootstrap <- function(results = NULL, progress = NULL) {
      ...
      if (response[["status"]] == "changed") {
        ...
        optsForBootstrap <- c(
            ...
            "thresholdBox", "thresholdString", "thresholdValue",
            "weightedNetwork", "signedNetwork", "missingValues",
            "numberOfBootstraps", "BootstrapType", "StatisticsEdges",
            "StatisticsStrength", "StatisticsCloseness",
            "StatisticsBetweenness", "StatisticsBetweenness",
        ...
        if (!any(unlist(change[optsForBootstrap]))) response[["status"]] <- "ok"
    ...
    if (options[["BootstrapType"]] == "jacknife")
      ...
      noTicks <- options[["numberOfBootstraps"]] * nGraphs

    progressbar <- .newProgressbar(
        ticks = noTicks * 2,  # times two because there are two possibly
                              # parallel loops
        callback = callbackBootstrap, response = TRUE, parallel = nCores > 1)
    ...
    for (nm in names(allNetworks)) {
      ...
      network[["bootstrap"]][[nm]] <- .networkAnalysisBootnetBootnet(
          # bootnet arguments
          data = allNetworks[[nm]], nBoots = options[["numberOfBootstraps"]],
          type = options[["BootstrapType"]], nCores = nCores,
          statistics = statistics, labels = variables,
          # progress bar variables
          progressbar = progressbar, resultsForProgressbar = NULL,  # results,
...
.networkAnalysisGeneralTable <- function(network, dataset, options, perform) {
  ...
  table <- list(title = "Summary of Network",
                schema = list(fields = list(list(name = "info", title = "",
                                                 type = "string")#list(name = "value", title = "", type = "number", format="sf:4;dp:3")
                                                                 )))
  for (i in seq_len(nGraphs)) {
    table[["schema"]][["fields"]][[i + 1]] <- list(
        name = paste0("value", i), title = names(network[["network"]])[i],
  ...
  TBcolumns <- list(
      info = c("Number of nodes", "Number of non-zero edges", "Sparsity"))
  ...
  if (is.null(network[["network"]])) {  # fill in with .
    ...
    if (options[["estimator"]] %in% c("IsingFit", "IsingSampler") &&
      ...
      msg <- c(msg, sprintf("Data was binarized using %s. ", options$split))
    ...
    if (!is.null(options[["colorNodesByData"]]) &&
      ...
      msg <- c(
          msg,
          sprintf(
              "Only the first %d values of %s were used to color nodes (%d provided). ",
              length(options[["variables"]]), options[["colorNodesBy"]],
              length(options[["colorNodesByData"]])))
  ...
  if (!is.null(options[["layoutMessage"]])) {
    .addFootnote(footnotes = footnotes, text = options[["layoutMessage"]],
                 symbol = "<em>Warning: </em>")
  }
  if (!is.null(options[["colorNodesByDataMessage"]])) {
    .addFootnote(
        footnotes = footnotes, text = options[["colorNodesByDataMessage"]],
        symbol = "<em>Warning: </em>")
  }
...
.networkAnalysisBootstrapTable <- function(network, dataset, options, perform) {
  ...
  substr(bootstrapType, 1, 1) <- toupper(
      substr(bootstrapType, 1, 1))  # capitalization
  ...
  table <- list(
      ...
      schema = list(
          fields = list(list(name = "type", title = "Type", type = "string"),
                        list(name = "numberOfBootstraps",
                             title = "Number of bootstraps",
                             type = "integer"),  # type = "number",
                                                 # format="sf:4;dp:3"),
                        list(name = "when", title = "Status", type = "string"),
                        list(name = "start", title = "Start time",
                             type = "string"),
                        list(name = "ETA", title = "Expected end time",
                             type = "string"))),
      data = list(list(type = bootstrapType, numberOfBootstraps = nBoot,
                       when = "In progress")))
  ...
  if (is.null(network[["bootstrap"]])) {
    ...
    if (nCores == 0)  # possible if nCores == 1, avoid div / 0.
      nCores = 1
    duration <- network[["network"]][[1]][["timing"]] *
        nGraphs * (1 + ceiling(nBoot / nCores))
    duration <- 60 * ceiling(duration / 60)  # round up to nearest minute

    table[["data"]][[1]][["start"]] <- format(Sys.time(), format = timeFormat)
    table[["data"]][[
        1]][["ETA"]] <- format(Sys.time() + duration, format = timeFormat)
...
.networkAnalysisMgmVariableInfoTable <- function(
    network, variables, options, perform) {
  ...
  types <- c("Gaussian", "Categorical", "Poisson")[
      match(options[["mgmVariableTypeData"]], c("g", "c", "p"))]
...
.networkAnalysisCentralityTable <- function(network, options, perform) {
  ...
  table <- list(
      ...
      schema = list(fields = list(list(name = "Variable", title = "Variable",
                                       type = "string"))))
  ...
  for (i in seq_len(nGraphs)) {
    ...
    table[["schema"]][["fields"]][[2 + 3 * (i - 1)]] <- list(
        name = paste0("Betweenness", i), title = "Betweenness", type = "number",
        format = "sf:4;dp:3", overTitle = overTitles[i])
    table[["schema"]][["fields"]][[3 + 3 * (i - 1)]] <- list(
        name = paste0("Closeness", i), title = "Closeness", type = "number",
        format = "sf:4;dp:3", overTitle = overTitles[i])
    table[["schema"]][["fields"]][[4 + 3 * (i - 1)]] <- list(
        name = paste0(nameCol3, i), title = "Strength", type = "number",
        format = "sf:4;dp:3", overTitle = overTitles[i])
  ...
  if (is.null(network[["centrality"]])) {  # make empty table
    ...
    if (!is.null(options[["variables"]]) ||
      ...
      TBcolumns <- data.frame(
          .v(options[["variables"]]), rep(".", length(options[["variables"]])),
    ...
    for (i in seq_len(nGraphs)) {
      ...
      names(toAdd) <- c(
          "Variable", paste0(c("Betweenness", "Closeness", "Strength"), i))
...
.networkAnalysisWeightMatrixTable <- function(
    network, options, variables, perform) {
  ...
  for (i in seq_len(nGraphs)) {
    for (v in seq_len(nVar)) {  # loop is not entered if nVar == 0
      table[["schema"]][["fields"]][[1 + v + nVar * (i - 1)]] <- list(
          name = paste0(variables[v], i), title = variables[v], type = "number",
          format = "sf:4;dp:3", overTitle = overTitles[i])
    }
  }
  ...
  if (is.null(network[["network"]])) {  # make empty table
    ...
    if (is.null(options[["variables"]]) ||
        !length(options[["variables"]]) > 0) {  # 1 by 1 table
    ...
    for (i in seq_len(nGraphs)) {
      ...
      if (!options[["weightedNetwork"]]) toAdd <- sign(toAdd)
      if (!options[["signedNetwork"]]) toAdd <- abs(toAdd)
...
.networkAnalysisLayoutTable <- function(network, options, variables, perform) {
  ...
  table <- list(
      title = "Layout",
      schema = list(fields = list(list(name = "x", title = "x", type = "number",
                                       format = "sf:4;dp:3"),
                                  list(name = "y", title = "y", type = "number",
                                       format = "sf:4;dp:3"))))
  ...
  if (is.null(network[["layout"]])) {  # make empty table
    ...
    if (options[["tableLayoutValuesOnly"]]) {
      ...
      TBcolumns <- list(
          x = network[["layout"]][, 1], y = network[["layout"]][, 2])
    }
  }
...
# functions for plots ----
.networkAnalysisCentralityPlot <- function(
    network, options, perform, oldPlot = NULL) {

  if (!is.null(oldPlot) && !identical(oldPlot[["data"]], "")) return(oldPlot)

  plot <- list(
      title = "Centrality Plot", width = options[["plotWidthCentrality"]],
      height = options[["plotHeightCentrality"]],
      custom = list(
          width = "plotWidthCentrality", height = "plotHeightCentrality"),
  ...
  if (perform == "run" && !is.null(network)) {
    ...
    if (length(wide) > 1) {
      wideDf[["type"]] <- rep(
          names(network[["centrality"]]), each = nrow(wideDf) / length(wide))
    ...
    if (options[["abbreviateLabels"]])
      Long[["node"]] <- base::abbreviate(
          Long[["node"]], options[["abbreviateNoChars"]])
    ...
    if (length(unique(Long$graph)) > 1) {
      g <- ggplot2::ggplot(Long, ggplot2::aes(x = value, y = node,
                                              group = graph, colour = graph)) +
          ggplot2::guides(
              color = ggplot2::guide_legend(
                  title = options[["groupingVariable"]]))  # change the name
                                                           # graph into the
                                                           # variable name for
                                                           # splitting
    } else {
      g <- ggplot2::ggplot(
          Long, ggplot2::aes(x = value, y = node, group = graph))
    }
    ...
    content <- .writeImage(
        width = plot[["width"]], height = plot[["height"]], plot = g)
  ...
  eval(quote(
           {
             ...
             qgraph::qgraph(
                 input = wMat, layout = layout,  # options[["layout"]],
                 groups = groups,
                 repulsion = options[["repulsion"]],  # redundant
                 cut = options[["cut"]], edge.width = options[["edgeSize"]],
                 ...
                 shape = shape, color = nodeColor, edge.color = edgeColor,
                 nodeNames = nodeNames, label.scale = options[["scaleLabels"]],
                 label.cex = options[["labelSize"]])
           }), envir = parent.frame(2))

}

.networkAnalysisNetworkPlot <- function(
    network, options, perform, oldPlot = NULL) {


  if (!is.null(oldPlot) &&
      !identical(oldPlot[["collection"]][[1]][["data"]], "")) return(oldPlot)

  plot <- list(title = "Network Plot", collection = list(), status = "complete")
  ...
  if (options[["graphSize"]] == "graphSizeFree") {
    ...
    subPlotBase <- list(width = options[["plotHeightNetwork"]],
                        height = options[["plotHeightNetwork"]],
                        custom = list(width = "plotHeightNetwork",
                                      height = "plotHeightNetwork"))
  }
  ...
  if (perform == "run" && !is.null(network)) {
    ...
    if (minE == 0) minE <- NULL
    if (maxE == 0 || (!is.null(minE) && maxE <= minE)) maxE <- NULL
    ...
    if (!is.null(options[["colorNodesByData"]])) {  # define groups
      ...
      groups <- lapply(
          u, function(x, y) which(y == x), y = options[["colorNodesByData"]])
      names(groups) <- u
    ...
    for (v in names(allNetworks)) {
      ...
      content <- .writeImage(
          width = subPlot[["width"]], height = subPlot[["height"]],
          plot = .networkAnalysisOneNetworkPlot)
...
.networkAnalysisBootstrapPlot <- function(
    network, options, perform, statistic, oldPlot = NULL) {

  if (!is.null(oldPlot) &&
      !identical(oldPlot[["collection"]][[1]][["data"]], "")) return(oldPlot)

  plot <- list(
      title = "Edge Stability", collection = list(), status = "complete")
  if (!("edge" %in% statistic)) plot[["title"]] <- "Centrality Stability"

  subPlotBase <- list(width = options[["plotWidthBootstrapPlot"]],
                      height = options[["plotHeightBootstrapPlot"]],
                      custom = list(width = "plotWidthBootstrapPlot",
                                    height = "plotHeightBootstrapPlot"))
  ...
  if (perform == "run" && !is.null(network)) {
    ...
    for (v in names(allBootstraps)) {
      ...
      content <- .writeImage(
          width = subPlot[["width"]], height = subPlot[["height"]],
          plot = plot(bt, statistic = statistic, order = "sample")  # returns a
                                                                    # ggplot
                                                                    # object
              )
...
.networkAnalysisAddReferencesToTables <- function(results, options) {
  ...
  for (nm in namesOfTables) {  # use names to index
    if (!is.null(results[[nm]])) {  # if table is not empty add reference
      ...
      results[[nm]][["citation"]] <- as.list(
          .networkAnalysisBootnetGetRefs(options[["estimator"]]))
    }
  }
...
.TBcolumns2TBrows <- function(infoByCol)
  do.call(mapply,
          c(FUN = base::list, infoByCol, SIMPLIFY = FALSE, USE.NAMES = FALSE))
...
.networkAnalysisNetworkHasErrors <- function(network) {
  ...
  allNetworks <- network[["network"]]
  if (any(sapply(allNetworks, function(x) anyNA(x[["graph"]])))) return("error")

  return("success")

}

.networkAnalysisFindDataType <- function(
    variables, variableMatch, asNumeric = FALSE, char = "=",
    inputCheck = NULL) {
  ## Input:
  ...
  if (!all(lens == 2)) {
    errors <- c(
        errors,
        list(list(matches = unlist(matches[lens != 2]), type = "missingChar",
                  message = sprintf(
                      "Some variables did not contain char (%s)", char))))
  ...
  if (any(trouble)) {  # TRUE if any variable did not appear in any matches
    ...
    errors <- c(
        ...
        list(
            list(
                matches = unlist(matches[trouble, ]), type = "missingMatch",
                message = sprintf(
                    "Some variables were not matched in the column names of the dataset."))))
  ...
  if (asNumeric) {
    ...
    if (anyNA(values)) {
      errors <- c(
          ...
          list(
              list(
                  matches = matches[is.na(values), ], type = "missingNumeric",
                  message = sprintf(
                      "Some variables were not matched in the column names of the dataset."))))
    }
  }
...
.networkAnalysisSanitizeMgmVariableType <- function(variables, options) {
  ...
  checks <- .networkAnalysisFindDataType(
      variables = variables, variableMatch = options[["mgmVariableTypeData"]],
      char = "=", inputCheck = function(x) x %in% c("g", "p", "c"))

  if (checks[["errors"]][["fatal"]]) {
    message <- paste0(
        "Data supplied in ", options[["mgmVariableType"]],
        " cannot be used to determine variables types. ", "Data should: ",
...
.networkAnalysisSanitizeLayoutData <- function(variables, options) {
  ...
  checksX <- .networkAnalysisFindDataType(
      variables = variables, variableMatch = options[["layoutXData"]],
      char = "=", inputCheck = Negate(is.na), asNumeric = TRUE)
  checksY <- .networkAnalysisFindDataType(
      variables = variables, variableMatch = options[["layoutYData"]],
      char = "=", inputCheck = Negate(is.na), asNumeric = TRUE)

  message <- NULL
  defMsg <-
      "Supplied data for layout was not understood and instead a circle layout was used."
  if (checksX[["errors"]][["fatal"]] || checksX[["errors"]][["fatal"]]) {

    if (checksX[["errors"]][["fatal"]] && checksX[["errors"]][["fatal"]]) {
      firstLine <- sprintf(
          "Data supplied in %s AND %s could not be used to determine node locations.",
          options[["layoutX"]], options[["layoutY"]])
    } else if (checksX[["errors"]][["fatal"]]) {
      firstLine <- sprintf(
          "Data supplied in %s could not be used to determine node locations.",
          options[["layoutX"]])
    } else {
      firstLine <- sprintf(
          "Data supplied in %s could not be used to determine node locations.",
    ...
    message <- sprintf(
        ...
        defMsg, firstLine)
  } else if (length(checksX[["unmatched"]]) > 0 ||
             length(checksY[["unmatched"]]) > 0) {
    ...
    if (unmatchedX != "") {
      if (length(checksX[["unmatched"]]) > 1) {
        message <- paste(
            message,
            sprintf("X-Coordinates for variables %s were not understood.",
                    unmatchedX))
      } else {
        message <- paste(
            message, sprintf(
                         "X-Coordinates for variable %s was not understood.",
                         unmatchedX))
      }
    }
    if (unmatchedY != "") {
      if (length(checksY[["unmatched"]]) > 1) {
        message <- paste(
            message,
            sprintf("Y-Coordinates for variables %s were not understood.",
                    unmatchedY))
      } else {
        message <- paste(
            message, sprintf(
                         "Y-Coordinates for variable %s was not understood.",
                         unmatchedY))
      }
    }
...
.networkAnalysisSanitizeColorNodesByData <- function(variables, options) {
  ...
  checks <- .networkAnalysisFindDataType(
      variables = variables, variableMatch = options[["colorNodesByData"]],
      char = "=", inputCheck = Negate(is.na))
  message <- NULL
  if (checks[["errors"]][["fatal"]]) {
    message <- sprintf(
        "Data supplied in %s could not be used to determine variables types. Data should:
                 Start with the column name of the variable.
  ...
  if (length(checks[["unmatched"]] > 0)) {
    ...
    message <- sprintf(
        "Some entries of %s were not understood. These are now grouped under '%s'.",
...
.networkAnalysisBootnetBootnet <- function(
    data, nBoots = 1000,
    default = c("none", "EBICglasso", "pcor", "IsingFit", "IsingSampler",
                "huge", "adalasso", "mgm", "relimp", "cor"),
    type = c(
        "nonparametric", "parametric", "node", "person", "jackknife", "case"),
    nCores = 1, statistics = c("edge", "strength", "closeness", "betweenness"),
    model = c("detect", "GGM", "Ising"), fun, prepFun, prepArgs, estFun,
    estArgs, graphFun, graphArgs, intFun, intArgs, verbose = TRUE,
    construct = c("default", "function", "arguments"), labels, alpha = 1,
    caseMin = 0.05, caseMax = 0.75, caseN = 10, subNodes = 2:(ncol(data) - 1),
    subCases = round((1 - seq(caseMin, caseMax, length = caseN)) * nrow(data)),
    computeCentrality = TRUE, propBoot = 1, replacement = TRUE, graph,
    sampleSize, intercepts, weighted, signed, directed, ..., progressbar = NULL,
    resultsForProgressbar = NULL, callback = NULL
                                                 ) {
  ...
  if (missing(data)) {
    if (type != "parametric") {
      warning(
          "'data' can only be missing if type = 'parametric'. Setting type = 'parametric' and performing parametric bootstrap instead.")
      type <- "parametric"
    }
    if (missing(graph)) {
      stop(
    ...
    if (missing(sampleSize)) {
      stop(
          "'sampleSize' may not be missing in parametric bootstrap when 'data' is missing.")
    }
    N <- ncol(graph)
    Np <- sampleSize
    ...
    if (is(data, "bootnetResult")) {
      ...
      if (!manual) {
        goodColumns <- sapply(
            data, function(x) is.numeric(x) | is.ordered(x) | is.integer(x))
        if (!all(goodColumns)) {
          if (verbose) {
  ...
  inputCheck <- bootnet:::checkInput(
      default = default, fun = fun, prepFun = prepFun, prepArgs = prepArgs,
      estFun = estFun, estArgs = estArgs, graphFun = graphFun,
      graphArgs = graphArgs, intFun = intFun, intArgs = intArgs,
      sampleSize = Np, construct = construct, .dots = dots)
  if (missing(weighted)) {
    weighted <- TRUE
  }
  if (missing(signed)) {
  ...
  if (type == "parametric" & model == "detect") {
    ...
    if (default != "none") {
      model <- ifelse(
          grepl("ising", default, ignore.case = TRUE), "Ising", "GGM")
    } else {
      model <- ifelse(any(grepl("ising", deparse(estFun), ignore.case = TRUE)),
                      "Ising", "GGM")
    }
  ...
  if (!manual) {
    ...
    sampleResult <- bootnet::estimateNetwork(
        data, default = default, fun = inputCheck$estimator,
        .dots = inputCheck$arguments, labels = labels, verbose = verbose,
        weighted = weighted, signed = signed, .input = inputCheck)
  } else {
    sampleResult <- list(
        graph = graph, intercepts = intercepts, labels = labels, nNodes = N,
        nPerson = Np, estimator = inputCheck$estimator,
  ...
  if (nCores == 1) {
      ...
      repeat  {
        if (!type %in% c("node", "person")) {
          ...
          if (type == "jackknife") {
            ...
            nPerson <- Np
            if (model == "Ising") {
              bootData <- IsingSampler::IsingSampler(
                  round(propBoot * Np), noDiag(sampleResult$graph),
                  sampleResult$intercepts)
            } else if (model == "GGM") {
              g <- -sampleResult$graph
              diag(g) <- 1
              bootData <- mvtnorm::rmvnorm(
                  round(propBoot * Np), sigma = corpcor::pseudoinverse(g))
            } else
              stop(paste0("Model '", model, "' not supported."))
          } else {
            nPerson <- Np
            bootData <- data[sample(seq_len(Np), round(propBoot * Np),
                                    replace = replacement), ]
          }
        } else if (type == "node") {
          nPerson <- Np
        ...
        if (!missing(prepFun)) {
          if (!missing(prepArgs) &
              is.list(prepArgs) & identical(prepFun, qgraph::cor_auto)) {
        ...
        res <- suppressWarnings(
            try({
                  bootnet::estimateNetwork(
                      bootData, default = default, fun = inputCheck$estimator,
                      .dots = inputCheck$arguments, labels = labels[inSample],
                      verbose = FALSE, weighted = weighted, signed = signed,
                      .input = inputCheck, memorysaver = TRUE)
                }))
        if (is(res, "try-error")) {
    ...
    setup <- .makeParallelSetup(
        pb = progressbar, objs = objToExport, env = environment())
    ...
    bootResults <- tryCatch(
        foreach::foreach(
            b = seq_len(nBoots), .options.snow = setup$progress,
            .inorder = FALSE, # .export = objToExport,
    ...
    if (inherits(bootResults, "warning")) {
      if (bootResults$message ==
          "progress function failed: Cancelled by callback") {
        return()
      }
    }
  }
  ...
  statTableOrig <- bootnet:::statTable(
      sampleResult, name = "sample", alpha = alpha,
      computeCentrality = computeCentrality, statistics = statistics,
      directed = directed)
  if (nCores == 1) {
    if (verbose) {
      pb <- txtProgressBar(0, nBoots, style = 3)
    ...
    for (b in seq_len(nBoots)) {
      statTableBoots[[b]] <- bootnet:::statTable(
          bootResults[[b]], name = paste("boot", b), alpha = alpha,
          computeCentrality = computeCentrality, statistics = statistics,
          directed = directed)
      response <- progressbar(resultsForProgressbar)
      if (response[["status"]] != "ok") return()
    ...
    objToExport <- c(
        "bootResults", "alpha", "computeCentrality", "statistics", "directed")
    snow::clusterExport(cl = cl, list = objToExport, envir = environment())
    statTableBoots <- tryCatch(
        foreach::foreach(
            b = seq_len(nBoots), .options.snow = setup$progress,
            .inorder = FALSE, # .export = objToExport,
                              .packages = c("bootnet"), .verbose = TRUE) %dopar%
        {
          bootnet:::statTable(
              bootResults[[b]], name = paste("boot", b), alpha = alpha,
              computeCentrality = computeCentrality, statistics = statistics,
  ...
  Result <- list(
      ...
      sample = sampleResult, boots = bootResults, type = type, sampleSize = Np)
