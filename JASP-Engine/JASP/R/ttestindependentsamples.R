...
TTestIndependentSamples <- function(
  ...
  init <- .initializeTTest(
      dataset, options, perform, type = "independent-samples")
  ...
  descriptivesTable <- .ttestIndependentSamplesDescriptives(
      dataset, options, perform)
  levene <- .ttestIndependentSamplesInequalityOfVariances(
      dataset, options, perform)
  shapiroWilk <- .ttestIndependentSamplesNormalityTest(
      dataset, options, perform)
  results[["assumptionChecks"]] <- list(
      shapiroWilk = shapiroWilk, levene = levene, title = "Assumption Checks")
  ...
  if (options$descriptivesPlots && length(options$variables) > 0) {
    ...
    plotTitle <- ifelse(length(options$variables) > 1, "Descriptives Plots",
                        "Descriptives Plot")
    descriptivesPlots <- .independentSamplesTTestDescriptivesPlot(
        dataset, options, perform)
    ...
    results[["descriptives"]] <- list(
        ...
        descriptivesPlots = list(
            collection = descriptivesPlots, title = plotTitle))
  ...
  if (perform == "init") {
    ...
    return(
        list(results = results, status = "complete",
             state = list(options = options, results = results), keep = keep))
  }
}
...
.ttestIndependentSamplesTTest <- function(dataset, options, perform) {
  ...
  wantsConfidenceMeanDiff <- (options$meanDiffConfidenceIntervalCheckbox &&
                              options$meanDifference)
  wantsConfidenceEffSize <- (options$effSizeConfidenceIntervalCheckbox &&
                             options$effectSize)
  ...
  if (wantsWilcox && onlyTest) {
    .addFootnote(
        footnotes, symbol = "<em>Note.</em>", text = "Mann-Whitney U test.")
    testStat <- "W"
    ...
    .addFootnote(
        footnotes, symbol = "<em>Note.</em>", text = "Student's t-test.")
    testStat <- "t"
  } else {
    testStat <- "Statistic"
  }
  ...
  if (wantsDifference) {
    fields[[length(fields) + 1]] <- list(
        name = "md", title = nameOfLocationParameter, type = "number",
        format = "sf:4;dp:3")
    if (!(wantsWilcox && onlyTest)) {  # Only add SE Difference if not only
                                       # MannWhitney is requested
      fields[[length(fields) + 1]] <- list(
          name = "sed", title = "SE Difference", type = "number",
          format = "sf:4;dp:3")
    }
  }
  if (wantsDifference && wantsWilcox && wantsStudents && wantsWelchs) {
    .addFootnote(
        ...
        text = "For the Student t-test and Welch t-test,
  ...
  if (wantsConfidenceMeanDiff) {
    ...
    fields[[length(fields) + 1]] <- list(
        name = "lowerCIlocationParameter", type = "number",
        format = "sf:4;dp:3", title = "Lower", overTitle = title)
    fields[[length(fields) + 1]] <- list(
        name = "upperCIlocationParameter", type = "number",
        format = "sf:4;dp:3", title = "Upper", overTitle = title)
  }
  ...
  if (wantsConfidenceEffSize) {
    ...
    fields[[length(fields) + 1]] <- list(
        name = "lowerCIeffectSize", type = "number", format = "sf:4;dp:3",
        title = "Lower", overTitle = title)
    fields[[length(fields) + 1]] <- list(
        name = "upperCIeffectSize", type = "number", format = "sf:4;dp:3",
  ...
  if (ready) {
    ...
    if (options$hypothesis == "groupOneGreater") {
      ...
      message <- paste0(
          ...
          levels[1], "</em> is greater than group <em>", levels[2], "</em>.")
      ...
      message <- paste0(
          "For all tests, the alternative hypothesis specifies that group  <em>",
          levels[1], "</em> is less than group <em>", levels[2], "</em>.")
      .addFootnote(footnotes, symbol = "<em>Note.</em>", text = message)
    ...
    rowNo <- 1
    whichTests <- list(
        "1" = wantsStudents, "2" = wantsWelchs, "3" = wantsWilcox)
    ...
    for (variable in options$variables) {
      ...
      for (test in seq_len(length(whichTests))) {
        ...
        if (!identical(errors, FALSE)) {
          ...
          row <- try(
              ...
              expr = {
                ...
                f <- as.formula(
                    paste(.v(variable), "~", .v(options$groupingVariable)))
                ...
                if (test == 3) {
                  ...
                  if (direction == "two.sided") {
                    confIntEffSize <- sort(
                        c(tanh(zRankBis +
                               qnorm((1 - ciEffSize) / 2) * rankBisSE),
                          tanh(zRankBis +
                               qnorm((1 + ciEffSize) / 2) * rankBisSE)))
                  } else if (direction == "less") {
                    confIntEffSize <- sort(
                        c(-Inf, tanh(zRankBis + qnorm(ciEffSize) * rankBisSE)))
                  } else if (direction == "greater") {
                    confIntEffSize <- sort(
                        c(tanh(zRankBis + qnorm((1 - ciEffSize)) * rankBisSE),
                          Inf))
                  }
                } else {
                  ...
                  r <- stats::t.test(f, data = dataset, alternative = direction,
                                     var.equal = test != 2,
                                     conf.level = ciMeanDiff, paired = FALSE)
                  ...
                  d <- .clean(as.numeric((ms[1] - ms[2]) / sdPooled))  # Cohen's
                                                                       # d
                  sed <- .clean((as.numeric(r$estimate[1]) -
                                 as.numeric(r$estimate[2])) / stat)
                  ...
                  if (wantsConfidenceEffSize) {
                    ...
                    ncp1 <- uniroot(function(x) alphaLevels[1] -
                                                pt(q = stat, df = df, ncp = x),
                    ...
                    ncp2 <- uniroot(function(x) alphaLevels[2] -
                                                pt(q = stat, df = df, ncp = x),
                                    c(end2, 2 * stat - end2))$root

                    confIntEffSize =
                        sort(c(ncp1 * sqrt(1 / ns[1] + 1 / ns[2]),
                               ncp2 * sqrt(1 / ns[1] + 1 / ns[2])))[
                            order(c(1 - ciEffSize, ciEffSize))]
                    if (direction == "greater") {
                      confIntEffSize[2] <- Inf
                    } else if (direction == "less") confIntEffSize[1] <- -Inf
                ...
                res <- list(v = variable, test = whatTest, df = df, p = p,
                            md = m, d = d, lowerCIlocationParameter = ciLow,
                            upperCIlocationParameter = ciUp,
                            lowerCIeffectSize = lowerCIeffectSize,
                            upperCIeffectSize = upperCIeffectSize, sed = sed,
        ...
        if (!is.null(errorMessage)) {
          ...
          row <- list(
              v = variable, test = "", df = "", p = "", md = "", d = "",
              lowerCIlocationParameter = "", upperCIlocationParameter = "",
              lowerCIeffectSize = "", upperCIeffectSize = "", sed = "",
...
.ttestIndependentSamplesDescriptives <- function(
      ...
      if (isAllright(variable, options, state, diff)) {
        ...
        for (i in 1:2) {
          ...
          if (class(groupDataOm) != "factor") {
            ...
            result <- list(variable = variable, group = level, N = n,
                           mean = mean, sd = std, se = sem)
...
.ttestIndependentSamplesInequalityOfVariances <- function(
    dataset, options, perform) {
      ...
      result <- try(
          ...
          expr = {
            ...
            levene <- car::leveneTest(
                dataset[[.v(variable)]], dataset[[.v(groups)]], "mean")
...
.ttestIndependentSamplesNormalityTest <- function(dataset, options, perform) {
  ...
  for (variable in variables) {
    ...
    for (level in levels) {
      ...
      if (perform == "run" &&
          length(variables) > 0 && !is.null(levels) && factor != "") {
        ...
        errors <- .hasErrors(
            dataset, perform, message = 'short',
            type = c('observations', 'variance', 'infinity'),
            all.target = variable, observations.amount = c('< 3', '> 5000'),
            all.grouping = factor, all.groupingLevel = level)
        ...
        if (!error) {
          ...
          result <- list(
              dep = variable, lev = level, W = W, p = p, .isNewGroup = newGroup)

          ## if there was a problem, foonote it
        } else {

          newGroup <- level == levels[1]
          result <- list(dep = variable, lev = level, W = "NaN", p = "NaN",
...
.independentSamplesTTestDescriptivesPlot <- function(
    dataset, options, perform) {
  ...
  if (perform == "run" && length(variables) > 0 && groups != "") {
    ...
    for (variableIndex in .indices(variables)) {
      ...
      errors <- .hasErrors(
          dataset, perform, message = 'short',
          type = c('observations', 'variance', 'infinity'),
          all.target = variables[variableIndex], observations.amount = '< 2',
      ...
      if (!identical(errors, FALSE)) {
        ...
        summaryStat <- .summarySE(
            ...
            measurevar = .v(options$variables[variableIndex]),
            groupvars = .v(options$groupingVariable),
            conf.interval = options$descriptivesPlotsConfidenceInterval,
            na.rm = TRUE, .drop = FALSE)

        colnames(summaryStat)[
            which(colnames(summaryStat) == .v(variables[variableIndex]))] <-
            "dependent"
        colnames(summaryStat)[which(colnames(summaryStat) == .v(groups))] <-
            "groupingVariable"
        ...
        p <- ggplot2::ggplot(
                 summaryStat,
                 ggplot2::aes(x = groupingVariable, y = dependent, group = 1)) +
            ggplot2::geom_errorbar(
                ggplot2::aes(ymin = ciLower, ymax = ciUpper), colour = "black",
                width = 0.2, position = pd) +
            ggplot2::geom_line(position = pd, size = 0.7) +
            ggplot2::geom_point(position = pd, size = 4) +
            ggplot2::ylab(unlist(options$variables[variableIndex])) +
            ggplot2::xlab(options$groupingVariable) + ggplot2::theme_bw() +
            ggplot2::theme(
