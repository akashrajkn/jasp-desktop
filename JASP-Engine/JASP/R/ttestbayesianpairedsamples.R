...
TTestBayesianPairedSamples <- function(
  ...
  for (pair in options$pairs) {
    ...
    if (options$descriptivesPlots) {
      ...
      if (!is.null(state) &&
          currentPair %in% state$descriptPlotPairs &&
          !is.null(diff) &&
          ((is.logical(diff) && diff == FALSE) ||
           (is.list(diff) && (diff$missingValues == FALSE &&
  ...
  if (options$plotPriorAndPosterior ||
    ...
    results[["inferentialPlots"]] <- list(
        title = ifelse(length(options$pairs) > 1 ||
  ...
  if (options$descriptivesPlots)
    results[["descriptives"]][["descriptivesPlots"]] <- list(
        title = ifelse(length(options$pairs) > 1, "Descriptives Plots",
  ...
  for (i in .indices(options$pairs)) {
    ...
    if (pair[[1]] == "" || pair[[2]] == "") {
      ...
      pair.statuses[[
          i]] <- list(ready = FALSE, error = FALSE, unplotable = TRUE)

      result <- list(.variable1 = p1, .separator = "-", .variable2 = p2,
                     BF = "", error = "")
      ...
      if (perform == "init") {
        ...
        if (!is.null(state) &&
          ...
          if (state$errorFootnotes[stateIndex] == "no") {
            ...
            result <- list(.variable1 = pair[[1]], .separator = "-",
                           .variable2 = pair[[2]], BF = .clean(NaN), error = "",
        ...
        if (!is.null(state) &&
          ...
          if (is.null(errorMessage)) {
            ...
            result <- try(
                ...
                expr = {
                  ...
                  r <- .generalTtestBF(x = c1, y = c2, paired = TRUE,
                  ...
                  list(.variable1 = pair[[1]], .separator = "-",
                       .variable2 = pair[[2]], BF = BF, error = error)
          ...
          if (is.null(errorMessage)) {
            ...
            pair.statuses[[i]] <- list(
                ready = FALSE, error = TRUE, errorMessage = errorMessage,
                unplotable = TRUE, unplotableMessage = unplotableMessage)
  ...
  if (length(ttest.rows) == 0)
    ttest.rows <- list(list(.variable1 = "...", .separator = "-",
                            .variable2 = "...", BF = "", error = ""))

  ttest[["data"]] <- ttest.rows
  ttest[["footnotes"]] <- as.list(footnotes)

  if (perform == "run") ttest[["status"]] <- "complete"
  ...
  if (perform == "run" &&
      ...
      (options$plotPriorAndPosterior ||
       options$plotBayesFactorRobustness ||
       options$plotSequentialAnalysis || options$descriptivesPlots)) {

    if (!.shouldContinue(callback(results))) return()

    n.plots.per.variable <- sum(
        options$plotPriorAndPosterior, options$plotBayesFactorRobustness,
    ...
    for (i in .indices(options$pairs)) {
      ...
      if (options$descriptivesPlots) {
        ...
        if (!is.null(state) &&
            tablePairs[[i]] %in% state$descriptPlotPairs &&
            !is.null(diff) &&
            ((is.logical(diff) && diff == FALSE) ||
             (is.list(diff) && (diff$missingValues == FALSE &&
                                diff$plotWidth == FALSE &&
            ...
            p <- try(
                ...
                expr = {
                  ...
                  p <- .plot2GroupMeansBayesIndTtest(
                      v1 = c1, v2 = c2, nameV1 = pair[[1]], nameV2 = pair[[2]],
          ...
          if (!is.null(errorMessage)) {
            ...
            p <- try(
                ...
                expr = {
                  ...
                  .plotFunc <- function() {
                    .plotPosterior.summarystats.ttest(
                        t = tValue[i], n1 = n[i], n2 = n[i], paired = TRUE,
          ...
          if (!is.null(errorMessage)) {
            ...
            p <- try(
                ...
                expr = {
                  ...
                  .plotFunc <- function() {
                    .plotBF.robustnessCheck.ttest(
                        x = c1, y = c2, BF10post = BF10post[i], paired = TRUE,
                ...
                expr = {
                  ...
                  .plotFunc <- function() {
                    .plotSequentialBF.ttest(
                        x = c1, y = c2, paired = TRUE, oneSided = oneSided,
                        rscale = options$priorWidth, BF10post = BF10post[i],
                        plotDifferentPriors = options$plotSequentialAnalysisRobustness,
                        BFH1H0 = BFH1H0, options = options)
                  }
                  content <- .writeImage(
                      width = 530, height = 400, plot = .plotFunc, obj = TRUE)
    ...
    return(
        list(results = results, status = "complete",
             state = list(
                 ...
                 plotTypes = plotTypes, plotPairs = plotPairs,
                 descriptPlotPairs = descriptPlotPairs,
                 descriptivesPlots = descriptivesPlots,
                 pairStatuses = pair.statuses, BF10post = BF10post,
                 tablePairs = tablePairs, errorFootnotes = errorFootnotes,
                 tValue = tValue, n = n), keep = keep))
  }
}

.plot2GroupMeansBayesPairedTtest <- function(
    v1 = NULL, v2 = NULL, nameV1 = NULL, nameV2 = NULL,
  ...
  p <- ggplot2::ggplot(summaryStat, ggplot2::aes(x = groupingVariable,
      ...
      ggplot2::ylab(NULL) + ggplot2::xlab(NULL) + ggplot2::theme_bw() +
      ggplot2::theme(
      ...
      singleRow <- try(
          ...
          expr = {
            ...
            list(v = variable, N = n, mean = m, sd = std, se = se,
                 lowerCI = ciLower, upperCI = ciUpper)
          })

      if (isTryError(singleRow)) {
        singleRow <- list(v = variable, N = "", mean = "", sd = "", se = "",
                          lowerCI = "", upperCI = "")
      }
