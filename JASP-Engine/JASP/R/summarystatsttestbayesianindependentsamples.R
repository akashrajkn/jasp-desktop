...
SummaryStatsTTestBayesianIndependentSamples <- function(
  ...
  hypothesis.variables <- .hypothesisType.summarystats.ttest.independent(
      options$hypothesis)
  ...
  fields[[length(fields) + 1]] <- list(
      name = "tStatistic", type = "number", format = "sf:4;dp:3", title = "t")
  fields[[length(fields) + 1]] <- list(
      name = "n1Size", type = "number", title = "n\u2081")
  fields[[length(fields) + 1]] <- list(
      name = "n2Size", type = "number", title = "n\u2082")
  fields[[length(fields) + 1]] <- list(
  ...
  if (run) {
    status <- "complete"
    state <- list(
        options = options, bayesFactorObject = bayesFactorObject,
...
.getOutputRow.summarystats.ttest.independent <- function(
  ...
  if (!is.null(state) && !is.null(diff) && !is.null(state$bayesFactorObject) &&
    ...
    status <- .isInputValid.summarystats.ttest(
        options = options, independent = TRUE)
    ...
    if (run) {
      if (status$ready) {
        ...
        bayesFactorObject <- .generalSummaryTtestBF(
            options = options, paired = FALSE)
        ...
        rowsTTestBayesianIndependentSamples$errorEstimate <- .clean(
  ...
  return(list(row = rowsTTestBayesianIndependentSamples,
...
.hypothesisType.summarystats.ttest.independent <- function(hypothesis) {
  ...
  if (hypothesis == "groupOneGreater") {
    ...
    oneSided <- "right"
    message <- paste(
        "For all tests, the alternative hypothesis specifies that group 1 is greater than group 2",
        sep = "")
    ...
    oneSided <- "left"
    message <- paste(
        "For all tests, the alternative hypothesis specifies that group 1 is lesser than group 2",
        sep = "")
  }
