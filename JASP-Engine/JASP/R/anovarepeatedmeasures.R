...
AnovaRepeatedMeasures <- function(
  ...
  numeric.variables <- c(
      unlist(options$repeatedMeasuresCells), unlist(options$covariates))
  ...
  if (is.null(dataset)) {
    ...
    if (perform == "run") {
      ...
      dataset <- .readDataSetToEnd(
          ...
          exclude.na.listwise = c(numeric.variables, factor.variables))

    } else {

      dataset <- .readDataSetHeader(columns.as.numeric = numeric.variables,
                                    columns.as.factor = factor.variables)
    }

  } else {

    dataset <- .vdf(dataset, columns.as.numeric = numeric.variables,
                    columns.as.factor = factor.variables)
  }
  ...
  if (!is.null(state)) {  # is there state?
    ...
    if (is.list(diff) && diff[['withinModelTerms']] == FALSE &&
                         diff[['betweenModelTerms']] == FALSE &&
                         diff[['repeatedMeasuresCells']] == FALSE &&
                         diff[['repeatedMeasuresFactors']] == FALSE &&
                         diff[['sumOfSquares']] == FALSE &&
                         diff[['covariates']] == FALSE &&
                         diff[['betweenSubjectFactors']] == FALSE) {
    ...
    if (is.list(diff) &&
        diff[['plotHorizontalAxis']] == FALSE &&
        diff[['plotSeparateLines']] == FALSE &&
        diff[['plotSeparatePlots']] == FALSE &&
        diff[['plotErrorBars']] == FALSE &&
        diff[['labelYAxis']] == FALSE &&
        !(diff[['errorBarType']] == TRUE && options$plotErrorBars == TRUE) &&
        !(diff[['confidenceIntervalInterval']] == TRUE &&
          options$errorBarType == "confidenceInterval" &&
          options$plotErrorBars == TRUE) &&
        diff[['plotWidthDescriptivesPlotLegend']] == FALSE &&
        diff[['plotHeightDescriptivesPlotLegend']] == FALSE &&
        diff[['plotWidthDescriptivesPlotNoLegend']] == FALSE &&
        diff[['plotHeightDescriptivesPlotNoLegend']] == FALSE &&
        diff[['repeatedMeasuresFactors']] == FALSE &&
        diff[['repeatedMeasuresCells']] == FALSE) {
    ...
    if (is.list(diff) && diff[['betweenSubjectFactors']] == FALSE &&
                         diff[['repeatedMeasuresFactors']] == FALSE &&
                         diff[['repeatedMeasuresCells']] == FALSE &&
    ...
    if (is.list(diff) && diff[['withinModelTerms']] == FALSE &&
                         ...
                         diff[['repeatedMeasuresCells']] == FALSE &&
                         diff[['repeatedMeasuresFactors']] == FALSE &&
                         diff[['homogeneityTests']] == FALSE &&
                         diff[["VovkSellkeMPR"]] == FALSE) {
  ...
  if (is.null(anovaModel)) {  # if not retrieved from state
    ...
    if (perform == "run" && status$ready && status$error == FALSE) {
      ...
      if (is.null(fullModel) ||
          (length(class(fullModel)) == 1 && class(fullModel) == "try-error")) {
        ...
        statePostHoc <- .resultsPostHoc(
            referenceGrid, options, dataset, fullModel)
  ...
  result <- .rmAnovaWithinSubjectsTable(
      dataset, options, perform, model, stateSphericity, status)
  ...
  result <- .rmAnovaBetweenSubjectsTable(
      dataset, options, perform, model, status)
  ...
  if (is.null(stateLevene)) {
    ...
    result <- .rmAnovaLevenesTable(
        dataset, options, perform, status, stateLevene, model)
  ...
  results[["assumptionsObj"]] <- list(
      title = "Assumption Checks", sphericity = resultSphericity,
      levene = resultLevene)
  ...
  results[[
      "contrasts"]] <- list(collection = result$result, title = "Contrasts")
  status <- result$status



  ## Create Post Hoc Tables
  result <- .rmAnovaPostHocTable(
      dataset, options, perform, status, statePostHoc)

  results[[
      "posthoc"]] <- list(collection = result$result, title = "Post Hoc Tests")
  status <- result$status
  ...
  if (is.null(stateDescriptivesTable)) {
    ...
    result <- .rmAnovaDescriptivesTable(
        dataset, options, perform, status, stateDescriptivesTable)
  ...
  if (is.null(stateDescriptivesPlot)) {
    ...
    result <- .rmAnovaDescriptivesPlot(
        dataset, options, perform, status, stateDescriptivesPlot)
  ...
  if (length(descriptivesPlot) == 1) {
    ...
    results[["descriptivesObj"]] <- list(
        ...
        descriptivesPlot = list(
            collection = descriptivesPlot, title = titleDescriptivesPlot))
  }
  ...
  .meta <- list(list(name = "withinSubjectsEffects", type = "table"),
                list(name = "betweenSubjectsEffects", type = "table"),
                list(name = "assumptionsObj", type = "object",
                     meta = list(list(name = "sphericity", type = "table"),
...
.rmAnovaCheck <- function(dataset, options, perform) {
  ...
  ready <- "" %in% options$repeatedMeasuresCells == FALSE &&
      length(options$withinModelTerms) > 0
  ...
  if (ready && perform == "run") {
    ...
    for (component in components) {
      ...
      if (nLevels < 2) independentsWithLessThanTwoLevels <- c(
                           independentsWithLessThanTwoLevels, component)
    }
    ...
    if (length(independentsWithLessThanTwoLevels) > 0) {
      ...
      if (length(independentsWithLessThanTwoLevels) == 1) {
        errorMessage <- paste(
            "Factor: <em>", independentsWithLessThanTwoLevels,
            "</em>, contains fewer than two levels.", sep = "")
      } else {
        errorMessage <- paste(
            "Factors: <em>",
            paste(independentsWithLessThanTwoLevels, collapse = ",", sep = ""),
            "</em>, contain fewer than two levels.", sep = "")
      }
    }
    ...
    infiniteRM <- unlist(
        lapply(repeatedMeasuresData, function(x) sum(is.infinite(x)) > 0))

    if (!is.null(infiniteRM) && sum(infiniteRM) > 0) {

      error <- TRUE
      if (sum(infiniteRM) == 1) {
        errorMessage <- paste("The repeated measure: <em>",
                              options$repeatedMeasuresCells[infiniteRM],
                              "</em>, contains infinite values.", sep = "")
      } else {
        errorMessage <- paste(
            "The repeated measures: <em>",
            paste(options$repeatedMeasuresCells[infiniteRM], collapse = ", "),
            "</em>, contain infinite values.", sep = "")
      }
    }
    ...
    infiniteCov <- unlist(lapply(covariatesData, function(x)
                                                   sum(is.infinite(x)) > 0))
    ...
    if (!is.null(infiniteCov) && sum(infiniteCov) > 0) {
      ...
      if (sum(infiniteCov) == 1) {
        errorMessage <- paste(
            "The covariate: <em>", options$covariates[infiniteCov],
            "</em>, contains infinite values.", sep = "")
      } else {
        errorMessage <- paste(
            "The covariates: <em>",
            paste(options$covariates[infiniteCov], collapse = ", "),
            "</em>, contain infinite values.", sep = "")
      }
    }

    allNames <- unlist(lapply(options$repeatedMeasuresFactors,
                              function(x) x$name))  # Factornames
...
.rmModelFormula <- function(options) {
  ...
  for (i in 1:length(termsRM.base64)) {
    if (is.null(termsBS.base64)) {
      ...
      terms.base64[[i + 1]] <- c(
          termsRM.base64[i],
          paste(termsRM.base64[i], termsBS.base64, sep = ":"))
      terms.normal[[i + 1]] <- c(
          termsRM.normal[i],
          paste(termsRM.normal[i], termsBS.normal, sep = " \u273B "))
    }
  }

  main <- paste(
      "(", paste(unlist(terms.base64), collapse = " + "), ")", sep = "")
  termsBS <- paste("(", paste(termsBS.base64, collapse = " + "), ")", sep = "")
  errorRM <- paste("Error(", paste("subject/(", termsRM.base64, ")", sep = "",
                                   collapse = " + "), ")", sep = "")
  ...
  if (is.null(termsBS.base64) && is.null(termsRM.base64)) {
    ...
    model.def <- paste(
        "dependent", "~", paste(main, errorRM, termsBS, sep = " + "))
  }

  list(model.def = model.def, terms.normal = terms.normal,
       terms.base64 = terms.base64, termsRM.normal = termsRM.normal,
       termsRM.base64 = termsRM.base64)
}
...
.rmAnovaModel <- function(dataset, options, status) {
  ...
  dataset <- .shortToLong(
      dataset, options$repeatedMeasuresFactors, options$repeatedMeasuresCells,
      c(options$betweenSubjectFactors, options$covariates))

  variables <- unlist(c(options$betweenSubjectFactors,
                        lapply(options$repeatedMeasuresFactors, function(x)
                                                                  x$name)))

  for (i in variables) dataset[[.v(i)]] <- .v(dataset[[.v(i)]])
  ...
  if (options$sumOfSquares == "type1") {
    ...
    fullModel <- try(afex::aov_car(model.formula, data = dataset, type = 3,
                                   factorize = FALSE), silent = TRUE)

  } else if (options$sumOfSquares == "type2") {

    result <- try(afex::aov_car(model.formula, data = dataset, type = 2,
                                factorize = FALSE), silent = TRUE)

  } else {

    result <- try(afex::aov_car(model.formula, data = dataset, type = 3,
                                factorize = FALSE), silent = TRUE)
  }
  ...
  if (length(class(result)) == 1 && class(result) == "try-error") {
    ...
    if (status$errorMessage ==
      ...
      status$errorMessage <-
          "Some parameters are not estimable, most likely due to empty cells of the design."
    ...
    } else if (options$sumOfSquares == "type1" &&
               class(fullModel) == "try-error") {
  ...
  list(model = model, epsilon = epsilon, mauchly = mauchly,
       fullModel = fullModel, status = status)
}
...
.resultsSphericity <- function(options, epsilon, mauchly) {
  ...
  for (i in .indices(termsRM.base64)) {
    ...
    index <- unlist(lapply(modelTermsResults,
                           function(x) .identicalTerms(x, modelTermsCase)))
...
.resultsPostHoc <- function(referenceGrid, options, dataset, fullModel) {
  ...
  factorNamesV <- colnames(postHocData)  # Names to use to refer to variables in
                                         # data
  ...
  for (var in variables) {
    ...
    resultBonf <- summary(
        pairs(referenceGrid[[var]], adjust = "bonferroni"), infer = TRUE)
    ...
    orderOfTerms <- unlist(
        options$withinModelTerms[[length(options$withinModelTerms)]]$components)
    indexofOrderFactors <- match(allNames, orderOfTerms)

    if (any(var == allNames)) {  # If the variable is a repeated measures factor

      levelsOfThisFactor <- unlist(
          lapply(options$repeatedMeasuresFactors[rmFactorIndex],
                 function(x) x$levels))  # Levels within Factor
      numberOfLevels <- length(unique(levelsOfThisFactor))
      splitNames <- unlist(lapply(strsplit(factorNamesV, split = "_"),
                                  function(x)
                                    x[indexofOrderFactors[rmFactorIndex]]))

      listVarNamesToLevel <- list()  # create a list of vectors of variable
                                     # names, used to group the dataset for the
                                     # post-hoc t-tests
      for (i in 1:numberOfLevels) {
        listVarNamesToLevel[[i]] <- factorNamesV[
            grep(splitNames, pattern = .v(levelsOfThisFactor[i]))]
      }

      countr <- 1
      allEstimates <- allTees <- allSE <- allPees <- numeric()
      for (k in 1:numberOfLevels) {  # Loop over all the levels within factor
                                     # and do pairwise t.tests on them
        for (i in .seqx(k + 1, numberOfLevels)) {
          tResult <- t.test(unlist(postHocData[listVarNamesToLevel[[k]]]),
                            unlist(postHocData[listVarNamesToLevel[[i]]]),
      ...
      bonferPvals <- p.adjust(allPees, method = "bonferroni")  # correct all
                                                               # pvalues
                                                               # according to
                                                               # bonf
      resultGeneral <- list(estimate = allEstimates, t.ratio = allTees,
                            SE = allSE, p.value = bonferPvals)
      resultBonf['t.ratio'] <- allTees
      resultBonf['p.value'] <- bonferPvals
      resultBonf['SE'] <- allSE
      resultBonf['estimate'] <- allEstimates
      resultHolm['p.value'] <- p.adjust(
          allPees, method = "holm")  # correct all pvalues according to holm
    ...
    resultsPostHoc[[var]] <- list(
        resultBonf = resultBonf, resultHolm = resultHolm,
...
.resultsContrasts <- function(dataset, options, referenceGrid) {
  ...
  datasetLong <- .shortToLong(
      dataset, options$repeatedMeasuresFactors, options$repeatedMeasuresCells,
      options$betweenSubjectFactors)

  contrastTypes <- c("none", "deviation", "simple", "Helmert", "repeated",
...
.rmAnovaBetweenSubjectsTable <- function(
    dataset, options, perform, model, status) {
  ...
  fields[[length(fields) + 1]] <- list(
      name = "case", type = "string", title = "", combine = TRUE)
  fields[[length(fields) + 1]] <- list(
      name = "SS", type = "number", format = "sf:4;dp:3",
      title = "Sum of Squares")
  fields[[length(fields) + 1]] <- list(name = "df", type = "integer")
  fields[[length(fields) + 1]] <- list(
      name = "MS", type = "number", format = "sf:4;dp:3", title = "Mean Square")
  fields[[length(fields) + 1]] <- list(
      name = "F", type = "number", format = "sf:4;dp:3")
  fields[[length(fields) + 1]] <- list(
  ...
  if (options$effectSizeEstimates) {
    ...
    if (options$effectSizeEtaSquared) {
      fields[[length(fields) + 1]] <- list(
          name = "eta", type = "number", title = "\u03B7\u00B2",
          format = "dp:3")
    }
    if (options$effectSizePartialEtaSquared) {
      fields[[length(fields) + 1]] <- list(
          name = "partialEta", type = "number", title = "\u03B7\u00B2\u209A",
          format = "dp:3")
    }
    if (options$effectSizeOmegaSquared) {
      fields[[length(fields) + 1]] <- list(
          name = "omega", type = "number", title = "\u03C9\u00B2",
          format = "dp:3")
    }
  }
  ...
  if (options$sumOfSquares == "type1") {
    ...
    .addFootnote(
        footnotes, text = "Type I Sum of Squares", symbol = "<em>Note.</em>")

  } else if (options$sumOfSquares == "type2") {

    .addFootnote(
        footnotes, text = "Type II Sum of Squares", symbol = "<em>Note.</em>")

  } else if (options$sumOfSquares == "type3") {

    .addFootnote(
        footnotes, text = "Type III Sum of Squares", symbol = "<em>Note.</em>")
  ...
  if (is.null(model)) {
    ...
    if (length(terms.base64) > 0) {
      ...
      for (j in .indices(terms.base64[[1]])) {
        ...
        row <- list("case" = terms.normal[[1]][j], "SS" = ".", "df" = ".",
                    "MS" = ".", "F" = ".", "p" = ".", ".isNewGroup" = newGroup)
        if (options$VovkSellkeMPR) {
          row[["VovkSellkeMPR"]] <- "."
        }
    ...
    row <- list("case" = "Residual", "SS" = ".", "df" = ".", "MS" = ".",
                "F" = "", "p" = "", "eta" = "", "partialEta" = "", "omega" = "",
                ".isNewSubGroup" = TRUE)
    if (options$VovkSellkeMPR) {
      row[['VovkSellkeMPR']] <- "."
    }
    ...
    if (status$error)
      anova[["error"]] <- list(
          errorType = "badData", errorMessage = status$errorMessage)
    ...
    if (options$sumOfSquares == "type1") {
      ...
      modelTermsResults <- strsplit(
          gsub(" ", "", rownames(resultTable), fixed = TRUE), ":")
      ...
      for (j in .indices(terms.base64[[1]])) {
        ...
        row <- list("case" = terms.normal[[1]][j], "SS" = SS, "df" = df,
                    "MS" = MS, "F" = F, "p" = p, ".isNewGroup" = newGroup)
        if (options$VovkSellkeMPR) {
        ...
        if (options$effectSizeEstimates) {
          ...
          if (sum(gsub(" ", "", row.names(resultTable), fixed = TRUE) ==
                  "Residuals") > 0) {
      ...
      row <- list("case" = "Residual", "SS" = SS, "df" = df, "MS" = MS,
                  "F" = "", "p" = "", "eta" = "", "partialEta" = "",
                  "omega" = "", ".isNewSubGroup" = TRUE)
      if (options$VovkSellkeMPR) {
        row[["VovkSellkeMPR"]] <- ""
      }
      anova.rows[[length(anova.rows) + 1]] <- row
      ...
      for (j in .indices(terms.base64[[1]])) {
        ...
        if (options$effectSizeEstimates) {
          ...
          for (case in .indices(terms.base64[[1]])) {
            indices <- c(indices, which(unlist(lapply(modelTermsResults,
                                                      function(x)
                                                        .identicalTerms(
                                                            x, modelTermsCases[[
                                                                   case]])))))
...
.rmAnovaWithinSubjectsTable <- function(
    dataset, options, perform, model, stateSphericity, status) {
  ...
  if (options$sphericityCorrections && !is.null(corrections))
    fields[[length(fields) + 1]] <- list(
        name = "cor", type = "string", title = "Sphericity Correction")
  ...
  if (options$sphericityCorrections && !is.null(corrections)) {
    fields[[length(fields) + 1]] <- list(
        name = "df", type = "number", format = "sf:4;dp:3")
  } else {
    fields[[length(fields) + 1]] <- list(name = "df", type = "integer")
  }
  ...
  if (is.null(model)) {
      ...
      for (i in 2:length(terms.base64)) {
        ...
        for (j in .indices(terms.base64[[i]])) {
          ...
          if (options$sphericityCorrections && !is.null(corrections)) {
            ...
            if (options$sphericityNone) {
              row <- list(
                  "case" = terms.normal[[i]][j], "cor" = "None", "SS" = ".",
                  "df" = ".", "MS" = ".", "F" = ".", "p" = ".",
                  ".isNewGroup" = (newGroup && counter == 1),
                  ".isNewSubGroup" = (!newGroup &&
                                      counter == 1 && length(corrections) > 1))
              if (options$VovkSellkeMPR) {
                row[["VovkSellkeMPR"]] <- "."
              }
              anova.rows[[length(anova.rows) + 1]] <- row
            ...
            if (options$sphericityGreenhouseGeisser) {
              row <- list(
                  "case" = terms.normal[[i]][j], "cor" = "Greenhouse-Geisser",
                  "SS" = ".", "df" = ".", "MS" = ".", "F" = ".", "p" = ".",
                  ".isNewGroup" = (newGroup && counter == 1),
                  ".isNewSubGroup" = (!newGroup &&
                                      counter == 1 && length(corrections) > 1))
            ...
            if (options$sphericityHuynhFeldt) {
              row <- list(
                  "case" = terms.normal[[i]][j], "cor" = "Huynh-Feldt",
                  "SS" = ".", "df" = ".", "MS" = ".", "F" = ".", "p" = ".",
                  ".isNewGroup" = (newGroup && counter == 1),
                  ".isNewSubGroup" = (!newGroup &&
                                      counter == 1 && length(corrections) > 1))
        ...
        if (options$sphericityCorrections && !is.null(corrections)) {
          ...
          if (options$sphericityNone) {
            row <- list(
                "case" = "Residual", "cor" = "None", "SS" = ".", "df" = ".",
                "MS" = ".", "F" = "", "p" = "", "eta" = "", "partialEta" = "",
                "omega" = "", ".isNewSubGroup" = (counter == 1))
            if (options$VovkSellkeMPR) {
              row[["VovkSellkeMPR"]] <- ""
            }
            anova.rows[[length(anova.rows) + 1]] <- row
          ...
          if (options$sphericityGreenhouseGeisser) {
            row <- list("case" = "Residual", "cor" = "Greenhouse-Geisser",
                        "SS" = ".", "df" = ".", "MS" = ".", "F" = "", "p" = "",
                        "eta" = "", "partialEta" = "", "omega" = "",
                        ".isNewSubGroup" = (counter == 1))
            if (options$VovkSellkeMPR) {
              row[["VovkSellkeMPR"]] <- ""
            }
            anova.rows[[length(anova.rows) + 1]] <- row
          ...
          if (options$sphericityHuynhFeldt) {
            row <- list("case" = "Residual", "cor" = "Huynh-Feldt", "SS" = ".",
                        "df" = ".", "MS" = ".", "F" = "", "p" = "", "eta" = "",
                        "partialEta" = "", "omega" = "",
                        ".isNewSubGroup" = (counter == 1))
            if (options$VovkSellkeMPR) {
    ...
    if (is.null(corrections)) corrections <- "empty"
    ...
    if (options$sumOfSquares == "type1") {
      ...
      for (i in 2:length(terms.base64)) {
        ...
        resultTable <- result[[paste("Error: subject", termsRM.base64[[i - 1]],
                                     sep = ":")]][[1]]
        ...
        for (j in .indices(terms.base64[[i]])) {
          ...
          index <- unlist(
              lapply(modelTermsResults, function(x)
                                          .identicalTerms(x, modelTermsCase)))
          ...
          for (cor in corrections) {
            ...
            if (!is.null(stateSphericity) &&
                !is.na(stateSphericity[i - 1, "p"]) &&
                stateSphericity[i - 1, "p"] < .05) {

              foot.index <- .addFootnote(
                  footnotes,
                  text = "Mauchly's test of sphericity indicates that the assumption of sphericity is violated (p < .05).")
              row.footnotes <- list(
                  SS = list(foot.index), df = list(foot.index),
                  MS = list(foot.index), F = list(foot.index),
                  p = list(foot.index))
            ...
            if (!options$sphericityCorrections || cor == "empty") {
              ...
              row <- list("case" = terms.normal[[i]][j], "SS" = SS, "df" = df,
                          "MS" = MS, "F" = F, "p" = p, ".isNewGroup" = newGroup,
              ...
              row <- list(
                  "case" = terms.normal[[i]][j], "cor" = "None", "SS" = SS,
                  "df" = df, "MS" = MS, "F" = F, "p" = p,
                  ".isNewGroup" = (newGroup && counter == 1),
                  ".isNewSubGroup" = (!newGroup &&
                                      counter == 1 && length(corrections) > 1),
              ...
              row <- list(
                  "case" = terms.normal[[i]][j], "cor" = "Greenhouse-Geisser",
                  "SS" = SS, "df" = dfGG, "MS" = MSGG, "F" = F, "p" = pGG,
                  ".isNewGroup" = (newGroup && counter == 1),
                  ".isNewSubGroup" = (!newGroup &&
                                      counter == 1 && length(corrections) > 1),
                  .footnotes = row.footnotes)
              ...
              row <- list(
                  "case" = terms.normal[[i]][j], "cor" = "Huynh-Feldt",
                  "SS" = SS, "df" = dfHF, "MS" = MSHF, "F" = F, "p" = pHF,
                  ".isNewGroup" = (newGroup && counter == 1),
                  ".isNewSubGroup" = (!newGroup &&
                                      counter == 1 && length(corrections) > 1),
                  .footnotes = row.footnotes)
        ...
        for (cor in corrections) {
          ...
          if (!options$sphericityCorrections || cor == "empty") {
            ...
            row <- list(
                "case" = "Residual", "cor" = "None", "SS" = SS, "df" = df,
                "MS" = MS, "F" = "", "p" = "", "eta" = "", "partialEta" = "",
                "omega" = "", ".isNewSubGroup" = (counter == 1))
            if (options$VovkSellkeMPR) {
              row[["VovkSellkeMPR"]] <- ""
            }
            ...
            row <- list("case" = "Residual", "cor" = "Huynh-Feldt", "SS" = SS,
                        "df" = dfHF, "MS" = MSHF, "F" = "", "p" = "",
                        "eta" = "", "partialEta" = "", "omega" = "",
                        ".isNewSubGroup" = (counter == 1))
            if (options$VovkSellkeMPR) {
          ...
          for (cor in corrections) {
            ...
            if (options$effectSizeEstimates) {
              ...
              for (case in .indices(terms.base64[[i]])) {
                indices <- c(
                    indices,
                    which(unlist(lapply(modelTermsResults,
                                        function(x)
                                          .identicalTerms(
                                              x, modelTermsCases[[case]])))))
              }
...
.sphericityTest <- function(options, stateSphericity, perform, status) {
  ...
  if (options$VovkSellkeMPR) {
    fields <- list(list(name = "case", type = "string", title = ""),
                   list(name = "W", type = "number", format = "sf:4;dp:3",
                        title = "Mauchly's W"),
                   list(name = "p", type = "number", format = "dp:3;p:.001"),
                   list(name = "VovkSellkeMPR", title = "VS-MPR",
                        type = "number", format = "sf:4;dp:3"),
                   list(name = "GG", type = "number", format = "sf:4;dp:3",
                        title = "Greenhouse-Geisser \u03B5"),
                   list(name = "HF", type = "number", format = "sf:4;dp:3",
                        title = "Huynh-Feldt \u03B5"))
  } else {
    fields <- list(list(name = "case", type = "string", title = ""),
                   list(name = "W", type = "number", format = "sf:4;dp:3",
  ...
  if (is.null(stateSphericity)) {
    ...
    for (i in .indices(termsRM.base64)) {
      ...
      row <- list("case" = termsRM.normal[[i]], "W" = ".", "p" = ".",
                  "GG" = ".", "HF" = ".", ".isNewGroup" = newGroup)
    ...
    for (i in .indices(stateSphericity$termsNormal)) {
      ...
      if (stateSphericity$twoLevels[i]) {
        ...
        foot.index <- .addFootnote(
            footnotes,
            text = "The repeated measure has only two levels. When the repeated measure has two levels, the assumption of sphericity is always met.")
        row.footnotes <- list(W = list(foot.index), p = list(foot.index),
                              GG = list(foot.index), HF = list(foot.index))
      ...
      row <- list(
          "case" = stateSphericity$termsNormal[i], "W" = stateSphericity$W[i],
          "p" = .clean(stateSphericity$p[i]), "GG" = stateSphericity$GG[i],
          "HF" = stateSphericity$HF[i], ".isNewGroup" = newGroup,
          .footnotes = row.footnotes)
...
.rmAnovaLevenesTable <- function(
  ...
  if (perform == "run" &&
      status$ready &&
      status$error == FALSE && length(options$betweenSubjectFactors) > 0) {
    ...
    for (i in .indices(options$repeatedMeasuresCells)) {
      ...
      interaction <- paste(
          .v(options$betweenSubjectFactors), collapse = ":", sep = "")
      if (length(options$covariates) > 0) {
        covterms <- paste(.v(options$covariates), collapse = "+", sep = "")
        combterms <- paste(c(interaction, covterms), collapse = "+", sep = "")
        levene.def <- paste(
            .v(options$repeatedMeasuresCells[i]), "~", combterms)
      } else {
        levene.def <- paste(
            .v(options$repeatedMeasuresCells[i]), "~", interaction)
      }
      ...
      error <- base::tryCatch(summary(aov(levene.formula, dataset)),
                              error = function(e) e, warning = function(w) w)

      row <- list("case" = options$repeatedMeasuresCells[i],
                  "F" = .clean(r[[1]]$`F value`[1]), "df1" = r[[1]]$Df[1],
                  "df2" = r[[1]]$Df[2], "p" = .clean(r[[1]]$`Pr(>F)`[1]),
                  ".isNewGroup" = newGroup)
    ...
    for (i in .indices(options$repeatedMeasuresCells)) {
      ...
      row <- list("case" = options$repeatedMeasuresCells[i], "F" = ".",
                  "df1" = ".", "df2" = ".", "p" = ".", ".isNewGroup" = newGroup)
      if (options$VovkSellkeMPR) {
        row["VovkSellkeMPR"] <- "."
      }
      levenes.rows[[length(levenes.rows) + 1]] <- row
  ...
  if (length(options$betweenSubjectFactors) == 0)
    levenes.table[["error"]] <- list(
        errorType = "badData",
        errorMessage = "No between subjects factors specified.")

  if (status$error) levenes.table[["error"]] <- list(error = "badData")
...
.rmAnovaContrastTable <- function(options, perform, status, stateContrasts) {
  ...
  for (contrast in options$contrasts) {
    ...
    if (contrast$contrast != "none") {
      ...
      contrastTable[["title"]] <- paste(
          contrastType, " Contrast", " - ", contrast$variable, sep = "")
      contrastTable[["name"]] <- paste(
          contrastType, "Contrast_", contrast$variable, sep = "")
      ...
      if (status$error) contrastTable[["error"]] <- list(errorType = "badData")
...
.rmAnovaPostHocTable <- function(
  ...
  for (posthoc.var in posthoc.variables) {
    ...
    posthoc.table[[
        "title"]] <- paste("Post Hoc Comparisons - ", posthoc.var, sep = "")
    ...
    if (options$postHocTestEffectSize) {
      fields[[length(fields) + 1]] <- list(
          name = "Cohen's d", title = "Cohen's d", type = "number",
          format = "sf:4;dp:3")
      posthoc.table[["footnotes"]] <- list(
          list(symbol = "<i>Note.</i>",
               text = "Cohen's d does not correct for multiple comparisons."))
    }

    if (options$postHocTestsTukey)
      fields[[length(fields) + 1]] <- list(
          name = "tukey", title = "p<sub>tukey</sub>", type = "number",
          format = "dp:3;p:.001")

    if (options$postHocTestsScheffe)
      fields[[length(fields) + 1]] <- list(
          name = "scheffe", title = "p<sub>scheffe</sub>", type = "number",
          format = "dp:3;p:.001")

    if (options$postHocTestsBonferroni)
      fields[[length(fields) + 1]] <- list(
          name = "bonferroni", title = "p<sub>bonf</sub>", type = "number",
          format = "dp:3;p:.001")

    if (options$postHocTestsHolm)
      fields[[length(fields) + 1]] <- list(
          name = "holm", title = "p<sub>holm</sub>", type = "number",
          format = "dp:3;p:.001")
    ...
    if (posthoc.var %in% options$betweenSubjectFactors) {
      ...
      variable.levels <- options$repeatedMeasuresFactors[[
                                     which(
                                         lapply(options$repeatedMeasuresFactors,
                                                function(x) x$name) ==
                                         posthoc.var)]]$levels
    ...
    for (i in 1:length(variable.levels)) {
      for (j in .seqx(i + 1, length(variable.levels))) {
        ...
        if (!is.null(statePostHoc)) {
          ...
          if (length(class(statePostHoc[[posthoc.var]]$resultBonf)) == 1 &&
              class(statePostHoc[[posthoc.var]]$resultBonf) == "try-error") {
            ...
            for (c in 1:length(statePostHoc[[posthoc.var]]$comparisons)) {
              if (all(statePostHoc[[posthoc.var]]$comparisons[[c]] %in%
                      c(.v(variable.levels[[i]]), .v(variable.levels[[j]])))) {
                index <- c

                reverse <- TRUE

                if (statePostHoc[[posthoc.var]]$comparisons[[c]][1] ==
                    .v(variable.levels[[i]])) reverse <- FALSE
              }
            }
            # index <- index + 1
            ...
            if (reverse) {
              md <- .clean(
                  -as.numeric(
                       statePostHoc[[posthoc.var]]$resultBonf$estimate[index]))
            } else {
              md <- .clean(
                  as.numeric(
                      statePostHoc[[posthoc.var]]$resultBonf$estimate[index]))
            ...
            SE <- .clean(
                as.numeric(statePostHoc[[posthoc.var]]$resultBonf$SE[index]))

            if (reverse) {
              t <- .clean(
                  -as.numeric(
                       statePostHoc[[posthoc.var]]$resultBonf$t.ratio[index]))
            ...
            if (options$postHocTestsTukey) {
              pTukey <- .clean(
                  as.numeric(
                      statePostHoc[[posthoc.var]]$resultTukey$p.value[index]))
              if (!is.numeric(statePostHoc[[
                                  posthoc.var]]$resultTukey$p.value[index])) {
                posthoc.table[["footnotes"]] <- list(
                    list(
                        symbol = "<i>Note.</i>",
            ...
            if (options$postHocTestsScheffe) {
              pScheffe <- .clean(
                  as.numeric(
                      statePostHoc[[posthoc.var]]$resultScheffe$p.value[index]))
              if (!is.numeric(statePostHoc[[
                                  posthoc.var]]$resultScheffe$p.value[index])) {
                posthoc.table[["footnotes"]] <- list(
                    list(
                        symbol = "<i>Note.</i>",
            ...
            if (options$postHocTestsBonferroni)
              pBonf <- .clean(
                  as.numeric(
                      statePostHoc[[posthoc.var]]$resultBonf$p.value[index]))

            if (options$postHocTestsHolm)
              pHolm <- .clean(
                  as.numeric(
                      statePostHoc[[posthoc.var]]$resultHolm$p.value[index]))
          }
    ...
    if (status$error) posthoc.table[["error"]] <- list(errorType = "badData")
...
.rmAnovaDescriptivesTable <- function(
    dataset, options, perform, status, stateDescriptivesTable) {
  ...
  for (variable in factors) {
    ...
    name <- paste(".", variable, sep = "")  # in case variable is "Mean", "SD"
                                            # or "N"
    fields[[length(fields) + 1]] <- list(
        name = name, type = "string", title = variable, combine = TRUE)

  }

  fields[[length(fields) + 1]] <- list(
      name = "Mean", type = "number", format = "sf:4;dp:3")
  fields[[length(fields) + 1]] <- list(
      name = "SD", type = "number", format = "sf:4;dp:3")
  fields[[length(fields) + 1]] <- list(
      name = "N", type = "number", format = "dp:0")
  ...
  if (length(factors) > 0) {
    ...
    if (perform == "run" && status$ready && status$error == FALSE) {
      ...
      dataset <- .shortToLong(
          dataset, options$repeatedMeasuresFactors,
          options$repeatedMeasuresCells, options$betweenSubjectFactors)
  ...
  if (status$error) descriptives.table[["error"]] <- list(error = "badData")
...
.rmAnovaDescriptivesPlot <- function(
    dataset, options, perform, status, stateDescriptivesPlot) {

  descriptivesPlotList <- list()

  if (perform == "run" &&
      status$ready && !status$error && options$plotHorizontalAxis != "") {

    dataset <- .shortToLong(
        dataset, options$repeatedMeasuresFactors, options$repeatedMeasuresCells,
        options$betweenSubjectFactors)

    groupVars <- c(options$plotHorizontalAxis, options$plotSeparateLines,
                   options$plotSeparatePlots)
    groupVars <- groupVars[groupVars != ""]
    groupVarsV <- .v(groupVars)

    betweenSubjectFactors <- groupVars[
        groupVars %in% options$betweenSubjectFactors]
    repeatedMeasuresFactors <- groupVars[
        groupVars %in% sapply(options$repeatedMeasuresFactors, function(x)
                                                                 x$name)]

    if (length(repeatedMeasuresFactors) == 0) {

      summaryStat <- .summarySE(
          as.data.frame(dataset), measurevar = "dependent",
          groupvars = .v(betweenSubjectFactors),
          conf.interval = options$confidenceIntervalInterval, na.rm = TRUE,
          .drop = FALSE, errorBarType = options$errorBarType)

    } else {

      summaryStat <- .summarySEwithin(
          as.data.frame(dataset), measurevar = "dependent",
          betweenvars = .v(betweenSubjectFactors),
          withinvars = .v(repeatedMeasuresFactors), idvar = "subject",
          conf.interval = options$confidenceIntervalInterval, na.rm = TRUE,
          .drop = FALSE, errorBarType = options$errorBarType)
    ...
    if (options$plotHorizontalAxis != "") {
      colnames(summaryStat)[
          which(colnames(summaryStat) == .v(options$plotHorizontalAxis))] <-
          "plotHorizontalAxis"
    }

    if (options$plotSeparateLines != "") {
      colnames(summaryStat)[
          which(colnames(summaryStat) == .v(options$plotSeparateLines))] <-
          "plotSeparateLines"
    }

    if (options$plotSeparatePlots != "") {
      colnames(summaryStat)[
          which(colnames(summaryStat) == .v(options$plotSeparatePlots))] <-
          "plotSeparatePlots"
    }
    ...
    for (i in 1:nPlots) {
      ...
      if (options$plotSeparateLines != "") {
        ...
        descriptivesPlot[["custom"]] <- list(
            width = "plotWidthDescriptivesPlotNoLegend",
            height = "plotHeightDescriptivesPlotNoLegend")

      }

      if (options$plotSeparatePlots != "") {
        summaryStatSubset <- subset(
            summaryStat, summaryStat[, "plotSeparatePlots"] == subsetPlots[i])
      } else {
      ...
      if (options$plotSeparateLines == "") {
        ...
        p <- ggplot2::ggplot(
            summaryStatSubset,
            ggplot2::aes(x = plotHorizontalAxis, y = dependent, group = 1))

      } else {

        p <- ggplot2::ggplot(
            summaryStatSubset,
            ggplot2::aes(x = plotHorizontalAxis, y = dependent,
                         group = plotSeparateLines, shape = plotSeparateLines,
      ...
      p <- p + ggplot2::geom_line(position = pd, size = .7) +
          ...
          ggplot2::scale_fill_manual(
              ...
              guide = ggplot2::guide_legend(nrow = 10)) +
          ...
          ggplot2::labs(shape = options$plotSeparateLines,
                        fill = options$plotSeparateLines) +
          ggplot2::theme_bw() +
          ggplot2::theme(  # legend.justification=c(0,1),
              ...
              legend.key = ggplot2::element_blank(),  # legend.key.width =
                                                      # grid::unit(10,"mm"),
      ...
      if (nPlots > 1) {
        descriptivesPlot[["title"]] <- paste(
            options$plotSeparatePlots, ": ", subsetPlots[i], sep = "")
      } else {
      ...
      if (options$plotSeparateLines != "") {
        ...
        content <- .writeImage(
            ...
            height = options$plotHeightDescriptivesPlotLegend, plot = p,
            obj = TRUE)
        ...
        content <- .writeImage(
            width = options$plotWidthDescriptivesPlotNoLegend,
            height = options$plotHeightDescriptivesPlotNoLegend, plot = p,
            obj = TRUE)
    ...
    if (options$plotSeparatePlots != "") {
      ...
      repeatedMeasuresNames <- sapply(
          options$repeatedMeasuresFactors, function(x) x$name)
      repeatedMeasuresLevels <- lapply(
          options$repeatedMeasuresFactors, function(x) x$levels)
...
.summarySE <- function(
    ...
    conf.interval = .95, .drop = TRUE, errorBarType = "confidenceInterval") {
...
.normDataWithin <- function(data = NULL, idvar, measurevar, betweenvars = NULL,
  ...
  data[, measureNormedVar] <- data[, measurevar] -
      data[, "subjMean"] + mean(data[, measurevar], na.rm = na.rm)
...
.summarySEwithin <- function(
    data = NULL, measurevar, betweenvars = NULL, withinvars = NULL,
    idvar = NULL, na.rm = FALSE, conf.interval = .95, .drop = TRUE,
    errorBarType = "confidenceInterval") {

  # Get the means from the un-normed data
  datac <- .summarySE(
      data, measurevar, groupvars = c(betweenvars, withinvars), na.rm = na.rm,
      conf.interval = conf.interval, .drop = .drop, errorBarType = errorBarType)
  ...
  ndata <- .normDataWithin(
      data, idvar, measurevar, betweenvars, na.rm, .drop = .drop)
  ...
  ndatac <- .summarySE(
      ndata, measurevar_n, groupvars = c(betweenvars, withinvars),
