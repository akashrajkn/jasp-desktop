...
.readBayesianRepeatedMeasuresDataOptions <- function(
    dataset = NULL, options = list(), perform = "init") {
    ...
    if (is.null(dataset)) {
      ...
      if (perform == "run") {
        dataset <- .readDataSetToEnd(
            ...
            columns.as.factor = bs.factors, exclude.na.listwise = all.variables)
        dataset <- .shortToLong(
            dataset, rm.factors, rm.vars, c(bs.factors, bs.covariates))
      } else {
        dataset <- data.frame(
            dependent = numeric(), subject = factor(levels = 2))
      ...
      options$modelTerms[[length(options$modelTerms) + 1]] <- list(
          components = "subject", isNuisance = TRUE)
    }
...
.readBayesianRepeatedMeasuresShortData <- function(
    options = list(), perform = "init") {
  numeric.vars <- c(
  ...
  if (perform == "run") {
    dataset <- .readDataSetToEnd(
        columns.as.numeric = numeric.vars, columns.as.factor = factor.vars,
...
.readBayesianLinearModelData <- function(
    dataset = NULL, options = list(), perform = "init") {
...
.setBayesianLinearModelStatus <- function(
    dataset = NULL, options = list(), perform = "init") {
  ...
  if (!exists("error.message") && perform == "run") {
    ...
    if (!is.null(variable.names))
      error.message <- paste("Bayes factor is undefined -- the variable(s) ",
                             variable.names, " contain(s) infinity", sep = "")
  }
  ...
  if (!exists("error.message") && perform == "run") {
    ...
    for (fact in options$fixedFactors) {
      if (length(unique(dataset[[.v(fact)]])) < 2) {
        ...
        error.message <- paste(
            "Bayes factor is undefined -- the factor(s) ",
            paste(factor.names, collapse = ", "),
            " contain(s) less than two levels (possibly only after rows with missing values are excluded)",
            sep = "")
      }
    }
  }
  ...
  if (!exists("error.message") &&
      perform == "run" && (length(dataset[[.v(options$dependent)]]) <=
                           (1 + length(options$modelTerms))))
    error.message <-
  ...
  if (!exists("error.message")) {
    for (term in options$modelTerms) {
      ...
      if (term$isNuisance) {
        components <- term$components
        withmain.conflict <- sapply(
            options$modelTerms, function(terms) {
                                  if (!terms$isNuisance) {
                                    term.components <- terms$components
                                    if (sum(term.components %in% components) ==
                                        length(term.components)) {
...
.updateResultsBayesianLinearModels <- function(
    results, model.object, effects.matrix, interactions.matrix, neverExclude,
    null.model, options, status) {
  results[[
      "model comparison"]] <- .theBayesianLinearModelsComparison(
                                  list(
                                      ...
                                      null.model = null.model), options,
                                  perform = "run", status,
                                  populate = TRUE)$modelTable

  results[["effects"]] <- .theBayesianLinearModelsEffects(
      list(models = model.object, effects = effects.matrix,
           interactions.matrix = interactions.matrix, nuisance = neverExclude,
           null.model = null.model), options, perform = "run", status,
      populate = TRUE)
  ...
  if (options$posteriorEstimates) {
    results[["effects"]] <- .theBayesianLinearModelEstimates(
        list(models = model.object, effects = effects.matrix,
             interactions.matrix = interactions.matrix, nuisance = neverExclude,
             null.model = null.model), options, perform = "init", status,
        populate = TRUE)
  }
...
.theBayesianLinearModels <- function(
    dataset = NULL, options = list(), perform = "init", status = list(),
    .callbackBayesianLinearModels, .callbackBFpackage, results = list(),
    analysisType = "") {
  ...
  for (term in options$modelTerms) {
    if (is.null(effects) & is.null(neverExclude)) {
      model.formula <- paste(
          model.formula, paste(.v(term$components), collapse = ":"), sep = "")
    } else {
      model.formula <- paste(
          model.formula, " + ", paste(.v(term$components), collapse = ":"),
          sep = "")
    }
    if (term$isNuisance) {
      neverExclude <- c(
          neverExclude, paste(.v(term$components), collapse = ":"))
    } else {
  ...
  model.list <- try(BayesFactor::enumerateGeneralModels(
                        model.formula, whichModels = "withmain",
                        neverExclude = paste("^", neverExclude, "$", sep = "")),
                    silent = TRUE)
  if (class(model.list) == "try-error") {
    if (!status$error) {
      ...
      status$error.message <-
          "An unknown error occured. Please contact the authors."
      return(list(model = list(
                      models = NULL, effects = NULL, interactions.matrix = NULL,
                      nuisance = neverExclude, null.model = NULL),
                  status = status))
    }
  ...
  if (perform == "run") {
    progressbar <- .newProgressbar(
        ticks = length(model.list), callback = .callbackBayesianLinearModels,
        response = TRUE)
  }
  ...
  if (!is.null(neverExclude)) {
    ...
    if (perform == "run" && status$ready) {
      bf <- try(
          BayesFactor::lmBF(
              null.formula, data = dataset,
              whichRandom = .v(unlist(options$randomFactors)), progress = FALSE,
              posterior = FALSE, callback = .callbackBFpackage,
              rscaleFixed = rscaleFixed, rscaleRandom = rscaleRandom,
              rscaleCont = rscaleCont, iterations = iter))

      null.model$bf <- bf

      if (inherits(bf, "try-error")) {
        message <- .extractErrorMessage(bf)
        if (message == "Operation cancelled by callback function.") return()
        status$ready <- FALSE
        status$error <- TRUE
        status$error.message <-
  ...
  if (no.models > 0 && no.effects > 0) {
    ...
    if (no.effects > 1) {
      effect.components <- sapply(
          effects, function(effect) {
                     base::strsplit(effect, split = ":", fixed = TRUE)
                   })
      for (e in 1:no.effects) {
        interactions.matrix[e, ] <- sapply(
            1:no.effects,
            function(ee) {
              (sum(effect.components[[e]] %in% effect.components[[ee]]) ==
               length(effect.components[[e]]))
            })
      }
    ...
    for (m in 1:no.models) {
      ...
      if (no.effects > 1) {
        for (effect in model.effects) {
          ...
          inclusion <- sapply(
              effect.components,
              function(effect.component) {
                if (length(components) != length(effect.component)) {
                  return(FALSE)
                } else {
                  if (sum(components %in% effect.component) ==
                      length(components)) {
                    return(TRUE)
                  } else {
                    return(FALSE)
      ...
      model.object[[m]]$ready <-
          FALSE  # to ensure that intermediate results can be called
    }

    #Create empty tables
    results <- .updateResultsBayesianLinearModels(
        results, model.object, effects.matrix, interactions.matrix,
        neverExclude, null.model, options, status)
    ...
    if (response$status != "ok") {
      ...
      if (!is.null(response$options)) options <- response$options
    }
    ...
    for (m in 1:no.models) {
      if (perform == "run" && status$ready) {
        bf <- try(BayesFactor::lmBF(
                      model.list[[m]], data = dataset,
                      whichRandom = .v(unlist(options$randomFactors)),
                      progress = FALSE, posterior = FALSE,
                      callback = .callbackBFpackage, rscaleFixed = rscaleFixed,
                      rscaleRandom = rscaleRandom, rscaleCont = rscaleCont,
                      iterations = iter))
    ...
    if (options$posteriorEstimates && perform == "run" && status$ready) {
      ...
      chains <- try(
          BayesFactor::lmBF(
              model.list[[m]], data = dataset,
              whichRandom = .v(unlist(options$randomFactors)), progress = FALSE,
              posterior = TRUE, callback = .callbackBFpackage,
      ...
      if (inherits(bf, "try-error")) {
        ...
        return(
            list(model = list(models = model.object, effects = effects.matrix,
                              interactions.matrix = interactions.matrix,
...
.theBayesianLinearModelsComparison <- function(
    model = NULL, options = list(), perform = "init", status = list(),
    populate = TRUE) {
  modelTable <- list()
  modelTable[["title"]] <- "Model Comparison"
  modelTable[["citation"]] <- list(
  ...
  if (options$bayesFactorType == "LogBF10") {
    fields[[4]] <- list(name = "BFM", type = "number", format = "sf:4;dp:3",
                        title = paste(bfm.title, sep = ""))
    fields[[5]] <- list(name = "BF10", type = "number", format = "sf:4;dp:3",
                        title = paste(bf.title, sep = ""))
  }
  ...
  if (length(model$nuisance) > 0) {
    null.model <- paste(
        "Null model (incl. ", paste(.unvf(model$nuisance), collapse = ", "),
        ")", sep = "")
    footnote <- paste(
        "All models include ", paste(.unvf(model$nuisance), collapse = ", "),
        ".", sep = "")
  ...
  if (perform == "run" && status$ready) {
    ...
    bestModelTop <- !is.null(options$bayesFactorOrder) &&
        options$bayesFactorOrder == "bestModelTop" && populate == FALSE
    if (bestModelTop) {
      ordering <- order(bayes.factors, decreasing = TRUE)
      bestModelIndex <- ordering[1] - 1  # The null model is not in the model
                                         # list, so shift by 1
      if (bestModelIndex != 0) {
    ...
    if (populate == FALSE) {
      ...
      ln.BF.model <- sapply(
          ...
          function(m) {
            if (m %in% index) {
              i <- which(index == m)
              bayes.factors[m] - maxLBF - log(sum(exp(bayes.factors[index][-i] -
                                                      maxLBF))) + log(no.models)
            } else {
              NA
            }
          })
    ...
    for (m in 1:no.models) {
      if (model$models[[m]]$ready) {
        ...
        if (populate == FALSE) {
          index <- .addFootnote(
              footnotes, text = model$models[[m]]$error.message)
  ...
  if (is.null(status$analysis.type) || status$analysis.type != "rmANOVA") {
    modelTable[[
        "title"]] <- paste("Model Comparison - ", options$dependent, sep = "")
  }
  modelTable[["data"]] <- rows
  modelTable[["footnotes"]] <- as.list(footnotes)

  if (status$error)
    modelTable[["error"]] <- list(
        errorType = "badData", errorMessage = status$error.message)

  return(list(modelTable = modelTable, model = model))
}

.theBayesianLinearModelsEffects <- function(
    model = NULL, options = list(), perform = "init", status = list(),
    populate = TRUE) {

  if (!options$effects) return(NULL)

  effectsTable <- list()
  effectsTable[["title"]] <- "Analysis of Effects"
  effectsTable[["citation"]] <- list(
  ...
  fields <- list(
      ...
      list(name = "BF<sub>Inclusion</sub>", type = "number",
           format = "sf:4;dp:3", title = paste(inclusion.title, sep = "")))

  effectsTable[["schema"]] <- list(fields = fields)

  if (!status$ready && !status$error) return(effectsTable)

  if (options$effectsType == "matchedModels") {
    footnotes <- .newFootnotes()
    .addFootnote(
        footnotes, symbol = "<em>Note.</em>",
        text = "Compares models that contain the effect to equivalent models
  ...
  if (perform == "run" && status$ready && !populate) {
    no.effects <- ncol(effects.matrix) - 2  # last 2 columns hold (unnamed)
                                            # prior & posterior probs
    effectNames <- effectNames[1:no.effects]

    if (options$effectsType == "allModels") {
      effects.matrix <- effects.matrix[-1, , drop = FALSE]  # remove the null
                                                            # model
      ...
      posterior.inclusion.probabilities <- posterior.probabilities %*%
          effects.matrix
      posterior.inclusion.probabilities[
          posterior.inclusion.probabilities > 1] <-
          1  # deals with numerical error
      posterior.inclusion.probabilities[
          posterior.inclusion.probabilities < 0] <- 0
      bayes.factor.inclusion <- (posterior.inclusion.probabilities /
      ...
      for (effect in effectNames) {
        ...
        higherInteractions <- effectNames[
            model$interactions.matrix[effect, ] == TRUE]
        ...
        if (nrow(modelsWithout) > 1) {
          refMatrix <- modelsWithout[
              , 1:(ncol(effects.matrix) - 2), drop = FALSE]
          refMatrix <- refMatrix[, names(refMatrix) != effect, drop = FALSE]
          indices <- apply(
              modelsWith[, names(modelsWith) != effect, drop = FALSE], 1,
              function(row) {
        ...
        sumInModelPost <- ifelse(
            sum(inModelPosteriors) < 0, 0, sum(inModelPosteriors))
        sumInModelPost <- ifelse(sumInModelPost > 1, 1, sumInModelPost)
        sumOutModelPost <- ifelse(
            sum(outModelPosteriors) < 0, 0, sum(outModelPosteriors))
        ...
        bayes.factor.inclusion[index] <- (sumInModelPost / sumOutModelPost) /
            (sum(inModelPriors) / sum(outModelPriors))
  ...
  if (is.null(status$analysis.type) || status$analysis.type != "rmANOVA") {
    effectsTable[["title"]] <- paste(
        "Analysis of Effects - ", options$dependent, sep = "")
  }

  if (!status$ready) effectsTable[["error"]] <- list(errorType = "badData")

  return(effectsTable)
}

.theBayesianLinearModelEstimates <- function(
    model = NULL, options = list(), perform = "init", status = list(),
    populate = FALSE) {
  if (!options$posteriorEstimates) return(NULL)

  estimatesTable <- list()
  estimatesTable[["title"]] <- "Parameter Estimates"
  estimatesTable[["citation"]] <- list(
      "Morey, R. D. & Rouder, J. N. (2015). BayesFactor (Version 0.9.10-2)[Computer software].",
      "Rouder, J. N., Morey, R. D., Speckman, P. L., Province, J. M., (2012) Default Bayes Factors for ANOVA Designs. Journal of Mathematical Psychology. 56. p. 356-374.")
  ...
  fields <- list(
      ...
      list(name = "Lower Bound", title = paste(alpha, "%", sep = ""),
           type = "number", format = "sf:4;dp:3"),
      list(name = "Upper Bound", title = paste(1 - alpha, "%", sep = ""),
           type = "number", format = "sf:4;dp:3"))
  estimatesTable[["schema"]] <- list(fields = fields)

  if (!status$ready && !status$error) return(estimatesTable)

  if (perform == "init" && !populate) return(estimatesTable)

  if (!populate) {
    effects.matrix <- matrix(
        model$effects[1:nrow(model$effects), 1:(ncol(model$effects) - 2)],
        nrow = nrow(model$effects), ncol = ncol(model$effects) - 2)
    complexity <- rowSums(effects.matrix)
    m <- which(complexity == max(complexity))
  } else {
    effects.matrix <- model$effects
  ...
  if (!populate && is.null(model$models[[m]]$error.message)) {
    ...
    if (length(options$randomFactors) > 0) {
      ...
      remove <- NULL
      for (name in rfnames)
        remove <- c(remove, base::grep(x = parameters, pattern = name,
      ...
      if (length(name) > 1) {
        ...
        if (base::substr(name, 1, 1) == "X") {
          ...
          if (name == "sig2") {
            ...
            if (base::substr(name, 1, 1) == "X") name <- .unvf(name)
    ...
    if (length(options$fixedFactors) == 0) {
      parameters <- c(
          .unvf(colnames(effects.matrix)), "\u03C3\u00B2", "g (prior)")
    } else {
      parameters <- c("Intercept", .unvf(colnames(effects.matrix)),
                      "\u03C3\u00B2", "g (prior)")
    }
  ...
  if (!status$ready) estimatesTable[["error"]] <- list(errorType = "badData")

  estimatesTable[[
      "title"]] <- paste("Parameter Estimates - ", options$dependent, sep = "")

  return(estimatesTable)
}


.anovaNullControlPostHocTable <- function(
    dataset, options, perform, status, analysisType) {

  if (length(options$postHocTestsVariables) == 0) return(NULL)
  ...
  for (posthoc.var in posthoc.variables) {
    ...
    if (analysisType == "RM-ANOVA" &&
        posthoc.var %in% options$fixedFactors &&
        !posthoc.var %in% options$betweenSubjectFactors) {
      variable.levels <- options$repeatedMeasuresFactors[[
                                     which(
                                         ...
                                         posthoc.var)]]$levels
      paired <- TRUE
    } else if (posthoc.var %in%
               c(options$fixedFactors, options$betweenSubjectFactors,
                 options$randomFactors)) {
    ...
    for (i in 1:ncol(pairs)) {
      ...
      if (options$postHocTestsNullControl == TRUE &&
          perform == "run" && status$ready && status$error == FALSE) {

        x <- dataset[
            dataset[[.v(posthoc.var)]] == pairs[1, i], .v(dep.variable)]
        y <- dataset[
            dataset[[.v(posthoc.var)]] == pairs[2, i], .v(dep.variable)]
        x <- na.omit(x)
        y <- na.omit(y)

        ttest <- try(BayesFactor::ttestBF(
                         x = x, y = y, rscale = priorWidth, paired = paired),
                     silent = TRUE)
