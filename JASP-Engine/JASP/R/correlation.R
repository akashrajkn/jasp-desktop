...
Correlation <- function(dataset = NULL, options, perform = "run",
                        callback = function(...) 0, state = NULL, ...) {
  #
  stateKey <- list(
      correlationPlot = c(
          "plotCorrelationMatrix", "plotDensities", "plotStatistics",
          "variables", "pearson", "kendallsTauB", "spearman",
          "confidenceIntervals", "confidenceIntervalsInterval", "hypothesis",
          "missingValues"), tableData = c("hypothesis", "missingValues",
                                          "confidenceIntervalsInterval"))
  ...
  tableData <- .calculateCorrelations(
      dataset, perform, options, stateTableData, variables = options$variables,
      pearson = options$pearson, kendallsTauB = options$kendallsTauB,
      spearman = options$spearman, hypothesis = options$hypothesis,
      CI = options$confidenceIntervalsInterval)

  correlation.table <- .fillCorrelationTable(
      tableData, displayPairwise = options$displayPairwise,
  ...
  if (perform == "init") {
    ...
    state <- list(options = options, correlationPlot = correlation.plot,
                  tableData = tableData)
...
.calculateCorrelations <- function(
    dataset, perform, options, stateTableData, variables = c(), pearson = TRUE,
    kendallsTauB = FALSE, spearman = FALSE, hypothesis = "correlated",
    CI = 0.95) {
  #
  ready <- FALSE
  ...
  if (length(variables) == 0) {
    variables <- c(variables, "...", "... ")  # we need this trailing space so 1
                                              # != 2
  ...
  results <- list(
      tests = tests, variables = variables, hypothesis = hypothesis, CI = CI)
  ...
  for (i in seq_along(pairs)) {
    ...
    for (test in tests) {
      ...
      if (!is.null(stateTableData) &&
          !is.null(stateTableData[["result"]][[pairName]][[test]])) {
        ...
        if (!identical(errors, FALSE)) {
          ...
          if (hypothesis == "correlated") {
            result <- cor.test(obs1, obs2, method = test,
                               alternative = "two.sided", conf.level = CI)
          } else if (hypothesis == "correlatedPositively") {
            result <- cor.test(obs1, obs2, method = test,
                               alternative = "greater", conf.level = CI)
          } else {
            result <- cor.test(obs1, obs2, method = test, alternative = "less",
                               conf.level = CI)
          }
          ...
          if (test == "pearson") {
            ...
            spearCI <- .createNonparametricConfidenceIntervals(
                obs1, obs2, obsCor = estimate, method = "spearman",
                hypothesis = hypothesis, confLevel = CI)

            upperCI <- as.numeric(spearCI[2])
            lowerCI <- as.numeric(spearCI[1])
          } else if (test == "kendall") {
            kendallCI <- .createNonparametricConfidenceIntervals(
                obs1, obs2, obsCor = estimate, method = "kendall",
                hypothesis = hypothesis, confLevel = CI)
      ...
      results[["result"]][[pairName]][[test]] <- list(
          estimate = .clean(estimate), p.value = .clean(p.value),
          MPR = .clean(MPR), upperCI = .clean(upperCI),
          lowerCI = .clean(lowerCI), errorMessage = errorMessage)
    }
  }
  return(results)
}

.fillCorrelationTable <- function(
    tableData, displayPairwise = FALSE, reportSignificance = FALSE,
  ...
  if (flagSignificant || reportSignificance) {
    if (hypothesis == "correlatedPositively") {
      .addFootnote(footnotes, "all tests one-tailed, for positive correlation",
                   symbol = "<i>Note</i>.")
    } else if (hypothesis == "correlatedNegatively") {
      .addFootnote(footnotes, "all tests one-tailed, for negative correlation",
                   symbol = "<i>Note</i>.")
    }
  }
  ...
  if (flagSignificant) {
    if (hypothesis == "correlated") {
      ...
      .addFootnote(footnotes, "p < .05, ** p < .01, *** p < .001, one-tailed",
                   symbol = "*")
    }
  }
  ...
  CI <- tableData$CI
  fields <- list()
  test.names <- list(pearson = "Pearson's r", spearman = "Spearman's rho",
  ...
  if (displayPairwise) {
    ...
    for (test in tests) {
      ...
      fields[[length(fields) + 1]] <- list(
          name = test, title = test.names[[test]], overTitle = overTitle,
          type = "number", format = "sf:4;dp:3")

      if (reportSignificance) {
        fields[[length(fields) + 1]] <- list(
            name = paste0(test, "-p"), title = "p", overTitle = overTitle,
      ...
      if (reportVovkSellkeMPR) {
        ...
        fields[[length(fields) + 1]] <- list(
            name = paste0(test, "-MPR"), title = title, overTitle = overTitle,
            type = "number", format = "sf:4;dp:3")
      }

      if (reportCI) {
        fields[[length(fields) + 1]] <- list(
            name = paste0(test, "-lowerCI"),
            title = paste0("Lower ", 100 * CI, "% CI"), overTitle = overTitle,
            type = "number", format = "sf:4;dp:3")
        fields[[length(fields) + 1]] <- list(
            name = paste0(test, "-upperCI"),
            title = paste0("Upper ", 100 * CI, "% CI"), overTitle = overTitle,
            type = "number", format = "sf:4;dp:3")
      }
    ...
    for (test in tests) {
      if (length(tests) > 1 ||
          (length(tests) == 1 &&
           (reportSignificance || reportCI || reportVovkSellkeMPR))) {
        fields[[length(fields) + 1]] <- list(
            name = paste0(".test[", test, "]"), title = "", type = "string")
      }

      for (variable.name in variables) {
        fields[[length(fields) + 1]] <- list(
            name = paste0(variable.name, "[", test, "]"), title = variable.name,
            type = "number", format = "dp:3")
      }

      if (reportSignificance) {
        fields[[length(fields) + 1]] <- list(
            name = paste0(".test[", test, "-p]"), title = "", type = "string")

        for (variable.name in variables) {
          fields[[length(fields) + 1]] <- list(
              name = paste0(variable.name, "[", test, "-p]"),
              title = variable.name, type = "number", format = "dp:3;p:.001")
        }
      }

      if (reportVovkSellkeMPR) {
        fields[[length(fields) + 1]] <- list(
            name = paste0(".test[", test, "-MPR]"), title = "", type = "string")

        for (variable.name in variables) {
          fields[[length(fields) + 1]] <- list(
              name = paste0(variable.name, "[", test, "-MPR]"),
              title = variable.name, type = "number", format = "sf:4;dp:3")
        }
      }

      if (reportCI) {
        fields[[length(fields) + 1]] <- list(
            name = paste0(".test[", test, "-upperCI]"), title = "",
            type = "string")
        fields[[length(fields) + 1]] <- list(
            name = paste0(".test[", test, "-lowerCI]"), title = "",
            type = "string")

        for (variable.name in variables) {
          fields[[length(fields) + 1]] <- list(
              name = paste0(variable.name, "[", test, "-upperCI]"),
              title = variable.name, type = "number", format = "dp:3")
          fields[[length(fields) + 1]] <- list(
              name = paste0(variable.name, "[", test, "-lowerCI]"),
              title = variable.name, type = "number", format = "dp:3")
        }
      }
    }
  ...
  for (i in seq_along(template)) {  # loop over row variables
    ...
    for (j in seq_along(row.pairs)) {  # loop over pairs containing that row var
      ...
      for (test in tests) {
        ...
        if (!displayPairwise) {
          ...
          col.mpr <- paste0(
              col.var, "[", col.mpr, "]")  # contNormal[pearson-MPR]
          col.upperCI <- paste0(
              col.var, "[", col.upperCI, "]")  # contNormal[pearson-upperCI]
          col.lowerCI <- paste0(
              col.var, "[", col.lowerCI, "]")  # contNormal[pearson-lowerCI]
          ...
          if (reportCI) {
            row[[paste0(".test", "[", test, "-upperCI]")]] <- paste(
                "Upper ", 100 * CI, "% CI", sep = "")
            row[[paste0(".test", "[", test, "-lowerCI]")]] <- paste(
                "Lower ", 100 * CI, "% CI", sep = "")
          }

          if (identical(pair, "-")) {  # auto correlation
            row[[col.est]] <-
                row[[col.p]] <- row[[col.mpr]] <- row[[col.upperCI]] <-
                                                      row[[col.lowerCI]] <-
                                                          "\u2014"  # em-dash
            next
        ...
        if (reportSignificance) row[[col.p]] <- result$p.value

        if (reportVovkSellkeMPR) row[[col.mpr]] <- result$MPR
    ...
    if (length(row.footnotes) > 0) row[[".footnotes"]] <- row.footnotes
...
.plotCorrelations <- function(dataset, perform, options) {
  ...
  if (perform == "init" && length(variables) > 1) {
    ...
    for (i in seq_along(variables)) {
      ...
      errors <- .hasErrors(
          dataset, perform, type = c("infinity", "variance", "observations"),
          all.target = variables[i], message = "short",
            ...
            if (row == col) {
              if (options$plotDensities) {
                if (!variable.statuses[[row]]$unplotable) {
                  ...
                  .displayError(
                      variable.statuses[[row]]$plottingError, cexText = cexText)
                }
              } else {
              ...
              if (l < 7) {
                if (options$plotStatistics) {
                  if (!variable.statuses[[col]]$unplotable &&
                      !variable.statuses[[row]]$unplotable) {
                    .plotCorValue(
                        dataset[[variables[col]]], dataset[[variables[row]]],
                        hypothesis = options$hypothesis,
                        pearson = options$pearson,
                        kendallsTauB = options$kendallsTauB,
                        spearman = options$spearman,
                        confidenceInterval = options$confidenceIntervalsInterval)  # plot r= ...
                  } else {
                    errorMessages <- c(variable.statuses[[row]]$plottingError,
                                       variable.statuses[[col]]$plottingError)
                    errorMessagePlot <- paste0(
              ...
              if (l >= 7) {
                if (options$plotStatistics) {
                  if (!variable.statuses[[col]]$unplotable &&
                    ...
                    .plotCorValue(
                        dataset[[variables[col]]], dataset[[variables[row]]],
                        cexCI = 1.2, hypothesis = options$hypothesis,
                        pearson = options$pearson,
                        kendallsTauB = options$kendallsTauB,
                        spearman = options$spearman,
                        confidenceInterval = options$confidenceIntervalsInterval)
                  } else {
                    errorMessages <- c(variable.statuses[[row]]$plottingError,
                                       variable.statuses[[col]]$plottingError)
                    errorMessagePlot <- paste0(
...
.plotMarginalCor <- function(variable, cexYlab = 1.3, lwd = 2, rugs = FALSE) {
  ...
  ax2 <- axis(2, at = c(0, max(max(h$density), max(density$y)) / 2,
              ...
              pos = range(ax1) - 0.08 * diff(range(ax1)), cex.axis = 1.7,
              mgp = c(3, 0.7, 0))
...
.plotCorValue <- function(
    xVar, yVar, cexText = 2.5, cexCI = 1.7, hypothesis = "correlated",
    pearson = options$pearson, kendallsTauB = options$kendallsTauB,
    spearman = options$spearman, confidenceInterval = 0.95) {
  ...
  for (i in seq_along(tests)) {
    if (round(cor.test(xVar, yVar, method = tests[i])$estimate, 8) == 1) {
    ...
    } else if (round(cor.test(xVar, yVar, method = tests[i])$estimate, 8) ==
               -1) {
      CIPossible <- FALSE
      ...
      if (tests[i] == "pearson") {
        lab[[i]] <- bquote(italic(r) ==
                           .(formatC(round(cor.test(xVar, yVar,
                                                    method = tests[i])$estimate,
                                           3), format = "f", digits = 3)))
      }
      ...
      if (tests[i] == "spearman") {
        lab[[i]] <- bquote(
            rho == .(formatC(
                         round(cor.test(xVar, yVar, method = tests[i])$estimate,
                               3), format = "f", digits = 3)))
      }
  ...
  if (hypothesis == "correlated" &
      length(tests) == 1 & any(tests == "pearson")) {
    alternative <- "two.sided"
    ctest <- cor.test(
        xVar, yVar, method = tests, conf.level = confidenceInterval)
  }
  ...
  if (any(tests == "pearson") & length(tests) == 1 && CIPossible) {
    ...
    text(1, 0.8, labels = paste(100 * confidenceInterval, "% CI: [", CIlow,
                                ", ", CIhigh, "]", sep = ""), cex = cexCI)
  }
...
.displayError <- function(errorMessage = NULL, cexText = 1.6, lwdAxis = 1.2) {
  plot(1, type = 'n', xlim = c(-1, 1), ylim = 0:1, bty = 'n', axes = FALSE,
       xlab = "", ylab = "")
  ...
  for (k in 1:length(x)) {
    if (k != i) {
      concordanceIndex <- concordanceIndex +
          .concordanceFunction(c(x[i], y[i]), c(x[k], y[k]))
    }
  }
  return(concordanceIndex)
}

.createNonparametricConfidenceIntervals <- function(
    x, y, obsCor, hypothesis = "two-sided", confLevel = 0.95,
    method = "kendall") {
  # Based on sections 8.3 and 8.4 of Hollander, Wolfe & Chicken, Nonparametric Statistical Methods, 3e.
  alpha <- 1 - confLevel
  missingIndices <- as.logical(is.na(x) + is.na(y))  # index those values that
                                                     # are missing
