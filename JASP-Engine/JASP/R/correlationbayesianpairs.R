...
CorrelationBayesianPairs <- function(
    dataset = NULL, options, perform = "run", callback = function(...) 0, ...) {
  # Note: This is the default failed bfObject for wrong data
  #
  failedBfObject <- list(
      n = NaN, r = NaN, stat = NA, bf10 = NA, bfPlus0 = NA, bfPlus0 = NA,
      bfMin0 = NA, ciValue = options$ciValue, ci = list())
    ...
    if (perform == "run") {
      if (options$missingValues == "excludeListwise") {
        dataset <- .readDataSetToEnd(columns.as.numeric = allVariables,
                                     exclude.na.listwise = allVariables)
      } else {
        dataset <- .readDataSetToEnd(columns.as.numeric = allVariables)
      }
    } else {
  ...
  fields <- list(
      ...
      list(name = "r", type = "number", format = "sf:4;dp:3",
           title = nameForFields),
  ...
  for (pair in options$pairs) {
    ...
    if (options$plotPriorAndPosterior ||
        ...
        options$plotSequentialAnalysis || options$plotScatter) {
      plotGroups[[i]] <- list()
    ...
    if (options$plotScatter) {
      if (!is.null(state) &&
          currentPair %in% state$plotPairs &&
          !is.null(diff) && (is.list(diff) && (diff$missingValues == FALSE &&
                                               diff$plotWidth == FALSE &&
                                               diff$plotHeight == FALSE)) &&
                            "plotScatter" %in% state$plotTypes) {
        ...
        stateIndex <- which(state$plotPairs == currentPair &
                            state$plotTypes == "plotScatter")[1]

        plots.correlation[[
            length(plots.correlation) + 1]] <- state$plotsCorrelation[[
                                                         stateIndex]]
        ...
        .plotFunc <- function() {
          .plotScatter.Bcorrelationpairs(
              xlab = pair[[1]], ylab = pair[[2]], dontPlotData = TRUE)
        }
        content <- .writeImage(
      ...
      plotGroups[[
          i]][["ScatterPlot"]] <- plots.correlation[[length(plots.correlation)]]
    ...
    if (options$plotPriorAndPosterior) {
      ...
      if (!is.null(state) &&
          ...
          (is.list(diff) &&
           (diff$priorWidth == FALSE &&
            ...
            diff$corcoefficient == FALSE &&
            diff$bayesFactorType == FALSE &&
            diff$missingValues == FALSE &&
            diff$plotWidth == FALSE && diff$plotHeight == FALSE)) &&
        ...
        stateIndex <- which(state$plotPairs == currentPair &
                            state$plotTypes == "posteriorPlotAddInfo")[1]
        ...
        stateIndex <- which(state$plotPairs == currentPair &
                            state$plotTypes == "posteriorPlot")[1]
        ...
        .plotFunc <- function() {
          .plotPosterior.correlation(
              n = NULL, r = NULL, oneSided = oneSided, dontPlotData = TRUE,
              addInformation = options$plotPriorAndPosteriorAdditionalInfo,
              corCoefficient = options$corcoefficient)
        }
        content <- .writeImage(
            width = 530, height = 400, plot = .plotFunc, obj = TRUE)
        plot[["convertible"]] <- TRUE
      ...
      plotPairs[[length(plotPairs) + 1]] <- paste(pair, collapse = " - ")
      plotGroups[[i]][["PriorPosteriorPlot"]] <- plots.correlation[[
          length(plots.correlation)]]
    ...
    if (options$plotBayesFactorRobustness) {
      if (!is.null(state) &&
        ...
        stateIndex <- which(state$plotPairs == currentPair &
                            state$plotTypes == "robustnessPlotAddInfo")[1]
        ...
        stateIndex <- which(state$plotPairs == currentPair &
                            state$plotTypes == "robustnessPlot")[1]
      ...
      plotPairs[[length(plotPairs) + 1]] <- paste(pair, collapse = " - ")
      plotGroups[[i]][[
          "BFrobustnessPlot"]] <- plots.correlation[[length(plots.correlation)]]
    }
    ...
    if (options$plotSequentialAnalysis) {
      if (!is.null(state) &&
        ...
        stateIndex <- which(state$plotPairs == currentPair &
                            state$plotTypes == "sequentialRobustnessPlot")[1]
        plots.correlation[[
            length(plots.correlation) + 1]] <- state$plotsCorrelation[[
                                                         stateIndex]]
      } else if (!is.null(state) &&
        ...
        stateIndex <- which(state$plotPairs == currentPair &
                            state$plotTypes == "sequentialPlot")[1]
        plots.correlation[[
            length(plots.correlation) + 1]] <- state$plotsCorrelation[[
                                                         stateIndex]]
      } else {
      ...
      plotPairs[[length(plotPairs) + 1]] <- paste(pair, collapse = " - ")
      plotGroups[[i]][[
          "BFsequentialPlot"]] <- plots.correlation[[length(plots.correlation)]]
    }
    i <- i + 1
  }
  ...
  for (i in .indices(options$pairs)) {
    ...
    if (pair[[1]] == "" || pair[[2]] == "") {
      ...
      pairStatuses[[i]] <- list(ready = FALSE, error = FALSE, unplotable = TRUE,
                                unplotableScatter = TRUE)
      result <- list(
          .variable1 = p1, .separator = "-", r = "", .variable2 = p2, BF = "")
    } else {
      if (perform == "init") {
        if (!is.null(state) &&
            tablePairs[[i]] %in% state$tablePairs &&
            !is.null(diff) &&
            (is.list(diff) &&
             (diff$priorWidth == FALSE &&
              diff$hypothesis == FALSE &&
              diff$bayesFactorType == FALSE &&
              diff$missingValues == FALSE && diff$corcoefficient == FALSE))) {

          stateIndex <- which(state$tablePairs ==
                              paste(pair, collapse = " - "))[1]
          ...
          if (state$errorFootnotes[stateIndex] == "no") {
            ...
            result <- list(
                .variable1 = pair[[1]], .separator = "-",
                .variable2 = pair[[2]], r = .clean(NaN), BF = .clean(NaN),
                .footnotes = list(r = list(index2)))
          }
        } else {
          pairStatuses[[i]] <- list(ready = FALSE, error = FALSE,
                                    unplotable = TRUE, unplotableScatter = TRUE)
          result <- list(.variable1 = pair[[1]], .separator = "-",
                         .variable2 = pair[[2]], r = ".", BF = ".")
        }
      } else {
        unplotable <- FALSE
        ...
        if (!is.null(state) &&
          ...
          if (!identical(errors, FALSE)) {
            ...
            if (options$missingValues == "excludeAnalysisByAnalysis") {
              subDataSet <- subset(
                  dataset, select = c(.v(pair[[1]]), .v(pair[[2]])))
              subDataSet <- na.omit(subDataSet)
          ...
          if (!is.null(index)) {
            result <- list(
                .variable1 = pair[[1]], .separator = "-",
                .variable2 = pair[[2]], r = .clean(bfObject$stat),
                BF = .clean(BF10post[i]), .footnotes = list(r = list(index)),
                upperCI = .clean(someUpperCi), lowerCI = .clean(someLowerCi))
          } else {
            result <- list(
                .variable1 = pair[[1]], .separator = "-",
                .variable2 = pair[[2]], r = .clean(bfObject$stat),
                BF = .clean(BF10post[i]), upperCI = .clean(someUpperCi),
                lowerCI = .clean(someLowerCi))
          }

          pairStatuses[[i]] <- list(
              ready = TRUE, error = FALSE, unplotable = unplotable,
              unplotableMessage = unplotableMessage,
              unplotableScatter = unplotableScatter,
              unplotableMessageScatter = unplotableMessageScatter)
        }
      }
    }
  ...
  if (length(correlationRows) == 0)
    correlationRows <- list(list(.variable1 = "...", .separator = "-",
                                 .variable2 = "...", r = "", BF = ""))
  ...
  if (perform == "run" &&
    ...
    for (i in .indices(options$pairs)) {
      ...
      if (options$plotScatter) {
        ...
        if (!is.null(state) &&
            tablePairs[[i]] %in% state$plotPairs &&
            !is.null(diff) &&
            ((is.list(diff) &&
              (diff$missingValues == FALSE &&
               diff$plotWidth == FALSE &&
               diff$plotHeight == FALSE && diff$corcoefficient == FALSE))) &&
            "plotScatter" %in% state$plotTypes) {
          ...
          results[["plots"]][[
              "collection"]][[i]][["ScatterPlot"]][["status"]] <- "running"
          ...
          } else if (status$unplotableScatter &&
                     "unplotableMessageScatter" %in% names(status)) {
      ...
      if (options$plotPriorAndPosterior) {
        ...
        if (!is.null(state) &&
          ...
          if (status$unplotable == FALSE) {
            ...
            p <- try(
                ...
                expr = {
                  ...
                  .plotFunc <- function() {
                    .plotPosterior.correlation(
                        r = rs[i], n = ns[i], oneSided = oneSided,
                        BF = plotBF10post, BFH1H0 = BFH1H0,
                        addInformation = options$plotPriorAndPosteriorAdditionalInfo,
          ...
          } else if (status$unplotable &&
                     "unplotableMessage" %in% names(status)) {

            message <- paste(
                "Plotting is not possible:", status$unplotableMessage)
      ...
      if (options$plotBayesFactorRobustness) {
        ...
        if (!is.null(state) &&
          ...
          results[["plots"]][["collection"]][[
              i]][["BFrobustnessPlot"]][["status"]] <- "running"
            ...
            p <- try(
                ...
                expr = {
                  ...
                  .plotFunc <- function() {
                    ...
                    .plotBF.robustnessCheck.correlation(
                        r = rs[i], n = ns[i], oneSided = oneSided,
                        BF10post = plotBF10post, BFH1H0 = BFH1H0,
                        kappa = options$priorWidth,
                        corCoefficient = options$corcoefficient,
                        additionalInformation = options$plotBayesFactorRobustnessAdditionalInfo)
                  }
      ...
      if (options$plotSequentialAnalysis) {
        ...
        if (!is.null(state) &&
          ...
          results[["plots"]][["collection"]][[
              i]][["BFsequentialPlot"]][["status"]] <- "running"
          ...
          if (status$unplotable == FALSE) {
            p <- try(
                ...
                expr = {
                  ...
                  .plotFunc <- function() {
                    .plotSequentialBF.correlation(
                        x = v1, y = v2, oneSided = oneSided, BF = plotBF10post,
                        BFH1H0 = BFH1H0, kappa = options$priorWidth,
                        corCoefficient = options$corcoefficient)
                  }
                  content <- .writeImage(
                      width = 530, height = 400, plot = .plotFunc, obj = TRUE)
  ...
  for (plot in plots.correlation) keep <- c(keep, plot$data)
  ...
  if (perform == "init") {
    ...
    return(list(results = results, status = "complete",
                state = list(
                    ...
                    plotsCorrelation = plots.correlation, plotTypes = plotTypes,
                    plotPairs = plotPairs, pairStatuses = pairStatuses,
                    BF10post = BF10post, tablePairs = tablePairs,
                    errorFootnotes = errorFootnotes, ns = ns, rs = rs),
                keep = keep))
  }
}


.plotScatter.Bcorrelationpairs <- function(
    xVar = NULL, yVar = NULL, xlab, ylab, dontPlotData = FALSE, cexPoints = 1.3,
    cexXAxis = 1.3, cexYAxis = 1.3, lwd = 2, lwdAxis = 1.2) {
  ...
  ylow <- min((min(yVar) - 0.1 * min(yVar)), min(pretty(yVar)),
              min(.poly.pred(fit, line = FALSE, xMin = xticks[1],
                             xMax = xticks[length(xticks)], lwd = lwd)))
  yhigh <- max((max(yVar) + 0.1 * max(yVar)), max(pretty(yVar)),
               max(.poly.pred(fit, line = FALSE, xMin = xticks[1],
                              xMax = xticks[length(xticks)], lwd = lwd)))
  ...
  plot(xVar, yVar, col = "black", pch = 21, bg = "grey", ylab = "", xlab = "",
       axes = F, ylim = range(yticks), xlim = range(xticks), cex = cexPoints)
...
.plotPosterior.correlation <- function(
    n, r, kappa = 1, oneSided = FALSE, BF, BFH1H0, addInformation = TRUE,
    dontPlotData = FALSE, lwd = 2, corCoefficient = "Pearson", cexPoints = 1.5,
    cexAxis = 1.2, cexYlab = 1.5, cexXlab = 1.5, cexTextBF = 1.4, cexCI = 1.1,
    cexLegend = 1.2, lwdAxis = 1.2) {
  ...
  if (dontPlotData) {
    ...
    mtext(expression("Population correlation" ~ rho), side = 1, cex = cexXlab,
          line = 2.6)
  ...
  if (usePearson) {
    ...
    if (any(is.na(c(CIlow, medianPosterior, CIhigh)))) drawCI <- FALSE
    ...
    if (oneSided == FALSE) {
      ...
      if (sum(is.na(posteriorLine)) > 1 ||
        ...
        if (any(is.na(c(betaA, betaB)))) stop("Posterior is too peaked")
      ...
      if (sum(is.na(posteriorLine)) > 1 ||
        ...
        posteriorLine <- .stretchedBeta(
                             alpha = betaA, beta = betaB, rho = rho) /
  ...
  if (useKendall) {
    betaApproximation <- FALSE
    if (oneSided == FALSE) {
      ci <- .credibleIntervalKendallTau(
          n = n, tauObs = r, ciValue = 0.95, kappa = kappa, test = "two-sided")
      CIlow <- ci[[1]]
      ...
      ci <- .credibleIntervalKendallTau(
          n = n, tauObs = r, ciValue = 0.95, kappa = kappa, test = "positive")
      CIlow <- ci[[1]]
      medianPosterior <- ci[[2]]
      CIhigh <- ci[[3]]
      ...
      ci <- .credibleIntervalKendallTau(
          n = n, tauObs = r, var = 1, ciValue = 0.95, kappa = kappa,
          test = "negative")
      CIlow <- ci[[1]]
      medianPosterior <- ci[[2]]
      CIhigh <- ci[[3]]
    ...
    if (oneSided == FALSE) {
      ...
      posteriorLine <- .posteriorTau(n = n, tauObs = r, tauPop = rho,
                                     kappa = kappa, var = 1, test = "two-sided")
  ...
  if (usePearson) {
    ...
    mtext(expression("Population correlation" ~ tau), side = 1, cex = cexXlab,
          line = 2.6)
  }
  ...
  if (oneSided == "right") {
    ...
    if (usePearson) {
      points(0, .priorRhoPlus(rho = 0, kappa = kappa), col = "black", pch = 21,
             bg = "grey", cex = cexPoints)
    } else if (useKendall) {
      points(0, .priorTauPlus(tauPop = 0, kappa = kappa), col = "black",
             pch = 21, bg = "grey", cex = cexPoints)
    }
    ...
    if (usePearson) {
      points(0, .priorRhoMin(rho = 0, kappa = kappa), col = "black", pch = 21,
             bg = "grey", cex = cexPoints)
    } else if (useKendall) {
      points(0, .priorTauMin(tauPop = 0, kappa = kappa), col = "black",
             pch = 21, bg = "grey", cex = cexPoints)
    }
    points(1e-15, heightPosteriorAtZero, col = "black", pch = 21, bg = "grey",
           cex = cexPoints)

  } else {
    if (betaApproximation && usePearson) {
      points(0, .priorRho(rho = 0, kappa = kappa), col = "black", pch = 21,
             bg = "grey", cex = cexPoints)
      points(0, .stretchedBeta(alpha = betaA, beta = betaB, rho = 0),
             col = "black", pch = 21, bg = "grey", cex = cexPoints)
    } else if (usePearson) {
      # Quentin: Will we ever get here?
      points(0, .priorRho(rho = 0, kappa = kappa), col = "black", pch = 21,
             bg = "grey", cex = cexPoints)
      points(1e-15, .posteriorRho(rho = 1e-8, n = n, r = r, kappa = kappa),
             col = "black", pch = 21, bg = "grey", cex = cexPoints)
    } else if (useKendall) {
      points(
          1e-15, .posteriorTau(n = n, tauObs = r, tauPop = 1e-8, kappa = kappa),
          col = "black", pch = 21, bg = "grey", cex = cexPoints)
      points(0, .priorTau(tauPop = 0, kappa = kappa), col = "black", pch = 21,
             bg = "grey", cex = cexPoints)
    }
  }
...
.plotBF.robustnessCheck.correlation <- function(
    r = NULL, n = NULL, paired = FALSE, BF10post = NULL, kappa = 1,
    callback = function(...) 0, oneSided = FALSE, lwd = 2, cexPoints = 1.4,
    cexAxis = 1.2, cexYXlab = 1.5, cexText = 1.2, cexLegend = 1.4,
    lwdAxis = 1.2, cexEvidence = 1.6, BFH1H0 = TRUE, dontPlotData = FALSE,
    corCoefficient = "Pearson", additionalInformation = TRUE) {
  useKendall <- corCoefficient == "Kendall"
  usePearson <- corCoefficient == "Pearson"
  ...
  for (i in seq_along(kappaValues)[-1]) {
    if (usePearson) {
      bfObject <- .bfPearsonCorrelation(
          n = n, r = r, kappa = kappaValues[i], ciValue = NULL)
    } else if (useKendall) {
      # TODO (Johnny): I removed var=1, because it's done by default already,
      bfObject <- .bfCorrieKernelKendallTau(
          n = n, tauObs = r, kappa = kappaValues[i], var = 1, ciValue = NULL)
    }
  ...
  if (additionalInformation) {
    ...
    points(
        kappa, log(BF10user), pch = 21, bg = "grey", cex = cexPoints, lwd = 1.3)
    points(
        maxBFkappaVal, log(maxBF10), pch = 21, bg = "red", cex = 1.3, lwd = 1.3)
    ...
    maxBFkappaValt <- formatC(
        maxBFkappaVal, digits = 4, format = "f", drop0trailing = TRUE)
    ...
    BFind <- sort(c(BF10userText, BF10maxText), decreasing = TRUE,
                  index.return = TRUE)$ix
    BFsort <- sort(c(BF10userText, BF10maxText), decreasing = TRUE,
                   index.return = TRUE)$x
    ...
    legend(xx, yy, legend = legend[BFind], pch = rep(21, 3),
           pt.bg = pt.bg[BFind], bty = "n", cex = cexLegend, lty = rep(NULL, 3),
           pt.lwd = rep(1.3, 3), pt.cex = pt.cex[BFind])
...
.plotSequentialBF.correlation <- function(
    x = NULL, y = NULL, BF10post, kappa = 1, callback = function(...) 0,
    oneSided = FALSE, lwd = 2, cexPoints = 1.4, cexAxis = 1.2, cexYlab = 1.5,
    cexXlab = 1.6, cexTextBF = 1.4, cexText = 1.2, cexLegend = 1.2,
    cexEvidence = 1.6, lwdAxis = 1.2, plotDifferentPriors = FALSE,
    BFH1H0 = TRUE, dontPlotData = FALSE, corCoefficient = "Pearson") {
  ...
  for (i in seq_along(x)) {
    ...
    if (i == 1) {
      ...
      if (usePearson) {
        ...
        bfObject <- .bfPearsonCorrelation(
            n = nObs, r = rObs, kappa = kappa, ciValue = NULL)
      } else if (useKendall) {
        rObs <- cor(x[1:i], y[1:i], method = "kendall")
        # TODO Johnny: I removed var=1 as it's default
        bfObject <- .bfKendallTau(
            n = nObs, tauObs = rObs, kappa = kappa, ciValue = NULL)
      }
      ...
      if (is.na(BF10[i])) stop("One or more Bayes factors cannot be computed")

      if (is.infinite(BF10[i])) stop("One or more Bayes factors are infinity")
    }
  }
  ...
  if (length(BF10) <= 60) {
    ...
    points(log(BF), pch = 21, bg = "grey", cex = cexPoints, lwd = 1.3)  # user
                                                                        # prior
