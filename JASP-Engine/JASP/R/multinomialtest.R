...
MultinomialTest <- function(
  ...
  if (is.null(dataset)) {
    if (perform == "run") {
      dataset <- .readDataSetToEnd(
          columns.as.numeric = asnum, columns.as.factor = factor,
    ...
    dataset <- .vdf(
        dataset, columns.as.numeric = asnum, columns.as.factor = factor)
  }
  ...
  chisqTable <- descriptivesTable <- NULL  # expected versus observed
  ...
  if (!is.null(state)) {
    ...
    if (is.list(diff)) {
      ...
      if (!any(diff[["factor"]], diff[["counts"]],
               diff[["confidenceIntervalInterval"]], diff[["hypothesis"]],
               diff[["exProbVar"]], diff[["expectedProbs"]])) {

        chisqResults <- state[["chisqResults"]]

        # the following depend on chisqResults so in same if statement
        if (!any(diff[["confidenceInterval"]], diff[["countProp"]])) {
  ...
  if (options[["descriptives"]]) {
    ...
    if (is.null(descriptivesTable)) {
      descriptivesTable <- .multinomialDescriptives(
          chisqResults, factor, options, perform)
    }
  ...
  if (options[["descriptivesPlot"]]) {
    ...
    if (is.null(descriptivesPlot)) {
      descriptivesPlot <- .multinomialDescriptivesPlot(
          chisqResults, options, perform)
    }
...
.chisqTable <- function(chisqResults, options, perform) {
  ...
  fields <- list(
      ...
      list(name = "chisquare", title = "\u03C7\u00B2", type = "number",
           format = "sf:4;dp:3"),
...
.multinomialDescriptives <- function(chisqResults, factor, options, perform) {
  ...
  if (is.null(factor)) {
    ...
    fields <- list(list(name = "factor", title = "Factor", type = "string"),
    ...
    if (options$confidenceInterval) {
      ...
      fields[[length(fields) + 1]] <- list(
          name = "lowerCI", title = "Lower", type = "number",
          format = "sf:4;dp:3", overTitle = title)
      fields[[length(fields) + 1]] <- list(
          name = "upperCI", title = "Upper", type = "number",
          format = "sf:4;dp:3", overTitle = title)
    }
    ...
    if (length(nms) == 1) {
      fields[[length(fields) + 1]] <- c(
          list(name = "expected", title = paste0("Expected: ", nms)),
          numberType)
    } else {
      for (i in 1:length(nms)) {
        fields[[length(fields) + 1]] <- c(list(name = nms[i], title = nms[i]),
                                          numberType, overTitle = "Expected")
      }
    }
    ...
    if (options$confidenceInterval) {
      ...
      # make a list of cis
      ci <- lapply(chisqResults[[1]][["observed"]],
                   function(l) {
                     bt <- binom.test(
                         l, n, conf.level = options$confidenceIntervalInterval)
...
.multinomialDescriptivesPlot <- function(chisqResults, options, perform) {
  ...
  if (perform == "run") {
    ...
    # Create plot
    p <- ggplot2::ggplot(
             mapping = ggplot2::aes(x = factor(names(obs), levels = names(obs)),
                                    y = as.numeric(obs))) +
        ggplot2::geom_bar(
            stat = "identity", size = 0.75, colour = "black", fill = "grey") +
        ggplot2::geom_errorbar(
            ggplot2::aes(ymin = ciDf$lowerCI, ymax = ciDf$upperCI), size = 0.75,
            width = 0.3) +
        base_breaks_y(ciDf$upperCI) + ggplot2::xlab(options$factor) +
    ...
    content <- .writeImage(width = options$plotWidth,
                           height = options$plotHeight, plot = p, obj = TRUE)
