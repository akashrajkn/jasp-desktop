...
RegressionLogLinear <- function(
  ...
  if (is.null(dataset)) {
    ...
    if (perform == "run") {
      ...
      dataset <- .readDataSetToEnd(
          columns.as.factor = options$factors, columns.as.numeric = counts.var)
  ...
  if (options$counts != "" && perform == "run") {
    ...
    if (!is.null(variable.names))
      error.message <- paste(
          "Poisson glm is undefined -- the count variable ", variable.names,
          " contain(s) empty cell or NaN. ", sep = "")
    ...
    if (length(list.of.errors) == 0) {
      ...
      if (!is.null(variable.names))
        error.message <- paste(
            "Poisson glm is undefined -- the count variable ", variable.names,
            " contain(s) infinity and/or negative numbers. ", sep = "")
      list.of.errors[[length(list.of.errors) + 1]] <- error.message
    }
  }
  ...
  if (perform == "run" && length(list.of.errors) == 0) {
    ...
    if (!is.null(variable.names))
      error.message <-
          "Poisson glm is undefined -- the factors contain(s) empty cell or NaN or incomplete contingency table."
    list.of.errors[[length(list.of.errors) + 1]] <- error.message
  }
  ...
  if (length(options$modelTerms) > 0) {
    ...
    if (perform == "run" &&
      ...
      loglm.fit <- try(
          stats::glm(model.formula, family = poisson(), data = dataset),
          silent = TRUE)

      if (class(loglm.fit) == "glm") {

        loglm.model <- list(
            loglm.fit = loglm.fit, variables = variables.in.model)
  ...
  fields <- list(
      ...
      list(name = "Deviance", title = "Deviance", type = "number",
           format = "sf:4;dp:3"), list(name = "Residual df", type = "integer"),
      ...
      if (length(loglm.model$variables) > 0) {
        ...
        for (var in 1:l) {
          ...
          if (var == 1) {
            ...
            logregressionanova.result[[
                len.logreg]]$"df" <- as.integer(loglm.estimates$Df[var])
            logregressionanova.result[[
                len.logreg]]$"Deviance" <- as.numeric(loglm.estimates$Deviance[var])
            logregressionanova.result[[
                len.logreg]]$"p" <- as.numeric(loglm.estimates$"Pr(>Chi)"[var])
            if (options$VovkSellkeMPR) {
              logregressionanova.result[[
                  len.logreg]]$"VovkSellkeMPR" <- .VovkSellkeMPR(
                  logregressionanova.result[[len.logreg]]$"p")
            }
          }
          logregressionanova.result[[len.logreg]]$"Residual df" <- as.integer(
              loglm.estimates$"Resid. Df"[var])
          res <- as.numeric(loglm.estimates$"Resid. Dev"[var])
          if (abs(res) < 10 ^ (-4)) res <- 0
  ...
  if (options$regressionCoefficientsEstimates == TRUE) {
    ...
    if (options$regressionCoefficientsConfidenceIntervals == TRUE) {
      ci.label <- paste(
          100 * options$regressionCoefficientsConfidenceIntervalsInterval,
    ...
    fields <- list(
        list(name = "Name", title = "  ", type = "string"),
        list(name = "Coefficient", title = "Estimate", type = "number",
             format = "dp:3"),
        list(name = "Standard Error", type = "number", format = "dp:3"))

    if (options$regressionCoefficientsConfidenceIntervals == TRUE) {
      fields <- c(fields, list(list(name = "Lower", overTitle = ci.label,
                                    type = "number", format = "dp:3"),
                               list(name = "Upper", overTitle = ci.label,
                                    type = "number", format = "dp:3")))
    }
    ...
    empty.line <- list(  # for empty elements in tables when given output
        "Name" = "", "Coefficient" = "", "Standard Error" = "", "Lower" = "",
        "Upper" = "", "Z" = "", "p" = "")
    if (options$VovkSellkeMPR) {
      empty.line[["VovkSellkeMPR"]] <- ""
    }
        ...
        if (length(loglm.model$variables) > 0) {
          ...
          for (i in seq_along(coef)) {
            ...
            for (j in seq_along(coefficient)) {
              actualName[[j]] <- paste(
                  lookup.table[[coefficient[j]]], collapse = " = ")
            }
            var <- paste0(actualName, collapse = "*")
            #print(var)

            logregression.result[[len.logreg]]$"Name" <- var
            logregression.result[[len.logreg]]$"Coefficient" <- as.numeric(
                unname(loglm.estimates[i, 1]))
            logregression.result[[len.logreg]]$"Standard Error" <- as.numeric(
                loglm.estimates[i, 2])
            ...
            logregression.result[[
                len.logreg]]$"Z" <- as.numeric(loglm.estimates[i, 3])
            logregression.result[[
                len.logreg]]$"p" <- as.numeric(loglm.estimates[i, 4])
            if (options$VovkSellkeMPR) {
              logregression.result[[
                  len.logreg]]$"VovkSellkeMPR" <- .VovkSellkeMPR(
                  logregression.result[[len.logreg]]$"p")
            }
            len.logreg <- len.logreg + 1
          }
        }
