...
.checkPackages <- function() {
  ...
  expected <- matrix(
      nrow = 0, ncol = 2, dimnames = list(NULL, c("Package", "Version")))
  ...
  for (package.name in expected.package.names) {
    ...
    if (package.name %in% installed.package.names) {
      ...
      if (installed.version != expected.version)
        messages <- c(messages, paste("Package ", package.name,
                                      " is not the correct version; expected: ",
                                      expected.version, ", installed: ",
                                      installed.version, sep = ""))

    } else {

      messages <- c(messages, paste("Package ", package.name, " not installed!",
                                    sep = ""))
    }
...
.initPackages <- function(installedPackages) {
  ...
  packages <- c("BayesFactor", "bootnet")  # Add any package that needs
                                           # pre-loading

  for (package in packages) {
    if (package %in% installedPackages &&
        base::isNamespaceLoaded(package) == FALSE) {
