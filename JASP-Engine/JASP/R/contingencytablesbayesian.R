...
ContingencyTablesBayesian <- function(
    dataset, options, perform, callback, ...) {
  ...
  if (is.null(dataset)) {
    if (perform == "run") {
      dataset <- .readDataSetToEnd(
          columns.as.factor = factors, columns.as.numeric = counts.var,
          exclude.na.listwise = counts.var)
    } else {
  ...
  populate <- FALSE
  if (perform == "run" &&
      length(options$columns) > 0 && length(options$rows) > 0) {
    populate <- TRUE
  }
  ...
  for (i in seq_along(analyses)) {
    ...
    res <- .contTablesBayesian(
        dataset, options, populate = populate, analysis, last.results)

    if (!is.null(res$tables[["counts.table"]])) {

      results[[paste("Counts Table", i)]] <- res$tables[["counts.table"]]
      meta[[length(meta) + 1]] <- list(
          name = paste("Counts Table", i), type = "table")
    ...
    if (!is.null(res$tables[["tests.table"]])) {
      ...
      meta[[length(meta) + 1]] <- list(
          name = paste("Tests Table", i), type = "table")

    }
    if (!is.null(res$tables[["odds.ratio.table"]])) {

      results[[
          paste("Odds Ratio Table", i)]] <- res$tables[["odds.ratio.table"]]
      meta[[length(meta) + 1]] <- list(
          name = paste("Odds Ratio Table", i), type = "table")
    ...
    if (!is.null(res$tables[["cramersV.table"]])) {
      ...
      meta[[length(meta) + 1]] <- list(
          name = paste("Kramers V Table", i), type = "table")
  ...
  if (length(plots) > 1) {
    ...
    if (length(options$layers) > 0) {
      results[["LogORplots"]] <- list(
          title = "Log Odds Ratio Plots", collection = plotGroups)
      meta[[length(meta) + 1]] <- list(
          name = "LogORplots", type = "collection",
      ...
      results[["LogORplots"]] <- list(
          title = "Log Odds Ratio Plots", collection = plotGroups)
      meta[[length(meta) + 1]] <- list(
          name = "LogORplots", type = "collection", meta = "image")
    }
...
.contTablesBayesian <- function(dataset, options, populate, analysis, state) {
  ...
  group.matrices <- .contTablesCreateGroupMatrices(
      dataset, analysis$rows, analysis$columns, groups, counts.var,
      options$rowOrder == "descending", options$columnOrder == "descending",
      populate)

  # retrieve states

  counts.state <- NULL
  if ("counts" %in% names(state)) counts.state <- state$counts

  tests.state <- NULL
  if ("tests" %in% names(state)) tests.state <- state$tests

  odds.ratio.state <- NULL
  if ("odds.ratio" %in% names(state)) odds.ratio.state <- state$odds.ratio

  CramersV.state <- NULL
  if ("CramersV" %in% names(state)) CramersV.state <- state$CramersV

  plots.state <- NULL
  if ("plots.state" %in% names(state)) plots.state <- state$plots.state
  ...
  old.options <- NULL
  if ("options" %in% names(state)) old.options <- state$options
  ...
  res <- .contTablesBayesianCreateCountsTable(
      dataset, analysis, group.matrices, groups, footnotes, options, populate,
      counts.state, old.options, status)
  ...
  res <- .contTablesBayesianCreateTestsTable(
      dataset, analysis, group.matrices, groups, footnotes, options, populate,
      tests.state, old.options, status)
  ...
  res <- .contTablesBayesianCreateOddsRatioTable(
      dataset, analysis, group.matrices, groups, footnotes, options, populate,
      bf.results, odds.ratio.state, old.options, status)
  ...
  res <- .contTablesBayesianCreateCramerVTable(
      dataset, analysis, group.matrices, groups, footnotes, options, populate,
      bf.results, CramersV.state, old.options, status)
  ...
  res <- .contTablesBayesianCreateOddsRatioPlots(
      dataset, analysis, group.matrices, groups, options, populate,
      odds.ratio.results, bf.results, plots.state, old.options, status)
  ...
  list(tables = tables, plots = plots, keep = keep, state = new.state,
       complete = complete)
}

.contTablesBayesianCreateOddsRatioPlots <- function(
    dataset, analysis, counts.matrices, groups, options, populate,
    odds.ratio.results, bf.results, state, state.options, status) {

  if (options$plotPosteriorOddsRatio == FALSE ||
      identical(dim(counts.matrices[[1]]), as.integer(c(2, 2))) == FALSE)
  ...
  if (is.null(state.options) ||
      ...
      base::identical(state.options$columnOrder, options$columnOrder) ==
      FALSE ||
      base::identical(state.options$rowOrder, options$rowOrder) == FALSE ||
      base::identical(state.options$samplingModel, options$samplingModel) ==
      FALSE ||
      base::identical(state.options$hypothesis, options$hypothesis) == FALSE ||
      base::identical(state.options$oddsRatioCredibleIntervalInterval,
                      options$oddsRatioCredibleIntervalInterval) == FALSE ||
      base::identical(state.options$plotPosteriorOddsRatioAdditionalInfo,
                      options$plotPosteriorOddsRatioAdditionalInfo) == FALSE ||
      base::identical(state.options$priorConcentration,
                      options$priorConcentration) == FALSE) {
  ...
  for (i in 1:length(counts.matrices)) {
    ...
    plot.state <- NULL
    if (i <= length(state)) plot.state <- state[[i]]
    ...
    bf.result <- NULL
    if (i <= length(bf.results)) bf.result <- bf.results[[i]]

    res <- .contTablesBayesianCreateOddsRatioPlot(
        analysis$rows, analysis$columns, counts.matrix, options, populate,
        group, odds.ratio.result, bf.result, plot.state, state.options, status)
  ...
  complete <- complete &&
      (populate || is.null(state) == FALSE ||
                   length(options$rows) == 0 || length(options$columns) == 0)

  list(plots = plots, keep = keep, state = new.state, complete = complete)
}

.contTablesBayesianCreateOddsRatioPlot <- function(
    row.var, col.var, counts.matrix, options, populate, group,
    odds.ratio.result, bf.result, plot.state, state.options, status) {
  ...
  if (is.null(plot.state) == FALSE) {
    ...
    if ("status" %in% names(plot.state) && plot.state$status$error) {
      odds.ratio.plot[["error"]] <- list(
          errorType = "badData",
          errorMessage = paste0(
              "Plotting is not possible: ", plot.state$status$errorMessage))

    }

    return(list(plot = odds.ratio.plot, state = plot.state,
                keep = plot.state$keep, complete = TRUE))
  }
  ...
  if (populate && status$error == FALSE) {
    ...
    if (is.null(bf.result$BF) == FALSE && inherits(bf.result$BF, "try-error")) {
      ...
      status <- list(
          error = TRUE,
          errorMessage = "Odds ratio for this model not yet implemented")
      ...
      if (is.na(BF10)) {
        ...
        status <- list(
            error = TRUE, errorMessage = "The Bayes factor is undefined")

      } else if (is.infinite(1 / BF10)) {

        status <- list(
            error = TRUE, errorMessage = "The Bayes factor is too small")

      } else if (is.infinite(BF10)) {

        status <- list(
            error = TRUE, errorMessage = "The Bayes factor is infinite")
      }
    }
  }
        ...
        expr = {
          ...
          .plotFunc <- function() {
            .contTablesBayesianPlotPosterior(
                ...
                addInformation = options$plotPosteriorOddsRatioAdditionalInfo,
  ...
  if (populate && status$error == FALSE) {
    ...
    if (status$error) {
      ...
      odds.ratio.plot[["error"]] <- list(
          error = "badData",
          errorMessage = paste("Plotting is not possible:", message))
      odds.ratio.plot[["status"]] <- "complete"
      complete <- TRUE
  ...
  list(plot = odds.ratio.plot, state = new.plot.state, keep = plot[["png"]],
       complete = complete)
}



.contTablesBayesianCreateCountsTable <- function(
    dataset, analysis, counts.matrices, groups, footnotes, options, populate,
    state, state.options, status) {
  ...
  if (length(analysis) >= 3) {
    ...
    for (j in length(analysis):3)
      counts.fields[[length(counts.fields) + 1]] <- list(
          name = analysis[[j]], type = "string", combine = TRUE)
  }

  counts.fields[[length(counts.fields) + 1]] <- list(
      name = analysis$rows, type = "string", combine = TRUE)
  ...
  counts.fp <- FALSE  # whether the counts are float point or not; changes
                      # formatting

  if (options$counts != "") {

    counts <- dataset[[.v(options$counts)]]
    if (identical(counts, as.integer(counts)) == FALSE)  # are the counts
                                                         # floating point?
      counts.fp <- TRUE
  }
  ...
  for (column.name in lvls) {
    ...
    if (counts.fp || options$countsExpected) {
      ...
      counts.fields[[length(counts.fields) + 1]] <- list(
          name = private.name, title = column.name, overTitle = overTitle,
  ...
  if (counts.fp || options$countsExpected) {
    ...
    counts.fields[[length(counts.fields) + 1]] <- list(
        name = "total[counts]", title = "Total", type = "number",
        format = "sf:4;dp:2")

  } else {

    counts.fields[[length(counts.fields) + 1]] <- list(
        name = "total[counts]", title = "Total", type = "integer")
  }

  if (options$countsExpected) {

    counts.fields[[length(counts.fields) + 1]] <- list(
        name = "total[expected]", title = "Total", type = "number",
        format = "sf:4;dp:2")
  }
  ...
  if (is.null(state) == FALSE) status <- state$status
  ...
  if (populate && options$counts != "") {
    ...
    if (any(counts < 0) || any(is.infinite(counts))) {
      ...
      status <- list(
          error = TRUE,
          errorMessage = "Counts may not contain negative numbers or infinities")
    }
  }
  ...
  for (i in 1:length(counts.matrices)) {
    ...
    next.rows <- .contTableBayesianCreateCountsRows(
        analysis$rows, counts.matrix, options, populate, group, rows.state)
  ...
  complete <- populate || is.null(state) == FALSE ||
                              length(options$rows) == 0 ||
                                  length(options$columns) == 0 || status$error
  ...
  list(table = counts.table, state = new.state, complete = complete,
       status = status)
}
  ...
  if (length(analysis) >= 3)  # if layers are specified
    ...
    if (length(analysis) >= 4) {
      ...
      for (j in 4:length(analysis)) {
        ...
        groups <- cbind(
            rep(lvls, each = dim(groups)[1]), groups, stringsAsFactors = FALSE)
...
.contTablesBayesianCreateTestsTable <- function(
    dataset, analysis, counts.matrices, groups, footnotes, options, populate,
    state, state.options, status) {
  ...
  if (status$error) populate <- FALSE
  ...
  fields[[length(fields) + 1]] <- list(
      name = "type[BF]", title = "", type = "string")
  fields[[length(fields) + 1]] <- list(name = "value[BF]", title = "Value",
                                       type = "number", format = "sf:4;dp:3")
  fields[[length(fields) + 1]] <- list(
      name = "type[N]", title = "", type = "string")
  fields[[length(fields) + 1]] <- list(
      name = "value[N]", title = "Value", type = "integer")
  ...
  for (i in 1:length(counts.matrices)) {
    ...
    rows.state <- NULL
    if (i <= length(state)) rows.state <- state[[i]]

    res <- .contTablesBayesianCreateTestsRows(
        analysis$rows, counts.matrix, footnotes, options, populate, group,
        rows.state)
  ...
  if (complete) table[["status"]] <- "complete"
  ...
  if (populate == FALSE && is.null(state)) new.state <- NULL

  list(table = table, state = new.state, complete = complete, status = status)
}

.contTablesBayesianCreateOddsRatioTable <- function(
    dataset, analysis, counts.matrices, groups, footnotes, options, populate,
    bf.results, state, state.options, status) {
  ...
  fields[[length(fields) + 1]] <- list(
      name = "value[oddsRatio]", title = "Log Odds Ratio", type = "number",
      format = "sf:4;dp:3")
  fields[[length(fields) + 1]] <- list(
      name = "low[oddsRatio]", title = "Lower", overTitle = ci.label,
      type = "number", format = "dp:3")
  fields[[length(fields) + 1]] <- list(
      name = "up[oddsRatio]", title = "Upper", overTitle = ci.label,
      type = "number", format = "dp:3")
  ...
  for (i in 1:length(counts.matrices)) {
    ...
    res <- .contTablesBayesianCreateOddsRatioRows(
        analysis$rows, counts.matrix, footnotes, options, populate, group,
        bf.result, rows.state, state.options, status)
  ...
  if (options$oddsRatio == FALSE) table <- NULL

  list(table = table, state = new.state, complete = complete, status = status)
}


.contTablesBayesianCreateCramerVTable <- function(
    dataset, analysis, counts.matrices, groups, footnotes, options, populate,
    bf.results, state, state.options, status) {
  ...
  if (is.null(state.options) ||
      ...
      base::identical(state.options$priorConcentration,
                      options$priorConcentration) == FALSE ||
      base::identical(state.options$effectSizeCredibleIntervalInterval,
                      options$effectSizeCredibleIntervalInterval) == FALSE ||
      base::identical(state.options$hypothesis, options$hypothesis) == FALSE) {
  ...
  ci.label <- paste(100 * options$effectSizeCredibleIntervalInterval,
                    "% Credible Interval", sep = "")

  fields[[length(fields) + 1]] <- list(
      name = "value[CramerV]", title = "Cramer's V", type = "number",
      format = "sf:4;dp:3")
  fields[[length(fields) + 1]] <- list(
      name = "low[CramerV]", title = "Lower", overTitle = ci.label,
      type = "number", format = "dp:3")
  fields[[length(fields) + 1]] <- list(
      name = "up[CramerV]", title = "Upper", overTitle = ci.label,
      type = "number", format = "dp:3")
  ...
  for (i in 1:length(counts.matrices)) {
    ...
    res <- .contTablesBayesianCreateCramerVRows(
        analysis$rows, counts.matrix, footnotes, options, populate, group,
        bf.result, rows.state, state.options, status)
  ...
  if (options$effectSize == FALSE) table <- NULL

  list(table = table, state = new.state, complete = complete, status = status)
}

.contTablesBayesianCreateTestsRows <- function(
    var.name, counts.matrix, footnotes, options, populate, group, state) {
  ...
  if (options$samplingModel == "poisson") {
    ...
    if (options$hypothesis == "groupsNotEqual") {
      ...
      if (options$bayesFactorType == "BF10") {
        ...
        bfLabel <-
            "Log\u2009(\u2009BF\u2081\u2080\u2009) independent multinomial"
      }
  ...
  results <- state
  if (is.null(results)) results <- list(N = NULL, BF = NULL)

  if (populate && is.null(results$N)) results$N = base::sum(counts.matrix)
    ...
    results$BF <- try(
        {
          ...
          BF <- BayesFactor::contingencyTableBF(
              counts.matrix, sampleType = sampleType,
          ...
          if (options$samplingModel == "hypergeometric") {
            ...
            if (options$hypothesis == "groupOneGreater" &&
                options$samplingModel == "poisson") {

              #ch.result = BayesFactor::posterior(BF, iterations = 10000)
              theta <- as.data.frame(ch.result)

              odds.ratio <- (theta[, 1] * theta[, 4]) /
                  (theta[, 2] * theta[, 3])
              ...
              prop.consistent <- mean(theta[, 1] > theta[, 3])  # sum(p1.sim >
                                                                # p2.sim)/N.sim
              bf1 <- bf1 * prop.consistent / 0.5
              lbf1 <- lbf1 + log(prop.consistent) - log(0.5)
  ...
  if (is.null(results$BF)) {
  ...
  } else if (!identical(dim(counts.matrix), as.integer(c(2, 2))) &&
    ...
    if (options$hypothesis == "groupOneGreater" &&
      ...
      message <- paste(
          "For all tests, the alternative hypothesis specifies that group <em>",
          gp1, "</em> is greater than group <em>", gp2, ".</em>", sep = "")
      ...
      message <- paste(
          "For all tests, the alternative hypothesis specifies that group <em>",
          gp1, "</em> is less than group <em>", gp2, ".</em>", sep = "")
      .addFootnote(footnotes, symbol = "<em>Note.</em>", text = message)
...
.contTablesCreateGroupMatrices <- function(
    dataset, rows, columns, groups, counts = NULL, rowOrderDescending = FALSE,
    columnOrderDescending = FALSE, populate) {
  ...
  if (is.null(groups)) {
    ...
    if (populate == FALSE) {
      ...
      if (rows != "") row.levels <- base::levels(dataset[[.v(rows)]])
      if (columns != "") col.levels <- base::levels(dataset[[.v(columns)]])

      ss.matrix <- base::matrix(
          0, nrow = length(row.levels), ncol = length(col.levels),
      ...
      ss.matrix <- base::matrix(
          ss.table, nrow = dim(ss.table)[1], ncol = dim(ss.table)[2],
          dimnames = dimnames(ss.table))

    } else {

      ss.dataset <- base::subset(dataset, select = .v(c(rows, columns, counts)))
      ss.matrix <- base::tapply(
          ss.dataset[[.v(counts)]],
          list(ss.dataset[[.v(rows)]], ss.dataset[[.v(columns)]]), base::sum)
    ...
    for (group in groups) {
      ...
      if (populate == FALSE) {
        ...
        ss.dataset <- base::subset(
            dataset, select = .v(c(rows, columns, counts)))

      } else if (populate) {

        ss.filter.string <- base::paste(
            .v(names(group)), "==\"", group, "\"", sep = "", collapse = "&")
...
.contTableBayesianCreateCountsRows <- function(
    var.name, counts.matrix, options, populate, group, last.rows) {
  ...
  for (i in 1:dim(counts.matrix)[[1]]) {
    ...
    row <- list()
    if (is.list(last.rows) && i <= length(last.rows)) row <- last.rows[[i]]
    ...
    if (options$percentagesRow) {
      ...
      names(row.proportions) <- paste(
          names(row.proportions), "[row.proportions]", sep = "")

      if (class(row.proportions.matrix[1, 1]) == "character") {
        row.proportions[["total[row.proportions]"]] <- ""
      } else {
        row.proportions[["total[row.proportions]"]] <- .clean(
            base::sum(row.proportions.matrix[i, ]))
      }
    ...
    if (options$percentagesColumn) {
      ...
      names(col.proportions) <- paste(
          names(col.proportions), "[col.proportions]", sep = "")
    ...
    if (options$percentagesTotal) {
      ...
      if (class(proportions.matrix[1, 1]) == "character") {
        ...
        proportions[[
            "total[proportions]"]] <- .clean(base::sum(proportions.matrix[i, ]))
      }
  ...
  if (populate) {
    ...
    if (options$percentagesRow) {
      ...
      if (class(row.proportions.matrix[1, 1]) == "character") {
        ...
        row.proportions[[
            "total[row.proportions]"]] <- .clean(base::sum(rowproportion))
      }
    ...
    if (options$percentagesColumn) {
      ...
      if (class(row.proportions.matrix[1, 1]) == "character") {
        ...
        col.proportions[[
            "total[col.proportions]"]] <- .clean(base::sum(row.prop))
      }
    ...
    if (options$percentagesTotal) {
      ...
      if (class(proportions.matrix[1, 1]) == "character") {
        ...
        proportions[[
            "total[proportions]"]] <- .clean(base::sum(proportions.matrix))
      }
...
.contTablesBayesianCreateOddsRatioRows <- function(
    var.name, counts.matrix, footnotes, options, populate, group, bf.result,
    state, state.options, status) {
  ...
  if (!identical(dim(counts.matrix), as.integer(c(2, 2)))) {
    ...
    sup <- .addFootnote(
        footnotes, "Odd ratio for this model not yet implemented")
    row[[".footnotes"]] <- list("value[oddsRatio]" = list(sup))
        ...
        result <- try(
            {
              ...
              if (options$samplingModel == "poisson") {
                ...
                odds.ratio.samples <- (lambda[, 1] * lambda[, 4]) /
                    (lambda[, 2] * lambda[, 3])
                ...
                #ch.result  <- BayesFactor::posterior(BF, iterations = 10000)
                theta <- as.data.frame(ch.result)
                odds.ratio.samples <- (theta[, 1] * theta[, 4]) /
                    (theta[, 2] * theta[, 3])

              } else if (options$samplingModel ==
                         "independentMultinomialRowsFixed") {
              ...
              alpha <- (1 - sig) / 2
              lower <- unname(
                  stats::quantile(log.odds.ratio.samples, p = alpha))
              upper <- unname(
                  stats::quantile(log.odds.ratio.samples, p = (1 - alpha)))

              list(log.odds.ratio.samples = log.odds.ratio.samples, BF = BF,
                   median = log.odds.ratio.median, lower.ci = lower,
                   upper.ci = upper)
            })
      }
    }
...
.contTablesBayesianCreateCramerVRows <- function(
    var.name, counts.matrix, footnotes, options, populate, group, bf.result,
    state, state.options, status) {
  ...
  if (options$samplingModel == "hypergeometric") {
    ...
    sup <- .addFootnote(
        footnotes, "Cramer's V for this model not yet implemented")
        ...
        result <- try(
            {
              ...
              if (options$samplingModel == "poisson") {
                ...
                chi2 <- apply(
                    theta0, 2, function(data)
                                 chisq.test(matrix(data, I))$statistic)
                ...
                theta <- as.data.frame(ch.result[, index])
                theta0 <- apply(
                    theta, 1, function(x) matrix(x, J, byrow = TRUE))
                chi2 <- apply(
                    theta0 * yc, 2, function(data)
                                      chisq.test(matrix(data, J))$statistic)
                Phi.indepMulti <- sqrt(chi2 / (N * (k - 1)))
                Cramer <- Phi.indepMulti
              ...
              list(CramersV.samples = CramersV.samples, BF = BF,
                   CVmedian = CramersV.median, CV.lower.ci = lower,
                   CV.upper.ci = upper)
            })
      }
    }
...
.contTablesBayesianPlotPosterior <- function(
    samples, CI, medianSamples, BF, oneSided = FALSE, iterations = 10000,
    lwd = 2, cexPoints = 1.5, cexAxis = 1.2, cexYlab = 1.5, cexXlab = 1.5,
    cexTextBF = 1.4, cexCI = 1.1, cexLegend = 1.2, lwdAxis = 1.2,
    addInformation = FALSE, dontPlotData = FALSE,
    selectedCI = options$oddsRatioCredibleIntervalInterval, options) {
  ...
  ylim[1] <- 0
  ylim[2] <- stretch *
      max(.dposteriorOR(seq(min(xticks), max(xticks), length.out = 10000),
                        mean(samples), sd(samples), oneSided = oneSided))
  ...
  plot(0, 0, xlim = range(xticks), ylim = c(0, range(yticks)[2]), ylab = "",
       xlab = "", type = "n", axes = FALSE)

  lines(
      seq(min(xticks), max(xticks), length.out = 10000),
      .dposteriorOR(seq(min(xticks), max(xticks), length.out = 10000),
                    mean(samples), sd(samples), oneSided = oneSided), lwd = lwd)

  if (oneSided == "right" || oneSided == "left")
    lines(rep(0, 2), c(0, .dposteriorOR(0, mean(samples), sd(samples),
                                        oneSided = oneSided)), lwd = lwd)
  ...
  # credible interval
  dmax <- optimize(function(x) .dposteriorOR(x, mean(samples), sd(samples),
                                             oneSided = oneSided),
                   interval = range(xticks),
                   maximum = TRUE)$objective  # get maximum density
  ...
  if (oneSided == FALSE)
    arrows(
        CIlow, yCI, CIhigh, yCI, angle = 90, code = 3, length = 0.1, lwd = lwd)
  ...
  if (addInformation) {
    ...
    CIText <- paste(CInumber, bquote(.(formatC(CIlow, 3, format = "f"))), ", ",
...
.dposteriorOR <- function(logOR, mean, sd, oneSided) {
  ...
  if (oneSided == FALSE) {
    ...
    ifelse(logOR >= 0, dnorm(logOR, mean, sd) /
                       pnorm(0, mean, sd, lower.tail = FALSE), 0)

  } else if (oneSided == "left") {

    ifelse(logOR <= 0, dnorm(logOR, mean, sd) /
                       pnorm(0, mean, sd, lower.tail = TRUE), 0)
  }
}
