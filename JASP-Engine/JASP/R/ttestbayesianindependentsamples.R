...
TTestBayesianIndependentSamples <- function(
    ...
    if (perform == "run") {
      ...
      if (options$missingValues == "excludeListwise") {
        ...
        dataset <- .readDataSetToEnd(
            columns.as.numeric = dependents, columns.as.factor = grouping,
            exclude.na.listwise = c(dependents, grouping))

      } else {

        dataset <- .readDataSetToEnd(
            columns.as.numeric = dependents, columns.as.factor = grouping,
            exclude.na.listwise = grouping)
      }
  ...
  if (length(options$variables) != 0 && options$groupingVariable != '') {
    errors <- .hasErrors(
        dataset, perform, type = 'factorLevels',
  ...
  ttest.results <- .ttestBayesianIndependentSamplesTTest(
      dataset, options, perform, state = state, diff = diff)
  ...
  descriptivesTable <- .ttestBayesianIndependentSamplesDescriptives(
      dataset, options, perform)
  results[["descriptives"]] <- list(
      descriptivesTable = descriptivesTable, title = "Descriptives")
  ...
  if (options$plotPriorAndPosterior ||
      ...
      options$plotBayesFactorRobustness || options$descriptivesPlots) {
    ...
    if (!is.null(state)) {
      ...
      if ((BFtypeState == "LogBF10" || BFtypeState == "BF10") &&
          BFtype == "BF01") {
        BFtypeRequiresNewPlot <- TRUE
      } else if (BFtypeState == "BF01" &&
                 (BFtype == "LogBF10" || BFtype == "BF10")) {
        BFtypeRequiresNewPlot <- TRUE
      }
    }
      ...
      if (options$descriptivesPlots) {
        ...
        if (!is.null(state) &&
            ...
            ((is.logical(diff) && diff == FALSE) ||
             (is.list(diff) && (diff$groupingVariable == FALSE &&
                                diff$missingValues == FALSE &&
                                diff$plotHeight == FALSE &&
                                diff$plotWidth == FALSE &&
                                diff$descriptivesPlotsCredibleInterval ==
                                FALSE))) && options$descriptivesPlots) {
    ...
    if (options$descriptivesPlots)
      results[["descriptives"]][["descriptivesPlots"]] <- list(
          title = ifelse(length(options[["variables"]]) > 1,
                         "Descriptives Plots", "Descriptives Plot"),
          collection = descriptivesPlots)


    if (perform == "run" &&
        length(options$variables) > 0 && !is.null(grouping)) {
      ...
      for (variable in options[["variables"]]) {
        ...
        subDataSet <- subset(
            dataset, select = c(.v(variable), .v(options$groupingVariable)))
        subDataSet <- na.omit(subDataSet)

        r.size <- options$priorWidth

        group2 <- subDataSet[
            subDataSet[[.v(options$groupingVariable)]] == g1, .v(variable)]
        group1 <- subDataSet[
            subDataSet[[.v(options$groupingVariable)]] == g2, .v(variable)]
            ...
            if (!is.null(errorMessage)) {
              ...
              p <- try(
                  ...
                  expr = {
                    ...
                    p <- .plot2GroupMeansBayesIndTtest(
                        v1 = group2, v2 = group1, nameV1 = g1, nameV2 = g2,
                        groupingName = options$groupingVariable,
                        dependentName = variable,
            ...
            if (!is.null(errorMessage)) {
              ...
              p <- try(
                  ...
                  expr = {
                    ...
                    .plotFunc <- function() {
                      .plotPosterior.summarystats.ttest(
                          t = tValue[i], n1 = n_group2[i], n2 = n_group1[i],
        ...
        if (options$plotBayesFactorRobustness) {
                    ...
                    .plotFunc <- function() {
                      .plotBF.robustnessCheck.ttest(
                          ...
                          BF10post = ifelse(
                              ...
                              exp(BF10post[i]), BF10post[i]), paired = FALSE,
                          oneSided = oneSided, rscale = options$priorWidth,
                          BFH1H0 = BFH1H0,
                          additionalInformation = options$plotBayesFactorRobustnessAdditionalInfo)
                    }
                    content <- .writeImage(
          ...
          results[["inferentialPlots"]][["collection"]] <-
              plotGroups  # add plots without image object to results
        ...
        if (options$plotSequentialAnalysis) {
          ...
          if (!is.null(state) &&
            ...
            plots.ttest[[z]] <- state$plotsTtest[[index]]
            results[["inferentialPlots"]][["collection"]][[
                i]][["BFsequentialPlot"]][["status"]] <- "complete"
                  ...
                  expr = {
                    ...
                    .plotFunc <- function() {
                      .plotSequentialBF.ttest(
                          x = group2, y = group1, paired = FALSE,
                          oneSided = oneSided, rscale = options$priorWidth,
                          BFH1H0 = BFH1H0, BF10post = BF10post[i],
                          plotDifferentPriors = options$plotSequentialAnalysisRobustness,
                          subDataSet = subDataSet, level1 = g1, level2 = g2,
                          options = options)
                    }
                    content <- .writeImage(
                        width = 530, height = 400, plot = .plotFunc, obj = TRUE)
    ...
    return(
        list(results = results, status = "complete",
             state = list(
                 ...
                 errorFootnotes = errorFootnotes, tValue = tValue,
                 n_group2 = n_group2, n_group1 = n_group1), keep = keep))
  }

}


.ttestBayesianIndependentSamplesTTest <- function(
    dataset, options, perform, state, diff) {
  ...
  if (options$bayesFactorType == "BF01") {
    ...
    if (options$hypothesis == "groupsNotEqual") {
      fields[[length(fields) + 1]] <- list(
          name = "BF", type = "number", format = "sf:4;dp:3",
          title = "BF\u2080\u2081")
    }
    if (options$hypothesis == "groupOneGreater") {
      fields[[length(fields) + 1]] <- list(
  ...
  if (options$hypothesis == "groupsNotEqual") {
    ...
    fields[[length(fields) + 1]] <- list(
        name = "error", type = "number", format = "sf:4;dp:3",
        title = "error %")
  ...
  if (options$hypothesis == "groupOneGreater") {
    ...
    message <- paste(
        ...
        g1, "</em> is greater than group <em>", g2, "</em>.", sep = "")
    ...
    message <- paste(
        "For all tests, the alternative hypothesis specifies that group <em>",
        g1, "</em> is less than group <em>", g2, "</em>.", sep = "")
    .addFootnote(footnotes, symbol = "<em>Note.</em>", text = message)
  }
      ...
      if (state$errorFootnotes[index] == "no") {
        ...
        if (!(is.logical(diff) && diff == FALSE) && diff$bayesFactorType) {
          ...
          if (state$options$bayesFactorType == "BF10") {
            ...
            if (options$bayesFactorType == "BF01") {
              ttest.rows[[length(ttest.rows)]]$BF <- 1 /
                  state$results$ttest$data[[index]]$BF
            } else if (options$bayesFactorType == "LogBF10") {
              ttest.rows[[length(ttest.rows)]]$BF <- log(
                  state$results$ttest$data[[index]]$BF)
            }
            ...
            if (options$bayesFactorType == "BF10") {
              ...
              ttest.rows[[length(ttest.rows)]]$BF <- log(
                  1 / state$results$ttest$data[[index]]$BF)
            }

          } else if (state$options$bayesFactorType == "LogBF10") {

            if (options$bayesFactorType == "BF10") {
              ttest.rows[[length(ttest.rows)]]$BF <- exp(
                  state$results$ttest$data[[index]]$BF)
            } else if (options$bayesFactorType == "BF01") {
              ttest.rows[[length(ttest.rows)]]$BF <- 1 /
                  exp(state$results$ttest$data[[index]]$BF)
            }
          }
        }
  ...
  if (!is.null(state) &&
      all(options[["variables"]] %in% state$options$variables) &&
      options$groupingVariable == state$options$groupingVariable &&
      all(rowCompleted)) ttest[["status"]] <- "complete"

  if (perform == "run" &&
      length(options$variables) != 0 && options$groupingVariable != "") {

    if (length(levels) != 2) {

      ttest[["error"]] <- list(
          errorType = "badData",
          errorMessage = "The Grouping Variable must have 2 levels")

      status <- rep("error", length(options$variables))
      plottingError <- rep(
          "Plotting is not possible: The Grouping Variable must have 2 levels",
        ...
        if (!is.null(state) &&
          ...
          if (state$errorFootnotes[index] == "no") {
            ...
            if (!(is.logical(diff) && diff == FALSE) && diff$bayesFactorType) {
              ...
              if (state$options$bayesFactorType == "BF10") {
                ...
                if (options$bayesFactorType == "BF01") {
                  ttest.rows[[rowNo]]$BF <- 1 /
                      state$results$ttest$data[[index]]$BF
                } else if (options$bayesFactorType == "LogBF10") {
                  ttest.rows[[
                      rowNo]]$BF <- log(state$results$ttest$data[[index]]$BF)
                }
                ...
                if (options$bayesFactorType == "BF10") {
                  ttest.rows[[
                      rowNo]]$BF <- exp(state$results$ttest$data[[index]]$BF)
                } else if (options$bayesFactorType == "BF01") {
                  ttest.rows[[rowNo]]$BF <- 1 /
                      exp(state$results$ttest$data[[index]]$BF)
                }
          ...
          result <- try(
              ...
              expr = {
                ...
                r <- .generalTtestBF(x = group2, y = group1, paired = FALSE,
                ...
                if (options$bayesFactorType == "BF01") bf.raw <- 1 / bf.raw
                ...
                if (idData > 1 &&
                    idData2 > 1 && (options$plotSequentialAnalysis |
                                    options$plotSequentialAnalysisRobustness)) {
  ...
  list(ttest = ttest, status = status, g1 = g1, g2 = g2, BFH1H0 = BFH1H0,
       plottingError = plottingError, BF10post = BF10post,
       errorFootnotes = errorFootnotes, tValue = tValue, n_group2 = n_group2,
       n_group1 = n_group1)
}
...
.plot2GroupMeansBayesIndTtest <- function(
    v1 = NULL, v2 = NULL, nameV1 = NULL, nameV2 = NULL, groupingName = NULL,
    dependentName = NULL, descriptivesPlotsCredibleInterval = .95) {

  v1 <- na.omit(v1)
  v2 <- na.omit(v2)

  posteriorSummary1 <- .posteriorSummaryGroupMean(
      variable = v1,
      descriptivesPlotsCredibleInterval = descriptivesPlotsCredibleInterval)
  posteriorSummary2 <- .posteriorSummaryGroupMean(
      variable = v2,
      descriptivesPlotsCredibleInterval = descriptivesPlotsCredibleInterval)
  summaryStat <- data.frame(
      groupingVariable = c(nameV1, nameV2),
      dependent = c(posteriorSummary1$median, posteriorSummary2$median),
      ciLower = c(posteriorSummary1$ciLower, posteriorSummary2$ciLower),
      ciUpper = c(posteriorSummary1$ciUpper, posteriorSummary2$ciUpper))
  ...
  p <- ggplot2::ggplot(summaryStat, ggplot2::aes(x = groupingVariable,
      ...
      ggplot2::xlab(groupingName) + ggplot2::theme_bw() +
      ggplot2::theme(
...
.ttestBayesianIndependentSamplesDescriptives <- function(
    dataset, options, perform, state = NULL, diff = NULL) {
  ...
  fields <- list(
      ...
      list(
          name = "mean", title = "Mean", type = "number", format = "sf:4;dp:3"),
  ...
  isAllright <- function(variable, options, state = NULL, diff = NULL) {
    ...
    cond2 <- (!is.null(diff) &&
              (is.logical(diff) && diff == FALSE) ||
              (is.list(diff) && !any(diff$descriptives, diff$groupingVariable,
                                     diff$missingValues)))
  ...
  done <- (!is.null(state) && state$options$descriptives &&
        ...
        if (isAllright(variable, options, state, diff)) {
          ...
          for (i in 1:2) {
            ...
            if (class(groupDataOm) != "factor") {
              ...
              result <- list(
                  variable = variable, group = level, N = n, mean = mean,
                  sd = std, se = sem, lowerCI = ciLower, upperCI = ciUpper)

            } else {

              n <- .clean(length(groupDataOm))
              result <- list(variable = variable, group = "", N = n, mean = "",
                             sd = "", se = "", lowerCI = "", upperCI = "")
            }
