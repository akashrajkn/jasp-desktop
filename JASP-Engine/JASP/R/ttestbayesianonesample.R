...
TTestBayesianOneSample <- function(
  ...
  meta[[
      2]] <- list(name = "descriptives", type = "object",
                  meta = list(list(name = "descriptivesTable", type = "table"),
                              list(name = "descriptivesPlots",
                                   type = "collection", meta = "image")))
  meta[[3]] <- list(
      name = "inferentialPlots", type = "collection",
  ...
  if (options$hypothesis == "notEqualToTestValue") {
    ...
    fields <- list(
        ...
        list(name = "BF", type = "number", format = "sf:4;dp:3",
             title = bf.title), list(name = "error", type = "number",
                                     format = "sf:4;dp:3", title = "error %"))
  ...
  if (options$hypothesis == "greaterThanTestValue") {
    ...
    note <-
        "For all tests, the alternative hypothesis specifies that the mean
					is greater than "
    ...
    if (options$testValue != 0) {
      ...
      message <- paste(
          "All tests, hypothesis is population mean is different from ",
  ...
  for (variable in options[["variables"]]) {
    ...
    if (options$plotPriorAndPosterior ||
        options$plotBayesFactorRobustness || options$plotSequentialAnalysis) {
    ...
    if (!is.null(state) &&
        variable %in% state$options$variables &&
        !is.null(diff) && ((is.logical(diff) && diff == FALSE) ||
                           (is.list(diff) && (diff$priorWidth == FALSE &&
                                              diff$hypothesis == FALSE &&
                                              diff$bayesFactorType == FALSE &&
                                              diff$testValue == FALSE &&
      ...
      if (state$errorFootnotes[index] == "no") {
        ...
        ttest.rows[[
            length(ttest.rows) + 1]] <- state$results$ttest$data[[index]]
        ...
        ttest.rows[[length(ttest.rows) + 1]] <- list(
            Variable = variable, BF = .clean(NaN), error = "",
    ...
    if (options$descriptivesPlots) {
      ...
      if (!is.null(state) &&
          variable %in% state$descriptPlotVariables &&
          !is.null(diff) &&
          ((is.logical(diff) && diff == FALSE) ||
           (is.list(diff) &&
            (diff$testValue == FALSE &&
             diff$descriptivesPlotsCredibleInterval == FALSE &&
             diff$missingValues == FALSE &&
             diff$plotHeight == FALSE && diff$plotWidth == FALSE))) &&
          options$descriptivesPlots) {
        ...
        descriptivesPlots[[
            length(descriptivesPlots) + 1]] <- state$descriptivesPlots[[index]]
    ...
    if (options$plotPriorAndPosterior) {
      ...
      if (!is.null(state) &&
          variable %in% state$plotVariables &&
          !is.null(diff) &&
          ((is.logical(diff) && diff == FALSE) ||
           (is.list(diff) &&
            (diff$priorWidth == FALSE &&
             diff$hypothesis == FALSE &&
             diff$bayesFactorType == FALSE &&
             diff$testValue == FALSE &&
             diff$missingValues == FALSE &&
             ...
             diff$effectSizeStandardized == FALSE &&
             diff$informativeStandardizedEffectSize == FALSE &&
             diff$informativeCauchyLocation == FALSE &&
             diff$informativeCauchyScale == FALSE &&
             diff$informativeTLocation == FALSE &&
             diff$informativeTScale == FALSE &&
             diff$informativeTDf == FALSE &&
             diff$informativeNormalMean == FALSE &&
        ...
        index <- which(state$plotVariables == variable &
                       state$plotTypes == "posteriorPlotAddInfo")
        ...
        .plotFunc <- function() {
          .plotPosterior.summarystats.ttest(
              addInformation = options$plotPriorAndPosteriorAdditionalInf,
              dontPlotData = TRUE)
        }
      ...
      plotGroups[[
          i]][["PriorPosteriorPlot"]] <- plots.ttest[[length(plots.ttest)]]
    ...
    if (options$plotBayesFactorRobustness) {
      ...
      if (!is.null(state) &&
        ...
        # then, if the requested plot already exists, use it
        index <- which(state$plotVariables == variable &
                       state$plotTypes == "robustnessPlotAddInfo")
        ...
        # then, if the requested plot already exists, use it
        index <- which(state$plotVariables == variable &
                       state$plotTypes == "robustnessPlot")
        ...
        .plotFunc <- function() {
          .plotBF.robustnessCheck.ttest(
              oneSided = oneSided, BFH1H0 = BFH1H0, dontPlotData = TRUE,
      ...
      plotGroups[[
          i]][["BFrobustnessPlot"]] <- plots.ttest[[length(plots.ttest)]]
    ...
    if (options$plotSequentialAnalysis) {
      ...
      if (!is.null(state) &&
        ...
        index <- which(state$plotVariables == variable &
                       state$plotTypes == "sequentialRobustnessPlot")
        ...
        index <- which(state$plotVariables == variable &
                       state$plotTypes == "sequentialPlot")
        ...
        .plotFunc <- function() {
          .plotSequentialBF.ttest(oneSided = oneSided, BFH1H0 = BFH1H0,
                                  dontPlotData = TRUE, options = options)
        }
        content <- .writeImage(
            width = 530, height = 400, plot = .plotFunc, obj = TRUE)
      ...
      plotVariables[[length(plotVariables) + 1]] <- variable
      plotGroups[[
          i]][["BFsequentialPlot"]] <- plots.ttest[[length(plots.ttest)]]
    }
  ...
  if (options$plotPriorAndPosterior ||
    ...
    results[["inferentialPlots"]] <- list(
        title = ifelse(length(options[["variables"]]) > 1 ||
                       sum(c(options$plotPriorAndPosterior,
                             options$plotBayesFactorRobustness,
                             options$plotSequentialAnalysis)) > 1,
                       "Inferential Plots", "Inferential Plot"),
        collection = plotGroups)

  if (options$descriptivesPlots)
    results[["descriptives"]][["descriptivesPlots"]] <- list(
        title = ifelse(length(options[["variables"]]) > 1, "Descriptives Plots",
                       "Descriptives Plot"), collection = descriptivesPlots)
  ...
  if (options$descriptives) {
    ...
    if (options$descriptivesPlots) {
      ...
      fields[[length(fields) + 1]] <- list(
          name = "lowerCI", type = "number", format = "sf:4;dp:3",
          title = "Lower", overTitle = title)
      fields[[length(fields) + 1]] <- list(
          name = "upperCI", type = "number", format = "sf:4;dp:3",
    ...
    for (variable in variables) {
      ...
      if (perform == "run" && length(options[["variables"]]) > 0) {
        ...
        if (class(data) != "factor") {
          ...
          posteriorSummary <- .posteriorSummaryGroupMean(
              variable = data,
          ...
          result <- list(
              v = variable, N = n, mean = mean, sd = stdDeviation,
              se = stdErrorMean, lowerCI = ciLower, upperCI = ciUpper)
        } else {
    ...
    if (descriptivesComplete) descriptives[["status"]] <- "complete"
  ...
  if (perform == "run") {
    ...
    for (variable in options[["variables"]]) {
      ...
      if (!is.null(state) &&
        ...
        errors <- .hasErrors(dataset, perform, message = "short",
        ...
        if (!is.null(errorMessage)) {
          ...
          result <- list(Variable = variable, BF = .clean(NaN), error = "",
                         .footnotes = list(BF = list(index)))
          ...
          result <- try(
              ...
              expr = {
                ...
                r <- .generalTtestBF(
                    x = variableData, oneSided = oneSided, options = options)
                bf.raw <- r[["bf"]]
                tValue[i] <- r[["tValue"]]
                n[i] <- r[["n1"]]
                ...
                if (is.na(bf.raw)) {
                  ...
                  status[i] <- "error"
                  plottingError[i] <-
                      "Plotting is not possible: Bayes factor could not be calculated"
                } else if (bf.raw == Inf &
                           (options$plotPriorAndPosterior |
                            options$plotBayesFactorRobustness |
                            options$plotSequentialAnalysis |
                            options$plotSequentialAnalysisRobustness)) {

                  status[i] <- "error"
                  plottingError[i] <-
                      "Plotting is not possible: Bayes factor is infinite"
                } else if (is.infinite(1 / bf.raw)) {

                  status[i] <- "error"
                  plottingError[i] <-
                      "Plotting is not possible: The Bayes factor is too small"

                }

                if (bf.type == "BF01") bf.raw <- 1 / bf.raw
          ...
          if (idData > 1 & (options$plotSequentialAnalysis |
                            options$plotSequentialAnalysisRobustness)) {
    ...
    if (!.shouldContinue(callback())) return()
    ...
    for (variable in options[["variables"]]) {
      ...
      if (options$descriptivesPlots) {
        ...
        if (!is.null(state) &&
          ...
          results[["descriptives"]][["descriptivesPlots"]][[
              "collection"]][[descriptInd]][["status"]] <- "running"
          ...
          if (is.null(errorMessage)) {
            ...
            p <- try(
                ...
                expr = {
                  ...
                  obj <- .plotGroupMeanBayesOneSampleTtest(
                      variable = variableDataDescriptivesPlot,
                      variableName = variable, testValueOpt = options$testValue,
                      descriptivesPlotsCredibleInterval = options$descriptivesPlotsCredibleInterval)
                  content <- .writeImage(
          ...
          if (!is.null(errorMessage)) {
            ...
            plot[[
                "error"]] <- list(error = "badData", errorMessage = errorMessage)
        ...
        results[["descriptives"]][["descriptivesPlots"]][["collection"]] <-
            descriptivesPlots
      ...
      if (options$plotPriorAndPosterior) {
        ...
        if (!is.null(state) &&
          ...
          results[["inferentialPlots"]][["collection"]][[
              i]][["PriorPosteriorPlot"]][["status"]] <- "running"
          ...
          if (is.null(errorMessage)) {
            ...
            p <- try(
                ...
                expr = {
                  ...
                  .plotFunc <- function() {
                    .plotPosterior.summarystats.ttest(
                        t = tValue[i], n1 = n[i], oneSided = oneSided,
                        BF = BF10post[i], BFH1H0 = BFH1H0,
      ...
      if (options$plotBayesFactorRobustness) {
        ...
        if (!is.null(state) &&
          ...
          results[["inferentialPlots"]][[
              "collection"]][[i]][["BFrobustnessPlot"]][["status"]] <- "running"
          ...
          if (!is.null(errorMessage)) {
            ...
            plot[["error"]] <- list(
                error = "badData", errorMessage = errorMessage)
            ...
            p <- try(
                ...
                expr = {
                  ...
                  .plotFunc <- function() {
                    .plotBF.robustnessCheck.ttest(
                        x = variableData, oneSided = oneSided,
                        BF10post = BF10post[i], rscale = options$priorWidth,
                        BFH1H0 = BFH1H0,
                        additionalInformation = options$plotBayesFactorRobustnessAdditionalInfo)
                  }
                  content <- .writeImage(
      ...
      if (options$plotSequentialAnalysis) {
        ...
        if (!is.null(state) &&
          ...
          results[["inferentialPlots"]][[
              "collection"]][[i]][["BFsequentialPlot"]][["status"]] <- "running"
  ...
  for (plot in plots.ttest) keep <- c(keep, plot$data)

  for (plot in descriptivesPlots) keep <- c(keep, plot$data)
    ...
    return(
        list(results = results, status = "complete",
             state = list(
                 ...
                 descriptivesPlots = descriptivesPlots, status = status,
                 plottingError = plottingError, BF10post = BF10post,
                 errorFootnotes = errorFootnotes, tValue = tValue, n = n),
             keep = keep))
  }

}

.oneSidedTtestBFRichard <- function(
    x = NULL, y = NULL, paired = FALSE, oneSided = "right", r = sqrt(2) / 2,
    iterations = 10000) {

  # sample from delta posterior
  samples <- BayesFactor::ttestBF(
      x = x, y = y, paired = paired, posterior = TRUE, iterations = iterations,
      rscale = r)
  ...
  if (is.null(y) || paired) {
    ...
    meanBeta <- varBeta /
        samples[, 'sig2'] * ((sum(x) - sum(y)) + samples[, 'mu'] * (diffN)) / 2
  }

  logProbMin <- BayesFactor::logSummaryStats(
                    pnorm(0, meanBeta, sqrt(varBeta), log = TRUE))$logMean
...
.oneSidedTtestBFRichardAdaptive <- function(
    x = NULL, y = NULL, paired = FALSE, oneSided = "right", r = sqrt(2) / 2,
    nTests = 5, nIterations = 2000, criterion = .02, nMaxIterations = 2050000) {
  ...
  while (variability > criterion && nIterations < nMaxIterations) {
    ...
    for (i in seq_len(nTests)) {
      ...
      BF[i] <- .oneSidedTtestBFRichard(
          x = x, y = y, paired = paired, r = r, oneSided = oneSided,
...
.dposteriorShiftedT <- function(x, parameters, oneSided) {
  ...
  if (oneSided == FALSE) {
    ...
    ifelse(x >= 0, (dt((x - parameters[1]) / parameters[2], parameters[3]) /
                    parameters[2]) / pt((0 - parameters[1]) / parameters[2],
                                        parameters[3], lower.tail = FALSE), 0)

  } else if (oneSided == "left") {

    ifelse(x <= 0, (dt((x - parameters[1]) / parameters[2], parameters[3]) /
                    parameters[2]) / pt((0 - parameters[1]) / parameters[2],
                                        parameters[3], lower.tail = TRUE), 0)
...
.qShiftedT <- function(q, parameters, oneSided) {
  ...
  if (oneSided == FALSE) {
    ...
    areaSmallerZero <- pt(
        (0 - parameters[1]) / parameters[2], parameters[3], lower.tail = TRUE)

    qt(areaSmallerZero + q * (1 - areaSmallerZero), df = parameters[3]) *
...
.plotPosterior.ttest <- function(
    x = NULL, y = NULL, paired = FALSE, oneSided = FALSE, BF, BFH1H0,
    callback = function(...) 0, iterations = 10000, rscale = "medium", lwd = 2,
    cexPoints = 1.5, cexAxis = 1.2, cexYlab = 1.5, cexXlab = 1.5,
    cexTextBF = 1.4, cexCI = 1.1, cexLegend = 1.2, lwdAxis = 1.2,
  ...
  if (sigmaStart < .01) sigmaStart <- .01

  parameters <- try(
      silent = TRUE,
      expr = optim(par = c(deltaHat, sigmaStart, df), fn = .likelihoodShiftedT,
                   data = delta, method = "BFGS")$par)

  if (class(parameters) == "try-error") {

    parameters <- try(
        silent = TRUE, expr = optim(par = c(deltaHat, sigmaStart, df),
  ...
  ylim[2] <- max(stretch * .dprior(0, r, oneSided = oneSided),
                 stretch * dmax)  # get maximum density
  ...
  posteriorLine <- .dposteriorShiftedT(
      ...
      parameters = parameters, oneSided = oneSided)
  ...
  lines(seq(min(xticks), max(xticks), length.out = 1000),
        .dprior(seq(min(xticks), max(xticks), length.out = 1000), r = r,
                oneSided = oneSided), lwd = lwd, lty = 3)
  ...
  if (oneSided == FALSE) {
    ...
    heightPosteriorAtZero <- .dposteriorShiftedT(
        0, parameters = parameters, oneSided = oneSided)
    ...
    posteriorLineLargerZero <- posteriorLine[posteriorLine > 0]
    heightPosteriorAtZero <- posteriorLineLargerZero[
        length(posteriorLineLargerZero)]
  }
  ...
  if (addInformation) {
    ...
    CIText <- paste(
        "95% CI: [", bquote(.(formatC(CIlow, 3, format = "f"))), ", ",
    ...
    if (max(nchar(BF10t), nchar(BF01t)) == 7) {
      xx <- grconvertX(
          0.44 + 0.002 * max(nchar(BF10t), nchar(BF01t)), "ndc", "user")
    }

    if (max(nchar(BF10t), nchar(BF01t)) == 8) {
      xx <- grconvertX(
          0.44 + 0.003 * max(nchar(BF10t), nchar(BF01t)), "ndc", "user")
    }

    if (max(nchar(BF10t), nchar(BF01t)) > 8) {
      xx <- grconvertX(
          0.44 + 0.005 * max(nchar(BF10t), nchar(BF01t)), "ndc", "user")
    }
    ...
    plotrix::floating.pie(
        xx, yy, c(BF10, 1), radius = radius, col = c("darkred", "white"),
...
.plotSequentialBF.ttest <- function(
    x = NULL, y = NULL, paired = FALSE, BF10post, callback = function(...) 0,
    formula = NULL, data = NULL, rscale = 1, oneSided = FALSE, lwd = 2,
    cexPoints = 1.4, cexAxis = 1.2, cexYlab = 1.5, cexXlab = 1.6,
    cexTextBF = 1.4, cexText = 1.2, cexLegend = 1.2, cexEvidence = 1.6,
    lwdAxis = 1.2, plotDifferentPriors = FALSE, BFH1H0 = TRUE,
    dontPlotData = FALSE, level1 = NULL, level2 = NULL, subDataSet = NULL,
    options) {
  ...
  if (dontPlotData) {
    ...
    if (oneSided == FALSE) {
      ...
      if (BFH1H0) {
        ...
        mtext(text = expression(BF[1][0]), side = 2, las = 0, cex = cexYlab,
              line = 3.1)
      } else {

        mtext(text = expression(BF[0][1]), side = 2, las = 0, cex = cexYlab,
              line = 3.1)
      }
    }
    ...
    if (oneSided == "right") {
      ...
      if (BFH1H0) {
        ...
        mtext(text = expression(BF["+"][0]), side = 2, las = 0, cex = cexYlab,
              line = 3.1)
      } else {
  ...
  if (is.null(y) || paired) {
    ...
    while ((i <= length(x) | j <= length(y)) & k <= length(BF10)) {
      ...
      bfObject <- .generalTtestBF(x = x[1:i], y = y[1:j], paired = paired,
    ...
    if (plotDifferentPriors) {
      ...
      while ((i <= length(x) | j <= length(y)) & k <= length(BF10u)) {
        ...
        if (oneSided == FALSE) {
          ...
          BF <- BayesFactor::ttestBF(
              x = x[1:i], y = y[1:j], paired = paired, rscale = "ultrawide",
              nullInterval = nullInterval)
          BF10u[k] <- BayesFactor::extractBF(BF, logbf = FALSE, onlybf = F)[
              1, "bf"]

        } else {

          BF10u[k] <- .oneSidedTtestBFRichard(
              x = x[1:i], y = y[1:j], paired = paired, r = "ultrawide",
              oneSided = oneSided)
        }
      ...
      while ((i <= length(x) | j <= length(y)) & k <= length(BF10w)) {
        ...
        if (oneSided == FALSE) {
          ...
          BF <- BayesFactor::ttestBF(
              x = x[1:i], y = y[1:j], paired = paired, rscale = "wide",
              nullInterval = nullInterval)
          BF10w[k] <- BayesFactor::extractBF(BF, logbf = FALSE, onlybf = F)[
              1, "bf"]

        } else {

          BF10w[k] <- .oneSidedTtestBFRichard(
              x = x[1:i], y = y[1:j], paired = paired, r = "wide",
              oneSided = oneSided)
        }
    ...
    for (i in seq_len(nrow(subDataSet))) {
      ...
      if (length(xx) > 1 && length(yy) > 1 && (sd(xx) > 0 || sd(yy) > 0)) {
        ...
        bfObject <- .generalTtestBF(x = xx, y = yy, paired = paired,
                                    oneSided = oneSided, options = options)
        BF10[i] <- bfObject[["bf"]]
        # if (oneSided == FALSE) {
        #
      ...
      for (i in seq_len(nrow(subDataSet))) {
        ...
        if (length(xx) > 1 && length(yy) > 1 && (sd(xx) > 0 || sd(yy) > 0)) {
          ...
          if (oneSided == FALSE) {
            ...
            BF <- BayesFactor::ttestBF(
                x = xx, y = yy, paired = paired, rscale = "ultrawide",
                nullInterval = nullInterval)
            BF10u[
                i] <- BayesFactor::extractBF(BF, logbf = FALSE, onlybf = F)[1, "bf"]

          } else if (oneSided == "right") {

            BF10u[i] <- .oneSidedTtestBFRichard(
                xx, yy, oneSided = "right", r = "ultrawide")

          } else if (oneSided == "left") {

            BF10u[
                i] <- .oneSidedTtestBFRichard(xx, yy, oneSided = "left", r = "ultrawide")
          }
      ...
      for (i in seq_len(nrow(subDataSet))) {
        ...
        if (length(xx) > 1 && length(yy) > 1 && (sd(xx) > 0 || sd(yy) > 0)) {
          ...
          if (oneSided == FALSE) {
            ...
            BF <- BayesFactor::ttestBF(
                x = xx, y = yy, paired = paired, rscale = "wide",
                nullInterval = nullInterval)
            BF10w[i] <- BayesFactor::extractBF(BF, logbf = FALSE, onlybf = F)[
                1, "bf"]

          } else if (oneSided == "right") {

            BF10w[i] <- .oneSidedTtestBFRichard(
                xx, yy, oneSided = "right", r = "wide")

          } else if (oneSided == "left") {

            BF10w[i] <- .oneSidedTtestBFRichard(
                xx, yy, oneSided = "left", r = "wide")
          }
  ...
  while (eval(parse(text = y1h[i])) < max(BF)) {
    ...
    if (grepl(pattern = "e", y1h[i])) {
      ...
      newy <- paste(strsplit(y1h[i], split = "+", fixed = TRUE)[[1]][1], "+",
                    as.numeric(strsplit(y1h[i], split = "+", fixed = TRUE)[[1]][
                                   2]) + 1, sep = "")
    } else {
  ...
  while (eval(parse(text = y3h[i])) < max(BF)) {
    ...
    if (grepl(pattern = "e", y3h[i])) {
      ...
      newy <- paste(strsplit(y3h[i], split = "+", fixed = TRUE)[[1]][1], "+",
                    as.numeric(strsplit(y3h[i], split = "+", fixed = TRUE)[[1]][
                                   2]) + 1, sep = "")
    } else {
  ...
  while (eval(parse(text = y1l[i])) > min(BF)) {
    ...
    if (grepl(pattern = "e", y1l[i])) {
      ...
      newy <- paste(strsplit(y1l[i], split = "+", fixed = TRUE)[[1]][1], "+",
                    as.numeric(strsplit(y1l[i], split = "+", fixed = TRUE)[[1]][
                                   2]) + 1, sep = "")
    } else {
  ...
  while (eval(parse(text = y3l[i])) > min(BF)) {
    ...
    if (grepl(pattern = "e", y3l[i])) {
      ...
      newy <- paste(strsplit(y3l[i], split = "+", fixed = TRUE)[[1]][1], "+",
                    as.numeric(strsplit(y3l[i], split = "+", fixed = TRUE)[[1]][
                                   2]) + 1, sep = "")
    } else {
    ...
    if (eval(parse(text = newy)) <= 10 ^ (-6) &
        eval(parse(text = newy)) > 10 ^ (-9)) {

      newy <- format(eval(parse(text = newy)), digits = 3, scientific = TRUE)
      newy <- paste(
          substring(newy, 1, nchar(newy) - 1),
    ...
    if (max(BF) > eval(parse(text = yLab1s[length(yLab1s)]))) {
      ...
      for (i in 1:2) {
        ...
        if (grepl(pattern = "e", yLab1s[length(yLab1s)])) {
          ...
          newy <- paste(
              strsplit(
                  yLab1s[length(yLab1s)], split = "+", fixed = TRUE)[[1]][1],
              "+", as.numeric(strsplit(yLab1s[length(yLab1s)], split = "+",
                                       fixed = TRUE)[[1]][2]) + 1, sep = "")
        } else {
        ...
        if (eval(parse(text = newy)) >= 10 ^ 6) {
          ...
          newy <- format(
              eval(parse(text = newy)), digits = 3, scientific = TRUE)
        }
    ...
    if (min(BF) < eval(parse(text = yLab1s[1]))) {
      ...
      for (i in 1:2) {
        ...
        if (grepl(pattern = "e", yLab1s[1])) {
          ...
          newy <- paste(
              strsplit(yLab1s[1], split = "+", fixed = TRUE)[[1]][1],
              "+", as.numeric(strsplit(yLab1s[1], split = "+", fixed = TRUE)[[
                                  1]][2]) + 1, sep = "")
        } else {
  ...
  while (eval(parse(text = yLab[1])) > min(BF)) {
    ...
    for (i in 1:2) {
      ...
      newy <- paste(
          strsplit(yLab[1], "+", fixed = TRUE)[[1]][1], "+", pot, sep = "")
      yLab <- c(newy, yLab)
    }
  }
  ...
  while (eval(parse(text = yLab[length(yLab)])) < max(BF)) {
    ...
    for (i in 1:2) {
      ...
      interval <- as.numeric(
                      strsplit(yLab[length(yLab)], "+", fixed = TRUE)[[1]][2]) -
          as.numeric(
              strsplit(yLab[length(yLab) - 1], "+", fixed = TRUE)[[1]][2])
      pot <- as.numeric(strsplit(yLab[length(yLab)], "+", fixed = TRUE)[[
                            1]][2]) + interval
  ...
  if (!omit3s & eval(parse(text = yLab[1])) >= 1 / 300 &
      eval(parse(text = yLab[length(yLab)])) <= 300) {
  ...
  if (idData < length(BF10)) {
    ...
    BF10post <- as.numeric(BF10post)  # TO FIX: This needs to be done to make
                                      # sure that the BF shows up in the table.
    BF10e <- BF10post
  ...
  if (max(nchar(BF10t), nchar(BF01t)) > 8) {
    xx <- grconvertX(
        0.445 + 0.005 * max(nchar(BF10t), nchar(BF01t)), "ndc", "user")
  }
  ...
  plotrix::floating.pie(
      xx, yy, c(BF10e, 1), radius = radius, col = c("darkred", "white"),
      lwd = 2, startpos = startpos)
  ...
  if (length(BF10) <= 60) {
    ...
    points(log(BF10), pch = 21, bg = "grey", cex = cexPoints,
           lwd = 1.3)  # user prior
  } else {
  ...
  if (plotDifferentPriors) {
    ...
    if (length(BF10) <= 60) {
      ...
      points(log(BF10u), pch = 21, bg = "white", cex = 0.7,
             lwd = 1.3)  # "ultrawide" prior
      points(log(BF10w), pch = 21, bg = "black", cex = 0.7, lwd = 1.3)  # "wide"
                                                                        # prior
      ...
      lines(log(BF10u), col = greycol2, cex = 0.7, lwd = 1.3,
            lty = 1)  # "ultrawide" prior
      lines(log(BF10w), col = greycol, cex = 0.7, lwd = 1.3, lty = 3)  # "wide"
                                                                       # prior
    }
  }
  ...
  if (evidenceText) {
    ...
    if (length(BF10) <= 60) {
      ...
      legend(xx, yy, legend = legend[BFind], pch = rep(21, 3),
             pt.bg = pt.bg[BFind], bty = "n", cex = cexLegend,
             lty = rep(NULL, 3), pt.lwd = rep(1.3, 3), pt.cex = pt.cex[BFind])
    } else {
      ...
      legend(xx, yy, legend = legend[BFind], lty = lty[BFind], bty = "n",
             cex = cexLegend, lwd = lwd[BFind], col = col[BFind], seg.len = .7)
    }
  }
}


.plotBF.robustnessCheck.ttest <- function(
    x = NULL, y = NULL, paired = FALSE, BF10post, callback = function(...) 0,
    formula = NULL, data = NULL, rscale = 1, oneSided = FALSE, lwd = 2,
    cexPoints = 1.4, cexAxis = 1.2, cexYXlab = 1.5, cexText = 1.2,
    cexLegend = 1.4, lwdAxis = 1.2, cexEvidence = 1.6, BFH1H0 = TRUE,
    dontPlotData = FALSE, additionalInformation = FALSE) {
  ...
  if (dontPlotData) {
    ...
    if (oneSided == FALSE) {
      ...
      if (BFH1H0) {
        ...
        mtext(text = expression(BF[1][0]), side = 2, las = 0, cex = cexYXlab,
              line = 3.1)
      } else {

        mtext(text = expression(BF[0][1]), side = 2, las = 0, cex = cexYXlab,
              line = 3.1)
      }
    }
    ...
    if (oneSided == "right") {
      ...
      if (BFH1H0) {
        ...
        mtext(text = expression(BF["+"][0]), side = 2, las = 0, cex = cexYXlab,
              line = 3.1)
      } else {
  ...
  for (i in seq_along(rValues)) {
    ...
    if (oneSided == FALSE) {
      ...
      BF10[i] <- .oneSidedTtestBFRichard(
          x = x, y = y, paired = paired, oneSided = oneSided, r = rValues[i])
    }
  ...
  if (oneSided == FALSE) {
    ...
    BF10m <- .oneSidedTtestBFRichard(
        x = x, y = y, paired = paired, oneSided = oneSided, r = "medium")
  }
  ...
  if (oneSided == FALSE) {
    ...
    BF10w <- BayesFactor::ttestBF(x = x, y = y, paired = paired,
                                  nullInterval = nullInterval, rscale = "wide")
    BF10w <- BayesFactor::extractBF(BF10w, logbf = FALSE, onlybf = F)[1, "bf"]

  } else {

    BF10w <- .oneSidedTtestBFRichard(
        x = x, y = y, paired = paired, oneSided = oneSided, r = "wide")
  }
  ...
  if (oneSided == FALSE) {
    ...
    BF10ultra <- BayesFactor::ttestBF(
        x = x, y = y, paired = paired, nullInterval = nullInterval,
        rscale = "ultrawide")
    BF10ultra <- BayesFactor::extractBF(BF10ultra, logbf = FALSE, onlybf = F)[
        1, "bf"]

  } else {

    BF10ultra <- .oneSidedTtestBFRichard(
        x = x, y = y, paired = paired, oneSided = oneSided, r = "ultrawide")
  }
  ...
  if (length(yLab) > 9) {
    ...
    if (max(BF10) > eval(parse(text = yLab1s[length(yLab1s) - 1]))) {
      ...
      if (grepl(pattern = "e", yLab1s[length(yLab1s)])) {
        ...
        newy <- paste(
            strsplit(yLab1s[length(yLab1s)], split = "+", fixed = TRUE)[[1]][1],
            "+", as.numeric(strsplit(yLab1s[length(yLab1s)], split = "+",
                                     fixed = TRUE)[[1]][2]) + 1, sep = "")
      } else {
  ...
  while (eval(parse(text = yLab[2])) > min(BF10)) {
    ...
    interval <- as.numeric(strsplit(format(eval(parse(text = yLab[1])),
                                    ...
                                    fixed = TRUE)[[1]][2]) -
        as.numeric(
            strsplit(format(eval(parse(text = yLab[2])), digits = 3,
                            scientific = TRUE), "-", fixed = TRUE)[[1]][2])
    pot <- as.numeric(strsplit(format(eval(parse(text = yLab[1])), digits = 3,
                                      scientific = TRUE), "-", fixed = TRUE)[[
                          1]][2]) + interval

    if (nchar(pot) == 1) pot <- paste("0", pot, sep = "")
  ...
  while (eval(parse(text = yLab[length(yLab) - 1])) < max(BF10)) {
    ...
    interval <- as.numeric(
                    strsplit(format(eval(parse(text = yLab[length(yLab)])),
                                    digits = 3, scientific = TRUE), "+",
                             fixed = TRUE)[[1]][2]) -
        as.numeric(strsplit(format(eval(parse(text = yLab[length(yLab) - 1])),
                                   digits = 3, scientific = TRUE), "+",
                            fixed = TRUE)[[1]][2])
    ...
    newy <- paste(
        strsplit(format(eval(parse(text = yLab[length(yLab)])), digits = 3,
                        scientific = TRUE), "+", fixed = TRUE)[[1]][1], "+",
        pot, sep = "")
    yLab <- c(yLab, newy)
  }
  ...
  if (additionalInformation) {
    ...
    points(r, log(BF10user), pch = 21, bg = "grey", cex = cexPoints,
           lwd = 1.3)  # user prior
    points(1, log(BF10w), pch = 21, bg = "black", cex = 1.1,
           lwd = 1.3)  # "wide" prior
    points(sqrt(2), log(BF10ultra), pch = 21, bg = "white", cex = 1.1,
           lwd = 1.3)  # "ultrawide" prior
    points(maxBFrVal, log(maxBF10), pch = 21, bg = "red", cex = 1.1,
           lwd = 1.3)  # max BF value
    ...
    if (BF10maxText < 1000000) {
      BF10maxt <- formatC(
          BF10maxText, digits = 3, format = "f", drop0trailing = TRUE)
    }

    maxBFrValt <- formatC(
        maxBFrVal, digits = 4, format = "f", drop0trailing = TRUE)
    ...
    BFind <- sort(c(BF10userText, BF10ultraText, BF10wText, BF10maxText),
                  decreasing = TRUE, index.return = TRUE)$ix
    BFsort <- sort(c(BF10userText, BF10ultraText, BF10wText, BF10maxText),
                   decreasing = TRUE, index.return = TRUE)$x

    legend <- c("user prior:", "ultrawide prior:", "wide prior:",
                as.expression(maxBF10LegendText))
    pt.bg <- c("grey", "white", "black", "red")
    pt.cex <- c(cexPoints, 1.1, 1.1, 1.1)

    legend(xx, yy, legend = legend[BFind], pch = rep(21, 4),
           pt.bg = pt.bg[BFind], bty = "n", cex = cexLegend, lty = rep(NULL, 4),
           pt.lwd = rep(1.3, 4), pt.cex = pt.cex[BFind])
...
.posteriorSummaryGroupMean <- function(
    variable, descriptivesPlotsCredibleInterval = .95) {
  ...
  outTmp <- .qt.shiftedT(
      c(ciLower, .5, ciUpper), parameters = c(location, scale, df))
...
.base_breaks_y2 <- function(x, testValue) {
  ...
  list(ggplot2::geom_segment(
           data = d, ggplot2::aes(x = x, y = y, xend = xend, yend = yend),
           inherit.aes = FALSE, size = 1),
       ggplot2::scale_y_continuous(breaks = c(min(b), testValue, max(b))))
}

.plotGroupMeanBayesOneSampleTtest <- function(
    variable = 1:10, variableName = "test1", testValueOpt = 0,
    descriptivesPlotsCredibleInterval = .95) {
  ...
  posteriorSummary <- .posteriorSummaryGroupMean(
      variable = variable,
      descriptivesPlotsCredibleInterval = descriptivesPlotsCredibleInterval)
  summaryStat <- data.frame(
      groupingVariable = variableName, dependent = posteriorSummary$median,
      ciLower = posteriorSummary$ciLower, ciUpper = posteriorSummary$ciUpper)
  ...
  p <- ggplot2::ggplot(summaryStat, ggplot2::aes(x = groupingVariable,
      ...
      ggplot2::geom_hline(
          data = testValue, ggplot2::aes(yintercept = testValue),
...
.integrand <- function(g, t, n, nu, mu.delta, r, kappa) {
  ...
  pg_log <- kappa / 2 * (2 * log(r) + log(kappa / 2)) -
      lgamma(kappa / 2) - (kappa / 2 + 1) * log(g) - r ^ 2 * kappa / (2 * g)
  pg <- exp(pg_log)
  out <- tmp * pg
...
.dtss <- function(delta, mu.delta, r, kappa, log = FALSE) {
  ...
  out <- -log(r) + lgamma((kappa + 1) / 2) - .5 * (log(pi) + log(kappa)) -
      lgamma(kappa / 2) - (kappa + 1) /
      2 * log(1 + ((delta - mu.delta) / r) ^ 2 / kappa)
...
.dprior_informative <- function(delta, oneSided = FALSE, options) {
  ...
  if (options[["informativeStandardizedEffectSize"]] == "cauchy") {
    out <- .dtss(delta, mu.delta = options[["informativeCauchyLocation"]],
                 r = options[["informativeCauchyScale"]], kappa = 1)
    if (oneSided == "right") {
      out <- ifelse(
          delta < 0, 0,
          out /
          integrate(
              .dtss, 0, Inf, mu.delta = options[["informativeCauchyLocation"]],
              r = options[["informativeCauchyScale"]], kappa = 1)$value)
    } else if (oneSided == "left") {
      out <- ifelse(
          delta > 0, 0,
          out /
          integrate(
              .dtss, -Inf, 0, mu.delta = options[["informativeCauchyLocation"]],
              r = options[["informativeCauchyScale"]], kappa = 1)$value)
    }
  } else if (options[["informativeStandardizedEffectSize"]] == "t") {
    out <- .dtss(
        delta, mu.delta = options[["informativeTLocation"]],
        r = options[["informativeTScale"]], kappa = options[["informativeTDf"]])
    if (oneSided == "right") {
      out <- ifelse(delta < 0, 0,
                    out / integrate(.dtss, 0, Inf, mu.delta = options[[
                                                       "informativeTLocation"]],
                                    r = options[["informativeTScale"]],
                                    kappa = options[["informativeTDf"]])$value)
    } else if (oneSided == "left") {
      out <- ifelse(
          delta > 0, 0,
          out / integrate(.dtss, -Inf, 0,
    ...
    if (oneSided == "right") {
      out <- ifelse(delta < 0, 0,
                    out / pnorm(0, mean = options[["informativeNormalMean"]],
                                sd = options[["informativeNormalStd"]],
                                lower.tail = FALSE))
    } else if (oneSided == "left") {
      out <- ifelse(delta > 0, 0,
...
.posterior_t_tmp <- function(
    delta, t, ny, nx = NULL, independentSamples = FALSE, prior.location,
  ...
  numerator <- exp(-neff / 2 * delta ^ 2) * (1 + t ^ 2 / nu) ^ (-(nu + 1) / 2) *
      (.C(delta, t, neff, nu) +
       .D(delta, t, neff, nu)) * .dtss(delta, mu.delta, r, kappa)

  denominator <- integrate(.integrand, lower = 0, upper = Inf, t = t, n = neff,
                           nu = nu, mu.delta = mu.delta, r = r, kappa = kappa,
...
.cdf_t <- function(x, t, ny, nx = NULL, independentSamples = FALSE,
  ...
  integrate(
      .posterior_t, lower = -Inf, upper = x, t = t, ny = ny, nx = nx,
...
.quantile_t <- function(
    q, t, ny, nx = NULL, independentSamples = FALSE, prior.location,
    prior.scale, prior.df, tol = 0.0001, max.iter = 100) {
  ...
  dens <- .posterior_t(
      delta, t = t, ny = ny, nx = nx, independentSamples = independentSamples,
  ...
  while (is.finite(abs(x.cur - x.new)) &&
         abs(x.cur - x.new) > tol && i < max.iter) {

    x.cur <- x.new
    x.new <- x.cur -
        (.cdf_t(x.cur, t = t, ny = ny, nx = nx,
  ...
  if (is.infinite(abs(x.cur - x.new))) stop("Infinite value encountered.")

  return(x.new)

}

.ciPlusMedian_t <- function(
    t, ny, nx = NULL, independentSamples = FALSE, prior.location, prior.scale,
    prior.df, ci = .95, oneSided = FALSE, tol = 0.0001, max.iter = 100) {

  lower <- (1 - ci) / 2
  upper <- ci + (1 - ci) / 2
  med <- .5

  postAreaSmaller0 <- .cdf_t(
      x = 0, t = t, ny = ny, nx = nx, independentSamples = independentSamples,
      prior.location = prior.location, prior.scale = prior.scale,
      prior.df = prior.df)
  ...
  ciLower <- .quantile_t(
      lower, t = t, ny = ny, nx = nx, independentSamples = independentSamples,
      prior.location = prior.location, prior.scale = prior.scale,
      prior.df = prior.df)
  ciUpper <- .quantile_t(
      upper, t = t, ny = ny, nx = nx, independentSamples = independentSamples,
      prior.location = prior.location, prior.scale = prior.scale,
      prior.df = prior.df)
  median <- .quantile_t(
      med, t = t, ny = ny, nx = nx, independentSamples = independentSamples,
      prior.location = prior.location, prior.scale = prior.scale,
      prior.df = prior.df)
...
.posterior_normal_tmp <- function(
    delta, t, ny, nx = NULL, independentSamples = FALSE, prior.mean,
  ...
  numerator <- exp(-neff / 2 * delta ^ 2) * (1 + t ^ 2 / nu) ^ (-(nu + 1) / 2) *
      (.C(delta, t, neff, nu) +
       .D(delta, t, neff, nu)) * dnorm(delta, mu.delta, sqrt(g))

  denominator <- .term_normalprior(
      t = t, n = neff, nu = nu, mu.delta = mu.delta, g = g)
...
.quantile_normal <- function(
    q, t, ny, nx = NULL, independentSamples = FALSE, prior.mean, prior.variance,
  ...
  while (is.finite(abs(x.cur - x.new)) &&
    ...
    x.cur <- x.new
    x.new <- x.cur -
        (.cdf_normal(x.cur, t = t, ny = ny, nx = nx,
                     independentSamples = independentSamples,
                     prior.mean = prior.mean, prior.variance = prior.variance) -
         q) / .posterior_normal(
                  x.cur, t = t, ny = ny, nx = nx,
                  independentSamples = independentSamples,
                  prior.mean = prior.mean, prior.variance = prior.variance)
    i <- i + 1
...
.ciPlusMedian_normal <- function(
    t, ny, nx = NULL, independentSamples = FALSE, prior.mean, prior.variance,
  ...
  ciLower <- .quantile_normal(
      lower, t = t, ny = ny, nx = nx, independentSamples = independentSamples,
      prior.mean = prior.mean, prior.variance = prior.variance)
  ciUpper <- .quantile_normal(
      upper, t = t, ny = ny, nx = nx, independentSamples = independentSamples,
      prior.mean = prior.mean, prior.variance = prior.variance)
  median <- .quantile_normal(
      med, t = t, ny = ny, nx = nx, independentSamples = independentSamples,
      prior.mean = prior.mean, prior.variance = prior.variance)
...
.dposterior_informative <- function(
    delta, t, n1, n2 = NULL, paired = FALSE, oneSided = FALSE, options) {
  if (options[["informativeStandardizedEffectSize"]] == "cauchy") {
    out <- .posterior_t(
        delta, t = t, ny = n1, nx = n2, independentSamples = !paired &&
                                            !is.null(n2),
        prior.location = options[["informativeCauchyLocation"]],
        prior.scale = options[["informativeCauchyScale"]], prior.df = 1)
    if (oneSided == "right") {
      out <- ifelse(
          delta < 0, 0,
          out /
          (1 - .cdf_t(0, t = t, ny = n1, nx = n2,
    ...
    out <- .posterior_t(delta, t = t, ny = n1, nx = n2,
                        independentSamples = !paired && !is.null(n2),
...
.bf10_t <- function(
    t, ny, nx = NULL, independentSamples = FALSE, prior.location, prior.scale,
    prior.df, oneSided, rel.tol = .Machine$double.eps ^ 0.25) {
  ...
  r <- prior.scale
  kappa <- prior.df
  int <- integrate(.integrand, lower = 0, upper = Inf, t = t, n = neff, nu = nu,
                   mu.delta = mu.delta, r = r, kappa = kappa, rel.tol = rel.tol)
  numerator <- int$value
...
#-------------------------------------------------------------------------------
.generalTtestBF <- function(
    x = NULL, y = NULL, paired = FALSE, oneSided = FALSE, options) {
  ...
  if (options[["effectSizeStandardized"]] == "default") {
    ...
    bfObject <- BayesFactor::ttest.tstat(
        t = tValue, n1 = n1, n2 = n2, rscale = options$priorWidth,
        nullInterval = nullInterval)
    bf <- exp(bfObject$bf)
    ...
    if (options[["informativeStandardizedEffectSize"]] == "cauchy") {
      bfObject <- .bf10_t(
          t = tValue, ny = n1, nx = n2, oneSided = oneSided,
