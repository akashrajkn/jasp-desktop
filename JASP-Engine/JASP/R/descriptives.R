...
Descriptives <- function(
    ...
    if (run) {
      if (makeSplit) {
        dataset <- .readDataSetToEnd(
            columns.as.numeric = variables, columns.as.factor = splitName)
        dataset.factors <- .readDataSetToEnd(
            columns = variables, columns.as.factor = splitName)
      } else {
      ...
      if (makeSplit) {
        dataset <- .readDataSetHeader(
            columns.as.numeric = variables, columns.as.factor = splitName)
        dataset.factors <- .readDataSetHeader(
            columns = variables, columns.as.factor = splitName)
      } else {
  ...
  if (!is.null(state)) {
    ...
    if (is.list(diff)) {
      ...
      # mainTable
      if (!any(diff[["variables"]], diff[["splitby"]],
               ...
               diff[["percentileValuesPercentilesPercentiles"]], diff[["mean"]],
               diff[["median"]], diff[["mode"]], diff[["sum"]],
               diff[["statisticsValuesAreGroupMidpoints"]],
               diff[["standardDeviation"]], diff[["variance"]], diff[["range"]],
               diff[["minimum"]], diff[["maximum"]],
               diff[["standardErrorMean"]], diff[["skewness"]],
               diff[["kurtosis"]])) {
      ...
      if (options$frequencyTables &&
          !any(diff[["splitby"]], diff[["frequencyTables"]])) {
        for (var in stateVars) {
          # find table
          whichtab <- which(sapply(state$results$tables, function(x)
                                                           return(x$name),
                                   simplify = TRUE) == var)
      ...
      if (options$plotVariables &&
          !any(diff[["splitby"]], diff[["plotVariables"]], diff[["plotWidth"]],
               diff[["plotHeight"]])) {
        for (var in stateVars) {
          ...
          if (prevSplit) {
            distPlots[[var]] <- state$results$plots$distributionPlots[[var]]
          } else {
            distPlots[[
                var]] <- state$results$plots$distributionPlots$collection[[var]]
          }
        }
      }
      ...
      if (options$plotCorrelationMatrix &&
          !any(diff[["splitby"]], diff[["plotCorrelationMatrix"]],
               diff[["variables"]])) {
      ...
      if (options$splitPlots &&
          !any(diff[["splitby"]], diff[["splitPlots"]],
               diff[["splitPlotBoxplot"]], diff[["splitPlotViolin"]],
               diff[["splitPlotJitter"]], diff[["splitPlotOutlierLabel"]],
               diff[["splitPlotColour"]], diff[["plotWidth"]],
               diff[["plotHeight"]])) {
        for (var in stateVars) {
          # find plot
          whichPlot <- which(sapply(state$results$plots$splitPlots$collection,
                                    function(x) x$name, simplify = TRUE) == var)

          # Assign to results object
          boxPlots[[
              var]] <- state$results$plots$splitPlots$collection[[whichPlot]]
  ...
  if (is.null(mainTable)) {
    ...
    results[["stats"]] <- .descriptivesDescriptivesTable(
        dataset, options, run, last.stats.table)
  } else {
  ...
  # Frequency table
  if (options$frequencyTables) {
    results[["tables"]] <- .descriptivesFrequencyTables(
        dataset.factors, options, run, freqTabs)
  }
  ...
  if (options$plotCorrelationMatrix && is.null(corrPlot)) {
    if (makeSplit) {
      ...
      for (i in 1:length(splitLevels)) {
        corrPlot[["collection"]][[i]] <- .descriptivesMatrixPlot(
            splitDat[[i]], options, splitLevels[i], run)
      }
      corrPlot[["title"]] <- "Correlation plots"
    } else {
      # Create one plot
      corrPlot <- .descriptivesMatrixPlot(
          dataset, options, "Correlation plot", run)
    }
  }
      ...
      if (is.null(distPlots[[var]])) {
        if (makeSplit) {
          distPlots[[var]] <- .descriptivesFrequencyPlots(
              dataset = splitDat, options = options, run = run, variable = var)
        } else {
          distPlots[["collection"]][[var]] <- .descriptivesFrequencyPlots(
              dataset = dataset.factors, options = options, run = run,
              variable = var)
        }
      } else {
        if (!makeSplit) {
    ...
    for (var in variables) {
      if (is.null(boxPlots[[var]])) {
        splitPlots[["collection"]][[var]] <- .descriptivesSplitPlot(
            dataset = dataset, options = options, run = run, variable = var)
      } else {
  ...
  results[["plots"]] <- list(
      distributionPlots = distPlots, matrixPlot = corrPlot,
...
.descriptivesDescriptivesTable <- function(
    dataset, options, run, last.table = NULL) {
  ...
  percentilesPercentiles <- unique(
      options$percentileValuesPercentilesPercentiles)
  ...
  if (wantsSplit) {
    ...
    fields[[length(fields) + 1]] <- list(
        name = "Variable", title = "", type = "string")
    fields[[length(fields) + 1]] <- list(
        name = "Level", title = "", type = "string")
  } else {
    fields[[length(fields) + 1]] <- list(
        name = "Variable", title = "", type = "string")
  }
  ...
  if (options$mean)
    fields[[length(fields) + 1]] <- list(
        name = "Mean", type = "number", format = "sf:4")
  if (options$standardErrorMean)
    fields[[length(fields) + 1]] <- list(
        name = "Std. Error of Mean", type = "number", format = "sf:4")
  if (options$median)
    fields[[length(fields) + 1]] <- list(
        name = "Median", type = "number", format = "sf:4")
  if (options$mode)
    fields[[length(fields) + 1]] <- list(
        name = "Mode", type = "number", format = "sf:4")
  if (options$standardDeviation)
    fields[[length(fields) + 1]] <- list(
        name = "Std. Deviation", type = "number", format = "sf:4")
  if (options$variance)
    fields[[length(fields) + 1]] <- list(
        name = "Variance", type = "number", format = "sf:4")

  if (options$skewness) {

    fields[[length(fields) + 1]] <- list(
        name = "Skewness", type = "number", format = "sf:4")
    fields[[length(fields) + 1]] <- list(
        name = "Std. Error of Skewness", type = "number", format = "sf:4")
  }

  if (options$kurtosis) {

    fields[[length(fields) + 1]] <- list(
        name = "Kurtosis", type = "number", format = "sf:4")
    fields[[length(fields) + 1]] <- list(
        name = "Std. Error of Kurtosis", type = "text", format = "sf:4")
  }

  if (options$range)
    fields[[length(fields) + 1]] <- list(
        name = "Range", type = "number", format = "sf:4")
  if (options$minimum)
    fields[[length(fields) + 1]] <- list(
        name = "Minimum", type = "number", format = "sf:4")
  if (options$maximum)
    fields[[length(fields) + 1]] <- list(
        name = "Maximum", type = "number", format = "sf:4")
  if (options$sum)
    fields[[length(fields) + 1]] <- list(
        name = "Sum", type = "number", format = "sf:4")

  if (options$percentileValuesQuartiles) {

    fields[[length(fields) + 1]] <- list(name = "q1", title = "25th percentile",
                                         type = "number", format = "sf:4")
    fields[[length(fields) + 1]] <- list(name = "q2", title = "50th percentile",
                                         type = "number", format = "sf:4")
    fields[[length(fields) + 1]] <- list(name = "q3", title = "75th percentile",
                                         type = "number", format = "sf:4")
  }

  if (options$percentileValuesEqualGroups) {  # I've read that there are several
                                              # ways how to estimate percentiles
                                              # so it should be checked if it
                                              # match the SPSS way
    ...
    for (i in seq(equalGroupsNo - 1))
      fields[[length(fields) + 1]] <- list(
          name = paste("eg", i, sep = ""),
          title = paste(
              as.integer(100 * i / equalGroupsNo), "th percentile", sep = ""),
          type = "number", format = "sf:4")
  }
  ...
  if (options$percentileValuesPercentiles) {
    ...
    for (i in percentilesPercentiles)
      fields[[length(fields) + 1]] <- list(
          name = paste("pc", i, sep = ""),
          title = paste(i, "th percentile", sep = ""), type = "number",
          format = "sf:4")
  }
  ...
  na.for.categorical <-
      "Not all values are available for <i>Nominal Text</i> variables"
  ...
  if (wantsSplit) {
    ...
    for (variable in variables) {
      ...
      whichCols <- sapply(
          last.table$data, function(x) x$Variable == variable, simplify = TRUE)
      ...
      for (l in 1:nLevels) {
        resultsCol <- list(Variable = variable, Level = splitLevels[l])
        ...
        if (options$mode) {
          ...
          if (base::is.factor(na.omitted) == FALSE) {
            ...
            if (perform == "run") {
              ...
              mode <- as.numeric(
                  names(table(na.omitted)[table(na.omitted) ==
                                          max(table(na.omitted))]))
        ...
        if (options$sum) {
          ...
          if (base::is.factor(na.omitted) == FALSE) {
            ...
            if (perform == "run") resultsCol[["Sum"]] <- .clean(sum(na.omitted))
        ...
        if (options$range) {
          ...
          if (base::is.factor(na.omitted) == FALSE) {
            ...
            if (perform == "run")
              resultsCol[["Range"]] <- .clean(
                  range(na.omitted)[2] - range(na.omitted)[1])
        ...
        if (options$standardErrorMean) {
          ...
          if (base::is.factor(na.omitted) == FALSE) {
            ...
            if (perform == "run") resultsCol[["Std. Error of Mean"]] <- .clean(
                                      sd(na.omitted) / sqrt(length(na.omitted)))
        ...
        if (options$kurtosis) {
          ...
          if (base::is.factor(na.omitted) == FALSE) {
            ...
            if (perform == "run") {
              ...
              resultsCol[[
                  "Kurtosis"]] <- .clean(.descriptivesKurtosis(na.omitted))
              resultsCol[["Std. Error of Kurtosis"]] <- .clean(
                  .descriptivesSEK(na.omitted))
            }
        ...
        if (options$skewness) {
          ...
          if (base::is.factor(na.omitted) == FALSE) {
            ...
            if (perform == "run") {
              ...
              resultsCol[[
                  "Skewness"]] <- .clean(.descriptivesSkewness(na.omitted))
              resultsCol[["Std. Error of Skewness"]] <- .clean(
                  .descriptivesSES(na.omitted))
        ...
        if (options$percentileValuesQuartiles) {
          ...
          if (base::is.factor(na.omitted) == FALSE) {
            ...
            if (perform == "run") {
              ...
              resultsCol[[
                  "q1"]] <- .clean(quantile(na.omitted, c(.25), type = 6, names = F))
              resultsCol[[
                  "q2"]] <- .clean(quantile(na.omitted, c(.5), type = 6, names = F))
              resultsCol[[
                  "q3"]] <- .clean(quantile(na.omitted, c(.75), type = 6, names = F))
        ...
        for (row in names(resultsCol)) {
          ...
          if (substr(row, 1, 2) == "eg" &&
              ((row %in% equalGroupNames) == FALSE)) resultsCol[[row]] <- NULL
        }
        ...
        if (options$percentileValuesEqualGroups) {
          ...
          if (base::is.factor(na.omitted) == FALSE) {
            ...
            if (perform == "run") {
              ...
              for (i in seq(equalGroupsNo - 1))
                resultsCol[[paste("eg", i, sep = "")]] <- .clean(
                    quantile(
                        na.omitted, c(i / equalGroupsNo), type = 6, names = F))
        ...
        for (row in names(resultsCol)) {
          ...
          if (substr(row, 1, 2) == "pc" &&
              ((row %in% percentileNames) == FALSE)) resultsCol[[row]] <- NULL
        }
        ...
        if (options$percentileValuesPercentiles) {
          ...
          if (base::is.factor(na.omitted) == FALSE) {
            ...
            if (perform == "run") {
              ...
              for (i in percentilesPercentiles)
                resultsCol[[paste("pc", i, sep = "")]] <- .clean(
                    quantile(na.omitted, c(i / 100), type = 6, names = F))
    ...
    for (variable in variables) {
      ...
      if (options$mode) {
        ...
        if (base::is.factor(na.omitted) == FALSE) {
          ...
          if (perform == "run") {
            ...
            mode <- as.numeric(names(table(na.omitted)[table(na.omitted) ==
                                                       max(table(na.omitted))]))
      ...
      if (options$range) {
        ...
        if (base::is.factor(na.omitted) == FALSE) {
          ...
          if (perform == "run") variable.results[["Range"]] <- .clean(
                                    range(na.omitted)[2] - range(na.omitted)[1])
      ...
      if (options$standardErrorMean) {
        ...
        if (base::is.factor(na.omitted) == FALSE) {
          ...
          if (perform == "run")
            variable.results[["Std. Error of Mean"]] <- .clean(
                sd(na.omitted) / sqrt(length(na.omitted)))
      ...
      if (options$kurtosis) {
        ...
        if (base::is.factor(na.omitted) == FALSE) {
          ...
          if (perform == "run") {
            ...
            variable.results[[
                "Kurtosis"]] <- .clean(.descriptivesKurtosis(na.omitted))
            variable.results[["Std. Error of Kurtosis"]] <- .clean(
                .descriptivesSEK(na.omitted))
          }
      ...
      if (options$skewness) {
        ...
        if (base::is.factor(na.omitted) == FALSE) {
          ...
          if (perform == "run") {
            ...
            variable.results[[
                "Skewness"]] <- .clean(.descriptivesSkewness(na.omitted))
            variable.results[["Std. Error of Skewness"]] <- .clean(
                .descriptivesSES(na.omitted))
      ...
      if (options$percentileValuesQuartiles) {
        ...
        if (base::is.factor(na.omitted) == FALSE) {
          ...
          if (perform == "run") {
            ...
            variable.results[["q1"]] <- .clean(
                quantile(na.omitted, c(.25), type = 6, names = F))
            variable.results[["q2"]] <- .clean(
                quantile(na.omitted, c(.5), type = 6, names = F))
            variable.results[["q3"]] <- .clean(
                quantile(na.omitted, c(.75), type = 6, names = F))
      ...
      if (options$percentileValuesEqualGroups) {
        ...
        if (base::is.factor(na.omitted) == FALSE) {
          ...
          if (perform == "run") {
            ...
            for (i in seq(equalGroupsNo - 1))
              variable.results[[paste("eg", i, sep = "")]] <- .clean(
                  quantile(
                      na.omitted, c(i / equalGroupsNo), type = 6, names = F))
      ...
      if (options$percentileValuesPercentiles) {
        ...
        if (base::is.factor(na.omitted) == FALSE) {
          ...
          if (perform == "run") {
            ...
            for (i in percentilesPercentiles)
              variable.results[[paste("pc", i, sep = "")]] <- .clean(
                  quantile(na.omitted, c(i / 100), type = 6, names = F))
...
.descriptivesFrequencyTables <- function(dataset, options, run, stateTabs) {
  ...
  for (variable in options$variables) {
    ...
    if (wantsSplit) {
      fields[[1]] <- list(
          name = "factor", title = splitName, type = "string", combine = TRUE)
    }
    fields[[length(fields) + 1]] <- list(
        name = "Level", type = "string", title = variable)
    fields[[length(fields) + 1]] <- list(name = "Frequency", type = "integer")
    fields[[length(fields) + 1]] <- list(
        name = "Percent", type = "number", format = "dp:1")
    fields[[length(fields) + 1]] <- list(
        name = "Valid Percent", type = "number", format = "dp:1")
    fields[[length(fields) + 1]] <- list(
        name = "Cumulative Percent", type = "number", format = "dp:1")
    ...
    if (run) {
      if (wantsSplit) {
        ...
        for (lev in splitLevels) {
          ...
          rows[[length(rows) + 1]] <- list(
              "factor" = "", "Level" = "Total", "Frequency" = total,
              "Percent" = 100, "Valid Percent" = 100, "Cumulative Percent" = "")
        }
        ...
        rows[[length(rows) + 1]] <- list(
            "Level" = "Total", "Frequency" = total, "Percent" = 100,
            "Valid Percent" = 100, "Cumulative Percent" = "")
      }
      ...
      data[[length(data) + 1]] <- list(
          Level = "Total", "Cumulative Percent" = "")
...
.descriptivesMatrixPlot <- function(dataset, options, name, run) {
  ...
  if (run && length(unlist(options$variables)) > 0) {
    ...
    if (nrow(dataset) < 3) {
      matrix.plot[["error"]] <- list(
          error = "badData",
          errorMessage = "Plotting is not possible: Too few rows")
    ...
    if (length(variables) > 0) {
      .matrixPlotFunc <- function() {
        if (l == 1) {
          par(mfrow = c(1, 1), cex.axis = 1.3, mar = c(3, 4, 2, 1.5) + 0.1,
              oma = c(2, 0, 0, 0))
          ...
          par(mfrow = c(l, l), cex.axis = 1.3, mar = c(3, 4, 2, 1.5) + 0.1,
              oma = c(0.2, 2.2, 2, 0))
          for (row in seq_len(l)) {
            for (col in seq_len(l)) {
              if (row == col) {
                if (!variable.statuses[[row]]$unplotable) {
                  .plotMarginalCor(dataset[[variables[row]]])  # plot marginal
                                                               # (histogram with
                                                               # density
                                                               # estimator)
                } else {
              ...
              if (col > row) {
                if (!variable.statuses[[col]]$unplotable &&
                  ...
                  .plotScatterDescriptives(
                      dataset[[variables[col]]],
                      dataset[[variables[row]]])  # plot scatterplot
                } else {
                  errorMessages <- c(variable.statuses[[row]]$plottingError,
  ...
  if (options$splitby != "") {
    ...
    for (l in split) {
      ...
      if (run == FALSE) {
        ...
        imageObj <- .writeImage(
            options$plotWidth, options$plotHeight, plotFunc, obj = FALSE)

        plot[["data"]] <- imageObj[["png"]]
        plot[["obj"]] <- imageObj[["obj"]]
        plot[["error"]] <- list(
            error = "badData",
            errorMessage = "Plotting is not possible: Variable contains infinity")
        ...
        plot[["status"]] <- "complete"
                 ...
                 is.numeric(column) &&
                 length(column) > 0 &&
                 all(column %% 1 == 0) && length(unique(column)) <= 24) {
        ...
        if (any(is.infinite(column))) {
          ...
          imageObj <- .writeImage(
              options$plotWidth, options$plotHeight, plotFunc)
    ...
    plotResult <- list(collection = coll, title = variable)
...
.descriptivesSplitPlot <- function(dataset, options, run, variable) {
  ...
  if (run == FALSE) {
    ...
    image <- .writeImage(width = options$plotWidth, height = options$plotHeight,
                         plot = .initSplitPlot, obj = FALSE)
    splitPlot[["data"]] <- image[["png"]]
    splitPlot[["error"]] <- list(
        error = "badData",
        errorMessage = "Plotting is not possible: Variable is not numeric!")
    splitPlot[["status"]] <- "complete"

  } else if (!(options$splitPlotViolin ||
               options$splitPlotBoxplot || options$splitPlotJitter)) {
    ...
    splitPlot[["error"]] <- list(
        error = "badData",
        errorMessage = "Plotting is not possible: No plot type selected!")
    splitPlot[["status"]] <- "complete"
    ...
    for (level in levels(plotDat$group)) {
      ...
      plotDat[plotDat$group == level, ]$outlier <- v <
          (quantiles[1] - 1.5 * IQR) | v > (quantiles[2] + 1.5 * IQR)
    }
    ...
    if (options$splitPlotViolin &&
        options$splitPlotBoxplot && options$splitPlotJitter) {

      p <- p + ggplot2::geom_violin(
                   trim = F, size = 0.75, width = vioWidth, scale = "width") +
          ggplot2::stat_boxplot(
              geom = "errorbar", size = 0.75, width = boxWidth / 2) +
          ggplot2::geom_boxplot(
              size = 0.75, width = boxWidth, outlier.shape = NA) +
      ...
      p <- p + ggplot2::geom_violin(
          ...
          ggplot2::geom_boxplot(
              size = 0.75, outlier.size = 1.5, width = boxWidth) +
          ggplot2::geom_violin(trim = F, size = 0.75, width = vioWidth,
                               fill = "transparent", scale = "width")
      ...
      p <- p + ggplot2::stat_boxplot(
          ...
          ggplot2::geom_boxplot(
              size = 0.75, outlier.shape = NA, width = boxWidth) +
          ggplot2::geom_jitter(
              size = 2.5, shape = 1, stroke = 1,
              position = ggplot2::position_jitter(width = 0.05, height = 0),
              fill = "transparent")
      ...
      p <- p + ggplot2::geom_violin(trim = F, size = 0.75,
                                    width = 0.75 * boxWidth, scale = "width") +
          ggplot2::geom_jitter(
              size = 2.5, shape = 1, stroke = 1,
              position = ggplot2::position_jitter(width = 0.05, height = 0),
              fill = "transparent")
      ...
      p <- p + ggplot2::geom_jitter(
                   size = 2.5, ggplot2::aes(colour = group),
                   position = ggplot2::position_jitter(width = 0.1, height = 0))

    }

    if (options$splitPlotOutlierLabel &&
        (options$splitPlotBoxplot || options$splitPlotJitter)) {
      p <- p + ggplot2::geom_text(ggplot2::aes(label = label), hjust = -0.3)
    }

    ### Theming & Cleaning
    p <- p + ggplot2::xlab(xlab) + ggplot2::ylab(variable) + base_breaks_y(y) +
        ggplot2::theme_bw() +
        ggplot2::theme(
    ...
    image <- .writeImage(
        width = options$plotWidth, height = options$plotHeight, plot = p)
...
.plotMarginal <- function(
    variable, variableName, cexYlab = 1.3, lwd = 2, rugs = FALSE) {
  ...
  plot(1, xlim = range(xticks), ylim = c(ylow, yhigh), type = "n", axes = FALSE,
       ylab = "", xlab = "")
  h <- hist(variable, freq = F, main = "", ylim = c(ylow, yhigh), xlab = "",
            ylab = " ", axes = F, col = "grey", add = TRUE,
            nbreaks = round(length(variable) / 5))
...
.plotScatterDescriptives <- function(
    xVar, yVar, cexPoints = 1.3, cexXAxis = 1.3, cexYAxis = 1.3, lwd = 2) {
  ...
  ylow <- min((min(yVar) - 0.1 * min(yVar)), min(pretty(yVar)),
              min(.poly.pred(fit[[bestModel]], line = FALSE, xMin = xticks[1],
                             xMax = xticks[length(xticks)], lwd = lwd)))
  yhigh <- max((max(yVar) + 0.1 * max(yVar)), max(pretty(yVar)),
               max(.poly.pred(fit[[bestModel]], line = FALSE, xMin = xticks[1],
                              xMax = xticks[length(xticks)], lwd = lwd)))
  ...
  .poly.pred(fit[[bestModel]], line = TRUE, xMin = xticks[1],
             xMax = xticks[length(xticks)], lwd = lwd)
