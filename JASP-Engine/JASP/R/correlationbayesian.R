...
CorrelationBayesian <- function(
  ...
  correlationTableOutput <- .correlationTableBayesian(
      dataset, perform, variables = options$variables,
      pearson = options$pearson, kendallsTauB = options$kendallsTauB,
      spearman = options$spearman, hypothesis = options$hypothesis,
      reportBayesFactors = options$reportBayesFactors,
      flagSupported = options$flagSupported,
      credibleInterval = options$credibleInterval, ciValue = options$ciValue,
  ...
  if (!is.null(state) &&
      ...
      ((is.logical(diff) && diff == FALSE) ||
       (is.list(diff) &&
        (diff$credibleInterval == FALSE &&
         diff$ciValue == FALSE &&
         diff$hypothesis == FALSE &&
         diff$kendallsTauB == FALSE &&
         diff$missingValues == FALSE &&
         diff$pearson == FALSE &&
         diff$plotCorrelationMatrix == FALSE &&
         diff$plotDensitiesForVariables == FALSE &&
         diff$plotPosteriors == FALSE &&
         diff$spearman == FALSE &&
         diff$variables == FALSE && diff$priorWidth == FALSE)))) {

    results[["plot"]] <- state$correlationPlot

  } else {

    results[["plot"]] <- .correlationMatrixPlotBayesian(
        dataset, perform, options, hypothesis = options$hypothesis)
  }
  ...
  if (perform == "init") {
    if (length(options$variables) < 2) {
      ...
      results <- list(
          results = results, status = "inited", state = state, keep = keep)
      return(results)
    }
  } else {
    return(
        list(
            ...
            state = list(
                ...
                tableVariables = tableVariables, tableTests = tableTests,
                tableRows = tableRows,
...
.correlationTableBayesian <- function(
    dataset, perform, variables, pearson = TRUE, kendallsTauB = FALSE,
    spearman = FALSE, hypothesis = "correlated", reportBayesFactors = TRUE,
    flagSupported = FALSE, credibleInterval = FALSE, ciValue = 0.95,
    priorWidth = priorWidth, bayesFactorType = bayesFactorType,
    missingValues = "excludePairwise", state, diff) {
  # Note: This is the default failed bfObject for wrong data
  #
  failedBfObject <- list(
      n = NaN, r = NaN, stat = NA, bf10 = NaN, bfPlus0 = NA, bfPlus0 = NA,
      bfMin0 = NA, ciValue = ciValue, ci = list())
  ...
  if (flagSupported || reportBayesFactors) {
    if (hypothesis == "correlatedPositively") {
      .addFootnote(
          footnotes,
          "For all tests, the alternative hypothesis specifies that the correlation is positive.",
          symbol = "<i>Note</i>.")
    } else if (hypothesis == "correlatedNegatively") {
      .addFootnote(
          footnotes,
          "For all tests, the alternative hypothesis specifies that the correlation is negative.",
          symbol = "<i>Note</i>.")
    }
  }
  ...
  if (flagSupported) {
    if (bayesFactorType == "LogBF10") {
      .addFootnote(footnotes, paste(bfTitle, " > log(10), ** ", bfTitle,
                                    " > log(30), *** ", bfTitle, " > log(100)"),
                   symbol = "*")
    } else if (bayesFactorType == "BF01") {
      .addFootnote(
          footnotes, paste(bfTitle, " < .1, ** ", bfTitle, " < .03, *** ",
                           bfTitle, " < .01"), symbol = "*")
    } else {
      .addFootnote(
          footnotes, paste(bfTitle, " > 10, ** ", bfTitle, " > 30, *** ",
                           bfTitle, " > 100"), symbol = "*")
    }
  }
  ...
  if (numberOfVariables > 0) {
    ...
    #
    testNames <- list(pearson = "Pearson's r", spearman = "Spearman's rho",
    ...
    for (test in tests) {
      ...
      if (length(tests) > 1 || reportBayesFactors || credibleInterval) {
        ...
        fields[[length(fields) + 1]] <- list(
            name = columnName, title = "", type = "string")
      }
      ...
      for (variableName in variables) {
        ...
        columnNames[[length(columnNames) + 1]] <- columnName
        fields[[length(fields) + 1]] <- list(
            name = columnName, title = variableName, type = "number",
            format = "dp:3")
      }
      ...
      if (isTRUE(reportBayesFactors)) {
        ...
        columnNames[[length(columnNames) + 1]] <- columnName
        fields[[length(fields) + 1]] <- list(
            name = columnName, title = bfTitle, type = "string")
        for (variableName in variables) {
    ...
    for (i in 1:numberOfVariables) {
      ...
      for (test in tests) {
        ...
        if (isTRUE(credibleInterval)) {
          upperCiList[[length(upperCiList) + 1]] <- paste(
              "Upper ", 100 * ciValue, "% CI", sep = "")
          lowerCiList[[length(lowerCiList) + 1]] <- paste(
              "Lower ", 100 * ciValue, "% CI", sep = "")
        }
        ...
        for (j in 1:numberOfVariables) {
          ...
          if (missingValues == "excludePairwise") {
            ...
            bfObject <- memoryExcludePairwise[[priorLabel]][[variableName]][[
                columnName]]

            if (is.null(bfObject)) {
              # Try variableName then columnName
              bfObject <- memoryExcludePairwise[[priorLabel]][[columnName]][[
                  variableName]]
            }
            ...
            if (!is.null(bfObject)) {
              ...
              if (isTRUE(credibleInterval)) {
                if (ciValue != bfObject$ciValue) {
                  ...
                  if (test == "pearson") {
                    bfObject$ci <- .computePearsonCredibleInterval(
                        alpha = bfObject$betaA, beta = bfObject$betaB,
                        bfObject$ciValue)
                  } else if (test == "kendall") {
                    bfObject$ci <- .computeKendallCredibleInterval(
                        n = bfObject$n, tauObs = bfObject$stat,
                        kappa = priorWidth, ciValue = bfObject$ciValue)
                  }


                  # Store back into state
                  memoryExcludePairwise[[
                      priorLabel]][[variableName]][[columnName]] <- bfObject
                }
              }
              #
            ...
            #
            errors <- .hasErrors(
                dataset, perform = perform, message = 'short',
                type = c('observations', 'variance', 'infinity'),
                all.target = c(variableName, variable2Name),
            ...
            if (!identical(errors, FALSE)) {
              ...
              if (!is.null(v1) && !is.null(v2)) {
                ...
                # Try state retrieval
                #
                bfObject <- memoryExcludeListwise[[priorLabel]][[nLabel]][[
                    rLabel]]
                  ...
                  if (isTRUE(credibleInterval)) {
                    if (ciValue != bfObject$ciValue) {
                      ...
                      if (test == "pearson") {
                        ...
                        bfObject$ci <- .computeKendallCredibleInterval(
                            n = bfObject$n, tauObs = bfObject$tauObs,
                            kappa = bfObject$kappa, ciValue = bfObject$ciValue)
                      }

                      # Store back into state with new ciValue
                      memoryExcludeListwise[[
                          priorLabel]][[nLabel]][[rLabel]] <- bfObject
                    }
                  }
                  #
                  # Close succesfull retrieval of bf
          ...
          if (isTRUE(retrievalFailure)) {
            ...
            if (perform == "run") {
              ...
              if (!identical(errors, FALSE)) {
                ...
                if (missingValues == "excludePairwise") {
                  subDataSet <- subset(
                      dataset, select = c(.v(variableName), .v(variable2Name)))
                  subDataSet <- na.omit(subDataSet)
                ...
                if (test == "pearson") {
                  ...
                  bfObject <- .bfPearsonCorrelation(
                      n = nObs, r = rObs, kappa = priorWidth, ciValue = ciValue)
                } else if (test == "kendall") {
                  # TODO (Johnny): now by default var=1
                  bfObject <- .bfKendallTau(
                      n = nObs, tauObs = rObs, kappa = priorWidth,
                      ciValue = ciValue)
              ...
              if (missingValues == "excludePairwise") {
                ...
                memoryExcludeListwise[[priorLabel]][[nLabel]][[rLabel]] <-
                    bfObject
              }
              resultProcessing <- TRUE
            }  # Close perform == "run"
          ...
          if (isTRUE(resultProcessing)) {
            ...
            if (!is.null(retrievedFootnote)) {
              ...
              rowFootnotes[[
                  columnName]] <- c(rowFootnotes[[columnName]], list(tempList))
            }
            ...
            if (isTRUE(reportBayesFactors)) {
              ...
              bayesFactorsList[[
                  length(bayesFactorsList) + 1]] <- .clean(reportBf)
            }
      ...
      if (length(rowFootnotes) > 0) row[[".footnotes"]] <- rowFootnotes
      allRows[[i]] <- row
  ...
  return(list(correlationTable = correlationTable, test = tests,
              variables = variables, rows = allRows,
...
.scaledBetaTau <- function(tauPop, alpha = 1, beta = 1) {
  result <- ((pi * 2 ^ (-2 * alpha)) / beta(alpha, alpha)) *
      cos((pi * tauPop) / 2) ^ (2 * alpha - 1)
  return(result)
}
...
.aFunction <- function(n, r, rho) {
  ...
  hyperTerm <- Re(hypergeo::genhypergeo(U = c((n - 1) / 2, (n - 1) / 2),
                                        L = (1 / 2), z = (r * rho) ^ 2))
...
.bFunction <- function(n, r, rho) {
  ...
  #
  hyperTerm <- Re(hypergeo::genhypergeo(
                      U = c(n / 2, n / 2), L = (3 / 2), z = (r * rho) ^ 2))
  logTerm <- 2 *
      (lgamma(n / 2) - lgamma((n - 1) / 2)) + ((n - 1) / 2) * log(1 - rho ^ 2)
...
.hFunctionCombined <- function(nOri, rOri, nRep, rRep, rho) {
  result <- .hFunction(n = nOri, r = rOri, rho) *
      .hFunction(n = nRep, r = rRep, rho)
  return(result)
}

.hFunctionCombinedTwoSided <- function(nOri, rOri, nRep, rRep, rho) {
  result <- .aFunction(n = nOri, r = rOri, rho) *
      .aFunction(
          n = nRep, r = rRep, rho) + .bFunction(n = nOri, r = rOri, rho) *
      .bFunction(n = nRep, r = rRep, rho)
  return(result)
}

.hJeffreysApprox <- function(n, r, rho) {
  result <- ((1 - rho ^ (2)) ^ (0.5 * (n - 1))) /
      ((1 - rho * r) ^ (n - 1 - 0.5))
  return(result)
}
...
.m0MarginalLikelihood <- function(s, t, n) {
  ...
  result <- 1 / 4 * n ^ (0.5 * (1 - 2 * n)) * pi ^ (1 - n) * (s * t) ^ (1 - n) *
      exp(logTerm)
  return(result)
}
...
.bf10Exact <- function(n, r, kappa = 1) {
  ...
  logHyperTerm <- log(
      hypergeo::genhypergeo(U = c((n - 1) / 2, (n - 1) / 2),
                            L = ((n + 2 / kappa) / 2), z = r ^ 2))
  logResult <- log(2 ^ (1 - 2 / kappa)) + 0.5 * log(pi) -
      lbeta(1 / kappa, 1 / kappa) + lgamma((n + 2 / kappa - 1) / 2) -
      lgamma((n + 2 / kappa) / 2) + logHyperTerm
...
.bf10JeffreysIntegrate <- function(n, r, kappa = 1) {
  ...
  hyperTerm <- Re(hypergeo::genhypergeo(U = c((2 * n - 3) / 4, (2 * n - 1) / 4),
                                        L = (n + 2 / kappa) / 2, z = r ^ 2))
  logTerm <- lgamma((n + 2 / kappa - 1) / 2) - lgamma((n + 2 / kappa) / 2) -
...
.bfSavageDickeyBetaData <- function(n, r, kappa = 1, rho0 = 0) {
  ...
  return(.bfSavageDickeyBeta(
             a = fit$betaA, b = fit$betaB, kappa = kappa, rho0 = rho0))
}
...
.bfSavageDickeyBeta <- function(a, b, kappa = 1, rho0 = 0, bf10 = NULL) {
  ...
  if (is.finite(bf10)) {
    ...
    if (is.na(leftProportion)) {
      result <- utils::modifyList(
          result, list(bfPlus0 = NA, bfMin0 = NA, minSidedTooPeaked = TRUE,
                       plusSidedTooPeaked = TRUE))
      return(result)
    }
    ...
    if (leftProportion > 0 && leftProportion < 1) {
      ...
      rightProportion <- stats::pbeta(
          1 / 2, shape1 = a, shape2 = b, lower.tail = FALSE)
...
.logTarget <- function(z, n, r, kappa) {
  ...
  (0.5 * (n - 1)) * log(1 - tanh(z) ^ 2) -
      (n - 1 - 0.5) * log(1 - tanh(z) * r) + log(1 - tanh(z) ^ 2) / kappa
}
...
.metropolisOneStep <- function(rhoCurrent, n, r, kappa = 1) {
  ...
  candidateAcceptance <- .logTarget(zCandidate, n = n, r = r, kappa) +
      .logProposal(zCurrent, n, r) -
      (.logTarget(zCurrent, n = n, r = r, kappa) +
       .logProposal(zCandidate, n, r))
...
.mPlusExact <- function(n, r, kappa = 1) {
  ...
  hyperTerm <- Re(hypergeo::genhypergeo(U = c(1, n / 2, n / 2),
                                        L = c(3 / 2, (2 + kappa * (n + 1)) /
                                                     (2 * kappa)), z = r ^ 2))
  logTerm <- 2 * (lgamma(n / 2) - lgamma((n - 1) / 2)) -
      lbeta(1 / kappa, 1 / kappa)
  result <- 2 ^ ((3 * kappa - 2) / kappa) *
      kappa * r / (2 + (n - 1) * kappa) * exp(logTerm) * hyperTerm
  return(result)
}
...
.mPlusJeffreysIntegrate <- function(n, r, kappa = 1) {
  ...
  #
  hyperTerm <- Re(
      hypergeo::genhypergeo(U = c(1, (2 * n - 1) / 4, (2 * n + 1) / 4),
                            L = c(3 / 2, (n + 1 + 2 / kappa) / 2), z = r ^ 2))
  logTerm <- -lbeta(1 / kappa, 1 / kappa)
  result <- 2 ^ (1 - 2 / kappa) *
      r * (2 * n - 3) / (n + 2 / kappa - 1) * exp(logTerm) * hyperTerm
  return(result)
}
...
## Suit:
.bfHypergeo <- function(
    n, r, kappa = 1, methodNumber = 1, hyperGeoOverFlowThreshold = 24) {
  # Outputs:
  ...
  bf10 <- switch(
      methodNumber, try(silent = TRUE, .bf10Exact(n = n, r = r, kappa = kappa)),
  ...
  if (is.finite(bf10)) {
    ...
    if (log(bf10) < hyperGeoOverFlowThreshold) {
      ...
      switch(methodNumber,
             {
               bfPlus0 <- try(
                   silent = TRUE, bf10 + .mPlusExact(n = n, r = r, kappa))
               bfMin0 <- try(silent = TRUE, bf10 + .mPlusExact(n, -r, kappa))
             }, {
                  bfPlus0 <- try(
                      silent = TRUE, bf10 + .mPlusJeffreysIntegrate(
                                                n = n, r = r, kappa = kappa))
                  bfMin0 <- try(
                      silent = TRUE, bf10 + .mPlusJeffreysIntegrate(
                                                n = n, r = -r, kappa = kappa))
      ...
      if (is.finite(bfPlus0) && is.finite(bfMin0)) {
        tempList <- list(bfPlus0 = bfPlus0, bfMin0 = bfMin0,
                         plusSidedTooPeaked = FALSE, minSidedTooPeaked = FALSE)
        result <- utils::modifyList(result, tempList)
        return(result)
      }
    }
    ...
    tempList <- .bfSavageDickeyOneSidedAdapt(
        bf10, a = result$betaA, b = result$betaB, kappa = kappa)
    result <- utils::modifyList(result, tempList)
    return(result)
  }
  ...
  if (is.infinite(bf10)) {
    ...
    twoSidedIntegrand <- switch(
        methodNumber,
        {
          function(x) {
            .hFunction(n = n, r = r, x) * .priorRho(x, kappa = kappa)
          }
        }, {
             function(x) {
               .hJeffreysApprox(n = n, r = r, x) * .priorRho(x, kappa = kappa)
             }
           })
    ...
    if (is.finite(bf10)) {
      ...
      if (methodNumber == 1) {
        plusSidedIntegrand <- function(x) {
          .hFunction(n = n, r = r, x) * .priorRhoPlus(x, kappa = kappa)
        }
        minSidedIntegrand <- function(x) {
          .hFunction(n = n, r = r, x) * .priorRhoMin(x, kappa = kappa)
        }
      }

      if (methodNumber == 2) {
        plusSidedIntegrand <- function(x) {
          .hJeffreysApprox(n = n, r = r, x) * .priorRhoPlus(x, kappa = kappa)
        }
        minSidedIntegrand <- function(x) {
          .hJeffreysApprox(n = n, r = r, x) * .priorRhoMin(x, kappa = kappa)
        }
      }

      bfPlus0 <- try(
          silent = TRUE, exp = integrate(plusSidedIntegrand, 0, 1)$value)
      bfMin0 <- try(
          silent = TRUE, exp = integrate(minSidedIntegrand, -1, 0)$value)

      if (is.finite(bfPlus0) && is.finite(bfMin0)) {
        tempList <- list(bf10 = bf10, bfMin0 = bfMin0, bfPlus0 = bfPlus0,
                         plusSidedTooPeaked = FALSE, minSidedTooPeaked = FALSE)
        result <- utils::modifyList(result, tempList)
        return(result)
      }
...
.bfCorrieKernel <- function(n, r, kappa = 1, method = "exact", ciValue = 0.95,
  ...
  result <- list(
      n = n, r = r, kappa = kappa, bf10 = NULL, bfPlus0 = NULL, bfMin0 = NULL,
      methodNumber = NULL, betaA = NULL, betaB = NULL, twoSidedTooPeaked = NULL,
      plusSidedTooPeaked = NULL, minSidedTooPeaked = NULL, ci = tempList,
      ciValue = ciValue, acceptanceRate = 1)

  # When bf10 is na, modify result with naList
  #
  tooPeakedList <- list(twoSidedTooPeaked = TRUE, plusSidedTooPeaked = TRUE,
                        minSidedTooPeaked = TRUE)
  ...
  #
  predictiveMatchingList <- list(
      bf10 = 1, bfPlus0 = 1, bfMin0 = 1, twoSidedTooPeaked = FALSE,
      plusSidedTooPeaked = FALSE, minSidedTooPeaked = FALSE, methodNumber = 0)

  # Information consistent result
  #
  infoConsistentList <- append(
      list(bf10 = Inf, methodNumber = 0), tooPeakedList)
  ...
  if (method == "exact" || method == 1) {
    ...
    tempList <- .bfHypergeo(
        n = n, r = r, kappa = kappa, methodNumber = 1,
  ...
  if (method == "jeffreysIntegrate" || method == 2) {
    ...
    tempList <- .bfHypergeo(
        n = n, r = r, kappa = kappa, methodNumber = 2,
        hyperGeoOverFlowThreshold = hyperGeoOverFlowThreshold)
    result <- utils::modifyList(result, tempList)
  }
  ...
  if (method == "metropolisHastings" || method == 4) {
    ...
    tempResult <- .bfSavageDickeyBeta(
        a = marsmanResult$betaA, b = marsmanResult$betaB, kappa = kappa)
  ...
  if (!is.null(ciValue)) {
    ...
    result$ci <- .computePearsonCredibleInterval(
        alpha = result$betaA, beta = result$betaB, ciValue = result$ciValue)
  }
  ...
  if (result$bfPlus0 > 1 &&
      result$bfMin0 > 1 || any(c(result$bfPlus0, result$bfMin0) < 0)) {
    if (r > 0) {
      # Note: Data: OK,
      # 		bf10: OK.
      #		bfPlus0: OK
...
.bfPearsonCorrelation <- function(
    n, r, kappa = 1, ciValue = 0.95, hyperGeoOverFlowThreshold = 24) {
  ...
  while (any(is.na(c(result$bf10, result$bfPlus0, result$bfMin0)),
    ...
    result <- .bfCorrieKernel(
        n = n, r = r, kappa = kappa, method = methodNumber, ciValue = ciValue,
        hyperGeoOverFlowThreshold = hyperGeoOverFlowThreshold)
    #}
  ...
  result$call <- paste0(
      ".bfPearsonCorrelation(n=", n, ", r=", r, ", kappa=", kappa, ", ciValue=",
      ciValue, ", hyperGeoOverFlowThreshold=", hyperGeoOverFlowThreshold, ")")
  result$stat <- r
  return(result)
}

.postDensKendallTau <- function(
    n, Tstar, tauPop, kappa = 1, var = 1, test = "two-sided") {
  if (test == "two-sided") {
  ...
  priorDens <- .priorTau(tauPop, kappa)
  dens <- stats::dnorm(Tstar, (1.5 * tauPop * sqrt(n)), sd = sqrt(var)) *
      priorDens
  return(dens)
}

.posteriorTau <- function(
    n, tauObs, tauPop, kappa = 1, var = 1, test = "two-sided") {
  ...
  dens <- logicalCensor *
      .postDensKendallTau(
          n = n, Tstar = Tstar, tauPop = tauPop, kappa = kappa, var = var,
          test = test) / integrate(function(x) {
                                     .postDensKendallTau(
                                         n = n, Tstar = Tstar, tauPop = x,
                                         kappa = kappa, var = var, test = test)
                                   }, lims[1], lims[2])$value
}
...
.bfKendallTau <- function(n, tauObs, kappa = 1, var = 1, ciValue = 0.95) {
  ...
  #     methodNumber <- methodNumber+1
  # }
  result <- .bfCorrieKernelKendallTau(
      n = n, tauObs = tauObs, kappa = kappa, var = var, ciValue = ciValue)
  result$call <- paste0(".bfKendallTau(n=", n, ", tauObs=", tauObs, ", kappa=",
                        kappa, ", var=", var, ", ciValue=", ciValue, ")")
  result$stat <- tauObs
  return(result)
}

.bfCorrieKernelKendallTau <- function(
    n, tauObs, kappa = 1, var = 1, ciValue = 0.95) {
  tempList <- list(vector())
  result <- list(
      n = n, tauObs = tauObs, bf10 = NA, bfPlus0 = NA, bfMin0 = NA,
      methodNumber = NA, betaA = NA, betaB = NA, twoSidedTooPeaked = FALSE,
      plusSidedTooPeaked = FALSE, minSidedTooPeaked = FALSE, ci = tempList,
      ciValue = ciValue, acceptanceRate = 1)
  ...
  result$bf10 <- .priorTau(tauPop = 0, kappa) /
      .posteriorTau(n = n, tauObs = tauObs, tauPop = 0, kappa = kappa,
                    var = var, test = "two-sided")
  result$bfPlus0 <- .priorTauPlus(tauPop = 0, kappa) /
      .posteriorTau(n = n, tauObs = tauObs, tauPop = 0, kappa = kappa,
                    var = var, test = "positive")
  result$bfMin0 <- .priorTauMin(tauPop = 0, kappa) /
      .posteriorTau(n = n, tauObs = tauObs, tauPop = 0, kappa = kappa,
                    var = var, test = "negative")
  result$methodNumber <- 1

  # Calculate credible intervals
  if (!is.null(ciValue)) {
    result$ci <- .computeKendallCredibleInterval(
        n = n, tauObs = tauObs, kappa = kappa, var = var, ciValue = ciValue)
  }
  return(result)
}
...
.bfCorrieRepJosine <- function(
    nOri, rOri, nRep, rRep, kappa = 1, hyperGeoOverFlowThreshold = 24) {
  result <- list(combined = list(bf10 = NA, bfPlus0 = NA, bfMin0 = NA))

  methodNumber <- 1
  while (methodNumber <= 4 &&
         any(is.na(c(result$combined$bf10, result$combined$bfPlus0,
                     result$combined$bfMin0)),
             is.infinite(result$combined$bf10))) {
    result <- .bfCorrieRepJosineKernel(
        nOri = nOri, rOri = rOri, nRep = nRep, rRep = rRep, kappa = kappa,
  ...
  result$call <- paste0(
      ".bfCorrieRepJosine(nOri=", nOri, ", rOri=", rOri, ", nRep=", nRep,
      ", rRep=", rRep, ", kappa=", kappa, ", hyperGeoOverFlowThreshold=",
      hyperGeoOverFlowThreshold, ")")

  return(result)
}

.bfCorrieRepJosineKernel <- function(
    nOri, rOri, nRep, rRep, kappa = 1, methodNumber = 1,
    hyperGeoOverFlowThreshold = 24) {
  #
  ...
  oriObj <- .bfCorrieKernel(
      n = nOri, r = rOri, method = methodNumber, kappa = kappa)

  # Default is "NA" list
  result <- list(
      ori = oriObj, rep = list(NULL),
      combined = list(
          n = c(nOri, nRep), r = c(rOri, rRep), repMethodNumber = methodNumber,
          bf10 = NA, bfPlus0 = NA, bfMin0 = NA, betaA = NA, betaB = NA),
      repGivenOri = list(n = c(nOri, nRep), r = c(rOri, rRep), bf10 = NA,
                         bfPlus0 = NA, bfMin0 = NA),
  ...
  if (kappa == 1) {
    ...
    repObj <- .bfCorrieKernel(
        n = nRep, r = rRep, method = methodNumber, kappa = kappa)
    result$rep <- repObj
    ...
    if (methodNumber %in% 1:3) {
      oriLikelihoodFit <- .posteriorBetaParameters(
          n = nOri, r = rOri, kappa = 1)
      repLikelihoodFit <- .posteriorBetaParameters(
          n = nRep, r = rRep, kappa = 1)
    }
    ...
    # combine here
    result$combined$betaA <- oriLikelihoodFit$betaA -
        1 + repLikelihoodFit$betaA - 1 + 1 / kappa
    result$combined$betaB <- oriLikelihoodFit$betaB -
        1 + repLikelihoodFit$betaB - 1 + 1 / kappa
  ...
  if (methodNumber == "exact" || methodNumber == 1) {
    twoSidedIntegrand <- function(x) {
      .hFunctionCombinedTwoSided(nOri = nOri, rOri = rOri, nRep = nRep,
                                 rRep = rRep, x) * .priorRho(x, kappa = kappa)
    }
    plusSidedIntegrand <- function(x) {
      .hFunctionCombined(nOri = nOri, rOri = rOri, nRep = nRep, rRep = rRep,
                         x) * .priorRhoPlus(x, kappa = kappa)
    }
    minSidedIntegrand <- function(x) {
      .hFunctionCombined(nOri = nOri, rOri = rOri, nRep = nRep, rRep = rRep,
                         x) * .priorRhoMin(x, kappa = kappa)
    }
  } else if (methodNumber == "jeffreysIntegrate" || methodNumber == 2) {
    twoSidedIntegrand <- function(x) {
      .hJeffreysApprox(nRep, rRep, x) *
          .hJeffreysApprox(nOri, rOri, x) * .priorRho(x, kappa = kappa)
    }
    plusSidedIntegrand <- function(x) {
      .hJeffreysApprox(nRep, rRep, x) *
          .hJeffreysApprox(nOri, rOri, x) * .priorRhoPlus(x, kappa = kappa)
    }
    minSidedIntegrand <- function(x) {
      .hJeffreysApprox(nRep, rRep, x) *
          .hJeffreysApprox(nOri, rOri, x) * .priorRhoMin(x, kappa = kappa)
    }
  }
  ...
  if (methodNumber %in% 1:2) {
    bf10Combined <- try(
        silent = TRUE, exp = integrate(twoSidedIntegrand, -1, 1)$value)
    ...
    if (is.finite(bf10Combined)) {
      ...
      if (log(bf10Combined) > hyperGeoOverFlowThreshold) {
        ...
        tempList <- .bfSavageDickeyOneSidedAdapt(
            bf10Combined, a = result$combined$betaA, b = result$combined$betaB,
            kappa = kappa)
        ...
        if (is.na(bfPlus0Combined) ||
            is.na(bfMin0Combined) ||
            is.infinite(bfPlus0Combined) ||
            is.infinite(bfMin0Combined) ||
            (bfPlus0Combined > 1 && bfMin0Combined > 1) ||
            (bfPlus0Combined < 1 && bfMin0Combined < 1)) {
          tempList <- .bfSavageDickeyOneSidedAdapt(
              bf10Combined, a = result$combined$betaA,
              b = result$combined$betaB, kappa = kappa)
  ...
  if (methodNumber %in% 3:4) {
    ...
    if (!is.na(result$combined$betaA) && !is.na(result$combined$betaB)) {
      ...
      tempList <- .bfSavageDickeyBeta(
          a = result$combined$betaA, b = result$combined$betaB, kappa = kappa)
...
.posteriorRho <- function(n, r, rho, kappa = 1) {
  if (!is.na(r) && !r == 0) {
    return(1 / .bf10Exact(n = n, r = r, kappa) *
           .hFunction(n = n, r = r, rho) * .priorRho(rho, kappa))
  } else if (!is.na(r) && r == 0) {
    return(1 / .bf10JeffreysIntegrate(n = n, r = r, kappa) *
           .hJeffreysApprox(n = n, r = r, rho) * .priorRho(rho, kappa))
  }
}

.posteriorRhoPlus <- function(n, r, rho, kappa = 1) {
  if (!is.na(r) && !r == 0) {
    return(1 / .bfCorrieKernel(n = n, r = r, kappa, method = "exact")$bfPlus0 *
           .hFunction(n = n, r = r, rho) * .priorRhoPlus(rho, kappa))
  } else if (!is.na(r) && r == 0) {
    return(1 / .bfCorrieKernel(
                   n = n, r = r, kappa, method = "jeffreysIntegrate")$bfPlus0 *
...
.posteriorRhoMin <- function(n, r, rho, kappa = 1) {
  if (!is.na(r) && !r == 0) {
    return(1 / .bfCorrieKernel(n = n, r = r, kappa, method = "exact")$bfMin0 *
           .hFunction(n = n, r = r, rho) * .priorRhoMin(rho, kappa))
  } else if (!is.na(r) && r == 0) {
    return(1 / .bfCorrieKernel(
                   n = n, r = r, kappa, method = "jeffreysIntegrate")$bfMin0 *
...
.approximatePosteriorRho <- function(rho, n, r) {
  1 / (1 -
       rho ^ 2) * stats::dnorm(atanh(rho), mean = atanh(r), sd = 1 / sqrt(n))
}

.approximatePosteriorRhoPlus <- function(rho, n, r) {
  (.approximatePosteriorRho(rho, n, r) * (rho > 0)) /
      (stats::pnorm(0, mean = atanh(r), sd = 1 / sqrt(n), lower.tail = FALSE))
}

.approximatePosteriorRhoMin <- function(rho, n, r) {
  (.approximatePosteriorRho(rho, n, r) *
   (rho < 0)) / (stats::pnorm(0, mean = atanh(r), sd = 1 / sqrt(n)))
}
...
.posteriorMean <- function(n, r, kappa = 1) {
  ...
  #logHyperTerm <- log(hypergeo::hypergeo(((n-1)/2), ((n-1)/2), ((n+2/kappa)/2), r^2))
  hyperTerm1 <- Re(
      hypergeo::genhypergeo(U = c(n / 2, n / 2), L = c((2 + (n + 2) * kappa) /
                                                       (2 * kappa)), z = r ^ 2))
  hyperTerm2 <- Re(
      hypergeo::genhypergeo(U = c((n - 1) / 2, (n - 1) / 2),
                            L = c((2 + n * kappa) / (2 * kappa)), z = r ^ 2))

  logResult <- 2 * (lgamma(n / 2) - lgamma((n - 1) / 2))
  result <- (2 * kappa * r) /
      (2 + n * kappa) * exp(logResult) * hyperTerm1 / hyperTerm2
...
.posteriorVariance <- function(n, r, kappa = 1) {
  ...
  result <- .posteriorSecondMoment(n, r, kappa) -
      (.posteriorMean(n, r, kappa)) ^ 2
...
.posteriorBetaParameters <- function(n, r, kappa = 1) {
  ...
  if (is(some.mu, "try-error") ||
      is(some.var, "try-error") || is.na(some.mu) || is.na(some.var)) {
...
.computePearsonCredibleInterval <- function(alpha, beta, ciValue) {
  ...
  # One sided:
  result$minSided <- .computePearsonMinSidedCredibleInterval(
      alpha, beta, ciValue)
...
.computeKendallCredibleInterval <- function(
    n, tauObs, kappa = 1, var = 1, ciValue = 0.95) {
  ...
  result$twoSided <- .credibleIntervalKendallTau(
      n = n, tauObs = tauObs, kappa = kappa, var = var, test = "two-sided",
      ciValue = ciValue)
  result$plusSided <- .credibleIntervalKendallTau(
      n = n, tauObs = tauObs, kappa = kappa, var = var, test = "positive",
      ciValue = ciValue)
  result$minSided <- .credibleIntervalKendallTau(
      n = n, tauObs = tauObs, kappa = kappa, var = var, test = "negative",
      ciValue = ciValue)
  return(result)
}

# Compute credible intervals kendalls tau
.credibleIntervalKendallTau <- function(
    n, tauObs, kappa = 1, var = 1, test = "two-sided", ciValue = 0.95) {
  nSeqs <- 1000
  lowCI <- (1 - ciValue) / 2
  upCI <- (1 + ciValue) / 2
  tauPopDomain <- seq(-1, 1, length.out = (nSeqs - 1))
  densVals <- .posteriorTau(n = n, tauObs = tauObs, tauPop = tauPopDomain,
                            kappa = kappa, var = var, test = test)
  cdfVals <- cumsum((densVals[1:(nSeqs - 1)] + densVals[2:nSeqs]) *
                    0.5 * (tauPopDomain[2] - tauPopDomain[1]))
...
.displayErrorPosterior <- function(
    errorMessage = NULL, xticks, xlabels, xlim, cexText = 1.6, cexAxis = 1.2,
    cexYlab = 1.5, cexXlab = 1.28, lwdAxis = 1.2) {

  plot(1, 1, xlim = xlim, ylim = 0:1, ylab = "", xlab = "", type = "n",
       axes = FALSE)

  text(0, .5, errorMessage, cex = cexText)

  axis(1, at = xticks, labels = xlabels, cex.axis = cexAxis, lwd = lwdAxis)
  axis(2, at = c(0, .5, 1), pos = range(xticks) - 0.08 * diff(range(xticks)),
       labels = c("", "Density", ""), lwd.ticks = 0, cex.axis = 1.7,
       mgp = c(3, 0.7, 0), las = 0)
...
.plotPosterior.BayesianCorrelationMatrix <- function(
    x, y, kappa = 1, oneSided = FALSE, addInformation = FALSE, drawCI = FALSE,
    lwd = 2, cexPoints = 1.5, cexAxis = 1.2, cexYlab = 1.5, cexXlab = 1.28,
    cexTextBF = 1.4, cexCI = 1.1, cexLegend = 1.2, lwdAxis = 1.2, addRho = TRUE,
    addTau = FALSE) {
  ...
  if (oneSided == FALSE) {
    ...
    try(silent = TRUE,
        expr = {
          numIntegrate <- integrate(
                              function(x) {
                                .posteriorRho(x, n = n, r = r, kappa = kappa)
    ...
    posteriorLineTau <- .posteriorTau(
        n = n, tauObs = tau, tauPop = rho, kappa = kappa, var = 1,
        test = "two-sided")
    legendPosition <- c("topright", "topleft")[
        (which(posteriorLine == max(posteriorLine)) > 500) + 1]

    if (sum(is.na(posteriorLine)) > 1 ||
        any(posteriorLine < 0) || any(is.infinite(posteriorLine))) {

      betaApproximation <- TRUE

      if (any(is.na(c(betaA, betaB)))) tooPeaked <- TRUE
    ...
    try(silent = TRUE,
        expr = {
          numIntegrate <- integrate(function(x) {
                                      .posteriorRhoPlus(
                                          x, n = n, r = r, kappa = kappa)
                                    }, lower = -1, upper = 1)$value
          if (round(numIntegrate, digits = 2) != 1) {
            posteriorLine <- .approximatePosteriorRhoPlus(
                rho = rho, n = n, r = r)
          }
        })

    posteriorLineTau <- .posteriorTau(n = n, tauObs = tau, tauPop = rho,
                                      kappa = kappa, var = 1, test = "positive")
    ...
    if (sum(is.na(posteriorLine)) > 1 ||
      ...
      posteriorLine <- .stretchedBeta(alpha = betaA, beta = betaB, rho = rho) /
          pbeta(1 / 2, betaA, betaB, lower.tail = FALSE)
    ...
    try(silent = TRUE,
        expr = {
          numIntegrate <- integrate(
                              function(x) {
                                .posteriorRhoMin(x, n = n, r = r, kappa = kappa)
                              }, lower = -1, upper = 1)$value
          if (round(numIntegrate, digits = 2) != 1) {
            posteriorLine <- .approximatePosteriorRhoMin(
                rho = rho, n = n, r = r)
          }
        })

    posteriorLineTau <- .posteriorTau(n = n, tauObs = tau, tauPop = rho,
                                      kappa = kappa, var = 1, test = "negative")
    ...
    if (sum(is.na(posteriorLine)) > 1 ||
      ...
      posteriorLine <- .stretchedBeta(alpha = betaA, beta = betaB, rho = rho) /
          pbeta(1 / 2, betaA, betaB, lower.tail = TRUE)
      posteriorLine[rho > 0] <- 0
  ...
  if (tooPeaked) {
    ...
    .displayErrorPosterior(errorMessage = "Posterior is too peaked",
                           xticks = xticks, xlabels = xlabels, xlim = xlim)
    ...
    if (addRho & addTau) {
      ...
      legend(x = legendPosition[1], y = legendPosition[2],
             legend = c(expression(rho), expression(tau)), lty = 1:2,
             cex = cexYlab, bty = "n", lwd = lwd)
    } else if (addRho) {
      lines(rho, posteriorLine, lwd = lwd)
    ...
    axis(1, at = xticks, labels = xlabels, cex.axis = cexAxis, lwd = lwdAxis)
    axis(2, at = c(ylim[1], mean(ylim), ylim[2]),
         pos = range(xticks) - 0.08 * diff(range(xticks)),
         labels = c("", "Density", ""), lwd.ticks = 0, cex.axis = 1.7,
         mgp = c(3, 0.7, 0), las = 0)
...
.correlationMatrixPlotBayesian <- function(
    dataset, perform, options, hypothesis = options$hypothesis) {

  if (!options$plotCorrelationMatrix) return()
    ...
    if (length(variables) > 0) {
      ...
      p <- try(
          ...
          expr = {
            ...
            .plotFunc <- function() {
              ...
              if (l == 1) {
                ...
                # mtext(text = .unv(variables)[1], side = 1, cex=1.9, line = 3)
                         ...
                         !options$plotDensities && !options$plotPosteriors) {

                par(mfrow = c(1, 1), cex.axis = 1.3,
                    mar = c(3, 4, 2, 1.5) + 0.1, oma = c(2, 2.2, 2, 0))

                if (!variable.statuses[[1]]$unplotable &&
                    !variable.statuses[[2]]$unplotable) {

                  maxYlab <- .plotScatter(
                      dataset[[variables[1]]], dataset[[variables[2]]])
                  distLab <- maxYlab / 1.8

                  mtext(
                      text = .unv(variables)[1], side = 1, cex = 1.5, line = 3)
                  mtext(text = .unv(variables)[2], side = 2, cex = 1.5,
                        line = distLab + 2, las = 0)

                } else {

                  errorMessages <- c(variable.statuses[[1]]$plottingError,
                                     variable.statuses[[2]]$plottingError)
                  errorMessagePlot <- paste0(
                ...
                par(mfrow = c(l, l), cex.axis = 1.3,
                    mar = c(3, 4, 2, 1.5) + 0.1, oma = c(1, 2.2, 2, 0))
                  ...
                  for (col in seq_len(l)) {
                    ...
                    if (row == col) {
                      ...
                      if (options$plotDensities) {
                        ...
                        if (!variable.statuses[[row]]$unplotable) {
                          .plotMarginalCor(
                              dataset[[variables[row]]])  # plot marginal
                                                          # (histogram with
                                                          # density estimator)
                        } else {
                    ...
                    if (col > row) {
                      if (options$plotCorrelationMatrix) {
                        if (!variable.statuses[[col]]$unplotable &&
                            !variable.statuses[[row]]$unplotable) {
                          .plotScatter(dataset[[variables[col]]],
                                       dataset[[variables[row]]])  # plot
                                                                   # scatterplot
                        } else {
                          errorMessages <- c(
                              variable.statuses[[row]]$plottingError,
                              variable.statuses[[col]]$plottingError)
                          errorMessagePlot <- paste0(
                              "Correlation coefficient undefined:", "\n",
                              errorMessages[1])
                    ...
                    if (col < row) {
                      ...
                      if (options$plotPosteriors) {
                        ...
                        if (!variable.statuses[[col]]$unplotable &&
                            !variable.statuses[[row]]$unplotable) {
                          .plotPosterior.BayesianCorrelationMatrix(
                              dataset[[variables[col]]],
                              dataset[[variables[row]]], oneSided = oneSided,
                              kappa = options$priorWidth,
                              addRho = options$pearson,
                              addTau = options$kendallsTauB)
                        } else {
                          errorMessages <- c(
                              variable.statuses[[row]]$plottingError,
                              variable.statuses[[col]]$plottingError)
              ...
              if (l > 2 || ((l == 2 && options$plotDensities) ||
                            (l == 2 && options$plotPosteriors))) {
                ...
                if (!options$plotDensities && !options$plotPosteriors) {
                  ...
                  for (t in seq_along(textpos)) {
                    ...
                    mtext(text = .unv(variables)[t], side = 3, outer = TRUE,
                          at = textpos[t], cex = 1.5, line = -0.8)

                    if (t < length(textpos)) {
                      mtext(
                          text = .unv(variables)[t], side = 2, outer = TRUE,
                          at = rev(textpos)[t], cex = 1.5, line = -0.1, las = 0)
                    }
                  }
                  ...
                  for (t in seq_along(textpos)) {
                    ...
                    mtext(text = .unv(variables)[t], side = 3, outer = TRUE,
                          at = textpos[t], cex = 1.5, line = -0.8)
                    mtext(text = .unv(variables)[t], side = 2, outer = TRUE,
                          at = rev(textpos)[t], cex = 1.5, line = -0.1, las = 0)
                  }
                }
              }
            ...
            content <- .writeImage(
                width = width, height = height, plot = .plotFunc, obj = TRUE)
