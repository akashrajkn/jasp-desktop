...
RegressionLogLinearBayesian <- function(
    dataset, options, perform = "run", callback, ...) {
  ...
  if (is.null(dataset)) {
    ...
    if (perform == "run") {
      dataset <- .readDataSetToEnd(
          columns.as.factor = options$factors, columns.as.numeric = countsVar)
    } else {
  ...
  #
  initialObject <- list(
      "anthonyObj" = NULL, "variables" = c("...", "... "),
      "nModelsVisited" = NULL, "nBurnIn" = NULL, "bf10s" = rep(".", length = 2),
      "postModelProbs" = rep(".", length = 2), "modelNames" = NULL,
      "hasErrors" = FALSE, "errorMessages" = list())

  errorBfObj <- list(
      nModelsVisited = NA, postModelProbs = rep(NA, length = options$maxModels),
    ...
    if (numberOfModels == 0) {
      ...
      for (i in 1:length(options$modelTerms)) {
        ...
        if (length(components) == 1) {
          ...
          variablesInModel <- c(
              variablesInModel, paste0(componentsUnlisted, collapse = ":"))
        }
      }
  ...
  if (perform == "run") {
    ...
    if (options$counts != "") {
      ...
      if (!is.null(badVariableNames)) {
        errorMessage <- paste0(
            "Bayes factor is undefined -- incomplete contingency table, the count variable ",
            badVariableNames, " contain(s) empty cell and/or NaN.")
        listOfErrors[[length(listOfErrors) + 1]] <- errorMessage

        # TODO: Tim's error handling
        bfObject$hasErrors <- TRUE
        bfObject$errorMessages[[length(bfObject$errorMessages) + 1]] <- list(
            errorMessage = errorMessage, errorType = "badData")
      }
      ...
      if (length(listOfErrors) == 0) {
        ...
        for (counts in options$counts) {
          if (any(!is.finite(dataset[[.v(options$counts)]])) ||
              any(dataset[[.v(options$counts)]] < 0)) {
            badVariableNames <- c(badVariableNames, options$counts)
          }
        }
        ...
        if (!is.null(badVariableNames)) {
          errorMessage <- paste0(
              "Bayes factor is undefined -- the count variable ",
              badVariableNames, " contain(s) infinity and/or negative numbers.")
          listOfErrors[[length(listOfErrors) + 1]] <- errorMessage
    ...
    if (length(listOfErrors) == 0) {
      if (isTRUE(anyNA(dataset[.v(options$factors)]))) {
        errorMessage <-
            "Bayes factor is undefined -- the factors contain(s) empty cell and/or NaN or incomplete contingency table."
        listOfErrors[[length(listOfErrors) + 1]] <- errorMessage
    ...
    if (length(variablesInModel) == 0) {
      ...
      modelDefinition <- paste(
          dependentBase64, "~", paste(variablesInModelBase64, collapse = "+"))
    } else {
      # Nothing worked out:
      modelDefinition <- NULL  # this model has no parameters
      ...
      bfObject$hasErrors <- TRUE
      bfObject$errorMessages[[length(bfObject$errorMessages) + 1]] <- list(
          errorMessage = "variables cannot be read", errorType = "input")
    }
    ...
    if (!is.null(modelDefinition) && !isTRUE(bfObject$hasErrors)) {
      ...
      anthonyObj <- try(
          conting::bcct(
              formula = modelFormula, data = dataset, prior = "SBH",
              n.sample = 2000, a = options$priorShape, b = options$priorScale),
          silent = TRUE)
      ...
      if (class(anthonyObj) == "bcct") {
        ...
        bfObject$hasErrors <- TRUE
        bfObject$errorMessages[[length(bfObject$errorMessages) + 1]] <- list(
            errorMessage = error, errorType = "package")
      }
      ...
      bfObject$hasErrors <- TRUE
      bfObject$errorMessages[[length(bfObject$errorMessages) + 1]] <- list(
          errorMessage = "Cannot define model", errorType = "input")
    }
    # No clue error changed in to exception
    # else {
    #     # NAs: nModelsVisited, bf10s, postModelProbs
    ...
    if (!isTRUE(bfObject$hasErrors)) {
      if (class(bfObject$anthonyObj) == "bcct") {
        ...
        anthonySummary <- try(conting::mod_probs(bfObject$anthonyObj, scale = 0,
                                                 best = options$maxModels),
                              silent = TRUE)

        if (inherits(anthonySummary, "modprobs")) {
          # Good case
          bfObject$nModelsVisited <- anthonySummary$totmodsvisit
          bfObject$modelNames <- substring(
              as.character(anthonySummary$table$model_formula), first = 2)
          ...
          if (bfObject$nModelsVisited == 1) {
            ...
            if (!is.null(anthonySummary$table$prob.Freq)) {
              ...
              bfObject$bf10s <- anthonySummary$table$prob.Freq /
                  max(anthonySummary$table$prob.Freq)
            } else {
              # NAs: nModelsVisited, bf10s, postModelProbs
              bfObject <- modifyList(bfObject, errorBfObj)

              bfObject$hasErrors <- TRUE
              bfObject$errorMessages <- list(
                  errorMessages = "R Package error: Cannot retrieve table probabilities",
                  errorType = "package")
            }
          }
          # No clue error changed in to exception
          ...
          bfObject$errorMessages[[length(bfObject$errorMessages) + 1]] <- list(
              errorMessage = paste0(
                  "R Package error: ", .extractErrorMessage(anthonySummary)),
              errorType = "package")
        }
        # No clue error changed in to exception
        # else {
        ...
        bfObject$hasErrors <- TRUE
        bfObject$errorMessages[[length(bfObject$errorMessages) + 1]] <- list(
            errorMessage = "R Package error: Object of wrong class",
            errorType = "package")
      }
    } else {
  ...
  if (isTryError(nModelsReport) ||
      isTRUE(bfObject$hasErrors) || perform == "init") {
    # Report error Bfs
    nModelsReport <- 2
  }
  ...
  message <- paste(
      "Total number of models visited =", bfObject$nModelsVisited, sep = " ")
  ...
  if (options$regressionCoefficientsEstimates == TRUE) {
    ...
    ci.label <- paste0(
        100 * options$regressionCoefficientsCredibleIntervalsInterval,
        "% Credible intervals")
    # Declare table elements
    fields <- list(
        list(name = "Name", title = " ", type = "string"),
        list(name = "post_prob", title = "P(incl|data)", type = "number",
             format = "dp:3"), list(name = "post_mean", title = "Mean",
                                    type = "number", format = "dp:3"),
        list(name = "post_var", title = "Variance", type = "number",
             format = "dp:3"))
    if (options$regressionCoefficientsCredibleIntervals == TRUE) {
      fields <- c(
          fields,
          list(list(name = "lower_lim", title = "Lower", overTitle = ci.label,
                    type = "number", format = "sf:4;dp:3"),
               list(name = "upper_lim", title = "Upper", overTitle = ci.label,
                    type = "number", format = "sf:4;dp:3")))
    }

    emptyRow <- list(  # for empty elements in tables when given output
        "Name" = "", "post_prob" = "", "post_mean" = "", "post_var" = "",
        "lower_lim" = "", "upper_lim" = "")
    ...
    lookup.table <- .regressionLogLinearBayesianBuildLookup(
        dataset, options$factors)
    ...
    if (perform == "run" && length(listOfErrors) == 0) {
      if (inherits(bfObject$anthonyObj, "try-error")) {
        ...
        Bayesianlogregression[[
            "error"]] <- list(errorType = "badData", errorMessage = error)
      } else if (class(bfObject$anthonyObj) == "bcct") {
        logBlm.summary =
            summary(
                bfObject$anthonyObj, n.burnin = bfObject$nBurnIn,
                cutoff = options$posteriorProbabilityCutOff,
                prob.level = options$regressionCoefficientsCredibleIntervalsInterval)
        ...
        if (length(bfObject$variables) > 0) {
          ...
          for (var in seq_along(coef)) {
            ...
            for (j in seq_along(terms)) {
              actualName[[
                  j]] <- paste(lookup.table[[terms[j]]], collapse = " = ")
            }
            varName <- paste0(actualName, collapse = "*")

            Bayesianlogregression.result[[len.Blogreg]]$"Name" <- varName
            Bayesianlogregression.result[[
                len.Blogreg]]$"post_prob" <- as.numeric(
                logBlm.estimates$prob[var])
            Bayesianlogregression.result[[
                len.Blogreg]]$"post_mean" <- as.numeric(
                logBlm.estimates$post_mean[var])
            Bayesianlogregression.result[[
                len.Blogreg]]$"post_var" <- as.numeric(
                logBlm.estimates$post_var[var])

            if (options$regressionCoefficientsCredibleIntervals == TRUE) {
              Bayesianlogregression.result[[
                  len.Blogreg]]$"lower_lim" <- as.numeric(
                  logBlm.estimates$lower[var])
              Bayesianlogregression.result[[
                  len.Blogreg]]$"upper_lim" <- as.numeric(
                  logBlm.estimates$upper[var])
            }
      ...
      if (length(listOfErrors) == 1) {
        Bayesianlogregression[["error"]] <- list(
            errorType = "badData", errorMessage = listOfErrors[[1]])
      }
    }
  ...
  if (options$regressionCoefficientsSubmodel == TRUE) {
    ...
    BayesianSublogregression[["title"]] <- paste(
        "Posterior Summary Statistics For Submodel",
        options$regressionCoefficientsSubmodelNo, sep = " ")
    BayesianSublogregression[["citation"]] <- logLinearBayesianCitations
    ci.label <- paste(
        100 * options$regressionCoefficientsSubmodelCredibleIntervalsInterval,
        "% Credible intervals", sep = "")
    # Declare table elements
    fields <- list(
        list(name = "Name", title = " ", type = "string"),
        list(name = "post_mean", title = "Mean", type = "number",
             format = "dp:3"), list(name = "post_var", title = "Variance",
                                    type = "number", format = "dp:3"))
    ...
    emptyRow <- list(  # for empty elements in tables when given output
        "Name" = "", "post_mean" = "", "post_var" = "", "lower_lim" = "",
        "upper_lim" = "")
    ...
    if (perform == "run" &&
        length(listOfErrors) == 0 && !is.null(bfObject$anthonyObj)) {
      logBlm.subestimates =
          try(conting::sub_model(
                  bfObject$anthonyObj, n.burnin = bfObject$nBurnIn,
                  order = options$regressionCoefficientsSubmodelNo,
                  prob.level = options$regressionCoefficientsSubmodelCredibleIntervalsInterval),
              silent = TRUE)

      if (inherits(logBlm.subestimates, "try-error")) {
        error <- .extractErrorMessage(logBlm.subestimates)
        BayesianSublogregression[[
            "error"]] <- list(errorType = "badData", errorMessage = error)
        ...
        extractedModelFormula <- substring(
            extractedModelFormula, first = 2)  # trim leading ~
        extractedModelFormula <- .unvf(extractedModelFormula)
        message1 <- extractedModelFormula
        .addFootnote(
            footnotes, symbol = "<em>Model formula:</em>", text = message1)

        Post.pob <- round(logBlm.subestimates$post_prob, 3)
        .addFootnote(
            footnotes, symbol = "<em>Posterior model probability =</em>",
            text = Post.pob)
        ...
        if (length(bfObject$variables) > 0) {
          ...
          for (var in seq_along(coef)) {
            ...
            BayesianSublogregression.result[[
                len.Blogreg]]$"post_mean" <- as.numeric(
                logBlm.subestimates$post_mean[var])
            BayesianSublogregression.result[[
                len.Blogreg]]$"post_var" <- as.numeric(
                logBlm.subestimates$post_var[var])

            if (options$regressionCoefficientsSubmodelCredibleIntervals ==
                TRUE) {
              BayesianSublogregression.result[[
                  len.Blogreg]]$"lower_lim" <- as.numeric(
                  logBlm.subestimates$lower[var])
              BayesianSublogregression.result[[
                  len.Blogreg]]$"upper_lim" <- as.numeric(
                  logBlm.subestimates$upper[var])
            }
      ...
      if (length(listOfErrors) == 1) {
        BayesianSublogregression[["error"]] <- list(
            errorType = "badData", errorMessage = listOfErrors[[1]])
      }
    }
  ...
  if (perform == "init") {
    if (options$counts == "" && numberOfModels < 2) {
      ...
      endResults <- list(results = results, status = "inited")  # , state=state,
                                                                # keep=keep)
    }
  } else {
