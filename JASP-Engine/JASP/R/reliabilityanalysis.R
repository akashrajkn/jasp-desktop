...
ReliabilityAnalysis <- function(dataset = NULL, options, perform = "run",
  ...
  if (is.null(dataset)) {
    ...
    if (perform == "run") {
      ...
      dataset <- .readDataSetToEnd(
          columns.as.numeric = variables, columns.as.factor = NULL,
    ...
    dataset <- .vdf(
        dataset, columns.as.numeric = variables, columns.as.factor = NULL)
  ...
  stateKey <- list(
      resultsAlpha = defaults, confAlpha = c(defaults, "confAlphaLevel"))
  ...
  meta <- list(
      list(name = "reliabilityScale", type = "table"),
      list(name = "reliabilityItemsObj", type = "object",
           meta = list(list(name = "reliabilityItems", type = "table"))))
  ...
  if (!is.null(variables) && length(variables) > 0) {
    ...
    } else if (is.null(state[["confAlpha"]])) {  # resultsAlpha retrieved from
                                                 # state, only recalculate CI

      resultsAlpha[["ciAlpha"]] <- .reliabilityAlphaCI(
          relyFit = resultsAlpha, ci = options[["confAlphaLevel"]])

    }
  }
  results[["reliabilityScale"]] <- .reliabalityScaleTable(
      resultsAlpha, dataset, options, variables, perform)

  if (options$alphaItem ||
      options$gutmannItem ||
      options$itemRestCor ||
      options$meanItem || options$sdItem || options[["mcDonaldItem"]]) {
    results[["reliabilityItemsObj"]] <- list(
        title = "Item Statistics",
        reliabilityItems = .reliabalityItemsTable(
            resultsAlpha, options, variables, perform))
  } else {
...
.reliabilityResults <- function(dataset, options, variables, perform) {
  ...
  if (perform == "run" && !is.null(variables) && length(variables) > 1) {
    ...
    if (length(options[["reverseScaledItems"]]) > 0) {
      ...
      key[match(.v(unlist(options[["reverseScaledItems"]])), colnames(dataset))] <-
          -1
    ...
    relyFit[["ciAlpha"]] <- .reliabilityAlphaCI(
        relyFit = relyFit, ci = options[["confAlphaLevel"]])
    ...
    if (nVar < 3) {
      ...
      relyFit[["glb"]] <- try(psych::glb(r = dataList[["correlation"]],
                                         key = key)[["glb.max"]], silent = TRUE)

    }

    # calculate McDonalds omega
    omega <- psych::omega(
                 m = dataList[["correlation"]], nfactors = 1, flip = FALSE,
    ...
    if (nVar > 2) {
      ...
      for (i in 1:nVar) {
        omegaDropped[
            i] <- psych::omega(m = dataList[["correlation"]][-i, -i],
...
.reliabalityScaleTable <- function(r, dataset, options, variables, perform) {
  ...
  if (options$meanScale)
    fields[[length(fields) + 1]] <- list(
        name = "mu", title = "mean", type = "number", format = "sf:4;dp:3")

  if (options$sdScale)
    fields[[length(fields) + 1]] <- list(
        name = "sd", title = "sd", type = "number", format = "sf:4;dp:3")

  if (options$alphaScale)
    fields[[length(fields) + 1]] <- list(
        name = "alpha", title = "Cronbach's \u03B1", type = "number",
        format = "sf:4;dp:3")

  if (options$gutmannScale)
    fields[[length(fields) + 1]] <- list(
        name = "lambda", title = "Gutmann's \u03BB6", type = "number",
        format = "sf:4;dp:3")

  if (options[["mcDonaldScale"]])
    fields[[length(fields) + 1]] <- list(
        name = "omega", title = "McDonald's \u03C9", type = "number",
        format = "sf:4;dp:3")

  if (options[["glbScale"]])
    fields[[length(fields) + 1]] <- list(
        name = "glb", title = "Greatest lower bound", type = "number",
        format = "sf:4;dp:3")

  if (options[["averageInterItemCor"]])
    fields[[length(fields) + 1]] <- list(
        name = "rho", title = "Average interitem correlation", type = "number",
        format = "sf:4;dp:3")

  if (options[["confAlpha"]]) {
    overTitle <- sprintf(
        "%.1f%% Confidence Interval", 100 * options[["confAlphaLevel"]])
    fields[[length(fields) + 1]] <- list(
        name = "lower", title = "Lower", type = "number", format = "sf:4;dp:3",
        overTitle = overTitle)
    fields[[length(fields) + 1]] <- list(
        name = "upper", title = "Upper", type = "number", format = "sf:4;dp:3",
        overTitle = overTitle)
  }
  ...
  if (!is.null(r)) {
    ...
    message <- sprintf(
        "Of the observations, %d were used, %d were excluded%s, and %d were provided.",
    ...
    if (options[["glbScale"]]) {
      ...
      if (length(variables) <= 2) {
        ...
        message <- paste(
            message,
            "Warning: Greatest lower bound can only be calculated for three or more variables.")

      } else if (isTryError(r[["glb"]])) {

        message <- paste(
            message, "Warning: Greatest lower bound could not be calculated.")
    ...
    if (options$alphaScale) alpha <- .clean(r$total$raw_alpha)

    if (options$gutmannScale) lambda <- .clean(r$total[["G6(smc)"]])

    if (options$meanScale) mu <- .clean(r$total$mean)

    if (options$sdScale) sd <- .clean(r$total$sd)

    if (options[["averageInterItemCor"]])
      rho <- .clean(r[["total"]][["average_r"]])

    if (options[["mcDonaldScale"]]) omega <- .clean(r[["omega"]])
    ...
    data[[1]] <- list(
        case = "scale", alpha = alpha, lambda = lambda, omega = omega,
        glb = glb, rho = rho, mu = mu, sd = sd, lower = lower, upper = upper)
    ...
    data[[1]] <- list(
        case = "scale", alpha = ".", lambda = ".", omega = ".", glb = ".",
        rho = ".", mean = ".", sd = ".", lower = ".", upper = ".")
...
.reliabalityItemsTable <- function(r, options, variables, perform) {
  ...
  fields =
      list(list(name = "case", title = "", type = "string", combine = TRUE))
  ...
  if (options$itemRestCor)
    fields[[length(fields) + 1]] <- list(
        name = "itemRestCor", title = "item-rest correlation", type = "number",
        format = "sf:4;dp:3")
  ...
  if (options[["mcDonaldItem"]] && length(variables) < 3) {
    ...
    message <-
        "Warning: McDonald's \u03C9 if item dropped can only be calculated for three or more variables."
  ...
  if (!is.null(r)) {
    ...
    for (var in variables) {
      ...
      if (options$alphaItem) alpha <- .clean(r$alpha.drop[index, "raw_alpha"])

      if (options$gutmannItem) lambda <- .clean(r$alpha.drop[index, "G6(smc)"])

      if (options$itemRestCor)
        itemRestCor <- .clean(r$item.stats[index, "r.drop"])

      if (options$meanItem) mu <- .clean(r$item.stats[index, "mean"])

      if (options$sdItem) sd <- .clean(r$item.stats[index, "sd"])

      if (options[["mcDonaldItem"]]) omega <- .clean(r[["omegaDropped"]][index])

      data[[length(data) + 1]] <- list(
          case = case, alpha = alpha, lambda = lambda, omega = omega,
          itemRestCor = itemRestCor, mu = mu, sd = sd)
    }
    ...
    if (is.null(variables)) variablesTemp <- "..."

    for (var in variablesTemp) {

      data[[length(data) + 1]] <- list(
          case = var, alpha = ".", lambda = ".", omega = ".", itemRestCor = ".",
          mu = ".", sd = ".")
...
.reliabilityAlphaCI <- function(relyFit, ci, nullAlpha = 0) {
  ...
  nDen <- (nObs - 1) * (nVar - 1)
  nNum <- nObs - 1
  null.p <- stats::pf(f, nNum, nDen)  # set the upper and lower p values for the
                                      # desired C.I.
  p1 <- (1 - ci) / 2
...
.reliabilityConvertDataToCorrelation <- function(dataset, options) {
  ...
  cormat <- psych::cor.smooth(
      stats::cov2cor(covmat), eig.tol = sqrt(.Machine[["double.eps"]]))

  return(list(correlation = cormat, itemSds = stdev, itemMeans = means,
