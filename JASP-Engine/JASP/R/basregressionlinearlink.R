...
BASRegressionLinearLink <- function(
  ...
  modelOpts <- c("dependent", "covariates", "wlsWeights", "modelTerms",
                 "priorRegressionCoefficients", "gPriorParameter", "modelPrior",
                 "betaBinomialParamA", "betaBinomialParamB", "bernoulliParam",
                 "samplingMethod", "iterationsMCMC", "numberOfModels")
  stateKey <- list(bas_obj = modelOpts,
                   descriptives = c("dependent", "covariates"),
                   plotPosteriorLogOdds = c(modelOpts, "plotLogPosteriorOdds"),
                   plotCoefficientsPosterior = c(
                       modelOpts, "plotCoefficientsPosterior"),
                   plotResidualsVsFitted = c(
                       modelOpts, "plotResidualsVsFitted"),
                   plotModelProbabilities = c(
                       modelOpts, "plotModelProbabilities"),
                   plotModelComplexity = c(modelOpts, "plotModelComplexity"),
                   plotInclusionProbabilities = c(
                       modelOpts, "plotInclusionProbabilities"))  # TODO: check
                                                                  # if this
                                                                  # dependence
                                                                  # is correct
  ...
  if (perform == "run" && status$ready) {
    ...
    if (is.null(bas_obj)) {
      bas_output <- .calcModel.basReg(
          dataset = dataset, status = status, options = options)
      ...
      if (!status$error) {
        ...
        bas_obj$namesx[2:len.bas_obj] <- .unvf(
            bas_obj$namesx[2:len.bas_obj])  # first is intercept
      }
    }
  ...
  if (!is.null(bas_obj)) {
    ...
    regTableData <- .calcDataRegTable.basReg(
        bas_obj = bas_obj, status = status, options = options)
  }

  if (options$descriptives && is.null(descriptives)) {
    descriptives <- .descriptivesTable.basReg(dataset, status, perform, options)
  }
  ...
  if (options$plotInclusionProbabilities &&
      is.null(plotInclusionProbabilities)) {
  ...
  results[["regressionTable"]] <- .fillRegTable.basReg(
      data = regTableData, status = status, perform = perform,
      options = options)
  ...
  if (options$plotLogPosteriorOdds ||
      options$plotCoefficientsPosterior ||
    ...
    results[["inferentialPlots"]] <- list(
        title = "Inferential Plots", PosteriorPlotModels = plotPosteriorLogOdds,
        ...
        coefficentsPlots = list(
            title = "Coefficent plots", collection = plotCoefficientsPosterior))
  ...
  if (perform == "init") {
    ...
    keep <- c(plotPosteriorLogOdds$data,
              sapply(plotCoefficientsPosterior, function(x)
                                                  x$PosteriorCoefficients$data),
              plotResidualsVsFitted$data, plotModelProbabilities$data,
              plotModelComplexity$data, plotInclusionProbabilities$data)

    state <- list(
        options = options, bas_obj = bas_obj, descriptives = descriptives,
  ...
  return(list(keep = keep, results = results, status = statusAnalysis,
              state = state))
}
...
.createMeta.basReg <- function() {
  ...
  meta[[3]] <- list(
      ...
      meta = list(
          ...
          list(name = "coefficentsPlots", type = "collection",
               meta = list(name = "plotGroups", type = "object",
                           meta = list(list(name = "PosteriorCoefficients",
                                            type = "image"))))))
...
.readData.basReg <- function(perform, options) {
  ...
  if (perform == "run") {
    dataset <- .readDataSetToEnd(
        columns.as.numeric = vars, exclude.na.listwise = vars)
  } else {
...
.setStatus.basReg <- function(dataset, perform, options) {
  ...
  customChecks <- list(
      function() {
        nuisanceTerms <- sapply(options$modelTerms, function(term)
                                                      term$isNuisance)
      ...
      function() {
        maxModelComponents <- max(
            sapply(options$modelTerms, function(term) length(term$components)))
  ...
  .hasErrors(
      ...
      infinity.target = c(
          options$covariates, options$dependent, options$wlsWeight),
...
.calcModel.basReg <- function(dataset, status, options) {
  ...
  if (grepl("MCMC", samplingMethod, fixed = TRUE)) {
    ...
    if (is.null(n.models)) {
      ...
      MCMC.iterations <- options$numberOfModels * 10  # FIXME: should we allow
                                                      # models to influence MCMC
                                                      # samples?
    }
  }

  # hyper parameter for g-prior
  alpha <- switch(options$priorRegressionCoefficients, g_prior =, hyper_g =,
                  hyper_g_laplace =, hyper_g_n =, zs_null =,
                  zs_full = options$gPriorParameter, NULL)

  # Bayesian Adaptive Sampling
  bas_lm <- try(
      BAS::bas.lm(formula = formula, data = dataset,
                  prior = options$priorRegressionCoefficients, alpha = alpha,
                  modelprior = modelPrior, n.models = n.models,
                  method = samplingMethod, MCMC.iterations = MCMC.iterations,
                  initprobs = initProbs, weights = wlsWeights))
  ...
  if (isTryError(bas_lm)) {
    ...
    bas_lm[["priorprobs"]] <- bas_lm[["priorprobs"]] /
        sum(bas_lm[["priorprobs"]])
  }
...
.calcDataRegTable.basReg <- function(bas_obj, status, options) {
  ...
  if (options$bayesFactorOrder == "nullModelTop") {
    ...
    if (1 %in% bestModelIndices) {  # null model has index 1
      ...
      models.ordered <- c(1, bestModelIndices[-index])  # change position of
                                                        # null model
    } else {
  ...
  if (sum(nuisanceTerms) > 0) {
    null.model <- paste(
        "Null model (incl. ", paste(.unvf(names(which(nuisanceTerms))),
                                    collapse = ", "), ")", sep = "")
  }
  ...
  if (options$bayesFactorType == "BF10") {
    ...
    models.bf <- bas_obj$logmarg[
                             models.ordered] - bas_obj$logmarg[
                                                           models.ordered][1]
  }
  ...
  for (i in 1:nRows) {
    output.rows[[i]] <- list(
        "Models" = model.names[i], "BF" = models.bf[i],
  ...
  if (sum(nuisanceTerms) > 0) {
    ...
    footnote <- paste(
        "All models include ", paste(.unvf(names(which(nuisanceTerms))),
                                     collapse = ", "), ".", sep = "")
...
.fillRegTable.basReg <- function(data = NULL, status, perform, options) {
  ...
  if (options$logmarg) {
    fields[[length(fields) + 1]] <- list(
        name = "logmarg", type = "number", format = "sf:4;dp:3",
...
.descriptivesTable.basReg <- function(dataset, status, perform, options) {
  ...
  if (status$ready) {
    ...
    for (variable in variables) {
      if (perform == "run") {
        ...
        rows[[length(rows) + 1]] <- list(
            v = variable, N = n, mean = mean, sd = sd)

      } else {

        rows[[length(rows) + 1]] <- list(
            v = variable, N = ".", mean = ".", sd = ".")
    ...
    names <- sapply(fields, function(x) x$name)
    descriptives[[
        "data"]][[1]] <- setNames(as.list(rep(".", length(names))), names)
  }
...
.plotLogOdds.basReg <- function(bas_obj, status, perform) {
  ...
  emptyPlot <- .makeEmptyPlot.basReg(
      title = "Posterior Log Odds", status = status)
...
.plotPosterior.basReg <- function(bas_obj, status, perform) {
  ...
  # if there were errors or we're in init, show just empty x and y-axes
  emptyPlot <- .makeEmptyPlot.basReg(
      title = "Posterior distribution", status = status)
    ...
    for (i in 1:number.parameters) {
      ...
      p <- try(
          ...
          expr = {
            ...
            plotFunc <- function() {
              BAS:::plot.coef.bas(BAS:::coef.bas(bas_obj), subset = list(i),
                                  ask = FALSE, bty = "n")
            }
            content <- .writeImage(width = 530, height = 400, plot = plotFunc)
      ...
      plots[[length(plots) + 1]] <- list(
          title = term, name = term, PosteriorCoefficients = plot)
    }

  } else {  # init or errors

    plots <- list(
        list(title = "", name = "", PosteriorCoefficients = emptyPlot))
...
.plotDiagnostics.basReg <- function(bas_obj, status, perform, which) {
  ...
  titles <- c("Residuals vs Fitted", "Model Probabilities", "Model Complexity",
  ...
  if (status$ready && perform == "run") {
    ...
    p <- try(
        ...
        expr = {
          ...
          plotFunc <- function() {
            BAS:::plot.bas(
                bas_obj, ask = FALSE, which = c(which), caption = "", bty = "n")
          }
          content <- .writeImage(width = 530, height = 400, plot = plotFunc)
...
.makeEmptyPlot.basReg <- function(
    xlab = NULL, ylab = NULL, title = NULL, status) {
