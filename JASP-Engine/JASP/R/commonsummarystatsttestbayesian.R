...
.calculateBF.summarystats.ttest <- function(
    options, state, diff, hypothesis.variables) {
      ...
      ((is.logical(diff) && diff == FALSE) ||
       (is.list(diff) &&
        (diff$priorWidth == FALSE &&
         ...
         diff$n1Size == FALSE &&
         (is.null(diff$n2Size) || diff$n2Size == FALSE) &&
...
.pValueFromCor <- function(corrie, n, method = "pearson") {
  ...
  if (method == "pearson") {
    ...
    if (n > 2 && n < 50) {
      ...
      # tau neq 0
      result$twoSided <- 1 - SuppDists::pKendall(q = abs(corrie), N = n) +
          SuppDists::pKendall(q = -abs(corrie), N = n)
      # tau < 0
      result$minSided <- SuppDists::pKendall(q = corrie, N = n)
      # tau > 0
      result$plusSided <- SuppDists::pKendall(
          q = corrie, N = n, lower.tail = FALSE)
    } else if (n >= 50) {
...
.isInputValid.summarystats.ttest <- function(options, independent) {
  ...
  row <- list(BF = ".", tStatistic = tStatValue, n1Size = n1Value,
              errorEstimate = ".", pValue = ".")
...
.getPriorAndPosteriorPlot.summarystats.ttest <- function(
    run, options, state, diff, bayesFactorObject, oneSided, paired) {
    ...
    p <- try(
        ...
        expr = {
          ...
          .plotFunc <- function() {
            .plotPosterior.summarystats.ttest(
                t = options$tStatistic, n1 = options$n1Size, n2 = n2Value,
                paired = paired, BFH1H0 = BFH1H0, dontPlotData = dontPlotData,
                rscale = options$priorWidth,
                addInformation = options$plotPriorAndPosteriorAdditionalInfo,
                BF = ifelse(
                    BFH1H0, bayesFactorObject$bf, 1 / bayesFactorObject$bf),
...
.getBayesFactorRobustnessPlot.summarystats.ttest <- function(
    run, options, state, diff, bayesFactorObject, oneSided) {
  ...
  if (!(is.null(state)) && BFtypeRequiresNewPlot) {
    ...
    if (bftype.current %in% equivalent.bf &&
        bftype.previous %in% equivalent.bf) {
      BFtypeRequiresNewPlot <- FALSE
    }
  }
  ...
  if (!is.null(state) &&
      ...
      ((is.logical(diff) && diff == FALSE) ||
       (is.list(diff) &&
        (diff$tStatistic == FALSE &&
         ...
         (is.null(diff$n2Size) || diff$n2Size == FALSE) &&
         diff$priorWidth == FALSE &&
         diff$effectSizeStandardized == FALSE && diff$hypothesis == FALSE))) &&
      plotType %in% state$plotTypes) {
    ...
    if (run) {
      ...
      if (!is.null(bayesFactorObject)) {
        BF10post <- ifelse(BFH1H0, .clean(bayesFactorObject$bf),
...
.plotBFrobustness.summarystats.ttest <- function(
    t = NULL, n1 = NULL, n2 = NULL, BF10post, callback = function(...) 0,
    ...
    dontPlotData = FALSE, addInformation = TRUE) {
  ...
  for (i in seq_along(rValues)) {
    ...
    BF <- BayesFactor::ttest.tstat(
        t = t, n1 = n1, n2 = n2, nullInterval = nullInterval,
        rscale = rValues[i])
  ...
  # BF10 "medium" prior
  BF10m <- BayesFactor::ttest.tstat(
      t = t, n1 = n1, n2 = n2, nullInterval = nullInterval, rscale = "medium")
  BF10m <- .clean(exp(BF10m$bf))
  BF10mText <- BF10m

  # BF10 "wide" prior
  BF10w <- BayesFactor::ttest.tstat(
      t = t, n1 = n1, n2 = n2, nullInterval = nullInterval, rscale = "wide")
  BF10w <- .clean(exp(BF10w$bf))
  BF10wText <- BF10w

  # BF10 "ultrawide" prior
  BF10ultra <- BayesFactor::ttest.tstat(
      t = t, n1 = n1, n2 = n2, nullInterval = nullInterval,
      rscale = "ultrawide")
...
.plotPosterior.summarystats.ttest <- function(
    t = NULL, n1 = NULL, n2 = NULL, paired = FALSE, oneSided = FALSE, BF,
    BFH1H0, callback = function(...) 0, iterations = 10000, rscale = "medium",
    lwd = 2, cexPoints = 1.5, cexAxis = 1.2, cexYlab = 1.5, cexXlab = 1.5,
    cexTextBF = 1.4, cexCI = 1.1, cexLegend = 1.2, lwdAxis = 1.2,
    addInformation = TRUE, dontPlotData = FALSE, options = NULL) {
  ...
  BF <- as.numeric(BF)  # TO FIX: This needs to be done to make sure the BF
                        # shows up in the table during bayesian t-tests.
  ...
  if ("effectSizeStandardized" %in% names(options) &&
      options$effectSizeStandardized == "informative") {
    # informative prior
    xlim <- vector("numeric", 2)
    if (options[["informativeStandardizedEffectSize"]] == "cauchy") {
      ci99PlusMedian <- .ciPlusMedian_t(
          t = t, ny = n1, nx = n2, independentSamples = !paired && !is.null(n2),
      ...
      priorLower <- .qShiftedT(
          .15, parameters = c(options[["informativeCauchyLocation"]],
                              options[["informativeCauchyScale"]], 1),
          oneSided = oneSided)
      priorUpper <- .qShiftedT(
          .85, parameters = c(options[["informativeCauchyLocation"]],
                              options[["informativeCauchyScale"]], 1),
          oneSided = oneSided)
      # compute 95% credible interval & median:
      ci95PlusMedian <- .ciPlusMedian_t(
          t = t, ny = n1, nx = n2, independentSamples = !paired && !is.null(n2),
          prior.location = options[["informativeCauchyLocation"]],
          prior.scale = options[["informativeCauchyScale"]], prior.df = 1,
      ...
      ci99PlusMedian <- .ciPlusMedian_t(
          ...
          prior.df = options[["informativeTDf"]], ci = .99, oneSided = oneSided)
      priorLower <- .qShiftedT(
          .15,
          parameters = c(
              options[["informativeTLocation"]], options[["informativeTScale"]],
              options[["informativeTDf"]]), oneSided = oneSided)
      priorUpper <- .qShiftedT(
          .85,
          parameters = c(
              options[["informativeTLocation"]], options[["informativeTScale"]],
      ...
      ci99PlusMedian <- .ciPlusMedian_normal(
          t = t, ny = n1, nx = n2, independentSamples = !paired && !is.null(n2),
          prior.mean = options[["informativeNormalMean"]],
          prior.variance = options[["informativeNormalStd"]] ^ 2, ci = .99,
          oneSided = oneSided)

      priorAreaSmaller0 <- pnorm(0, options[["informativeNormalMean"]],
      ...
      priorLower <- qnorm(lowerp, options[["informativeNormalMean"]],
                          options[["informativeNormalStd"]])
      priorUpper <- qnorm(upperp, options[["informativeNormalMean"]],
                          options[["informativeNormalStd"]])

      # compute 95% credible interval & median:
      ci95PlusMedian <- .ciPlusMedian_normal(
          t = t, ny = n1, nx = n2, independentSamples = !paired && !is.null(n2),
          prior.mean = options[["informativeNormalMean"]],
          prior.variance = options[["informativeNormalStd"]] ^ 2, ci = .95,
          oneSided = oneSided)
      CIlow <- ci95PlusMedian[["ciLower"]]
      CIhigh <- ci95PlusMedian[["ciUpper"]]
      medianPosterior <- ci95PlusMedian[["median"]]
    ...
    ylim[1] <- 0
    dmax1 <- optimize(function(x)
                        .dposterior_informative(
                            x, t = t, n1 = n1, n2 = n2, paired = paired,
                            oneSided = oneSided, options = options),
                      interval = range(xticks), maximum = TRUE)$objective
    dmax2 <- optimize(function(x) .dprior_informative(x, oneSided = oneSided,
                                                      options = options),
                      interval = range(xticks), maximum = TRUE)$objective
    ...
    priorLine <- .dprior_informative(
        xxx, oneSided = oneSided, options = options)
    posteriorLine <- .dposterior_informative(
        xxx, t = t, n1 = n1, n2 = n2, paired = paired, oneSided = oneSided,
        options = options)
    ...
    library(BayesFactor)
    samples <- BayesFactor::posterior(
        model = bfObject, iterations = iterations, index = 1)
    delta <- samples[, "delta"]
    ...
    priorLine <- .dprior(seq(min(xticks), max(xticks), length.out = 1000),
  ...
  lines(seq(min(xticks), max(xticks), length.out = 1000), posteriorLine,
        lwd = lwd)
  lines(seq(min(xticks), max(xticks), length.out = 1000), priorLine, lwd = lwd,
        lty = 3)
  ...
  if ("effectSizeStandardized" %in% names(options) &&
      options$effectSizeStandardized == "informative") {
    points(0, .dprior_informative(0, oneSided = oneSided, options = options),
           col = "black", pch = 21, bg = "grey", cex = cexPoints)
    heightPosteriorAtZero <- .dposterior_informative(
        0, t = t, n1 = n1, n2 = n2, paired = paired, oneSided = oneSided,
        options = options)
  } else {
  ...
  if ("effectSizeStandardized" %in% names(options) &&
    ...
    if (options[["informativeStandardizedEffectSize"]] == "cauchy") {
      mostPosterior <- 1 -
          .cdf_t(
              midPoint, t = t, ny = n1, nx = n2,
              prior.location = options[["informativeCauchyLocation"]],
              prior.scale = options[["informativeCauchyScale"]], prior.df = 1)
    } else if (options[["informativeStandardizedEffectSize"]] == "t") {
      mostPosterior <- 1 -
          .cdf_t(midPoint, t = t, ny = n1, nx = n2,
                 prior.location = options[["informativeTLocation"]],
                 prior.scale = options[["informativeTScale"]],
                 prior.df = options[["informativeTDf"]])
    } else if (options[["informativeStandardizedEffectSize"]] == "normal") {
      mostPosterior <- 1 -
          .cdf_normal(midPoint, t = t, ny = n1, nx = n2,
                      prior.mean = options[["informativeNormalMean"]],
