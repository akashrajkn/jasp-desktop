...
BinomialTestBayesian <- function(
  ...
  if (is.null(dataset)) {
    ...
    if (perform == "run") {
      ...
      dataset <- .readDataSetToEnd(
          columns.as.numeric = NULL, columns.as.factor = variables,
    ...
    dataset <- .vdf(
        dataset, columns.as.numeric = NULL, columns.as.factor = variables)
  }
  ...
  if (options$hypothesis == "notEqualToTestValue") {
    ...
    hyp <- "two.sided"
    message <- paste0(
        "Proportions tested against value: ", options$testValue, ".")
    ...
    hyp <- "greater"
    note <-
        "For all tests, the alternative hypothesis specifies that the proportion
					is greater than "
  ...
  schema <- list(
      fields = list(
          ...
          list(name = "proportion", title = "Proportion", type = "number",
               format = "sf:4;dp:3"),
          list(name = "BF", title = "BF", type = "number", format = "sf:4;dp:3",
               title = bf.title)))
  ...
  if (!is.null(state)) diff <- .diff(options, state$options)
  ...
  if (options$testValue == 1 && hyp == "greater") {
    ...
    errorMessageTable <-
        "Cannot test the hypothesis that the test value is greater than 1."

  } else if (options$testValue == 0 && hyp == "less") {

    errorMessageTable <-
        "Cannot test the hypothesis that the test value is less than 0."
  }
  ...
  if (perform == "run" && !is.null(variables)) {
    ...
    for (var in variables) {
      ...
      for (lev in levels) {
        ...
        if (!is.null(state) &&
            rowIdentifier[r] %in% state$rowIdentifier &&
            !diff$testValue &&
            !diff$hypothesis &&
            !diff$priorA && !diff$priorB && !diff$bayesFactorType) {
          ...
          BF10 <- .bayesBinomialTest(
              counts, n, options$testValue, hypothesis = hyp, a = a, b = b)
          rowsBF10[r] <- BF10
          ...
          row <- list(
              case = var, level = lev, counts = .clean(counts),
              total = .clean(n), proportion = .clean(prop), BF = .clean(BF))
        ...
        if (options$plotPriorAndPosterior && is.null(errorMessageTable)) {
          ...
          plotType <- ifelse(
              ...
              "PriorPosteriorAddInfoPlot:", "PriorPosteriorPlot:")
          plotIdentifier[j] <- paste0(plotType, plotGroups[[i]][["name"]])


          if (!is.null(state) &&
              plotIdentifier[j] %in% state$plotIdentifier &&
              !diff$testValue &&
              !diff$hypothesis && !diff$priorA && !diff$priorB) {
                ...
                expr = {
                  ...
                  func <- function() {
                    .plotPosterior.binomTest(
                        counts, n, options$testValue, a = a, b = b, BF10,
                        hypothesis = hyp,
        ...
        if (options$plotSequentialAnalysis && is.null(errorMessageTable)) {
          ...
          plotIdentifier[
              j] <- paste("SequentialAnalysisPlot:", plotGroups[[i]][["name"]])

          if (!is.null(state) &&
              plotIdentifier[j] %in% state$plotIdentifier &&
              !diff$testValue &&
              !diff$hypothesis &&
              !diff$bayesFactorType && !diff$priorA && !diff$priorB) {
            ...
            p <- try(
                ...
                expr = {
                  ...
                  .plotFunc <- function() {
                    .plotSequentialBF.binomTest(
                        d, lev, options$testValue, a = a, b = b,
                        BF10table = BF10, hypothesis = hyp, BFH1H0 = BFH1H0)
                  }
                  content <- .writeImage(
    ...
    if (is.null(variables)) {
      ...
      data[[length(data) + 1]] <- list(case = ".", level = ".", counts = ".",
                                       total = ".", proportion = ".", p = ".")
      variables <- ""
    }
    ...
    for (var in variables) {
      ...
      for (lev in levels) {
        ...
        rowIdentifier[
            length(rowIdentifier) + 1] <- paste(c(var, lev), collapse = " - ")

        if (!is.null(state) &&
            rowIdentifier[length(rowIdentifier)] %in% state$rowIdentifier &&
            !diff$testValue &&
            !diff$hypothesis &&
            !diff$priorA && !diff$priorB && !diff$bayesFactorType) {

          index <- which(state$rowIdentifier ==
                         rowIdentifier[length(rowIdentifier)])
          data[[length(data) + 1]] <- state$rowsBinomtest[[index]]
          rowsBinomtest[[
              length(rowsBinomtest) + 1]] <- state$rowsBinomtest[[index]]

        } else {

          data[[length(data) + 1]] <- list(
              case = var, level = lev, counts = ".", total = ".",
              proportion = ".", p = ".")
          rowsBinomtest[[length(rowsBinomtest) + 1]] <- list(
              case = var, level = lev, counts = ".", total = ".",
              proportion = ".", p = ".")
        }
        ...
        if (options$plotPriorAndPosterior || options$plotSequentialAnalysis) {
          ...
          plotGroups[[
              length(plotGroups)]][["title"]] <- paste(c(var, lev), collapse = " - ")
          plotGroups[[length(plotGroups)]][[
              "name"]] <- plotGroups[[length(plotGroups)]][["title"]]
        ...
        if (options$plotPriorAndPosterior && is.null(errorMessageTable)) {
          ...
          plotType <- ifelse(
              options$plotPriorAndPosteriorAdditionalInfo,
              "PriorPosteriorAddInfoPlot:", "PriorPosteriorPlot:")
          plotIdentifier[length(plotIdentifier) + 1] <- paste0(
              plotType, plotGroups[[length(plotGroups)]][["name"]])

          if (!is.null(state) &&
              plotIdentifier[length(plotIdentifier)] %in%
              state$plotIdentifier &&
              !diff$testValue &&
              !diff$hypothesis && !diff$priorA && !diff$priorB) {

            index <- which(state$plotIdentifier ==
                           plotIdentifier[length(plotIdentifier)])
            plot <- state$plotsBinomtest[[index]]
        ...
        if (options$plotSequentialAnalysis && is.null(errorMessageTable)) {
          ...
          plotIdentifier[length(plotIdentifier) + 1] <- paste(
              "SequentialAnalysisPlot:",
              plotGroups[[length(plotGroups)]][["name"]])
          ...
          if (!is.null(state) &&
            ...
            .plotFunc <- function() {
              .plotSequentialBF.binomTest(
                  dontPlotData = TRUE, hypothesis = hyp, BFH1H0 = BFH1H0)
            }
            content <- .writeImage(
                width = 530, height = 400, plot = .plotFunc, obj = TRUE)
  ...
  if (!is.null(errorMessageTable))
    table[["error"]] <- list(
        errorType = "badData", errorMessage = errorMessageTable)
  ...
  if (options$plotPriorAndPosterior || options$plotSequentialAnalysis)
    results[["plots"]] <- list(
        title = ifelse(length(plotGroups) == 1 && length(plotGroups[[1]]) == 3,
                       "Plot", "Plots"), collection = plotGroups)

  keep <- NULL

  for (plot in plotsBinomtest) keep <- c(keep, plot$data)
    ...
    return(
        list(results = results, status = "complete",
             state = list(
                 ...
                 plotsBinomtest = plotsBinomtest,
                 plotIdentifier = plotIdentifier, rowsBinomtest = rowsBinomtest,
                 rowIdentifier = rowIdentifier, rowsBF10 = rowsBF10),
             keep = keep))
  }
}
...
.bayesBinomialTest.twoSided <- function(counts, n, theta0, a, b) {
  ...
  if (theta0 == 0 && counts == 0) {
    ...
    logBF10 <- lbeta(counts + a, n - counts + b) -
        lbeta(a, b) - (n - counts) * log(1 - theta0)
    ...
    logBF10 <- lbeta(counts + a, n - counts + b) -
        lbeta(a, b) - counts * log(theta0)

  } else {

    logBF10 <- lbeta(counts + a, n - counts + b) -
        lbeta(a, b) - counts * log(theta0) - (n - counts) * log(1 - theta0)
  }
...
.bayesBinomialTest.oneSided <- function(counts, n, theta0, a, b, hypothesis) {
  ...
  term1 <- pbeta(theta0, a + counts, b + n - counts, lower.tail = lowerTail,
                 log.p = TRUE) + lbeta(a + counts, b + n - counts)
  term2 <- lbeta(a, b) +
      pbeta(theta0, a, b, lower.tail = lowerTail, log.p = TRUE)
...
.bayesBinomialTest <- function(counts, n, theta0, hypothesis, a, b) {
  ...
  if (hypothesis == "two.sided") {
    ...
    BF10 <- try(
        .bayesBinomialTest.twoSided(counts, n, theta0, a, b), silent = TRUE)
    ...
    BF10 <- try(.bayesBinomialTest.oneSided(
                    counts, n, theta0, a, b, hypothesis), silent = TRUE)

    #}
  }

  if (class(BF10) == "try-error") BF10 <- NA

  return(BF10)

}

.dpriorTheta <- function(
    x, a = 1, b = 1, hypothesis = "two.sided", theta0 = .5) {
  ...
  if (hypothesis == "two.sided") {
    ...
    ifelse(x >= theta0, dbeta(x, a, b) /
                        pbeta(theta0, a, b, lower.tail = FALSE), 0)

  } else if (hypothesis == "less") {

    ifelse(x <= theta0, dbeta(x, a, b) / pbeta(theta0, a, b), 0)

  }

}

.dposteriorTheta <- function(x, a = 1, b = 1, counts = 10, n = 20,
                             hypothesis = "two.sided", theta0 = .5) {
  ...
  if (hypothesis == "two.sided") {
    ...
    ifelse(x >= theta0,
           dbeta(x, a + counts, b + n - counts) /
           pbeta(theta0, a + counts, b + n - counts, lower.tail = FALSE), 0)

  } else if (hypothesis == "less") {

    ifelse(x <= theta0, dbeta(x, a + counts, b + n - counts) /
                        pbeta(theta0, a + counts, b + n - counts), 0)

  }

}

.credibleIntervalPlusMedian <- function(
    credibleIntervalInterval = .95, a = 1, b = 1, counts = 10, n = 20,
    hypothesis = "two.sided", theta0 = .5) {
  ...
  if (hypothesis == "two.sided") {
    ...
    quantiles <- qbeta(
        leftArea + rightArea * c(lower, .5, upper), a + counts, b + n - counts)

  } else if (hypothesis == "less") {

    leftArea <- pbeta(theta0, a + counts, b + n - counts)
    quantiles <- qbeta(
        leftArea * c(lower, .5, upper), a + counts, b + n - counts)

  }

  return(list(ci.lower = quantiles[1], ci.median = quantiles[2],
              ci.upper = quantiles[3]))

}

.plotPosterior.binomTest <- function(
    counts, n, theta0, a = 1, b = 1, BF10, hypothesis = "two.sided",
    addInformation = TRUE, dontPlotData = FALSE, lwd = 2, cexPoints = 1.5,
    cexAxis = 1.2, cexYlab = 1.5, cexXlab = 1.5, cexTextBF = 1.4, cexCI = 1.1,
    cexLegend = 1.2, lwdAxis = 1.2, credibleIntervalInterval = .95) {
  ...
  if (dontPlotData) {
    ...
    mtext(expression("Population proportion" ~ theta), side = 1, cex = cexXlab,
          line = 2.6)

    return()
  }

  if (is.infinite(BF10)) stop("Bayes factor is infinite")

  if (is.infinite(1 / BF10)) stop("Bayes factor is too small")
  ...
  quantiles <- try(
      .credibleIntervalPlusMedian(
          credibleIntervalInterval = credibleIntervalInterval, a = a, b = b,
          counts = counts, n = n, hypothesis, theta0 = theta0), silent = TRUE)
  ...
  dmax <- max(c(posteriorLine[is.finite(posteriorLine)],
                priorLine[is.finite(priorLine)]))
  ...
  plot(1, xlim = xlim, ylim = range(yticks), ylab = "", xlab = "", type = "n",
       axes = FALSE)
  ...
  if (theta0 == 0) {
    ...
    points(theta[1], heightPriorTheta0, col = "black", pch = 21, bg = "grey",
           cex = cexPoints)
    points(theta[1], heightPosteriorTheta0, col = "black", pch = 21,
    ...
    points(theta[length(theta)], heightPriorTheta0, col = "black", pch = 21,
           bg = "grey", cex = cexPoints)
    points(theta[length(theta)], heightPosteriorTheta0, col = "black", pch = 21,
           bg = "grey", cex = cexPoints)

  } else {

    heightPriorTheta0 <- .dpriorTheta(theta0, a, b, hypothesis, theta0)
    heightPosteriorTheta0 <- .dposteriorTheta(
        theta0, a, b, counts, n, hypothesis, theta0)

    points(theta0, heightPriorTheta0, col = "black", pch = 21, bg = "grey",
           cex = cexPoints)
    points(theta0, heightPosteriorTheta0, col = "black", pch = 21, bg = "grey",
           cex = cexPoints)
  }
...
.plotSequentialBF.binomTest <- function(
    d, level, theta0 = .5, a = 1, b = 1, BF10table, hypothesis = "two.sided",
    callback = function(...) 0, lwd = 2, cexPoints = 1.4, cexAxis = 1.2,
    cexYlab = 1.5, cexXlab = 1.6, cexTextBF = 1.4, cexText = 1.2,
    cexLegend = 1.2, cexEvidence = 1.6, lwdAxis = 1.2,
    plotDifferentPriors = FALSE, dontPlotData = FALSE, BFH1H0 = TRUE) {
  ...
  if (is.infinite(BF10table)) stop("Bayes factor is infinite")

  if (is.infinite(1 / BF10table)) stop("Bayes factor is too small")
  ...
  if (is.infinite(yhigh)) yhigh <- 1e+308
  ...
  if (evidenceText) {
    ...
    if (BF10e < 1) text(xxT, yyT, paste("Evidence for H0:\n", lab), cex = 1.4,
                        pos = 2, offset = -.2)
  }
}
