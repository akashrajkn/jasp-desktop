...
.jaspGlm <- function(dataset, options, perform, type) {
  ...
  if (perform == "run" && options[["dependent"]] != "") {
    if (!is.null(type) && type == "binomial") {
      ...
      glmRes <- .glmStep(
          nullMod, fullMod, dataset, method = options[["method"]], test = "LR",
          pentry = options[["entryPval"]], premove = options[["removalPval"]],
          maxk = maxStep)
  ...
  if (method == "enter") {
    ...
    while (!converged) {
      ...
      a <- try(add1(currentModel, full, test = test,
                    x = model.matrix(fullModel)), silent = TRUE)
...
.glmModelSummary <- function(glmObj, options, perform, type) {
  ...
  if (type == "binomial") {
    ...
    fields <- list(
        ...
        list(name = "dev", title = "Deviance", type = "number",
             format = "sf:4;dp:3"),
        ...
        list(name = "chi", title = "\u03A7\u00B2", type = "number",
             format = "dp:3"), list(name = "pvl", title = "p", type = "number",
                                    format = "dp:3;p:.001"),
        list(name = "fad", title = "McFadden R²", type = "number",
             format = "sf:4;dp:3"), list(name = "nag", title = "Nagelkerke R²",
                                         type = "number", format = "sf:4;dp:3"),
        list(name = "tju", title = "Tjur R²", type = "number",
             format = "sf:4;dp:3"))
    ...
    if (perform == "run" && !is.null(glmObj)) {
      ...
      if (options[["method"]] == "enter") {
        ...
        rows <- list(
            list(mod = "H\u2080", dev = .clean(glmObj[[1]][["deviance"]]),
                 aic = .clean(glmObj[[1]][["aic"]]),
                 bic = .clean(.bic(glmObj[[1]])),
                 dof = .clean(glmObj[[1]][["df.residual"]]), chi = .clean(NULL),
                 pvl = .clean(NULL), fad = .clean(NULL), nag = .clean(NULL),
                 tju = .clean(NULL)),
            list(mod = "H\u2081", dev = .clean(glmObj[[2]][["deviance"]]),
                 aic = .clean(glmObj[[2]][["aic"]]),
                 bic = .clean(.bic(glmObj[[2]])),
                 dof = .clean(glmObj[[2]][["df.residual"]]),
                 chi = .clean(lr[["stat"]]), pvl = .clean(lr[["pval"]]),
        ...
        for (midx in 1:length(glmObj)) {
          ...
          if (midx > 1) {
            ...
            rows[[midx]] <- list(
                mod = as.character(midx), dev = .clean(mObj[["deviance"]]),
                aic = .clean(mObj[["aic"]]), bic = .clean(.bic(mObj)),
                dof = .clean(mObj[["df.residual"]]), chi = .clean(lr[["stat"]]),
                pvl = .clean(lr[["pval"]]), fad = .clean(fadden),
                nag = .clean(nagel), tju = .clean(.tjur(mObj)))
          } else {
            rows[[midx]] <- list(
                mod = as.character(midx), dev = .clean(mObj[["deviance"]]),
                aic = .clean(mObj[["aic"]]), bic = .clean(.bic(mObj)),
                dof = .clean(mObj[["df.residual"]]), chi = .clean(NULL),
                pvl = .clean(NULL), fad = .clean(.mcFadden(mObj, mObj)),
      ...
      rows <- list(list(mod = "H\u2080", dev = ".", fad = .clean(NULL),
                        nag = .clean(NULL), tju = .clean(NULL), aic = "."),
                   list(mod = "H\u2081", dev = ".", fad = ".", nag = ".",
                        tju = ".", aic = "."))
    }
...
.glmEstimatesTable <- function(glmObj, options, perform, type) {
  ...
  if (type == "binomial" && options[["coeffEstimates"]]) {
    ...
    if (options[["coeffCIOR"]]) {
      ciTitle <- paste0(options[["coeffCIInterval"]],
                        "% Confidence interval <br> (odds ratio scale)")
    } else {
    ...
    fields <- list(
        ...
        list(
            name = "est", title = "Estimate", type = "number", format = "dp:3"),
        list(name = "se", title = seTitle, type = "number", format = "dp:3"),
        list(name = "std", title = "Standardized\u207A", type = "number",
             format = "dp:3"), list(name = "or", title = "Odds Ratio",
                                    type = "number", format = "sf:4;dp:3"),
        list(name = "zval", title = "z", type = "number", format = "sf:4;dp:3"),
        list(name = "pval", title = "p", type = "number",
             format = "dp:3;p:.001"),
        list(name = "vsmpr", title = "VS-MPR\u002A", type = "number",
             format = "sf:4;dp:3"),
        list(name = "cilo", title = "Lower bound", type = "number",
             format = "dp:3", overTitle = ciTitle),
        list(name = "ciup", title = "Upper bound", type = "number",
             format = "dp:3", overTitle = ciTitle))


    # then determine which ones we need
    selectFields <- with(
        options, c(multimod, TRUE, TRUE, TRUE, stdCoeff, oddsRatios, TRUE, TRUE,
                   VovkSellkeMPR, coeffCI, coeffCI))
    ...
    if (perform == "run" && !is.null(glmObj)) {
      ...
      if (!multimod) {
        ...
        if (length(rn) == 1) {
          ...
          rows[[1]] <- list(
              param = .clean(.formatTerm(rn, glmObj[[2]])), est = .clean(s[1]),
              se = .clean(s[2]), std = .clean(as.numeric(beta)),
              or = .clean(exp(s[1])), zval = .clean(s[3]), pval = .clean(s[4]),
          ...
          for (i in seq_along(rn)) {
            ...
            rows[[i]] <- list(
                param = .clean(.formatTerm(rn[i], glmObj[[2]])),
                est = .clean(s[i, 1]), se = .clean(s[i, 2]),
                std = .clean(as.numeric(beta[i])), or = .clean(exp(s[i, 1])),
                zval = .clean(s[i, 3]), pval = .clean(s[i, 4]),
        ...
        for (midx in 1:length(glmObj)) {
          ...
          if (length(rn) == 1) {
            ...
            rows[[length(rows) + 1]] <- list(
                ...
                param = .clean(.formatTerm(rn, mObj)), est = .clean(s[1]),
                se = .clean(s[2]), std = .clean(as.numeric(beta)),
                or = .clean(exp(s[1])), zval = .clean(s[3]),
                pval = .clean(s[4]), vsmpr = .clean(.VovkSellkeMPR(s[4])),
                cilo = .clean(expon(s[1] - c * s[2])),
                ciup = .clean(expon(s[1] + c * s[2])), .isNewGroup = TRUE)
          } else {
            if (options[["robustSEOpt"]]) {
              s[, 2] <- unname(.glmRobustSE(mObj))  # new se
              s[, 3] <- s[, 1] / s[, 2]  # new z
...
.glmConfusionMatrix <- function(glmObj, options, perform, type) {
  ...
  if (type == "binomial") {
    ...
    if (perform == "run" && !is.null(glmObj)) {
      ...
      fields <- list(list(name = "obs", title = "Observed", type = "string"),
                     list(name = "pred0", title = levs[1], type = typ,
                          format = frm, overTitle = "Predicted"),
                     list(name = "pred1", title = levs[2], type = typ,
                          format = frm, overTitle = "Predicted"))
      ...
      fields <- list(list(name = "obs", title = "Observed", type = "string"),
                     list(name = "pred0", title = "0", type = "number",
                          format = "sf:4;dp:3", overTitle = "Predicted"),
                     list(name = "pred1", title = "1", type = "number",
                          format = "dp:3", overTitle = "Predicted"))
      rows <- list(list(obs = "0", pred0 = ".", pred1 = "."),
...
.glmPerformanceMetrics <- function(glmObj, options, perform, type) {
  ...
  if (type == "binomial") {
    ...
    if (perform == "run" && !is.null(glmObj) && any(scrNeed)) {
      ...
      # make fields
      fields <- list(list(name = "met", title = "", type = "string"),
                     list(name = "val", title = "Value", type = "number",
                          format = "sf:4;dp:3"))

      m <- .confusionMatrix(mObj, cutoff = 0.5)[["metrics"]]
      rows <- list(list(met = "AUC", val = m[["AUC"]]),
      ...
      rows <- list(
          list(met = "AUC", val = "."), list(met = "Sensitivity", val = "."),
    ...
    if (length(predictors) > 0 && !is.null(glmObj)) {
      ...
      if (perform == "run") {
        ...
        predictors <- predictors[
            .v(predictors) %in% attr(mObj[["terms"]], "term.labels")]
        plots <- vector("list", length(predictors))
    ...
    if (length(predictors) > 0 && !is.null(glmObj)) {
      ...
      if (perform == "run") {
        ...
        for (i in seq_along(predictors)) {
          resPlot <- list()
          gg <- .plotGlmResiduals(
              mObj, predictors[i], type = options[["residualType"]])
          plotObj <- .writeImage(
              width = options[["plotWidth"]], height = options[["plotHeight"]],
              plot = gg, obj = TRUE)
...
.glmPredictedResidualsPlot <- function(glmObj, options, perform, type) {
  ...
  if (type == "binomial") {
    if (!is.null(glmObj)) {
      resPlot <- list()
      gg <- .plotGlmResiduals(
          glmObj[[length(glmObj)]], type = options[["residualType"]])
      plotObj <- .writeImage(
          width = options[["plotWidth"]], height = options[["plotHeight"]],
          plot = gg, obj = TRUE)
      resPlot[["width"]] <- options[["plotWidth"]]
      resPlot[["height"]] <- options[["plotHeight"]]
      resPlot[["custom"]] <- list(width = "plotWidth", height = "plotHeight")
...
.bic <- function(glmModel) {
  return(log(length(glmModel[["y"]])) * length(coef(glmModel)) +
         glmModel[["deviance"]])
}
...
.plotLogCurve <- function(glmModel, var, points = TRUE) {
  ...
  if (attr(ribdat, "factor")) {
    ...
    if (points) {
      p <- p + ggplot2::geom_point(
                   data = ggdat, size = 2, position = ggplot2::position_jitter(
                                               height = 0.01, width = 0.04),
                   color = "dark grey", alpha = 0.3)
    }

    p <- p + ggplot2::geom_point(data = ribdat, mapping = ggplot2::aes(
                                                    x = x, y = y), size = 4) +
        ggplot2::geom_errorbar(
            mapping = ggplot2::aes(x = x, ymin = lo, ymax = hi), data = ribdat,
            width = 0.2)
    ...
    p <- ggplot2::ggplot(ribdat, ggplot2::aes(x = x, y = y)) +
        ggplot2::geom_ribbon(
            data = ribdat, mapping = ggplot2::aes(x = x, ymax = hi, ymin = lo),
            fill = "light grey", alpha = 0.5) +
        ggplot2::geom_line(data = ribdat, mapping = ggplot2::aes(x = x, y = y),
                           size = .75, color = "black")
  ...
  p + ggplot2::xlab(var) + ggplot2::ylab(ytitle) + ggplot2::theme_bw() +
      ...
      ggplot2::theme(
          ...
          axis.text.x = ggplot2::element_text(
              size = 15, margin = ggplot2::margin(t = 1, unit = "mm")),
          axis.text.y = ggplot2::element_text(
              size = 15, margin = ggplot2::margin(r = 1, unit = "mm")),
...
.glmLogRegRibbon <- function(logRes, var, ciInt = 0.95) {
  ...
  if (length(factors) == 0 || !(var %in% factors)) {
    ...
    xs <- factor(
        logRes[["xlevels"]][[var]], levels = logRes[["xlevels"]][[var]])
    newDat[[var]] <- xs
  }
...
.plotGlmResiduals <- function(glmModel, var = NULL, type = "deviance") {
  ...
  custom_y_axis <- function(val) {
    d <- data.frame(
        x = -Inf, xend = -Inf, y = min(pretty(val)), yend = max(pretty(val)))
    list(ggplot2::geom_segment(
             data = d, ggplot2::aes(x = x, y = y, xend = xend, yend = yend),
             inherit.aes = FALSE, size = 1),
  ...
  custom_x_axis <- function(val) {
    if (class(val) == "factor") {
      ...
      d <- data.frame(
          y = -Inf, yend = -Inf, x = min(pretty(val)), xend = max(pretty(val)))
      l <- list(
          ggplot2::geom_segment(
              data = d, ggplot2::aes(x = x, y = y, xend = xend, yend = yend),
              inherit.aes = FALSE, size = 1),
  ...
  p <- ggplot2::ggplot(data = ggdat, mapping = ggplot2::aes(x = x, y = resid)) +
      ggplot2::geom_point(
          position = pos, size = 3, colour = "black", fill = "grey", pch = 21)

  p <- p + ggplot2::xlab(var) + ggplot2::ylab("Residuals") +
      ggplot2::theme_bw() +
