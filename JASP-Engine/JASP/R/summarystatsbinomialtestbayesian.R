...
SummaryStatsBinomialTestBayesian <- function(
  ...
  hypothesis.variables <- .hypothesisType.summarystats.binomial(
      hypothesis = options$hypothesis, test.value = options$testValue)
  ...
  outputTableElements <- .getOutputRow.summarystats.binomial(
      run = run, options = options, state = state, diff = diff, hyp = hyp)
  ...
  if (options$plotPriorAndPosterior) {
    priorAndPosteriorPlot <- .getPriorAndPosteriorPlot.summarystats.binomial(
        ...
        bayesFactorObject = bayesFactorObject, hyp = hyp)
  }

  # Add footnotes to the analysis result
  footnotes <- .newFootnotes()
  .addFootnote(
      footnotes, symbol = "<em>Note.</em>", text = hypothesis.variables$message)
  ...
  fields[[length(fields) + 1]] <- list(
      name = "successes", type = "integer", title = "Successes")
  fields[[length(fields) + 1]] <- list(
      name = "failures", type = "integer", title = "Failures")
  fields[[length(fields) + 1]] <- list(
      name = "testValue", type = "number", title = "Test value")
  fields[[length(fields) + 1]] <- list(
  ...
  if (options$plotPriorAndPosterior) {
    results[["inferentialPlots"]] <- list(
        title = "Inferential Plot", PriorPosteriorPlot = priorAndPosteriorPlot)
  }
  results[["table"]] <- table
...
.getPriorAndPosteriorPlot.summarystats.binomial <- function(
    run, options, state, diff, bayesFactorObject, hyp) {
  ...
  if (!is.null(state) &&
      ...
      ((is.logical(diff) && diff == FALSE) ||
       (is.list(diff) && (diff$successes == FALSE &&
                          diff$failures == FALSE &&
                          diff$testValue == FALSE &&
                          diff$hypothesis == FALSE &&
                          diff$betaPriorParamA == FALSE &&
                          diff$betaPriorParamB == FALSE &&
        ...
        expr = {
          ...
          .plotFunc <- function() {
            .plotPosterior.binomTest(
                ...
                n = (options$failures + options$successes),
                theta0 = options$testValue, a = options$betaPriorParamA,
                b = options$betaPriorParamB, BF10 = bayesFactorObject,
                hypothesis = hyp,
                addInformation = options$plotPriorAndPosteriorAdditionalInfo,
                dontPlotData = dontPlotData)
          }
...
.getOutputRow.summarystats.binomial <- function(
    run, options, state, diff, hyp) {
      ...
      ((is.logical(diff) && diff == FALSE) ||
       (is.list(diff) &&
        (diff$bayesFactorType == FALSE &&
         diff$successes == FALSE &&
         diff$failures == FALSE &&
         diff$testValue == FALSE &&
         diff$betaPriorParamA == FALSE &&
         diff$betaPriorParamB == FALSE && diff$hypothesis == FALSE)))) {
    ...
    if (run) {
      if (status$ready) {
        bayesFactorObject <- .bayesBinomialTest(
            ...
            theta0 = options$testValue, hypothesis = hyp,
            a = options$betaPriorParamA, b = options$betaPriorParamB)
        ...
        pValue <- stats::binom.test(
                      x = c(options$successes, options$failures),
                      p = options$testValue, alternative = hyp)$p.value
        # For successes > 1, failures > 1, test_value = 1 or 0, pValue retured is FALSE
        rowsBinomialTest$pValue <- .clean(
            switch(as.character(pValue), "TRUE" = 1, "FALSE" = 0, pValue))
      }
    }
  }
...
.isInputValid.summarystats.binomial <- function(options) {
  ...
  row <- list(BF = ".", successes = successes, failures = failures,
              testValue = testValue)
