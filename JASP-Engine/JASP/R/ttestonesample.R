...
TTestOneSample <- function(
  ...
  if (!is.null(dataset)) {
    ...
    dataset <- .vdf(
        dataset, column.as.numeric = variables, exclude.na.listwise = exclude)
  ...
  if (options$descriptivesPlots) {
    ...
    plotTitle <- ifelse(length(options$variables) > 1, "Descriptives Plots",
                        "Descriptives Plot")
    descriptivesPlots <- .ttestOneSamplesDescriptivesPlot(
...
.ttestOneSample <- function(dataset, options, perform) {
  ...
  if (sum(allTests) == 2) {
    fields <- append(
        fields, list(list(name = "test", type = "string", title = "Test")), 1)
  }
  ...
  if (wantsDifference) {
    fields[[length(fields) + 1]] <- list(
        name = "m", title = nameOfLocationParameter, type = "number",
        format = "sf:4;dp:3")
  ...
  if (options$hypothesis == "greaterThanTestValue") {
    ...
    direction <- "two.sided"
    note <-
        "For all tests, the alternative hypothesis specifies that the population mean is different from "
  }
  ...
  if (options$testValue != 0) {
    ...
    if (wantsWilcox && !wantsStudents) {
      ...
      tMessage <- gsub(
          pattern = "For all tests", replacement = "For the Student t-tests",
          x = paste0(note, options$testValue))
      wilcoxMessage <- gsub(pattern = "mean", replacement = "median",
                            x = paste0(note, options$testValue))
      wilcoxMessage <- gsub(
          pattern = "For all tests", replacement = "for the Wilcoxon test",
          x = wilcoxMessage)
      ...
      if (perform == "run" && length(options$variables) > 0) {
        ...
        row <- try(
            ...
            expr = {
              ...
              if (test == 2) {
                r <- stats::wilcox.test(
                    dat, alternative = direction, mu = options$testValue,
                ...
                if (direction == "two.sided") {
                  confIntEffSize <- sort(
                      c(tanh(zmbiss +
                             qnorm((1 - percentConfidenceEffSize) / 2) * mrSE),
                        tanh(zmbiss +
                             qnorm((1 + percentConfidenceEffSize) / 2) * mrSE)))
        ...
        if (!is.null(errorMessage)) {
          ...
          row <- list(v = variable, df = "", p = "", m = "", lowerCI = "",
                      upperCI = "", .footnotes = row.footnotes)
          row[[testStat]] <- .clean(NaN)
        }

        ## if we are not yet ready to perform, just create an empty table
      } else {
        row <- list(v = variable, df = ".", p = ".", m = ".", d = ".",
                    lowerCI = ".", upperCI = ".")
        row[[testStat]] <- "."
      }
    ...
    if (perform == "run" && length(options$variables) > 0) {
      ...
      if (class(data) != "factor") {
        ...
        result <- list(v = variable, N = n, mean = mean, sd = stdDeviation,
                       se = stdErrorMean, .isNewGroup = newGroup)
      } else {
    ...
    base_breaks_y <- function(x, options) {
      ...
      list(ggplot2::geom_segment(
           ...
           ggplot2::scale_y_continuous(
               breaks = c(min(b), options$testValue, max(b))))
    }
