...
SummaryStatsTTestBayesianOneSample <- function(
  ...
  hypothesis.variables <- .hypothesisType.summarystats.ttest.one(
      options$hypothesis)
  ...
  # Define table schema
  fields <- list(list(name = "tStatistic", type = "number",
                      format = "sf:4;dp:3", title = "t"),
  ...
  if (rowsTTestBayesianOneSample$errorEstimate != "NaN") {
    fields[[length(fields) + 1]] <- list(
        name = "errorEstimate", type = "number", format = "sf:4;dp:3",
        title = "error %")
  }
  fields[[length(fields) + 1]] <- list(
  ...
  if (options$plotPriorAndPosterior) {
    priorAndPosteriorPlot <- .getPriorAndPosteriorPlot.summarystats.ttest(
        run = run, options = options, state = state, diff = diff,
        bayesFactorObject = bayesFactorObject, oneSided = oneSided,
        paired = FALSE)
    plots.sumstats.ttest[[length(plots.sumstats.ttest) + 1]] <-
...
.getOutputRow.summarystats.ttest.one <- function(
    run, options, state, diff, hypothesis.variables) {
  ...
  if (!is.null(state) && !is.null(diff) && !is.null(state$bayesFactorObject) &&
    ...
    status <- .isInputValid.summarystats.ttest(
        options = options, independent = FALSE)
    ...
    if (run) {
      if (status$ready) {
        ...
        pValue <- switch(as.character(hypothesis.variables$oneSided),
                         left = allPValues$minSided,
        ...
        rowsTTestBayesianOneSample$errorEstimate <- .clean(
            bayesFactorObject$properror)
        rowsTTestBayesianOneSample$pValue <- .clean(pValue)

      }
    }
  }
  return(list(row = rowsTTestBayesianOneSample,
...
.hypothesisType.summarystats.ttest.one <- function(hypothesis) {
  ...
  if (hypothesis == "greaterThanTestValue") {
    ...
    oneSided <- "right"
    message <- paste(
        "For all tests, the alternative hypothesis specifies that the mean is greater than 0.",
        sep = "")
    ...
    oneSided <- "left"
    message <- paste(
        "For all tests, the alternative hypothesis specifies that the mean is lesser than 0.",
        sep = "")
  }
...
.generalSummaryTtestBF <- function(
    tValue = options$tStatistic, size = options$n1Size, options,
    paired = TRUE) {
  ...
  # help vars
  n1 <- size
  n2 <- if (!is.null(options$n2Size)) options$n2Size else 0  # single sample
                                                             # case
  oneSided =
      !(options$hypothesis %in% c("notEqualToTestValue", "groupsNotEqual"))

  ### Default case: a non-informative zero-centered Cauchy prior
  if (options$effectSizeStandardized == "default") {
    nullInterval <- switch(
        options$hypothesis, greaterThanTestValue = c(0, Inf),
        groupOneGreater = c(0, Inf), lessThanTestValue = c(-Inf, 0),
        groupTwoGreater = c(-Inf, 0), c(-Inf, Inf))  # default is
                                                     # notEqualToTestValue
  ...
  if (options$effectSizeStandardized == "informative") {
    ...
    side = switch(options$hypothesis, greaterThanTestValue = "right",
                  groupOneGreater = "right", lessThanTestValue = "left",
                  groupTwoGreater = "left", FALSE)
    ...
    if (options[["informativeStandardizedEffectSize"]] == "cauchy") {
      bfObject <- .bf10_t(
          t = tValue, ny = n1, nx = n2, oneSided = side,
