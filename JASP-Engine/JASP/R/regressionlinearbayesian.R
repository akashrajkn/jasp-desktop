...
RegressionLinearBayesian <- function(
  ...
  .callbackBayesianLinearModels <- function(results = NULL, progress = NULL) {
    ...
    if (response$status == "changed") {
      ...
      if (change$modelTerms || change$dependent || change$priorCovariates)
        return(response)
      response$status <- "ok"
    }
    return(response)
  ...
  if (!is.null(state)) {
    ...
    if (!base::identical(change, FALSE) &&
        (change$dependent || change$modelTerms || change$priorCovariates)) {
      state <- NULL
    } else {
      perform <- "run"
    }
  ...
  if (is.null(state)) {
    ...
    model.object <- .theBayesianLinearModels(
        ...
        .callbackBFpackage, results = results, analysisType = "Regression")
