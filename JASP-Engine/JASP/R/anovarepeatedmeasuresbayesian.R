...
AnovaRepeatedMeasuresBayesian <- function(
  ...
  .callbackBayesianLinearModels <- function(results = NULL, progress = NULL) {
    ...
    if (response$status == "changed") {
      ...
      new.options$modelTerms[[length(new.options$modelTerms) + 1]] <- list(
      ...
      if (change$modelTerms ||
          ...
          change$covariates || change$repeatedMeasuresFactors ||
  ...
  ## DATA
  dataANDoptions <- .readBayesianRepeatedMeasuresDataOptions(
  ...
  if (perform2 != perform) {  # we changed from init to run and need the full
                              # dataset
    dataset <- .readBayesianRepeatedMeasuresDataOptions(
                   NULL, originalOptions, "run")$dataset
  }
  ...
  if (is.null(state)) {
    ...
    model.object <- .theBayesianLinearModels(
        ...
        .callbackBFpackage, results, analysisType = "RM-ANOVA")
  ...
  results[["posthoc"]] <- .anovaNullControlPostHocTable(
      dataset, options, perform, status, analysisType = "RM-ANOVA")

  ## Descriptives Table
  descriptivesDataset <- .readBayesianRepeatedMeasuresShortData(
      options, perform)
  descriptivesTable <- .rmAnovaDescriptivesTable(
                           descriptivesDataset, options, perform, status,
  ...
  descriptivesPlot <- .rmAnovaDescriptivesPlot(
                          descriptivesDataset, options, perform, status,
                          stateDescriptivesPlot = NULL)[["result"]]
  ...
  if (length(descriptivesPlot) == 1) {
    ...
    if (plotOptionsChanged)
      results[[".meta"]][[
          6]][["meta"]][[2]] <- list(name = "descriptivesPlot", type = "image")
