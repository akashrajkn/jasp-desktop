...
BinomialTest <- function(
  ...
  if (!is.null(state)) {
    ...
    if (is.list(diff) &&
        !any(diff[["variables"]], diff[["confidenceIntervalInterval"]],
             diff[["hypothesis"]], diff[["testValue"]])) {
    ...
    if (is.list(diff) && options[["descriptivesPlots"]] &&
                         !any(diff[["variables"]], diff[["testValue"]],
                              ...
                              diff[["plotWidth"]], diff[["plotHeight"]])) {
  ...
  for (j in .indices(variables)) {
    descriptivesPlotsMeta[[
        j]] <- list(name = variables[j], type = "collection", meta = "image")
  }
  ...
  results[[
      "binomial"]] <- .binomialTable(binomResults, options, variables, perform)


  if (options[["descriptivesPlots"]]) {

    if (is.null(descriptPlots)) {
      descriptPlots <- .binomialDescriptivesPlot(
          dataset, options, variables, perform)
    }
    ...
    for (i in 1:length(descriptPlots)) {
      if (is.list(descriptPlots[[i]])) {
        for (j in 1:length(descriptPlots[[i]]$collection)) {
          plotPaths[[
              length(plotPaths) + 1]] <- descriptPlots[[i]]$collection[[j]]$data
        }
      }
    }
...
.binomialTable <- function(r, options, variables, perform) {
  ...
  if (options$hypothesis == "notEqualToTestValue") {
    ...
    note <-
        "For all tests, the alternative hypothesis specifies that the
  ...
  if (!is.null(r)) {
    ...
    if (options$VovkSellkeMPR) {
      for (row in 1:length(table[["data"]])) {
        table[["data"]][[row]][[
            "VovkSellkeMPR"]] <- .VovkSellkeMPR(table[["data"]][[row]][["p"]])
      }
    }
    ...
    for (var in variables) {
      if (options$VovkSellkeMPR) {
        data[[length(data) + 1]] <- list(
            case = var, level = ".", counts = ".", total = ".",
            proportion = ".", p = ".", VovkSellkeMPR = ".", lowerCI = ".",
            upperCI = ".")
      } else {
        data[[length(data) + 1]] <- list(
            case = var, level = ".", counts = ".", total = ".",
            proportion = ".", p = ".", lowerCI = ".", upperCI = ".")
      }
    }
  ...
  if (perform == "run" && !is.null(variables)) {
    ...
    for (var in variables) {
      ...
      for (lev in levels) {
        ...
        r <- stats::binom.test(
            counts, n, p = options$testValue, alternative = hyp,
  ...
  for (i in .indices(options$variables)) {
    ...
    for (k in .indices(levels(d))) {
      ...
      if (perform == "run") {
        ...
        r <- stats::binom.test(
            x = count, n = nObs, p = testValue, alternative = "two.sided",
            conf.level = confLevel)
        ciLower <- r$conf.int[1]
        ciUpper <- r$conf.int[2]

        summaryStat <- data.frame(
            label = level, rate = rate, ciLower = ciLower, ciUpper = ciUpper)

        dfTestValue <- data.frame(testValue = testValue)

        p <- ggplot2::ggplot(
                 summaryStat, ggplot2::aes(x = label, y = rate, group = 1)) +
            ggplot2::geom_errorbar(
                ggplot2::aes(ymin = ciLower, ymax = ciUpper), colour = "black",
                width = 0.2, position = pd) +
            ggplot2::geom_point(position = pd, size = 4) +
            ggplot2::geom_hline(
                data = dfTestValue, ggplot2::aes(yintercept = testValue),
                linetype = "dashed") + ggplot2::ylab(NULL) +
            ggplot2::xlab(NULL) +
            ggplot2::theme_bw() + ggplot2::ylim(min = 0, max = 1) +
            ggplot2::theme(
