...
SummaryStatsRegressionLinearBayesian <- function(
    dataset = NULL, options, perform = 'run', callback = function(...) 0, ...) {
  state <- .retrieveState()

  diff <- NULL

  if (!is.null(state))  # difference between the previous state
                        # variables(options) and current options
      {
  ...
  if (options$numberOfCovariatesNull == 0 &&
  ...
  meta[[2]] <- list(
      name = "inferentialPlots", type = "object",
  ...
  if (nullModelSpecified) {
    ...
    fields[[length(fields) + 1]] <- list(
        name = "sampleSize", type = "string", title = sampleSize.title)
    ...
    fields[[length(fields) + 1]] <- list(
        name = "sampleSize", type = "integer", title = "n")
  }
  fields[[length(fields) + 1]] <- list(
      name = "numberOfCovariates", type = "integer",
      title = "Number of covariates")
  fields[[length(fields) + 1]] <- list(
      name = "unadjustedRSquared", type = "number", title = "R\u00B2",
      format = "dp:3")

  fields[[length(fields) + 1]] <- list(
      name = "BF", type = "number", format = "sf:4;dp:3", title = bf.title)

  fields[[length(fields) + 1]] <- list(name = "properror", type = "number",
  ...
  if (run) {
    status <- .isInputValidRegressionSummaryStatistics(
        options, nullModelSpecified)
      ...
      if (status$error) {
        ...
        if (nullModelSpecified) {
          ...
          if (!is.null(state)) {
            ...
            if ((BFTypeState == "BF10" || BFTypeState == "BF01") &&
                BFType == "LogBF10") {
              BFRequiresNewRow <- TRUE
            } else if ((BFType == "BF10" || BFType == "BF01") &&
                       BFTypeState == "LogBF10") {
              BFRequiresNewRow <- TRUE
            }
          }
          ...
          if (!is.null(state) &&
              ...
              ((is.logical(diff) && diff == FALSE) ||
               (is.list(diff) && (diff$sampleSize == FALSE &&
                                  diff$numberOfCovariatesNull == FALSE &&
                                  diff$unadjustedRSquaredNull == FALSE &&
                                  diff$numberOfCovariatesAlternative == FALSE &&
                                  diff$unadjustedRSquaredAlternative == FALSE &&
                                  diff$priorWidth == FALSE))) &&
              !is.null(state$bayesFactorObjectAlternative) &&
              !BFRequiresNewRow) {
            ...
            bayesFactorObjectNull <- BayesFactor::linearReg.R2stat(
                N = options$sampleSize, p = options$numberOfCovariatesNull,
                R2 = options$unadjustedRSquaredNull,
                rscale = options$priorWidth)
            bayesFactorObjectAlternative <- BayesFactor::linearReg.R2stat(
                N = options$sampleSize,
                p = options$numberOfCovariatesAlternative,
                R2 = options$unadjustedRSquaredAlternative,
                rscale = options$priorWidth)

            BFNull <- exp(bayesFactorObjectNull[["bf"]]) /
                exp(bayesFactorObjectAlternative[["bf"]])
            BFAlternative <- exp(bayesFactorObjectAlternative[["bf"]]) /
                exp(bayesFactorObjectNull[["bf"]])
            ...
            row[[1]] <- list(
                numberOfCovariates = options$numberOfCovariatesNull,
                sampleSize = "Null model",
                unadjustedRSquared = .clean(options$unadjustedRSquaredNull),
                BF = .clean(BFNull),
                properror = .clean(bayesFactorObjectNull[["properror"]]))
            row[[2]] <- list(
                sampleSize = "Alternative model",
                numberOfCovariates = .clean(
                    options$numberOfCovariatesAlternative),
                unadjustedRSquared = .clean(
                    options$unadjustedRSquaredAlternative),
                BF = .clean(BFAlternative),
                properror = .clean(bayesFactorObjectAlternative[["properror"]]))
          }
        } else {
          if (!is.null(state) &&
              !is.null(diff) &&
              ((is.logical(diff) && diff == FALSE) ||
               (is.list(diff) && (diff$bayesFactorType == FALSE &&
                                  diff$sampleSize == FALSE &&
                                  diff$numberOfCovariatesNull == FALSE &&
                                  diff$unadjustedRSquaredNull == FALSE &&
                                  diff$numberOfCovariatesAlternative == FALSE &&
                                  diff$unadjustedRSquaredAlternative == FALSE &&
            ...
            bayesFactorObject <- BayesFactor::linearReg.R2stat(
                N = options$sampleSize,
                p = options$numberOfCovariatesAlternative,
                R2 = options$unadjustedRSquaredAlternative,
                rscale = options$priorWidth)
            ...
            row <- list(sampleSize = .clean(options$sampleSize),
                        numberOfCovariates = .clean(
                            options$numberOfCovariatesAlternative),
                        unadjustedRSquared = .clean(
                            options$unadjustedRSquaredAlternative),
                        BF = .clean(BF), properror = .clean(errorEstimate))
          }
        }
                    ...
                    .plotFunc <- function() {
                      .plotBF.robustnessCheck.regression.summaryStatistics(
                          ...
                          BF10post = ifelse(
                              ...
                              .clean(exp(bayesFactorObjectAlternative[["bf"]]) /
                                     exp(bayesFactorObjectNull[["bf"]])),
                              .clean(exp(bayesFactorObjectNull[["bf"]]) /
                                     exp(bayesFactorObjectAlternative[["bf"]]))))
                    }
                    content <- .writeImage(
      ...
      if (!is.null(state) &&
          ...
          ((is.logical(diff) && diff == FALSE) ||
           (is.list(diff) && (diff$sampleSize == FALSE &&
                              ...
                              diff$numberOfCovariatesNull == FALSE &&
                              diff$unadjustedRSquaredNull == FALSE))) &&
          !is.null(state$bayesFactorObjectAlternative) && !BFRequiresNewRow) {
        rowsRegressiontest <- state$rowsRegressiontest
        bayesFactorObjectNull <- state$bayesFactorObjectNull
        bayesFactorObjectAlternative <- state$bayesFactorObjectAlternative
      } else {
        rowsRegressiontest <- .isInputValidRegressionSummaryStatistics(
                                  options, nullModelSpecified)$row
      }
    } else {
      if (!is.null(state) &&
          !is.null(diff) &&
    ...
    return(
        list(results = results, status = "complete",
             state = list(
                 ...
                 bayesFactorRobustnessPlot = bayesFactorRobustnessPlot,
                 rowsRegressiontest = rowsRegressiontest,
                 bayesFactorObjectAlternative = bayesFactorObjectAlternative,
                 bayesFactorObjectNull = bayesFactorObjectNull), keep = keep))
  }
}


.isInputValidRegressionSummaryStatistics <- function(
    options, nullModelSpecified) {
  error <- FALSE
  errorMessage <- NULL
  ready <- TRUE
  ...
  if (nullModelSpecified) {
    if (numberOfCovariatesNullValue == 0 ||
        is.null(numberOfCovariatesNullValue)) {
      ready <- FALSE
    ...
    if (options$numberOfCovariatesNull != 0 && options$sampleSize != 0) {
      if ((options$sampleSize - options$numberOfCovariatesNull) < 2) {
        ...
        error <- TRUE
        errorMessage <- paste(
            "Number of Covariates must be less than N-1 (sample size minus 1)")
      }
    }
  }

  if (!error) {
    if ((sampleSizeValue == 0 && numberOfCovariatesAlternativeValue == 0 &&
                                 unadjustedRSquaredAlternativeValue == 0) ||
        is.null(unadjustedRSquaredAlternativeValue)) {
      ready <- FALSE
    ...
    if (numberOfCovariatesAlternativeValue == 0 ||
        is.null(numberOfCovariatesAlternativeValue)) {
      ready <- FALSE
      numberOfCovariatesAlternativeValue <- "."
    }

    if (options$numberOfCovariatesAlternative >
        options$numberOfCovariatesNull) {
      if (options$unadjustedRSquaredAlternative <
          options$unadjustedRSquaredNull) {
        ready <- TRUE
        error <- TRUE
        errorMessage <- paste(
            "Input: When number of covariates for Alternative hypothesis is greater than that of Null hypothesis, the R\u00B2 has to be higher under Alternative than under Null hypothesis")
      }
    }
  ...
  if (nullModelSpecified) {
    row[[1]] <- list(sampleSize = "Null model",
                     numberOfCovariates = numberOfCovariatesNullValue,
                     unadjustedRSquared = unadjustedRSquaredNullValue, BF = ".",
                     properror = ".")
    row[[2]] <- list(sampleSize = "Alternative model",
                     numberOfCovariates = numberOfCovariatesAlternativeValue,
                     unadjustedRSquared = unadjustedRSquaredAlternativeValue,
                     BF = ".", properror = ".")
  } else {
    row <- list(sampleSize = sampleSizeValue,
                numberOfCovariates = numberOfCovariatesAlternativeValue,
                unadjustedRSquared = unadjustedRSquaredAlternativeValue,
                BF = ".", properror = ".")
  }
...
.plotBF.robustnessCheck.regression.summaryStatistics <- function(
    sampleSize = NULL, numberOfCovariatesNull = NULL,
    unadjustedRSquaredNull = NULL, numberOfCovariatesAlternative = NULL,
    unadjustedRSquaredAlternative = NULL, nullModelSpecified = FALSE, BF10post,
    callback = function(...) 0, formula = NULL, data = NULL, rscale = 1,
  ...
  if (nullModelSpecified) {
    for (i in seq_along(rValues)) {
      ...
      BFNull <- BayesFactor::linearReg.R2stat(
          N = sampleSize, p = numberOfCovariatesNull,
          R2 = unadjustedRSquaredNull, rscale = rValues[i])
      BFAlternative <- BayesFactor::linearReg.R2stat(
          N = sampleSize, p = numberOfCovariatesAlternative,
          R2 = unadjustedRSquaredAlternative, rscale = rValues[i])
    ...
    for (i in seq_along(rValues)) {
      ...
      BF <- BayesFactor::linearReg.R2stat(
          N = sampleSize, p = numberOfCovariatesAlternative,
          R2 = unadjustedRSquaredAlternative, rscale = rValues[i])
  ...
  if (nullModelSpecified) {
    # BF10 "medium" prior
    BF10mNull <- BayesFactor::linearReg.R2stat(
        N = sampleSize, p = numberOfCovariatesNull, R2 = unadjustedRSquaredNull,
        rscale = sqrt(2) / 2)
    BF10mAlternative <- BayesFactor::linearReg.R2stat(
        N = sampleSize, p = numberOfCovariatesAlternative,
        R2 = unadjustedRSquaredAlternative, rscale = sqrt(2) / 2)
    BF10m <- .clean(exp(BF10mAlternative[["bf"]]) / exp(BF10mNull[["bf"]]))
    BF10mText <- BF10m

    # BF10 "wide" prior
    BF10wNull <- BayesFactor::linearReg.R2stat(
        N = sampleSize, p = numberOfCovariatesNull, R2 = unadjustedRSquaredNull,
        rscale = 1)
    BF10wAlternative <- BayesFactor::linearReg.R2stat(
        N = sampleSize, p = numberOfCovariatesAlternative,
        R2 = unadjustedRSquaredAlternative, rscale = 1)
    BF10w <- .clean(exp(BF10wAlternative[["bf"]]) / exp(BF10wNull[["bf"]]))
    BF10wText <- BF10w

    # BF10 "ultrawide" prior
    BF10ultraNull <- BayesFactor::linearReg.R2stat(
        N = sampleSize, p = numberOfCovariatesNull, R2 = unadjustedRSquaredNull,
        rscale = sqrt(2))
    BF10ultraAlternative <- BayesFactor::linearReg.R2stat(
        N = sampleSize, p = numberOfCovariatesAlternative,
        R2 = unadjustedRSquaredAlternative, rscale = sqrt(2))
    BF10ultra <- .clean(exp(BF10ultraAlternative[["bf"]]) /
                        exp(BF10ultraNull[["bf"]]))
    BF10ultraText <- BF10ultra
  } else {
    # BF10 "medium" prior
    BF10m <- BayesFactor::linearReg.R2stat(
        N = sampleSize, p = numberOfCovariatesAlternative,
        R2 = unadjustedRSquaredAlternative, rscale = sqrt(2) / 2)
    ...
    # BF10 "wide" prior
    BF10w <- BayesFactor::linearReg.R2stat(
        N = sampleSize, p = numberOfCovariatesAlternative,
        R2 = unadjustedRSquaredAlternative, rscale = 1)
    ...
    # BF10 "ultrawide" prior
    BF10ultra <- BayesFactor::linearReg.R2stat(
        N = sampleSize, p = numberOfCovariatesAlternative,
        R2 = unadjustedRSquaredAlternative, rscale = sqrt(2))
  ...
  points(r, log(BF10user), pch = 21, bg = "grey", cex = cexPoints,
         lwd = 1.3)  # user prior
  points(1, log(BF10w), pch = 21, bg = "black", cex = 1.1, lwd = 1.3)  # "wide"
                                                                       # prior
  ...
  legend(xx, yy, legend = legend[BFind], pch = rep(21, 3), pt.bg = pt.bg[BFind],
         bty = "n", cex = cexLegend, lty = rep(NULL, 3), pt.lwd = rep(1.3, 3),
         pt.cex = pt.cex[BFind])
