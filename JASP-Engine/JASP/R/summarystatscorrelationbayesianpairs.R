...
SummaryStatsCorrelationBayesianPairs <- function(
  ...
  hypothesis.variables <- .hypothesisType.summarystats.correlation(
      options$hypothesis)
  ...
  if (options$hypothesis != "correlated") {
    .addFootnote(footnotes, symbol = "<em>Note.</em>",
                 text = hypothesis.variables$message)
  }
  ...
  outputTableElements <- .getOutputRow.summarystats.correlation(
      run = run, options = options, diff = diff, state = state)
  ...
  if (options$plotPriorAndPosterior) {
    priorAndPosteriorPlot <- .getPriorAndPosteriorPlot.summarystats.correlation(
        run = run, options = options, state = state, diff = diff,
        bfObject = bfObject, oneSided = oneSided)
    plots.sumstats.correlation[[length(plots.sumstats.correlation) + 1]] <-
        priorAndPosteriorPlot
  ...
  fields[[length(fields) + 1]] <- list(
      name = "sampleSize", type = "number", title = "n")
  fields[[length(fields) + 1]] <- list(
      name = "pearsonsR", type = "number", format = "sf:4;dp:3",
      title = correlation.title)
  fields[[length(fields) + 1]] <- list(
      name = "BF", type = "number", format = "sf:4;dp:3", title = bf.title)
  fields[[length(fields) + 1]] <- list(
      name = "pValue", type = "number", format = "sf:4;dp:3", title = "p")
  ...
  if (run) {
    ...
    state <- list(options = options, bfObject = bfObject,
                  ...
                  plotsCorrelationTest = plots.sumstats.correlation,
                  plotTypes = plotTypes)
  } else {
    status <- "inited"
  }
...
.getPriorAndPosteriorPlot.summarystats.correlation <- function(
    run, options, state, diff, bfObject, oneSided) {
  ...
  if (!is.null(state) &&
      ...
      ((is.logical(diff) && diff == FALSE) ||
       (is.list(diff) &&
        (diff$bayesFactorType == FALSE &&
         ...
         diff$correlationCoefficient == FALSE &&
         diff$pearsonRhoValue == FALSE &&
         diff$kendallTauValue == FALSE &&
    ...
    p <- try(
        ...
        expr = {
          ...
          someBf <- switch(options$hypothesis, correlated = bfObject$bf10,
...
.getBayesFactorRobustnessPlot.summarystats.correlation <- function(
    run, options, state, diff, bfObject, oneSided) {
    ...
    p <- try(
        ...
        expr = {
          ...
          .plotFunc <- function() {
            .plotBF.robustnessCheck.summarystats.correlation(
                r = cor.value, n = options$sampleSize, oneSided = oneSided,
                BFH1H0 = BFH1H0, corCoefficient = cor.coefficient,
                kappa = options$priorWidth, dontPlotData = dontPlotData,
                BF10post = BF10post,
                addInformation = options$plotBayesFactorRobustnessAdditionalInfo)
          }
          content <- .writeImage(
...
.calculateBF.summarystats.correlation <- function(options, state, diff) {
  ...
  if (options$correlationCoefficient == "pearsonRho") {
    ...
    bfObject <- .bfPearsonCorrelation(
        n = some.n, r = some.r, kappa = options$priorWidth)
    allPValues <- .pValueFromCor(
        corrie = some.r, n = some.n, method = "pearson")
  } else if (options$correlationCoefficient == "kendallTau") {
    some.r <- options$kendallTauValue
    bfObject <- .bfKendallTau(
        n = some.n, tauObs = some.r, kappa = options$priorWidth)
    allPValues <- .pValueFromCor(
        corrie = some.r, n = some.n, method = "kendall")
      ...
      ((is.logical(diff) && diff == FALSE) ||
       (is.list(diff) &&
        (diff$bayesFactorType == FALSE &&
         ...
         diff$pearsonRhoValue == FALSE && diff$kendallTauValue == FALSE))) &&
      ...
      if (status$ready) {
        ...
        switch(options$hypothesis,
               correlated = {
                 ...
                 rowsCorrelationBayesianPairs$pValue <- .clean(
                     bfObject$pValue$twoSided)
               }, correlatedPositively = {
                    someBf <- bfObject$bfPlus0
                    rowsCorrelationBayesianPairs$pValue <- .clean(
                        bfObject$pValue$plusSided)
                  }, correlatedNegatively = {
                       someBf <- bfObject$bfMin0
                       rowsCorrelationBayesianPairs$pValue <- .clean(
                           bfObject$pValue$minSided)
                     })

        # Note: Choose the display mode for Bf
        rowsCorrelationBayesianPairs$BF <- switch(
            options$bayesFactorType, BF10 = .clean(someBf),
            BF01 = .clean(1 / someBf), LogBF10 = .clean(log(someBf)))
      }
    }
  }
...
.hypothesisType.summarystats.correlation <- function(hypothesis) {
  ...
  return(
      list(nullInterval = nullInterval, oneSided = oneSided, message = message))
}


.plotBF.robustnessCheck.summarystats.correlation <- function(
  ...
  for (i in seq_along(kappaValues)[-1]) {
    ...
    if (usePearson) {
      ...
      bfObject <- .bfKendallTau(
          n = n, tauObs = r, kappa = kappaValues[i], ciValue = NULL)
    }
