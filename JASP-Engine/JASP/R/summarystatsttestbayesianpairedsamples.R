...
SummaryStatsTTestBayesianPairedSamples <- function(
  ...
  hypothesis.variables <- .hypothesisType.summarystats.ttest.paired(
      options$hypothesis)
...
.getOutputRow.summarystats.ttest.paired <- function(
  ...
  if (!is.null(state) && !is.null(diff) && !is.null(state$bayesFactorObject) &&
    ...
    if (run) {
      if (status$ready) {
        ...
        rowsTTestBayesianPairedSamples$errorEstimate <- .clean(
  ...
  return(list(row = rowsTTestBayesianPairedSamples,
...
.hypothesisType.summarystats.ttest.paired <- function(hypothesis) {
  ...
  if (hypothesis == "groupOneGreater") {
    ...
    oneSided <- "right"
    message <- paste(
        "For all tests, the alternative hypothesis specifies that measure 1 is greater than measure 2",
        sep = "")
    ...
    oneSided <- "left"
    message <- paste(
        "For all tests, the alternative hypothesis specifies that measure 1 is lesser than measure 2",
        sep = "")
  }
